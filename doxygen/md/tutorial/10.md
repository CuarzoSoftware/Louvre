...
Lastly, to conclude the session, we call Louvre::LPointer::stopMovingToplevel() when releasing the left pointer button in a pointer button event as we also saw in [Chapter 8: Pointer Events](md_md_tutorial_08.html).
...
Now, let's override override these methods to configure the toplevels based on the output size, subtracting the topbar region. Additionally, we will introduce auxiliary variables and methods to store and recover the toplevel's state to its previous configuration before maximizing it.
...
In the 'Louvre::LToplevelRole::setMaximizedRequest()' method, we configure the toplevel with the 'Maximized' flag, adjusting it to the primary output size minus the topbar region. We also store the current primary output, as well as the toplevel's position and size for later restoration. We then await a configure ACK response through 'Louvre::LToplevelRole::maximizedChanged()' to ensure the toplevel is correctly positioned behind the topbar. If the toplevel isn't configured with the specified size, we revert it to its previous state. We perform this check whenever the window geometry changes.

Lastly, in 'Louvre::LToplevelRole::unsetMaximizedRequest()', we simply configure the toplevel with its previous size and state. When unmaximized, it restores its previous position. After recompiling and running the compositor, you'll observe that toplevels no longer maximize themselves behind the topbar, and upon unmaximizing, they return to their previous position and size.

### Handling Fullscreen Requests

In order to display toplevels in fullscreen mode, we'll create a new black Louvre::LSolidColorView and relocate the toplevel, along with all its child elements, into this view. This approach is necessary because toplevels can have a defined 'Louvre::LToplevelRole::maxSize()' that might be smaller than the output size. In such cases, it's important to center the toplevel on the screen to ensure a visually appealing appearance. If other surfaces were left in the background during fullscreen mode, it could appear somewhat unusual.
...
void EToplevel::setFullscreenRequest(LOutput *output)
{
    ...
    // but we will ignore it and use the one we selected.
    L_UNUSED(output);
...
In the constructor, we add the black fullscreen view to the fullscreen layer and initially hide it. When a client requests to set a toplevel into fullscreen mode, we configure it with the size of the chosen output. Clients may specify their preferred output for fullscreen mode, but we prioritize our selection over their preference.

Subsequently, when the toplevel's state transitions to fullscreen, we conduct a size check against the output. If the toplevel's size exceeds that of the output, we revert it to the normal state. Conversely, if its size is smaller, we add it to the fullscreen background view, including all its child surfaces. In cases where the size is smaller, we center it as needed. We repeat this process whenever the toplevel's window geometry changes.
...
After recompiling and running the compositor, you'll observe toplevels seamlessly transitioning into fullscreen mode and returning to their normal state when exiting fullscreen mode.

### Managing Surface Parent Changes

As we relocate the fullscreen toplevels and their child surfaces into the black fullscreen view, it's essential to ensure that any newly created child surfaces are also moved when the toplevel is in fullscreen mode. To accomplish this, we can override the 'Louvre::LSurface::parentChanged()' virtual method.
...
In a manner similar to our approach for minimizing surfaces, we also need to address situations where the output stored in "normalOutput" becomes uninitialized while the toplevel is in a maximized or fullscreen state. Additionally, we haven't yet covered the scenario where we need to relocate surfaces to ensure their visibility after such an event occurs. To handle these scenarios, we will implement code in 'Louvre::LOutput::uninitializeGL()' and introduce a new method named 'updateSurfaces()' for the precise reallocation of surfaces.
...
So now, when an output is uninitialized, we perform the following actions: we check all toplevels with their 'normalOutput' variable set to the output and set it to 'nullptr,' and we return these toplevels to their normal state.

Additionally, whenever there are changes in the outputs' size or position, we update the toplevels to ensure their correct positioning. We also move any surfaces into random positions within one of the still-initialized outputs to ensure they remain visible to the user.

This chapter is undoubtedly one of the most complex sections in this entire tutorial. In the next chapter, we'll explore the handling of popup surfaces, which are notably simpler to manage than toplevels. See you there!
