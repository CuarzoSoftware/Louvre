...
void ESurface::mappingChanged()
{
    if (mapped() && firstMapping)
    {
        ...
        if (toplevel())
        {
            ...
            // Center the toplevel
            setPos( (output->pos() + LPoint(0, TOPBAR_HEIGHT)) + (availableSize - toplevel()->windowGeometry().size()) / 2);
...
Lastly, to conclude the session, we call Louvre::LPointer::stopMovingToplevel() when releasing the left pointer button in a pointer button event as we also saw in [Chapter 8](md_md_tutorial_08.html).
...
{
    if (!fullscreen() && !maximized() &&  seat()->pointer()->focusSurface() == surface())
...
We don't need to override the Louvre::LToplevelRole::geometryChanged() virtual method  because it already automatically invokes Louvre::LToplevelRole::updateResizingPos() for us.
...
Now, let's override Louvre::LToplevelRole::setMaximizedRequest() and Louvre::LToplevelRole::maximizedChanged() to configure them with the output size minus the topbar region and also let's create some auxiliary variables and methods to save and restore the toplevel state to its configuration before being maximized.
