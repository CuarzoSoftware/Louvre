...
Popups involve complex rules for their positioning relative to their parent surface. These rules dictate how popups should be repositioned or scaled when their position is constrained, ensuring they always remain within the visible area, such as an output.

All these positioning rules are defined in their Louvre::LPositioner instance which can be accessed through Louvre::LPopupRole::positioner(). If you wish to understand how these rules work, you can refer to the Louvre::LPositioner documentation.

Fortunately, Louvre simplifies this, implementing these rules by default in 'Louvre::LPopupRole::rolePos()'. This leaves you with the straightforward task of defining the bounding area where the popup should be positioned. In our case, we should set the bounding area equal to the output size minus the topbar region, or equal to the entire output size if the popup is a child of a fullscreen toplevel. 
...
We configure popups using Louvre::LPopupRole::configure(). However, unlike toplevels, popups don't have states, but they do require configuration of both their position relative to the parent surface and their size.
...
So, within Louvre::LPopupRole::configureRequest(), we begin by defining the boundaries within which the popup can be positioned. If it's a child of a fullscreen toplevel, we set the bounding rectangle equal to the output rectangle; otherwise, we subtract the topbar region. To determine the position, we call Louvre::LPopupRole::rolePos() to obtain the popup position and subtract its parent's size to calculate the local parent offset.

Moreover, calling Louvre::LPopupRole::rolePos() also stores the size the popup should have to remain unconstrained (in case it is constrained), and this information is saved in Louvre::LPositioner::unconstrainedSize(). In the end, we configure the popup using the calculated local parent position and unconstrained size.

### Explicit Grab

Popups can also request to be created with a keyboard grab through Louvre::LPopupRole::grabSeatRequest(). This ensures that all keyboard events are redirected to that surface while it is active. If the popup is dismissed, the grab is returned to the parent surface (if it is also a popup previously created with a keyboard grab).
...
The Louvre::LKeyboard::setGrabbingSurface() method accepts two arguments: the target surface that will receive the keyboard grab and the [wl_keyboard](https://wayland.app/protocols/wayland#wl_keyboard) used by the client.
...
As you may recall from previous chapters, we dismissed all popups, for example when clicking on a surface that is not a popup or when no surface is clicked, using Louvre::LPointer::dismissPopups(). To dismiss a specific popup, you can use LPopupRole::dismiss(). This method will dismiss the specified popup along with any child or subchild popups, in reverse order of their creation.
