...
In this chapter, we will delve into the process of rendering content onto the outputs that we have previously initialized. Each Louvre::LOutput operates within its own thread and provides a set of events that can be customized to suit our rendering needs.

#### initializeGL()

The Louvre::LOutput::initializeGL() event is invoked once, right after we initialize the output using Louvre::LCompositor::addOutput(). Within this event, your focus should be on setting up and loading any necessary OpenGL resources, such as shaders, programs, and textures. However, it's important to refrain from performing any painting operations at this stage, as they will not be visible on the screen.
...
The Louvre::LOutput::moveGL() event is asynchronously triggered each time we alter the position of the output using Louvre::LOutput::setPos(). It's crucial to avoid performing any painting operations within this event, as they will not be displayed on the screen.

#### resizeGL()

Similarly, the Louvre::LOutput::resizeGL() event is called asynchronously whenever we modify the output mode or scale using Louvre::LOutput::setMode() or Louvre::LOutput::setScale(). Each output has one or more modes (Louvre::LOutputMode), each of which dictates its refresh rate and resolution. The default output mode typically offers the highest refresh rate and resolution. To access all available output modes, you can use Louvre::LOutput::modes(). Like the other events, it's essential not to perform any painting operations within Louvre::LOutput::resizeGL(), as they will not be visible on the screen.
...
As you might expect, Louvre provides a default mechanism for handling all these events, which is why you were able to see content rendered on your screens previously. Now, let's take control by overriding these virtual methods and creating our custom subclass of Louvre::LOutput.
...
To enable Louvre to use our custom `EOutput` class, we need to take one final step: overriding its virtual constructor (Louvre::LCompositor::createOutputRequest()).
...
In `initializeGL()`, we load our wallpaper image using the Louvre::LOpenGL::loadTexture() method. This method supports all image formats that FreeImage supports and returns `nullptr` if it fails. 
...
You can use the Louvre::LRegion class to perform efficient boolean rectangle operations to calculate the region that needs repainting each frame. However, due to its complexity, we won't cover it here. You can refer to the **louvre-weston-clone** compositor example to see how it's done, but be warned that it can be quite complex.
