...
If you recompile and execute the compositor now, you will only see a black background color, probably the cursor as well, but no application windows (surfaces). 

Let's proceed to learn how to render surfaces.
...
To get started, let's first render our background wallpaper. I'll use this beautiful Louvre-themed wallpaper, which is expected to be located on your system at `/usr/etc/Louvre/assets/wallpaper.png`. However, you're welcome to select any image of your choice for this purpose.
...
void EOutput::initializeGL()
{
    wallpaper = LOpenGL::loadTexture("/usr/etc/Louvre/assets/wallpaper.png");
...
Buffer coordinates represent the maximum granularity within the object, independently of the scale associated with it. For example, if the wallpaper image was 1000x500px, its size in buffer coordinates would be the same, 1000x500. Methods and properties that use buffer coordinates have the "B" suffix, such as Louvre::LTexture::sizeB().
