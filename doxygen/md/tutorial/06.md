...
ETopbar::ETopbar(EOutput *output) :
    output(output),
    view(0.f, 0.f, 0.f, 0.6f, &G::compositor()->overlayLayer)
{
    update();
}
...
As cursor hardware composition can vary across different outputs, we should enable or disable the `softwareCursor` view within a Louvre::LOutput::paintGL() event, just before invoking Louvre::LScene::handlePaintGL(), like this:
