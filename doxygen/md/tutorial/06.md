...
5. **Cursor Layer**: Here, we'll display a cursor view if hardware composition is not avaliable.
...
In this section, we construct the scene and layers, designating them as children of Louvre::LScene::mainView(). We also initialize the `softwareCursor` view with a `nullptr` texture, which automatically hides it. Furthermore, we set the default background color for the Louvre::LScene and enable a custom destination size for the `softwareCursor`, allowing us to specify a size that is independent from its texture's size.
...
If you recompile and run the compositor, you will only see a black background because we've added layer views, which are invisible by themselves, and the cursor view doesn't have a texture yet.
...
Notice that we've replaced the wallpaper Louvre::LTexture pointer with an Louvre::LTextureView and added an `updateWallpaper()` method. Within this method, we will accurately clip and scale our wallpaper each time the output size changes.
...
First, in Louvre::LOutput::initializeGL(), we are creating our wallpaper view without attaching a texture to it yet and adding it to the background layer. We also call our new `updateWallpaper()` method.
...
Finally, in Louvre::LOutput::uninitializeGL(), we check if the wallpaper view has a texture attached and delete it before deleting the view itself. It's worth noting that a texture is not automatically deleted when a view using it is deleted. This is because you can use the same texture in multiple views at a time. However, you need to be cautious when deleting the texture and set the view's texture to `nullptr`. Doing so hides the view and also prevents the scene from trying to access the texture, which could lead to a segmentation fault.
...
Furthermore, we've incorporated a Louvre::LSolidColorView to visually represent the top bar within the scene. As the name implies, Louvre::LSolidColorView is designed for displaying solid color rectangles.
...
We perform a check to determine whether the output supports hardware cursor compositing. If it does, we appropriately update the `softwareCursor` view texture, position, and size to match those of Louvre::LCursor. 

Importantly, if hardware compositing is supported, we assign `nullptr` as the texture, which effectively hides the cursor view. While it's possible to hide a view using Louvre::LView::setVisible(), it's not necessary in this case.
