...
> Each view has a Louvre::LView::mapped() property, which the scene uses to determine whether it satisfies the conditions for being drawn.
...
Notice that we've replaced the wallpaper Louvre::LTexture pointer with an Louvre::LTextureView and added an `updateWallpaper()` method. Within this method, we will accurately crop and scale our wallpaper each time the output size or scale factor change.
...
void EOutput::updateWallpaper()
{
    ...
    LTexture *originalWallpaper { LOpenGL::loadTexture("/usr/local/share/Louvre/assets/wallpaper.png") };
    ...
    Float32 scaledWidth { Float32(size().w() * originalSize.h()) / Float32(size().h()) };
...
In `copyThumbnailsFromAnotherTopbar()`, for now, all we do is call `updateGUI()`. Later, we will see how to create the thumbnails.
