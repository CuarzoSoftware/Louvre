...
First, let's define the padding, margin, and height of the thumbnails in a macro within the `Global.h` header. Additionally, we will create an auxiliary method to automatically cast the Louvre::LCompositor::surfaces() lists into our custom subtype.
...
2. When Louvre::LSurface::minimized() returns `true`, indicating that the surface is minimized:
   ...
   c. We use the `capture()` method to create a thumbnail of the surface, resizing the resulting texture to match `THUMBNAIL_HEIGHT` while preserving its aspect ratio. This thumbnail is stored in the `thumbnail` variable.
   It's essential to note that we created the scene view in stack memory, so it's automatically destroyed after the method ends. 
