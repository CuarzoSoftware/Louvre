...
You may already have an idea of what a surface is, but let's clarify it. Think of a surface as a canvas created by a Wayland client, which the compositor can render on the screen. The client can request the compositor to use it as a cursor, a drag & drop icon, a toplevel window, and so on. This specific use assigned by the client is known as the surface role. Each surface role provides rules on how the compositor should position, stack, and interact with it. Refer to the Louvre::LSurface documentation for additional information on the available roles.

You can determine the assigned role of a surface using Louvre::LSurface::roleId() and access it through Louvre::LSurface::role(). Alternatively, you can directly cast it using functions like Louvre::LSurface::cursorRole(), Louvre::LSurface::dndIcon(), Louvre::LSurface::toplevel(), etc.\n 
...
If you consistently observe that the entire screen or a great portion of it is being rendered in every frame, it is probable that there is an issue in your implementation. This might occur for example if you are frequently altering an output's position, adjusting a wallpaper's position, or manually introducing damage to the scene using functions like Louvre::LSceneView::damageAll() or Louvre::LSceneView::addDamage().
...
First, let's define the padding, margin, and height of the thumbnails in a macro within the `Global.h` header. Additionally, we will create an auxiliary function to automatically cast the Louvre::LCompositor::surfaces() lists into our custom subtype.
...
Now, let's proceed to create a Louvre::LTextureView subclass to serve as our thumbnail views, which we'll name `ETopbarItem`.
...
In the `update()` function of `ETopbar`, we now need to arrange the thumbnails from left to right, spacing them apart by a distance of `THUMBNAIL_MARGIN`.
...
ETopbar::ETopbar(EOutput *output) :
    output(output),
    view(0.f, 0.f, 0.f, 0.3f, &G::compositor()->overlayLayer)
{
...
void ESurface::minimizedChanged()
{
    ...
    if (minimized())
    {
        ...
        // Make it twice the size so it looks good
        // in both low and high DPI outputs
        LSceneView tmpScene(size() * 2, 2);

        capture(&tmpScene);

        // Scale it to the thumbnail size
        thumbnail = tmpScene.texture()->copyB(
            LSize((THUMBNAIL_HEIGHT * tmpScene.texture()->sizeB().w())/tmpScene.texture()->sizeB().h(), THUMBNAIL_HEIGHT) * 2);
...
Stay calm, take a breath, and let's examine what each function is doing:

#### Destructor

In the destructor, we simply destroy all the thumbnails associated with this surface.

#### capture()

In the `capture()` function, we pass a temporary Louvre::LSceneView with a buffer size twice the size of the surface (in surface coordinates) and with a scale factor of 2. This ensures that the thumbnail will look good on both low and high DPI outputs. We don't assign a parent to the scene view because it's solely for offscreen rendering and shouldn't be displayed in our main scene.

Instead of moving the surface view and any subsurface view to the scene view position, we position the scene view at the surface view's location. We also embed the surface view and any subsurface views within the temporary scene view. We use the Louvre::LSurface::isSubchildOf() method to check if a surface child of the minimized surface and also the Louvre::LSurface::subsurface() method to check if it has the Louvre::LSubsurfaceRole, excluding other child surfaces like popups or child toplevels.

Next, we simply call Louvre::LSceneView::render(), which renders the views into its framebuffer and finally return the views back to the surfaces layer.
