...
You may already have an idea of what a surface is, but let's clarify it. Think of a surface as a canvas created by a Wayland client, which the compositor can render on the screen. The client can request the compositor to use it as a cursor, a drag & drop icon, a toplevel window, and so on. This specific use assigned by the client is known as the surface role. Each surface role provides instructions on how the compositor should position, stack, and interact with it. Refer to the Louvre::LSurface documentation for additional information on the available roles.

You can determine the assigned role of a surface using Louvre::LSurface::roleId() and access it through Louvre::LSurface::role(). Alternatively, you can directly cast it using functions like Louvre::LSurface::cursorRole(), Louvre::LSurface::dndIcon(), Louvre::LSurface::toplevel(), etc. For example, to check if a surface has a toplevel role, you can call Louvre::LSurface::toplevel(), which may return an Louvre::LToplevelRole instance or `nullptr` if it has a different role or none at all. We will delve into the details of each specific role in future chapters, so please be patient.
...
In the Louvre::LSurface::roleChanged() event we hide the views that get assigned the cursor role (remember that we are using Louvre::LCursor instead).

And in Louvre::LSurface::orderChanged(), we update the view stacking order with Louvre::LView::insertAfter(). This inserts the view on top of the surface that precedes it in the surfaces list. If there's no previous surface, we pass `nullptr`, which places the view at the beginning of its parent's children list (the `surfacesLayer` view).
...
The `params` argument simply contain Wayland resources and parameters necessary for Louvre to correctly construct each Louvre::LSurface instance.
...
<img src="https://lh3.googleusercontent.com/pw/AIL4fc_0FjIAysmysZ2osxWqvE8VA7TEwmBMnD6E1poZjJ3ZsD3j3mXBuvWL5BffFoKZPhw966HK8gc4IIeWMS4figWarGhQAJZuMSFryp0oq0wSZpm94e8=w2400" />

If you consistently observe that the entire screen is being rendered in every frame, it is probable that there is an issue in your implementation. This might occur if you are frequently altering the output position, adjusting the wallpaper position, or manually introducing damage using functions like Louvre::LSceneView::damageAll() or Louvre::LSceneView::addDamage().

Delete the `painter()->clearScreen();` line to restore your compositor to its normal state.

### Displaying Minimized Windows

Let's continue with how to present minimized windows on our topbar. You can modify the minimized state of any surface, regardless of its role, using Louvre::LSurface::setMinimized() and get its current state with Louvre::LSurface::minimized(). Additionally, you can listen to changes in this property by implementing the Louvre::LSurface::minimizedChanged() virtual method.
...
First, let's define the padding, margin, and height of the thumbnails in a macro within the `Global.h` file. Additionally, we create some auxiliary functions to automatically cast the Louvre::LCompositor::outputs() and Louvre::LCompositor::surfaces() lists into our custom subtypes.
...
public:
    ...
    static LScene *scene();
    static std::list<EOutput*> &outputs();
    static std::list<ESurface*> &surfaces();
};

#endif // G_H
```

We define the thumbnail height as the topbar height minus twice the topbar padding.
...
std::list<EOutput *> &G::outputs()
{
    return (std::list<EOutput*>&)compositor()->outputs();
}

std::list<ESurface *> &G::surfaces()
{
    return (std::list<ESurface*>&)compositor()->surfaces();
}
```

The two added functions simply cast `std::list<LOutput*>` into `std::list<EOutput*>` and `std::list<LSurface*>` into `std::list<ESurface*>` to avoid the need for casting each time we use them.

Now, let's proceed to create a custom Louvre::LTextureView to serve as our thumbnail views, which we'll name `ETopbarItem`.
...
This item accepts two arguments: the topbar in which it will be embedded and the surface it represents. The `surfaceLink` is an iterator that we will utilize later to remove the item from the surface items list that we will create.
...
In the constructor, we set the surface thumbnail as the view texture (we'll cover how to create it shortly) and add it to the topbar view. Additionally, we add the item to the surface items list, set its buffer scale to 2, and update the topbar.
...
In the `update()` function of `ETopbar`, we now need to arrange the thumbnails from left to right, spacing them apart by a distance of `THUMBNAIL_MARGIN`. It's important to note that the parent offset is enabled by default, which means we're positioning the thumbnail views relative to the topbar view rather than considering the output position. Additionally, we manually trigger Louvre::LOutput::repaint() because there may be situations where the newly added thumbnail views are not yet aware of the outputs they are visible in. This calculation is executed each time Louvre::LScene::handlePaintGL() is called.
...
public:
    ...
    // Single thumbnail texture shared by all topbar items
    ...
    // The % surface position relative to minimizedOutput before it was minimized
...
Here, several additions have been made:
...
We've also introduced a new function called `primaryOutput()`, which will be used to determine the output in which the surface is most visible before it's minimized. Additionally, we'll store the position of the surface relative to that output as a percentage before it's minimized. This will allow us to restore the surface's position later even if the arrangement of outputs changes during the time it's minimized.

The `capture()` function will be used to render both the surface and its subsurfaces into a texture, which will serve as our thumbnail.
...
void ESurface::minimizedChanged()
{
    ...
    if (minimized())
    {
        ...
        if (minimizedOutput)
        {
            // Save the current local output pos so we can restore
            // it later even if the outputs arrangement change
            LPointF localPos = rolePos() - minimizedOutput->pos();
            prevMinimizedPos = localPos / LSizeF(minimizedOutput->size());
        }
        else
        {
            // Just in case, the surface is not currently visible in any output
            minimizedOutput = G::outputs().front();
            prevMinimizedPos = LPointF(0.25f, 0.25f);
        }

        LTexture *capturedTexture = capture();

        // Scale it to the thumbnail size
        thumbnail = capturedTexture->copyB(
            LSize((THUMBNAIL_HEIGHT * capturedTexture->sizeB().w())/capturedTexture->sizeB().h(), THUMBNAIL_HEIGHT) * 2);

        // We will use it later, meanwhile just delete it
        delete capturedTexture;
    ...
    {
        // This could happen if the output was uninitialized while
        ...
        // Restore back the prev unminimized position
...
EOutput *ESurface::primaryOutput()
{
    ...
    UInt32 bestArea = 0;
    LRect surfaceRect = LRect(rolePos(), size());
...
// Creates a capture of the surface including any subsurface
LTexture *ESurface::capture()
{
    // Making it always twice the size so it looks good
    // in both low and high DPI outputs
    LSceneView tmpScene(size() * 2, 2, nullptr);
    ...
    tmpScene.setPos(rolePos());

    // Add the view and any subsurface view to the scene
    view.setParent(&tmpScene);
    ESurface *next = this;
    while ((next = (ESurface*)nextSurface()))
        if (next->isSubchildOf(this) && next->subsurface())
            next->view.setParent(&tmpScene);

    tmpScene.render();

    // Restore views to the surfaces layer
    while (!tmpScene.children().empty())
        tmpScene.children().front()->setParent(&G::compositor()->surfacesLayer);

    // Return a copy of the rendered scene
    return tmpScene.texture()->copyB();
}
```

In the constructor, we've disabled the parent offset for the surface view using Louvre::LView::enableParentOffset().\n
Most view types typically default to a position of (0,0), and their position is inherently relative to their parent view.\n
By setting `Louvre::LView::enableParentOffset(false)`, we decouple the view's position from its parent's position.

Up until now, our surface views have been children of the surfaces layer, which has consistently remained at position (0,0). Consequently, we haven't encountered any positioning issues with our surface views.

However, we're now considering changing the parent of the surface views. This is why it's crucial to preserve the original surface position and prevent it from being influenced by its new parent's offset.

#### capture()

In the `capture()` function, we create a temporary Louvre::LSceneView with a buffer size twice the size of the surface (in surface coordinates) and set its buffer scale to 2. This ensures that the thumbnail will look good on both low and high DPI outputs. We don't assign a parent to this view because it's solely for offscreen rendering and shouldn't be displayed in our main scene.

Instead of moving the surface view and any subsurface view into the scene view, we position the scene view at the surface view's location. We also embed the surface view and any subsurface views within the temporary scene view. We use the Louvre::LSurface::isSubchildOf() method and check if they are subsurfaces, excluding other child surfaces like popups or even child toplevels.

Next, we simply call Louvre::LSceneView::render(), which renders the views into its framebuffer. Finally, we reattach the views to the surfaces layer and return a copy of the scene view framebuffer texture.\n
It's essential to note that we created the scene view in stack memory, so it's automatically destroyed after the function ends. 

#### minimizedChanged()

In the Louvre::LSurface::minimizedChanged() virtual method, we take the following actions:
...
2. When Louvre::LSurface::minimized() returns true, indicating that the surface is minimized:
   ...
   a. We determine the output where the surface is currently most visible using the `primaryOutput()` function. This could yield `nullptr` if the surface is not visible in any output, although it's an unusual scenario. If an output is found, we save it in the `minimizedOutput` variable.

   b. We calculate the current surface position relative to the output and as a percentage of its size. This enables us to restore the surface's position when unminimizing it. If the surface wasn't visible in any output, we opt for the first initialized one and set the position to 25% of its size.

   c. We use the `capture()` function to create a thumbnail of the surface, resizing the resulting texture to match `THUMBNAIL_HEIGHT` while preserving its aspect ratio. This thumbnail is stored in the `thumbnail` variable.
...
<img src="https://lh3.googleusercontent.com/pw/AIL4fc_lXcWQ_E5uPnZkmJs4R4jDbri_NT15jKGeNsN4hXURZjKXU13SyFybyd28tyDcylMwiV88lzpAM5qXFrkSjWNmxWL3bktw-UbmeNGlchPRbaEDPys=w2400"/>

Also, take the time to appreciate our dear biologist Humberto Maturana, may he rest in peace.

There's a scenario we haven't discussed yet, and it involves plugging or unplugging an output while surfaces are minimized. 

If a new output is initialized, we can easily copy the thumbnails from another output that has already been initialized.
...
And if an output becomes uninitialized, we can simply remove the thumbnails from its top bar and update the `minimizedOutput` variable for each minimized surface like this:
...
void EOutput::uninitializeGL()
{
    for (ESurface *surface : G::surfaces())
    {
        ...
        for (ETopbarItem *item : surface->topbarItems)
        {
            if (item->topbar == topbar)
            {
                delete item;
                break;
            }
...
Finally, in Louvre::LSurface::minimizedChanged(), we have already addressed the situation where a surface is unminimized, and `minimizedOutput` is `nullptr`.
