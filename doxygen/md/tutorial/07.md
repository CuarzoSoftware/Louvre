...
EOutput *ESurface::primaryOutput()
{
    ...
    Int32    bestArea     { 0 };
    LRect    surfaceRect  { rolePos(), size() };
...
void ESurface::minimizedChanged()
{
    ...
    if (minimized())
    {
        ...
        {
            LPointF localPos = rolePos() - outputBeforeMinimize->pos();
    ...
    else
    {
        ...
        setPos(outputBeforeMinimize->pos() + (relativePosBeforeMinimize * outputBeforeMinimize->size()));
...
The final step we need to take is to set the `outputBeforeMinimize` variable to `nullptr` when the output is uninitialized while the surface is minimized. This ensures that the surface correctly handles unminimization in Louvre::LSurface::minimizedChanged() and prevents our compositor from crashing.

> In Louvre::LSurface::minimizedChanged(), we have already addressed the situation where a surface is unminimized, and `outputBeforeMinimize` is `nullptr`.
...
{
    /* Set outputBeforeMinimize to nullptr to prevent the compositor 
     * from crashing when a surface is unminimized */
    for (ESurface *surface : G::surfaces())
        if (surface->outputBeforeMinimize == this)
            surface->outputBeforeMinimize = nullptr;
