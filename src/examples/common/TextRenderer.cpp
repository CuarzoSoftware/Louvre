...
TextRenderer *TextRenderer::loadFont(const char *fontName) {
  ...
  if (renderer->loadedFont) return renderer;
...
std::string TextRenderer::clipText(const char *text, Int32 fontSize,
                                   Int32 maxWidth, LSize &unclippedSize) {
  std::string str{""};
...
LTexture *TextRenderer::renderText(const char *text, Int32 fontSize,
  ...
  if (!character) {
    if (clippedText != text) delete[] clippedText;
  ...
  while (*character != 0) {
    ...
    y_off = face->size->metrics.ascender / 64 -
    ...
    for (UInt32 y = 0; y < face->glyph->bitmap.rows; y++) {
      for (UInt32 x = 0; x < face->glyph->bitmap.width; x++) {
        Int64 buffIndex =
            ((y + y_off) * bufferSize.width() + totalAdvanceX + x_off + x) * 4;
        buffer[buffIndex] = r;
  ...
  if (!texture->setDataFromMainMemory(bufferSize, bufferSize.w() * 4,
                                      DRM_FORMAT_ABGR8888, buffer)) {
    if (clippedText != text) delete[] clippedText;
...
UChar32 *TextRenderer::toUTF32(const char *utf8Str) {
  ...
  u_strFromUTF8(utf16Str, str16Capacity, &utf16Len, utf8Str, utf8Len,
                &errorCode);
  ...
  u_strToUTF32(utf32Str, str32Capacity, &utf32Len, utf16Str, utf16Len,
               &errorCode);
