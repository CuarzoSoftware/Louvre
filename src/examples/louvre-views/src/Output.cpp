...
Output::Output(const void *params) noexcept : LOutput(params) {
  ...
  wallpaper.enableSrcRect(compositor()->graphicBackendId() ==
...
void Output::moveGL() {
  ...
  if (!workspacesAnimation.running() && currentWorkspace &&
      currentWorkspace->toplevel) {
...
void Output::paintGL() {
  ...
  if (tryFullscreenScanoutIfNoOverlayContent()) return;
...
void Output::uninitializeGL() {
  ...
  for (Surface *s : G::surfaces()) {
    ...
    if (s->minimizedOutput == this) {
      ...
      s->minimizeStartRect.setPos(
          LPoint(rand() % 128, TOPBAR_HEIGHT + (rand() % 128)));
    }
  }
...
void Output::setWorkspace(Workspace *ws, UInt32 animMs, Float64 curve,
                          Float64 start) {
  ...
  if (ws && currentWorkspace->toplevel && currentWorkspace->toplevel->surf()) {
    ...
    currentWorkspace->toplevel->configureState(
        currentWorkspace->toplevel->pendingConfiguration().state |
    ...
    for (auto it = compositor()->layer(LSurfaceLayer::LLayerMiddle).rbegin();
      ...
      if ((*it)->toplevel() && (*it)->mapped() && !(*it)->minimized()) {
        foundSurface = true;
        (*it)->toplevel()->configureState(
            (*it)->toplevel()->pendingConfiguration().state |
            LToplevelRole::Activated);
    ...
    if (!foundSurface) seat()->keyboard()->setFocus(nullptr);
  ...
  for (Workspace *ws : workspaces) {
    if (ws->nativePos().x() != offset) {
      ...
      if (ws->toplevel) {
        ...
        ws->toplevel->configureState(
            ws->toplevel->pendingConfiguration().state);
      }
    }
...
void Output::updateFractionalOversampling() {
  ...
  if (!usingFractionalScale() || doingFingerWorkspaceSwipe ||
      workspacesAnimation.running())
    goto checkChange;

  if (currentWorkspace->toplevel) {
    if (currentWorkspace->toplevel->decoratedView &&
        currentWorkspace->toplevel->decoratedView->fullscreenTopbarVisibility !=
            0.f) {
    ...
    for (LSurface *surf : currentWorkspace->toplevel->surf()->children()) {
      if (surf->toplevel() &&
          surf->toplevel()->decorationMode() == LToplevelRole::ServerSide &&
    ...
    for (Surface *surf : G::surfaces()) {
      if (!surf->mapped() || surf->minimized()) continue;
...
bool Output::tryFullscreenScanoutIfNoOverlayContent() noexcept {
  ...
  if (sessionLockRole() && sessionLockRole()->surface()->mapped())
    fullscreenSurface = sessionLockRole()->surface();
           ...
           (!currentWorkspace->toplevel->decoratedView ||
            currentWorkspace->toplevel->decoratedView
                    ->fullscreenTopbarVisibility == 0.f))
  ...
  if (!fullscreenSurface ||
      ...
      !screenshotRequests().empty() || doingFingerWorkspaceSwipe ||
      workspacesAnimation.running() || dock.visiblePercent != 0.f ||
      hasMappedChildren(fullscreenSurface) || zoom != 1.f ||
      (shelf && shelf->surface()->size().h() + shelf->margins().bottom > 4))
    return false;

  const bool ret{setCustomScanoutBuffer(fullscreenSurface->texture())};

  if (ret) fullscreenSurface->requestNextFrame(true);

  return ret;
}

void Output::showAllWorkspaces() {
  for (auto *ws : workspaces) ws->show(true);
}

void Output::hideAllWorkspacesExceptCurrent() {
  for (auto *ws : workspaces)
    if (ws != currentWorkspace) ws->show(false);
}
...
void Output::onWorkspacesAnimationUpdate(LAnimation *anim) noexcept {
  ...
  // Early stop
  if (linear >= 0.85) linear = 1.0;

  Float64 ease{1.0 - pow(animStartOffset + (1.0 - animStartOffset) * linear,
                         animEasingCurve)};
  workspacesPosX = workspacesPosX * ease +
                   Float64(-currentWorkspace->nativePos().x()) * (1.0 - ease);
  workspacesContainer.setPos(workspacesPosX, 0);

  for (Output *o : G::outputs())
    for (Workspace *workspace : o->workspaces) workspace->clipChildren();
  ...
  if (animatedFullscreenToplevel) {
    ...
    if (tl->fullscreen()) {
       ...
       * Quick Reminder:
       *
       * - captureView: Displays a capture of the toplevel and subsurfaces
       * before going fullscreen.
       * - animScene: Blits the current state of the toplevel and subsurfaces
       * into a texture (can change each frame, e.g., if it is a web browser
       * displaying a video).
      ...
      LSizeF transRegionScale =
          LSizeF(cSize) / LSizeF(tl->prevBoundingRect.size());
      ...
      if (tl->decoratedView)
        tl->surf()->setPos(
            LPoint() - (LPoint(box.x1, box.y1) - tl->animScene->nativePos()));
      else
      ...
      tl->animView.setSrcRect(
          LRectF(0, boxSize * tl->animScene->bufferScale()));
    ...
    if (tl->decoratedView) tl->decoratedView->updateGeometry();
  }

  workspaceAnimationInFirstFrame = false;

  if (linear == 1.0) anim->stop();
}
...
void Output::onWorkspacesAnimationFinish(LAnimation * /*anim*/) noexcept {
  ...
  if (currentWorkspace->toplevel) {
    ...
    if (tl->destructorCalled || tl->quickUnfullscreen) goto returnChildren;
  ...
  if (animatedFullscreenToplevel) {
    ...
    if (tl->fullscreen()) {
      if (tl->decoratedView) {
        ...
        tl->decoratedView->topbarInput.setParent(
            &tl->fullscreenWorkspace->overlay);
        tl->decoratedView->buttonsContainer.setParent(
            &tl->fullscreenWorkspace->overlay);
      }

      tl->surf()->setPos(pos().x(), 0);
      G::reparentWithSubsurfaces(tl->surf(),
                                 &tl->fullscreenWorkspace->surfaces);
...
void Output::updateWallpaper() noexcept {
  ...
  if (!G::textures()->wallpaper || size().area() == 0) return;
  ...
  if (compositor()->graphicBackendId() == LGraphicBackendDRM) {
    ...
    if (scaledWallpaper && scaledWallpaper->sizeB() == bufferSize) return;

    LRect srcB;
    const Float32 w{
        Float32(bufferSize.w() * G::textures()->wallpaper->sizeB().h()) /
    ...
    if (w >= G::textures()->wallpaper->sizeB().w()) {
      ...
      srcB.setH((G::textures()->wallpaper->sizeB().w() * bufferSize.h()) /
                bufferSize.w());
      srcB.setY((G::textures()->wallpaper->sizeB().h() - srcB.h()) / 2);
    } else {
      srcB.setY(0);
      srcB.setH(G::textures()->wallpaper->sizeB().h());
      srcB.setW((G::textures()->wallpaper->sizeB().h() * bufferSize.w()) /
                bufferSize.h());
    ...
    if (outputScaledHeight >= G::textures()->wallpaper->sizeB().h()) {
      ...
      wallpaper.setSrcRect(LRectF((texSize.w() - outputScaledWidth) / 2, 0.f,
                                  outputScaledWidth, texSize.h()));
    } else
      wallpaper.setSrcRect(LRectF(0.f, (texSize.h() - outputScaledHeight) / 2,
...
void Output::zoomedDrawBegin() noexcept {
  ...
  const LPointF outputNormalizedCursorPos{outputRelativeCursorPos /
                                          LSizeF(size())};
  LPoint newPos{cursor()->pos() -
                (LSizeF(zoomScene.nativeSize()) * outputNormalizedCursorPos)};
