...
ToplevelButton::ToplevelButton(LView *parent, ToplevelView *toplevelView,
                               ButtonType type)
    : UITextureView(G::ButtonDisabled, parent) {
...
void ToplevelButton::update() {
  if (buttonType == Close) {
    ...
    bool altMode = !seat()->keyboard()->isKeyCodePressed(KEY_LEFTALT) ||
                   toplevelView->toplevel->fullscreen();
...
void ToplevelButton::pointerButtonEvent(const LPointerButtonEvent &event) {
  if (event.button() != LPointerButtonEvent::Left) return;
...
void ToplevelButton::pointerMoveEvent(const LPointerMoveEvent &) {
  if (!toplevelView->toplevel->resizeSession().isActive() &&
      !toplevelView->toplevel->moveSession().isActive())
    cursor()->useDefault();
}
