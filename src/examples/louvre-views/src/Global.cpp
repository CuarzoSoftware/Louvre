...
LScene *G::scene() { return &compositor()->scene; }

Pointer *G::pointer() { return (Pointer *)compositor()->seat()->pointer(); }
...
void G::enableDocks(bool enabled) {
  for (Output *o : outputs()) o->dock.setVisible(enabled);
}
...
void G::loadApps() {
  ...
  while (true) {
    // Read the app's name
    if (!fgets(appName, sizeof(appName), appsListFile)) break;
    ...
    if (!fgets(appExec, sizeof(appExec), appsListFile)) break;
    ...
    if (!fgets(appIcon, sizeof(appIcon), appsListFile)) break;
...
std::vector<App *> &G::apps() { return _apps; }

void G::setShelf(LClient *client) {
  _shelf.reset(client);

  for (Output *o : outputs()) o->dock.setVisible(client == nullptr);
}

LClient *G::shelf() { return _shelf; }

void G::createTooltip() { _tooltip = new Tooltip(); }

Tooltip *G::tooltip() { return _tooltip; }

void G::loadCursors() {
  xCursors.arrow = LXCursor::load("arrow");

  if (xCursors.arrow)
    cursor()->replaceDefaultB(xCursors.arrow->texture(),
                              xCursors.arrow->hotspotB());
...
G::Cursors &G::cursors() { return xCursors; }
...
void G::loadTextures() {
  ...
  _textures.wallpaper =
      LOpenGL::loadTexture(std::filesystem::path(getenvString("HOME")) /
                           ".config/Louvre/wallpaper.jpg");
    ...
    if (compositor()->graphicBackendId() == LGraphicBackendDRM) {
      ...
      for (LOutput *output : seat()->outputs())
        if (output->currentMode()->sizeB().area() > bestSize.area() &&
            output->currentMode()->sizeB().area() <
                _textures.wallpaper->sizeB().area())
      ...
      if (bestSize.area() != 0) {
        ...
        if (srcRect.w() <= _textures.wallpaper->sizeB().w()) {
          ...
          srcRect.setH((bestSize.h() * _textures.wallpaper->sizeB().w()) /
                       bestSize.w());
...
G::ToplevelRegions *G::toplevelRegions() { return &_toplevelRegions; }
...
void G::loadFonts() {
  ...
  if (_fonts.semibold)
    _textures.defaultTopbarAppName =
        G::font()->semibold->renderText("Louvre", 24);
}

G::Fonts *G::font() { return &_fonts; }

const char *G::transformName(LTransform transform) {
  switch (transform) {
    case LTransform::Normal: return "No Transform";
    case LTransform::Rotated90: return "Rotated 90°";
    case LTransform::Rotated180: return "Rotated 180°";
    case LTransform::Rotated270: return "Rotated 270°";
    case LTransform::Flipped: return "Flipped";
    case LTransform::Flipped90: return "Flipped & Rotated 90°";
    case LTransform::Flipped180: return "Flipped & Rotated 180°";
    case LTransform::Flipped270: return "Flipped & Rotated 270°";
  }

  return NULL;
}

LTexture *G::loadAssetsTexture(const std::filesystem::path &file,
                               bool exitOnFail) {
...
void G::reparentWithSubsurfaces(Surface *surf, LView *newParent,
                                bool onlySubsurfaces) {
  surf->getView()->setParent(newParent);

  for (Surface *s : surfaces()) {
    if (s->parent() == surf && s->roleId() != LSurface::Undefined &&
        !s->cursorRole()) {
...
void G::arrangeOutputs(Output *caller) noexcept {
  ...
  std::sort(vec.begin(), vec.end(),
            [](Output *a, Output *b) { return a->pos().x() < b->pos().x(); });
...
void G::repositionNonVisibleToplevelChildren(Output *target,
                                             Surface *toplevel) {
  for (Surface *s : (std::list<Surface *> &)toplevel->children()) {
    ...
    if (!LRect(box.x1, box.y1, box.x2 - box.x1, box.y2 - box.y1)
