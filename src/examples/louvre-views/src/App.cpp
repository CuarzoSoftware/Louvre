...
static inline Float32 periodicEasing(Float32 t, Float32 exponent_in,
                                     Float32 exponent_out) {
...
App::App(const char *appName, const char *appExec, const char *iconPath)
    : launchAnimation(
          ...
          [this](LAnimation *anim) {
            ...
            for (DockApp *dapp : dockApps) dapp->dock->update();
          },
          [this](LAnimation *) {
  ...
  if (iconPath) tmp = LOpenGL::loadTexture(iconPath);
  ...
  if (!texture) texture = G::textures()->defaultAppIcon;

  for (Output *output : G::outputs()) new DockApp(this, &output->dock);
...
App::~App() {
  ...
  while (!dockApps.empty()) delete dockApps.back();

  if (nameTexture) delete nameTexture;

  if (!pinned) LVectorRemoveOneUnordered(G::apps(), this);
}
        ...
        if (surf->minimized()) {
          ...
          if (surf->toplevel()->fullscreen()) {
            ...
            if (tl->fullscreenOutput && tl->fullscreenWorkspace) {
              tl->fullscreenOutput->setWorkspace(tl->fullscreenWorkspace, 600,
                                                 4.f);
              return;
            }
          } else {
            surf->toplevel()->configureState(
                surf->toplevel()->pendingConfiguration().state |
