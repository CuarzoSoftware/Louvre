...
class Compositor final : public LCompositor {
 public:
  ...
  LFactoryObject *createObjectRequest(LFactoryObject::Type type,
  ...
  // If true, we call scene->handlePointerEvent() once before
  // scene->handlePaintGL(). The reason for this is that pointer events are only
  // emitted when the pointer itself moves, and they do not trigger when, for
  // example, a view moves and positions itself under the cursor. As a result,
  // this call updates focus, cursor texture and so on...in those cases
