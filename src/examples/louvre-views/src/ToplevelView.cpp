...
static void onPointerEnterResizeArea(InputRect *rect, void *data,
                                     const LPoint &) {
  ...
  if (data) {
    G::compositor()->cursor()->setTextureB(cursor->texture(),
                                           cursor->hotspotB());
  ...
  // Topbar input
    ...
    if (view->toplevel->fullscreen() && !view->fullscreenTopbarAnim.running() &&
        view->fullscreenTopbarVisibility == 0.f) {
      // Stop if the user is interacting with a popup
      if (seat()->keyboard()->grab()) return;

      view->fullscreenTopbarAnim.setDuration(TOPBAR_ANIM_MS);

      view->fullscreenTopbarAnim.setOnUpdateCallback([view](LAnimation *anim) {
        view->fullscreenTopbarVisibility = 1.f - powf(1.f - anim->value(), 3.f);
        view->updateGeometry();
        G::compositor()->repaintAllOutputs();
      });

      view->fullscreenTopbarAnim.setOnFinishCallback([view](LAnimation *anim) {
...
static void onPointerLeaveResizeArea(InputRect *rect, void *data) {
  ...
  if (data) {
    ...
    if (view->toplevel->fullscreen() && !view->fullscreenTopbarAnim.running() &&
        view->fullscreenTopbarVisibility == 1.f) {
      view->fullscreenTopbarAnim.setDuration(TOPBAR_ANIM_MS);
...
static void onPointerButtonResizeArea(InputRect *rect, void *data,
  ...
  if (button != LPointerButtonEvent::Left) return;
  ...
  if (pointer->cursorOwner == view) pointer->cursorOwner = nullptr;
}
...
ToplevelView::ToplevelView(Toplevel *toplevel)
    : LLayerView(&G::compositor()->surfacesLayer),
      ...
      sceneBL(LSize(TOPLEVEL_BORDER_RADIUS, TOPLEVEL_BORDER_RADIUS) * 2, 2,
              this),
      sceneBR(LSize(TOPLEVEL_BORDER_RADIUS, TOPLEVEL_BORDER_RADIUS) * 2, 2,
              this),
      ...
      resizeBR(this, G::cursors().bottom_right_corner,
               LEdgeBottom | LEdgeRight),
  ...
  // Make them not primary so that the damage of the surface won't be cleared
  // after rendered
  ...
  // This blending func makes the alpha of the toplevel be replaced by the one
  // of the mask
  ...
  // Buttons
  buttonsContainer.setPos(TOPLEVEL_BUTTON_SPACING,
                          TOPLEVEL_BUTTON_SPACING - TOPLEVEL_TOPBAR_HEIGHT);
  buttonsContainer.setSize(
      3 * TOPLEVEL_BUTTON_SIZE + 2 * TOPLEVEL_BUTTON_SPACING,
      TOPLEVEL_BUTTON_SIZE);
  ...
  maximizeButton.setPos(2 * (TOPLEVEL_BUTTON_SIZE + TOPLEVEL_BUTTON_SPACING),
                        0);

  buttonsContainer.onPointerEnter = [](InputRect *, void *data,
                                       const LPoint &) {
    ToplevelView *view = (ToplevelView *)data;
    Pointer *pointer = (Pointer *)seat()->pointer();

    if (!seat()->toplevelResizeSessions().empty()) return;

    view->closeButton.update();
    view->minimizeButton.update();
    view->maximizeButton.update();

    if (!pointer->cursorOwner) cursor()->useDefault();
  };
  ...
  if (parent()) {
    class Toplevel *tl = G::searchFullscreenParent(
        (class Surface *)toplevel->surface()->parent());

    if (tl) {
      setParent(((class Surface *)tl->surface())->getView()->parent());
      enableParentOffset(true);

      for (Workspace *ws : tl->fullscreenOutput->workspaces) ws->clipChildren();
    }
  }
...
ToplevelView::~ToplevelView() {
  ...
  if (title.texture()) delete title.texture();
}

void ToplevelView::updateTitle() {
  if (!G::font()->semibold) return;

  Int32 maxWidth = (toplevel->windowGeometry().w() - 128) * 2;
  std::string newClippedTitle{G::font()->semibold->clipText(
      toplevel->title().c_str(), 28, maxWidth, unclippedTitleBufferSize)};

  if (newClippedTitle == prevClippedTitle) return;

  prevClippedTitle = std::move(newClippedTitle);

  LTexture *newTitle{
      G::font()->semibold->renderText(prevClippedTitle.c_str(), 28, -1)};
...
void ToplevelView::updateGeometry() {
  ...
  if (toplevel->windowGeometry().size().area() == 0) return;

  int TOPLEVEL_TOP_LEFT_OFFSET_X, TOPLEVEL_TOP_LEFT_OFFSET_Y,
      TOPLEVEL_BOTTOM_LEFT_OFFSET_X, TOPLEVEL_BOTTOM_LEFT_OFFSET_Y,
      TOPLEVEL_TOP_CLAMP_OFFSET_Y, TOPLEVEL_MIN_WIDTH_TOP,
      TOPLEVEL_MIN_WIDTH_BOTTOM, TOPLEVEL_MIN_HEIGHT;
  ...
  if (toplevel->fullscreen()) {
    if (!toplevel->fullscreenOutput) return;
    ...
    surf->view.setCustomPos((toplevel->fullscreenOutput->size() -
    ...
    decoT.setPos(toplevel->fullscreenOutput->pos().x(),
                 -(decoT.nativeSize().h() + 1) +
                     (TOPLEVEL_TOPBAR_HEIGHT + TOPLEVEL_TOP_CLAMP_OFFSET_Y) *
                         fullscreenTopbarVisibility);
    buttonsContainer.setPos(
        toplevel->fullscreenOutput->pos().x() + TOPLEVEL_BUTTON_SPACING,
        TOPLEVEL_BUTTON_SPACING -
            TOPLEVEL_TOPBAR_HEIGHT * (1.01f - fullscreenTopbarVisibility));

    // Set topbar center translucent regions
    LRegion transT;
    transT.addRect(0, 0, decoT.size().w(),
                   decoT.size().h() - TOPLEVEL_TOPBAR_HEIGHT -
                       TOPLEVEL_TOP_CLAMP_OFFSET_Y);
    transT.addRect(0, decoT.size().h() - TOPLEVEL_TOP_CLAMP_OFFSET_Y,
                   decoT.size().w(), TOPLEVEL_TOP_CLAMP_OFFSET_Y);

    decoT.setTranslucentRegion(&transT);

    topbarInput.setPos(
        toplevel->fullscreenOutput->pos().x(),
        -(TOPLEVEL_TOPBAR_HEIGHT) * (1.f - fullscreenTopbarVisibility));
    ...
    const Int32 clip{0};
    const LSize tlSize{toplevel->windowGeometry().size().w() < 60
                           ? 60
                           : toplevel->windowGeometry().size().w(),
                       toplevel->windowGeometry().size().h() < 60
                           ? 60
                           : toplevel->windowGeometry().size().h()};
    ...
    // Upper surface view
    clipTop.setSize(size.w(), size.h() - TOPLEVEL_BORDER_RADIUS);
    surf->view.setCustomPos(-clip, -clip);

    // Lower surface view (without border radius rects)
    clipBottom.setPos(TOPLEVEL_BORDER_RADIUS,
                      size.h() - TOPLEVEL_BORDER_RADIUS);
    clipBottom.setSize(size.w() - 2 * TOPLEVEL_BORDER_RADIUS,
                       TOPLEVEL_BORDER_RADIUS);
    surfB.setCustomPos(-TOPLEVEL_BORDER_RADIUS - clip,
                       TOPLEVEL_BORDER_RADIUS - size.h() - clip);

    // Bottom left / right surfaces views
    sceneBL.setPos(0, size.h() - TOPLEVEL_BORDER_RADIUS);
    sceneBR.setPos(size.w() - TOPLEVEL_BORDER_RADIUS,
                   size.h() - TOPLEVEL_BORDER_RADIUS);
    surfBL.setCustomPos(-clip, TOPLEVEL_BORDER_RADIUS - size.h() - clip);
    surfBR.setCustomPos(TOPLEVEL_BORDER_RADIUS - size.w() - clip,
                        TOPLEVEL_BORDER_RADIUS - size.h() - clip);

    // Decorations
    decoTL.setPos(TOPLEVEL_TOP_LEFT_OFFSET_X, TOPLEVEL_TOP_LEFT_OFFSET_Y);
    decoT.setDstSize(size.w() - TOPLEVEL_MIN_WIDTH_TOP, decoT.nativeSize().h());
    decoT.setPos(decoTL.nativePos().x() + decoTL.nativeSize().w(),
    ...
    decoL.setDstSize(decoL.nativeSize().w(), size.h() - TOPLEVEL_MIN_HEIGHT);
    decoL.setPos(-decoL.nativeSize().w(),
                 decoTL.nativePos().y() + decoTL.nativeSize().h());
    decoR.setDstSize(decoR.nativeSize().w(), size.h() - TOPLEVEL_MIN_HEIGHT);
    decoR.setPos(size.w(), decoTL.nativePos().y() + decoTL.nativeSize().h());
    decoBL.setPos(TOPLEVEL_BOTTOM_LEFT_OFFSET_X,
                  size.h() + TOPLEVEL_BOTTOM_LEFT_OFFSET_Y);
    decoBR.setPos(
        size.w() - decoBR.nativeSize().w() - TOPLEVEL_BOTTOM_LEFT_OFFSET_X,
        size.h() + TOPLEVEL_BOTTOM_LEFT_OFFSET_Y);
    decoB.setDstSize(size.w() - TOPLEVEL_MIN_WIDTH_BOTTOM,
                     decoB.nativeSize().h());
    decoB.setPos(decoBL.nativePos().x() + decoBL.nativeSize().w(), size.h());

    // Set topbar center translucent regions
    LRegion transT;
    transT.addRect(0, 0, decoT.size().w(),
                   decoT.size().h() - TOPLEVEL_TOPBAR_HEIGHT -
                       TOPLEVEL_TOP_CLAMP_OFFSET_Y + 1);
    transT.addRect(  // Bottom line
        0, decoT.size().h() - TOPLEVEL_TOP_CLAMP_OFFSET_Y, decoT.size().w(),
        TOPLEVEL_TOP_CLAMP_OFFSET_Y);
    ...
    resizeL.setSize(TOPLEVEL_RESIZE_INPUT_MARGIN * 2,
                    size.h() + TOPLEVEL_TOPBAR_HEIGHT);

    resizeR.setPos(size.w() - TOPLEVEL_RESIZE_INPUT_MARGIN,
                   -TOPLEVEL_TOPBAR_HEIGHT);
    resizeR.setSize(TOPLEVEL_RESIZE_INPUT_MARGIN * 2,
                    size.h() + TOPLEVEL_TOPBAR_HEIGHT);

    resizeTL.setPos(-TOPLEVEL_RESIZE_INPUT_MARGIN,
                    -TOPLEVEL_TOPBAR_HEIGHT - TOPLEVEL_RESIZE_INPUT_MARGIN);
    resizeTL.setSize(TOPLEVEL_RESIZE_INPUT_MARGIN + TOPLEVEL_BORDER_RADIUS,
                     TOPLEVEL_RESIZE_INPUT_MARGIN + TOPLEVEL_BORDER_RADIUS);

    resizeTR.setPos(size.w() - TOPLEVEL_BORDER_RADIUS,
                    -TOPLEVEL_TOPBAR_HEIGHT - TOPLEVEL_RESIZE_INPUT_MARGIN);
    resizeTR.setSize(TOPLEVEL_RESIZE_INPUT_MARGIN + TOPLEVEL_BORDER_RADIUS,
                     TOPLEVEL_RESIZE_INPUT_MARGIN + TOPLEVEL_BORDER_RADIUS);

    resizeBL.setPos(-TOPLEVEL_RESIZE_INPUT_MARGIN,
                    size.h() - TOPLEVEL_BORDER_RADIUS);
    resizeBL.setSize(TOPLEVEL_RESIZE_INPUT_MARGIN + TOPLEVEL_BORDER_RADIUS,
                     TOPLEVEL_RESIZE_INPUT_MARGIN + TOPLEVEL_BORDER_RADIUS);

    resizeBR.setPos(size.w() - TOPLEVEL_BORDER_RADIUS,
                    size.h() - TOPLEVEL_BORDER_RADIUS);
    resizeBR.setSize(TOPLEVEL_RESIZE_INPUT_MARGIN + TOPLEVEL_BORDER_RADIUS,
                     TOPLEVEL_RESIZE_INPUT_MARGIN + TOPLEVEL_BORDER_RADIUS);
  ...
  if (title.texture()) {
    ...
    Int32 h{topbarInput.size().h() -
            (TOPLEVEL_TOPBAR_HEIGHT + title.size().h()) / 2};

    if (toplevel->fullscreen()) h -= 2;
...
void ToplevelView::keyEvent(const LKeyboardKeyEvent &event) {
  if (event.keyCode() == KEY_LEFTALT) maximizeButton.update();
}
