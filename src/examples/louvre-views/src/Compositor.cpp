...
void Compositor::initialized() {
  ...
  clockMinuteTimer.setCallback([](LTimer *timer) {
    if (G::font()->regular) {
      ...
      if (newClockTexture) {
        ...
        if (G::compositor()->clockTexture) delete G::compositor()->clockTexture;
  ...
  oversamplingLabelTexture =
      G::font()->semibold->renderText("OVERSAMPLING", 22);
  ...
  if (outputs().empty()) {
    LLog::fatal(
        "[louvre-views] Failed to initialize outputs. Try launching the "
        "compositor from a free TTY or within a Wayland compositor.");
    finish();
  }
...
void Compositor::uninitialized() {
  for (Output *o : G::outputs()) o->workspacesAnimation.stop();

  for (Client *c : (std::vector<Client *> &)clients())
    if (c->pid != -1) kill(c->pid, SIGKILL);
}

LFactoryObject *Compositor::createObjectRequest(LFactoryObject::Type type,
                                                const void *params) {
  if (type == LFactoryObject::Type::LSurface) return new Surface(params);

  if (type == LFactoryObject::Type::LToplevelRole) return new Toplevel(params);

  if (type == LFactoryObject::Type::LPopupRole) return new Popup(params);

  if (type == LFactoryObject::Type::LLayerRole) return new LayerRole(params);

  if (type == LFactoryObject::Type::LClient) return new Client(params);

  if (type == LFactoryObject::Type::LOutput) return new Output(params);

  if (type == LFactoryObject::Type::LSeat) return new Seat(params);

  if (type == LFactoryObject::Type::LPointer) return new Pointer(params);

  if (type == LFactoryObject::Type::LKeyboard) return new Keyboard(params);

  if (type == LFactoryObject::Type::LTouch) return new Touch(params);
...
void Compositor::fadeOutSurface(LBaseSurfaceRole *role, UInt32 ms) {
  if (role->surface()) {
    ...
    LTextureView *fadeOutView =
        new LTextureView(surf->renderThumbnail(), &fullscreenLayer);
...
Int32 Compositor::millisecondsUntilNextMinute() {
  ...
  int msUntilNextMinute =
      secondsUntilNextMinute * 1000 - spec.tv_nsec / 1000000;
...
bool Compositor::checkUpdateOutputUnplug() {
  if (!outputUnplugHandled) {
    ...
    for (Surface *s : G::surfaces()) {
      if (s->outputUnplugHandled) continue;

      if (s->toplevel()) {
        Toplevel *tl = (Toplevel *)s->toplevel();

        if (tl->state().check(LToplevelRole::Fullscreen |
    ...
    if (outputUnplugHandled) {
      for (Output *o : G::outputs()) G::scene()->mainView()->damageAll(o);
    }
  }
