...
void Dock::initialize() noexcept {
  ...
  // Create app items
  for (App *app : G::apps()) new DockApp(app, this);
...
void Dock::update() {
  ...
  for (LView *item : appsContainer.children()) {
    if (item->userData() == DockSeparatorType) {
      static_cast<LSolidColorView *>(item)->setPos(
          dockWidth, DOCK_PADDING + (DOCK_ITEM_HEIGHT - item->size().h()) / 2);
      dockWidth += DOCK_SPACING;
    } else if (item->userData() == DockAppType) {
      DockApp &dockApp{static_cast<DockApp &>(*item)};
      dockApp.setPos(dockWidth, -2 - dockApp.app->dockAppsAnimationOffset.y() +
                                    DOCK_PADDING +
                                    (DOCK_ITEM_HEIGHT - item->size().h()) / 2);
      dockApp.dot.setVisible(dockApp.app->state == App::Running &&
                             dockApp.app->dockAppsAnimationOffset.y() == 0);

      if (pointerIsOver() && G::tooltip()->targetView == item)
        G::tooltip()->show(item->pos().x() + item->size().w() / 2,
                           item->pos().y());
    }

    dockWidth += item->size().w();

    if (item != appsContainer.children().back()) dockWidth += DOCK_SPACING;
  }

  for (LView *it : itemsContainer.children()) {
    LTextureView &item{static_cast<LTextureView &>(*it)};

    item.setPos(dockWidth,
                DOCK_PADDING + (DOCK_ITEM_HEIGHT - item.size().h()) / 2);
    dockWidth += item.size().w();

    if (it != itemsContainer.children().back()) dockWidth += DOCK_SPACING;
  ...
  setSize(output->rect().w(), DOCK_HEIGHT + 6);
  ...
  dockCenter.setDstSize(dockWidth - 2 * DOCK_BORDER_RADIUS,
                        dockCenter.nativeSize().h());
  dockRight.setPos(dockCenter.nativePos().x() + dockCenter.size().w(), 0);
  dockContainer.setSize(
      dockLeft.size().w() + dockCenter.size().w() + dockRight.size().w(),
      DOCK_HEIGHT);
...
void Dock::show() {
  ...
  if (anim.running() || visiblePercent != 0.f) return;
  ...
  anim.setOnUpdateCallback([this](LAnimation *anim) {
    visiblePercent = 1.f - powf(1.f - anim->value(), 2.f);
    update();
    output->repaint();
  });

  anim.setOnFinishCallback([this](LAnimation *) {
    if (!pointerIsOver()) {
      hide();
      return;
    }
    ...
    input.addRect(dockContainer.nativePos().x() + DOCK_SHADOW_SIZE / 2, 0,
...
void Dock::hide() {
  if (anim.running() || visiblePercent != 1.f) return;
...
void Dock::pointerLeaveEvent(const LPointerLeaveEvent &) {
  if (G::pointer()->cursorOwner == this) G::pointer()->cursorOwner = nullptr;
