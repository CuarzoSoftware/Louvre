...
void Keyboard::keyEvent(const LKeyboardKeyEvent &event) {
  ...
  if (LEFT_SHIFT && LEFT_META && event.state() == LKeyboardKeyEvent::Pressed) {
    ...
    if (event.keyCode() == KEY_O) {
      ...
      if (initOutputs) {
        for (LOutput *o : seat()->outputs()) compositor()->removeOutput(o);
      } else {
        for (LOutput *o : seat()->outputs()) compositor()->addOutput(o);
      }
    ...
    /**** Toggles the leasable() property for all outputs ****/
      ...
      for (LOutput *o : seat()->outputs()) o->setLeasable(leasable);
    ...
    /**** Toggles natural scrolling  ****/
      ...
      if (compositor()->inputBackendId() ==
          LInputBackendID::LInputBackendLibinput) {
        libinput_device *ldev;

        for (LInputDevice *dev : seat()->inputDevices()) {
          if (!dev->nativeHandle()) continue;
          ...
          libinput_device_config_scroll_set_natural_scroll_enabled(
              ldev, libinput_device_config_scroll_get_natural_scroll_enabled(
                        ldev) == 0
                        ? 1
                        : 0);
        }
  ...
  if (output && event.state() == LKeyboardKeyEvent::Pressed) {
    ...
    if (LEFT_ALT && LEFT_CTRL && output->currentWorkspace) {
      if (event.keyCode() == KEY_RIGHT &&
          std::next(output->currentWorkspace->outputLink) !=
              output->workspaces.end()) {
        if (!output->animatedFullscreenToplevel)
          output->setWorkspace(*std::next(output->currentWorkspace->outputLink),
                               450.f, 2.3f, 0.2f);
        return;
      } else if (event.keyCode() == KEY_LEFT &&
                 output->currentWorkspace != output->workspaces.front()) {
        if (!output->animatedFullscreenToplevel)
          output->setWorkspace(*std::prev(output->currentWorkspace->outputLink),
                               450.f, 2.3f, 0.2f);
        return;
      }
    }
    ...
    if (LEFT_META && LEFT_SHIFT) {
      ...
      if (isKeyCodePressed(KEY_L)) {
        if (event.keyCode() == KEY_DOWN && seat()->pointer()->focus() &&
            seat()->pointer()->focus()->layerRole()) {
          auto *layerRole{seat()->pointer()->focus()->layerRole()};
          if (layerRole->exclusiveZone().prevZone())
            layerRole->exclusiveZone().insertAfter(
                layerRole->exclusiveZone().prevZone()->prevZone());
          return;
        }

        if (event.keyCode() == KEY_UP && seat()->pointer()->focus() &&
            seat()->pointer()->focus()->layerRole()) {
          auto *layerRole{seat()->pointer()->focus()->layerRole()};
          if (layerRole->exclusiveZone().nextZone())
            layerRole->exclusiveZone().insertAfter(
                layerRole->exclusiveZone().nextZone());
          return;
        }
      }
      ...
      switch (event.keyCode()) {
        ...
        case KEY_M:
          if (output->currentMode() == output->modes().back())
            ...
            output->setMode(
                *(++std::find(output->modes().begin(), output->modes().end(),
                              output->currentMode())));
          break;
        ...
        /* This only prevents current managers from receiving information about
         * newly created toplevel windows */
        case KEY_F:
