...
Workspace::Workspace(Output *output, Toplevel *toplevel, Workspace *prev)
  ...
  if (prev)
    ...
    outputLink =
        output->workspaces.insert(std::next(output->workspaces.begin()), this);

  output->updateWorkspacesPos();
}

Workspace::~Workspace() { output->workspaces.erase(outputLink); }
...
void Workspace::returnChildren() {
  if (output->workspaces.front() == this) {
    ...
    for (class Surface *s : surfaces) s->raise();
...
void Workspace::clipChildren() {
  ...
  if (pos().x() + output->pos().x() < output->pos().x()) {
    ...
    if (rect.w() < 0) rect.setWidth(0);
  } else if (pos().x() + output->pos().x() >
             output->pos().x() + output->size().w()) {
