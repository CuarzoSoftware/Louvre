...
Surface::~Surface() {
  ...
  if (toplevel()) tl()->decoratedView.reset();

  while (!minimizedViews.empty()) delete minimizedViews.back();

  if (thumbnailFullsizeView) delete thumbnailFullsizeView;

  if (thumbnailFullSizeTex) delete thumbnailFullSizeTex;

  if (thumbnailTex) delete thumbnailTex;
}

Surface *Surface::searchSessionLockParent(Surface *parent) {
  if (parent) {
    if (parent->sessionLock()) return parent;
...
LView *Surface::getView() const {
  if (tl() && tl()->decoratedView) return tl()->decoratedView.get();
...
void Surface::parentChanged() {
  if (parent()) {
    Surface *sessionLockParent{
        searchSessionLockParent(static_cast<Surface *>(parent()))};
...
void Surface::mappingChanged() {
  ...
  if (mapped()) {
    ...
    if (tl() && tl()->decoratedView) tl()->decoratedView->updateTitle();
    ...
    if (client->app) {
      ...
      if (client->app->state != App::Running) client->app->state = App::Running;
    } else {  // If no App then is a non pinned dock App
    ...
    if (toplevel()) {
      ...
      while (next) {
        if (next->isSubchildOf(this)) next->requestNextFrame(false);

        next = next->nextSurface();
      }
    }

    if (dndIcon()) setPos(cursor()->pos());
    ...
    if ((!dndIcon() && !toplevel() && !subsurface()) ||
        (subsurface() && parent && parent->view.visible()))
      getView()->setVisible(true);
  } else {
    if (hasPointerFocus()) seat()->pointer()->setFocus(nullptr);

    if (tl()) {
      if (!tl()->fullscreen()) G::compositor()->fadeOutSurface(role(), 250);
    } else if (popup()) {
...
void Surface::bufferSizeChanged() { view.repaint(); }
...
void Surface::minimizedChanged() {
  if (minimized() && cursor()->output()) {
    ...
    thumbnailTex = thumbnailFullSizeTex->copy(
        LSize((s * thumbnailFullSizeTex->sizeB().w()) /
                  thumbnailFullSizeTex->sizeB().h(),
              s) *
        3.5f);

    // Create a view for thumbnailFullSizeTex (we only need one)
    thumbnailFullsizeView =
        new LTextureView(thumbnailFullSizeTex, getView()->parent());
    ...
    thumbnailFullsizeView->setDstSize(
        thumbnailFullsizeView->texture()->sizeB() /
        thumbnailFullsizeView->bufferScale());

    // Hide the surface as we will show thumbnailFullsizeView instead
    getView()->setVisible(false);

    // We will move the fullsize view to the dock where the cursor is currently
    // at
    ...
    for (Output *o : G::outputs()) {
      ...
      if (cursor()->output() == o) dstDockItem = minView;
    }

    minimizeAnim.setOnUpdateCallback([this, dstDockItem](LAnimation *anim) {
      ...
      thumbnailFullsizeView->setDstSize(
          (thumbnailFullsizeView->texture()->sizeB() /
           thumbnailFullsizeView->bufferScale()) *
          (1.f - x));
      thumbnailFullsizeView->setPos((dstDockItem->pos() + dstDockItem->size()) *
                                        x +
    ...
    if (minimizedOutput) {
      minimizedOutput->setWorkspace(minimizedOutput->workspaces.front(), 600,
                                    4.f);
...
void Surface::unminimize(DockItem *clickedItem) {
  ...
  minimizeAnim.setOnUpdateCallback([this, clickedItem](LAnimation *anim) {
    const Float32 x{1.f - powf(1.f - anim->value(), 3.f)};
    ...
    // Scale and move fullsize view to the dock
    thumbnailFullsizeView->setPos((clickedItem->pos() + clickedItem->size()) *
                                      (1.f - x) +
  ...
  minimizeAnim.setOnFinishCallback([this](LAnimation *) {
    ...
    if (G::tooltip()) G::tooltip()->hide();
  });

  minimizeAnim.start();
}

void Surface::damageChanged() { repaintOutputs(); }

void Surface::preferVSyncChanged() {
  if (tl() && tl()->fullscreenOutput &&
      tl()->fullscreenOutput->currentWorkspace == tl()->fullscreenWorkspace) {
...
void Surface::onToplevelFirstMap() noexcept {
  if (!cursor()->output() || !toplevel() || !mapped() || minimized()) return;
  ...
  if (pos().x() < outputPosG.x()) setX(outputPosG.x());

  if (pos().y() < outputPosG.y()) setY(outputPosG.y());

  if (!tl()->pendingConfiguration().state.check(LToplevelRole::Fullscreen) &&
      tl()->supportServerSideDecorations() &&
