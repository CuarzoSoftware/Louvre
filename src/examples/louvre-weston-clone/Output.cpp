...
{
    // Check if can access all buffers textures
    textureAccess = true;
...
void Output::paintWithTextureAccess()
{
    // Damage entire screen
    if (fullRefresh)
    {
        fullRefresh = false;
        damage.clear();
    ...
    list<Surface*> &surfaces = (list<Surface*>&)compositor()->surfaces();
    bool passFullscreen = false;
    LTexture *prevTex = bufferTexture(getPrevBufferIndex());
...
UInt32 Output::getPrevBufferIndex()
{
    if (currentBuffer() == 0)
        return buffersCount() - 1;
