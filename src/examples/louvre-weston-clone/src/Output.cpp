...
void Output::loadWallpaper() noexcept {
  ...
  if (!background)
    background.reset(LOpenGL::loadTexture(compositor()->defaultAssetsPath() /
                                          "wallpaper.png"));

  if (background) backgroundTexture.reset(background->copy(sizeB()));
}
...
void Output::fullDamage() noexcept {
  ...
  terminalIconRect.setSize(LSize(topbarExclusiveZone.size()) -
                           LSize(2 * terminalIconRect.pos().y()));
...
void Output::moveGL() noexcept { fullDamage(); }
...
void Output::paintGL() noexcept {
  ...
  std::list<Surface *> &surfaces{
      (std::list<Surface *> &)compositor()->surfaces()};
  ...
  if (seat()->pointer()->surfaceAt(cursor()->pos()) !=
      seat()->pointer()->focus())
  ...
  if (c->clock && c->clock->texture && redrawClock) {
    ...
    dstClockRect.setW(float(c->clock->texture->sizeB().w()) *
                      float(dstClockRect.h()) /
                      float(c->clock->texture->sizeB().h()));
    dstClockRect.setY(rect().y() + 9);
    dstClockRect.setX(rect().x() + rect().w() - dstClockRect.w() - 14);
    newDamage.addRect(dstClockRect);
  }

  if (seat()->dnd()->icon()) seat()->dnd()->icon()->surface()->raise();
  ...
  for (std::list<Surface *>::reverse_iterator it = surfaces.rbegin();
       it != surfaces.rend(); it++) {
    Surface *s = *it;
    ...
    for (LOutput *o : compositor()->outputs()) {
      if (o->rect().intersects(s->currentRect, false)) {
        ...
        if (o != this &&
            (s->outputsMap[o].lastRenderedDamageId < s->damageId())) {
          clearDamage = false;
          o->repaint();
        }
      } else
    ...
    if (!s->occluded && clearDamage) s->requestNextFrame();
  ...
  for (DestroyedToplevel &destroyed : c->destroyedToplevels) {
    ...
    for (LOutput *o : destroyed.outputs) o->repaint();
  }
  ...
  if (age > LOUVRE_WESTON_MAX_AGE) age = 0;
  ...
  if (age == 0) {
    ...
    for (Int32 i = 1; i < age; i++) {
      ...
      if (damageIndex < 0) damageIndex = LOUVRE_WESTON_MAX_AGE + damageIndex;
  ...
  for (std::list<Surface *>::reverse_iterator it = surfaces.rbegin();
    ...
    if (!s->isRenderable || s->occluded) continue;
  ...
  if (backgroundTexture) {
    ...
    p->setColor({.r = 0.15f, .g = 0.25f, .b = 0.35f});
  ...
  for (std::list<Surface *>::iterator it = surfaces.begin();
    ...
    s->currentTranslucentTranslated.subtractRegion(
        s->currentOpaqueTranslatedSum);
  ...
  if (!fullscreenSurface) {
    ...
    topbarRegion.addRect(
        LRect(rect().x(), rect().y(), rect().w(), topbarExclusiveZone.size()));
    topbarRegion.intersectRegion(newDamage);
    p->bindColorMode();
    p->setColor({.r = 1.f, .g = 1.f, .b = 1.f});
    p->setAlpha(0.9f);
  ...
  for (std::list<DestroyedToplevel>::iterator it =
           c->destroyedToplevels.begin();
       it != c->destroyedToplevels.end();) {
  ...
  for (LScreenshotRequest *ssReq : screenshotRequests()) ssReq->accept(true);
...
bool Output::tryFullscreenScanoutIfNoOverlayContent() noexcept {
  ...
  if (!fullscreenSurface ||
      ...
      !screenshotRequests().empty() || hasMappedChildren(fullscreenSurface))
    return false;
