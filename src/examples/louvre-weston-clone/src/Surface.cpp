  ...
  if (mapped()) {
    if (firstMap) {
      ...
      if (toplevel() && cursor()->output()) {
        const LRect availGeo{cursor()->output()->pos() +
                                 cursor()->output()->availableGeometry().pos(),
                             cursor()->output()->availableGeometry().size()};

        setPos(availGeo.pos() +
               (availGeo.size() - toplevel()->windowGeometry().size()) / 2);

        if (pos().x() < availGeo.x()) setX(availGeo.x());

        if (pos().y() < availGeo.y()) setY(availGeo.y());
      }
    }
    ...
    if (toplevel()) {
      ...
      for (Output *o : (std::vector<Output *> &)outputs()) {
        ...
        o->repaint();
        if (this == o->fullscreenSurface) o->fullscreenSurface = nullptr;
      }
    }
  }
}
...
void Surface::orderChanged() {
  for (auto &pair : outputsMap) pair.second.changedOrder = true;
...
void Surface::minimizedChanged() {
  if (minimized()) {
    ...
    if (toplevel()) toplevel()->configureState(LToplevelRole::Activated);
  }
}
