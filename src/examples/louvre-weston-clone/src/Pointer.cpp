...
void Pointer::pointerMoveEvent(const LPointerMoveEvent &event) {
  ...
  // Schedule repaint on outputs that intersect with the cursor if hardware
  // composition is not supported.
  cursor()->repaintOutputs(true);

  // Update terminal icon
  if (cursorOutput) {
    pointerOverTerminalIcon =
        cursorOutput->terminalIconRect.containsPoint(cursor()->pos());
    ...
    if (pointerOverTerminalIcon) {
      ...
      if (c->pointerCursor)
        cursor()->setTextureB(c->pointerCursor->texture(),
                              c->pointerCursor->hotspotB());
    } else {
      if (cursorOutput->terminalIconAlpha != 1.0f) {
        cursorOutput->newDamage.addRect(cursorOutput->terminalIconRect);
        cursorOutput->terminalIconRect += LRect(1, 1, -2, -2);
  ...
  if (surface) {
    ...
    if (activeDND) seat()->dnd()->setFocus(nullptr, LPointF());
  }
}
...
void Pointer::pointerButtonEvent(const LPointerButtonEvent &event) {
  ...
  if (event.state() == LPointerButtonEvent::Released &&
    ...
    if (pointerOverTerminalIcon) LLauncher::launch("weston-terminal");
