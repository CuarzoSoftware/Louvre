 ...
 public:
  ...
  static void closeRestricted(int fd, void * /*data*/) {
    if (libseatEnabled) {
      ...
      for (std::list<DEVICE_FD_ID>::iterator it = devices.begin();
           it != devices.end(); it++) {
        if ((*it).fd == fd) {
          dev = (*it);
          devices.erase(it);
  ...
  static Int32 processInput(int, unsigned int, void *) {
    ...
    if (ret != 0) {
      LLog::error("[Libinput Backend] Failed to dispatch libinput %s.",
                  strerror(-ret));
      return 0;
    }
    ...
    while ((ev = libinput_get_event(li)) != NULL) {
      ...
      switch (eventType) {
        case LIBINPUT_EVENT_POINTER_MOTION:
          ...
          pointerMoveEvent.setDxUnaccelerated(
              libinput_event_pointer_get_dx_unaccelerated(pointerEvent));
          pointerMoveEvent.setDyUnaccelerated(
          ...
          if (cursor() && cursor()->output()) {
            dx = Float32(cursor()->output()->pos().x()) +
                 libinput_event_pointer_get_absolute_x_transformed(
                     pointerEvent, cursor()->output()->size().w()) -
                 cursor()->pos().x();
            dy = Float32(cursor()->output()->pos().y()) +
                 libinput_event_pointer_get_absolute_y_transformed(
                     pointerEvent, cursor()->output()->size().h()) -
                 cursor()->pos().y();
          } else
            dx = dy = 0.f;
        ...
        case LIBINPUT_EVENT_POINTER_AXIS:
          ...
          pointerScrollEvent.setHasX(
              libinput_event_pointer_has_axis(
                  pointerEvent, LIBINPUT_POINTER_AXIS_SCROLL_HORIZONTAL) != 0);
          pointerScrollEvent.setHasY(
              libinput_event_pointer_has_axis(
                  pointerEvent, LIBINPUT_POINTER_AXIS_SCROLL_VERTICAL) != 0);
          pointerScrollEvent.setRelativeDirectionX(
              libinput_device_config_scroll_get_natural_scroll_enabled(dev) == 0
                  ? LPointerScrollEvent::Identical
                  : LPointerScrollEvent::Inverted);
          pointerScrollEvent.setRelativeDirectionY(
              pointerScrollEvent.relativeDirectionX());
          pointerScrollEvent.setMs(
              libinput_event_pointer_get_time(pointerEvent));
          pointerScrollEvent.setUs(
              libinput_event_pointer_get_time_usec(pointerEvent));
          pointerScrollEvent.setSerial(LTime::nextSerial());

          if (libinput_event_pointer_get_axis_source(pointerEvent) ==
              LIBINPUT_POINTER_AXIS_SOURCE_WHEEL) {
            if (pointerScrollEvent.hasX()) {
              pointerScrollEvent.setX(libinput_event_pointer_get_axis_value(
                  pointerEvent, LIBINPUT_POINTER_AXIS_SCROLL_HORIZONTAL));
              pointerScrollEvent.setDiscreteX(
            ...
            if (pointerScrollEvent.hasY()) {
              pointerScrollEvent.setY(libinput_event_pointer_get_axis_value(
                  pointerEvent, LIBINPUT_POINTER_AXIS_SCROLL_VERTICAL));
              pointerScrollEvent.setDiscreteY(
                  libinput_event_pointer_get_axis_value_discrete(
                      pointerEvent, LIBINPUT_POINTER_AXIS_SCROLL_VERTICAL));
            } else {
          ...
          } else if (libinput_event_pointer_get_axis_source(pointerEvent) ==
        ...
        case LIBINPUT_EVENT_POINTER_SCROLL_FINGER:
          ...
          if (pointerScrollEvent.hasX())
            pointerScrollEvent.setX(libinput_event_pointer_get_scroll_value(
                pointerEvent, LIBINPUT_POINTER_AXIS_SCROLL_HORIZONTAL));
          else
            pointerScrollEvent.setX(0.f);

          if (pointerScrollEvent.hasY())
            pointerScrollEvent.setY(libinput_event_pointer_get_scroll_value(
                pointerEvent, LIBINPUT_POINTER_AXIS_SCROLL_VERTICAL));
          else
            pointerScrollEvent.setY(0.f);
        ...
        case LIBINPUT_EVENT_POINTER_BUTTON:
          ...
          pointerButtonEvent.setButton(
              (LPointerButtonEvent::Button)libinput_event_pointer_get_button(
                  pointerEvent));
          pointerButtonEvent.setState(
              (LPointerButtonEvent::State)
                  libinput_event_pointer_get_button_state(pointerEvent));
          pointerButtonEvent.setMs(
              libinput_event_pointer_get_time(pointerEvent));
          pointerButtonEvent.setUs(
              libinput_event_pointer_get_time_usec(pointerEvent));
        ...
        case LIBINPUT_EVENT_GESTURE_SWIPE_END:
          ...
          inputDevice = (LInputDevice *)libinput_device_get_user_data(dev);
          gestureEvent = libinput_event_get_gesture_event(ev);
          pointerSwipeEndEvent.setCancelled(
              libinput_event_gesture_get_cancelled(gestureEvent));
          pointerSwipeEndEvent.setFingers(
              libinput_event_gesture_get_finger_count(gestureEvent));
          pointerSwipeEndEvent.setDevice(inputDevice);
          pointerSwipeEndEvent.setMs(
              libinput_event_gesture_get_time(gestureEvent));
          pointerSwipeEndEvent.setUs(
              libinput_event_gesture_get_time_usec(gestureEvent));
        ...
        case LIBINPUT_EVENT_GESTURE_PINCH_BEGIN:
          ...
          inputDevice = (LInputDevice *)libinput_device_get_user_data(dev);
          gestureEvent = libinput_event_get_gesture_event(ev);
          pointerPinchBeginEvent.setFingers(
              libinput_event_gesture_get_finger_count(gestureEvent));
          pointerPinchBeginEvent.setDevice(inputDevice);
          pointerPinchBeginEvent.setMs(
              libinput_event_gesture_get_time(gestureEvent));
          pointerPinchBeginEvent.setUs(
              libinput_event_gesture_get_time_usec(gestureEvent));
        ...
        case LIBINPUT_EVENT_GESTURE_PINCH_UPDATE:
          ...
          inputDevice = (LInputDevice *)libinput_device_get_user_data(dev);
          gestureEvent = libinput_event_get_gesture_event(ev);
          pointerPinchUpdateEvent.setDx(
              libinput_event_gesture_get_dx(gestureEvent));
          pointerPinchUpdateEvent.setDy(
              libinput_event_gesture_get_dy(gestureEvent));
          pointerPinchUpdateEvent.setDxUnaccelerated(
              libinput_event_gesture_get_dx_unaccelerated(gestureEvent));
          pointerPinchUpdateEvent.setDyUnaccelerated(
              libinput_event_gesture_get_dy_unaccelerated(gestureEvent));
          pointerPinchUpdateEvent.setScale(
              libinput_event_gesture_get_scale(gestureEvent));
          pointerPinchUpdateEvent.setRotation(
              libinput_event_gesture_get_angle_delta(gestureEvent));
          pointerPinchUpdateEvent.setFingers(
              libinput_event_gesture_get_finger_count(gestureEvent));
          pointerPinchUpdateEvent.setDevice(inputDevice);
          pointerPinchUpdateEvent.setMs(
              libinput_event_gesture_get_time(gestureEvent));
          pointerPinchUpdateEvent.setUs(
              libinput_event_gesture_get_time_usec(gestureEvent));
        ...
        case LIBINPUT_EVENT_GESTURE_PINCH_END:
          ...
          inputDevice = (LInputDevice *)libinput_device_get_user_data(dev);
          gestureEvent = libinput_event_get_gesture_event(ev);
          pointerPinchEndEvent.setCancelled(
              libinput_event_gesture_get_cancelled(gestureEvent));
          pointerPinchEndEvent.setFingers(
              libinput_event_gesture_get_finger_count(gestureEvent));
          pointerPinchEndEvent.setDevice(inputDevice);
          pointerPinchEndEvent.setMs(
              libinput_event_gesture_get_time(gestureEvent));
          pointerPinchEndEvent.setUs(
              libinput_event_gesture_get_time_usec(gestureEvent));
        ...
        case LIBINPUT_EVENT_GESTURE_HOLD_BEGIN:
          ...
          inputDevice = (LInputDevice *)libinput_device_get_user_data(dev);
          gestureEvent = libinput_event_get_gesture_event(ev);
          pointerHoldBeginEvent.setFingers(
              libinput_event_gesture_get_finger_count(gestureEvent));
          pointerHoldBeginEvent.setDevice(inputDevice);
          pointerHoldBeginEvent.setMs(
              libinput_event_gesture_get_time(gestureEvent));
          pointerHoldBeginEvent.setUs(
              libinput_event_gesture_get_time_usec(gestureEvent));
        ...
        case LIBINPUT_EVENT_GESTURE_HOLD_END:
          ...
          inputDevice = (LInputDevice *)libinput_device_get_user_data(dev);
          gestureEvent = libinput_event_get_gesture_event(ev);
          pointerHoldEndEvent.setCancelled(
              libinput_event_gesture_get_cancelled(gestureEvent));
          pointerHoldEndEvent.setFingers(
              libinput_event_gesture_get_finger_count(gestureEvent));
          pointerHoldEndEvent.setDevice(inputDevice);
          pointerHoldEndEvent.setMs(
              libinput_event_gesture_get_time(gestureEvent));
          pointerHoldEndEvent.setUs(
              libinput_event_gesture_get_time_usec(gestureEvent));
        ...
        case LIBINPUT_EVENT_KEYBOARD_KEY:
          ...
          keyboardKeyEvent.setKeyCode(
              libinput_event_keyboard_get_key(keyEvent));
          keyboardKeyEvent.setState(
              (LKeyboardKeyEvent::State)libinput_event_keyboard_get_key_state(
                  keyEvent));
          keyboardKeyEvent.setMs(libinput_event_keyboard_get_time(keyEvent));
          keyboardKeyEvent.setUs(
        ...
        case LIBINPUT_EVENT_TOUCH_DOWN:
          ...
          touchDownEvent.setX(
              libinput_event_touch_get_x_transformed(touchEvent, 1));
          touchDownEvent.setY(
              libinput_event_touch_get_y_transformed(touchEvent, 1));
        ...
        case LIBINPUT_EVENT_TOUCH_CANCEL:
          ...
          touchCancelEvent.setUs(
              libinput_event_touch_get_time_usec(touchEvent));
        ...
        case LIBINPUT_EVENT_DEVICE_ADDED:
          ...
          for (LInputDevice *idev : unpluggedDevices) {
            if (idev->name() == libinput_device_get_name(dev) &&
                idev->vendorId() == libinput_device_get_id_vendor(dev) &&
                idev->productId() == libinput_device_get_id_product(dev)) {
              inputDevice = idev;
          ...
          if (!inputDevice) inputDevice = new LInputDevice();
  ...
  static UInt32 backendGetId() { return LInputBackendLibinput; }

  static void *backendGetContextHandle() { return li; }
  ...
  static bool backendInitialize() {
    ...
    eventSource =
        wl_event_loop_add_fd(LCompositor::eventLoop(), fd, WL_EVENT_READABLE,
                             &LInputBackend::processInput, (LSeat *)seat);
    return true;
  ...
  static void backendSuspend() { libinput_suspend(li); }
  ...
  static void backendForceUpdate() { processInput(0, 0, NULL); }

  static void backendSetLeds(UInt32 leds) {
    for (auto device : pluggedDevices)
      libinput_device_led_update((libinput_device *)device->m_nativeHandle,
                                 (libinput_led)leds);
  }
};
