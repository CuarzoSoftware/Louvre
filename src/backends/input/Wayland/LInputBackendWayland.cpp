...
class Louvre::LInputBackend {
 public:
  ...
  static WaylandBackendShared &shared() {
    return *static_cast<WaylandBackendShared *>(
        compositor()->imp()->graphicBackendData);
  }

  static void unlockRenderThread() { eventfd_write(shared().fd[0].fd, 1); }

  static UInt32 backendGetId() { return LInputBackendWayland; }
  ...
  static void updateCursor() {
    ...
    if ((!shared().cursorChangedBuffer && !shared().cursorChangedHotspot) ||
        !pointer || !shared().cursorSurface) {
      shared().mutex.unlock();
      return;
    }
    ...
    wl_pointer_set_cursor(
        pointer, pointerEnterSerial,
        shared().cursorVisible ? shared().cursorSurface : NULL,
        shared().cursorHotspot.x(), shared().cursorHotspot.y());
  ...
  static bool initWayland() {
    setupListeners();
    display =
        static_cast<wl_display *>(compositor()->graphicBackendContextHandle());
    ...
    eventfdEventSource = wl_event_loop_add_fd(
        LCompositor::eventLoop(), shared().fd[2].fd, WL_EVENT_READABLE,
        &LInputBackend::processInput, nullptr);

    waylandEventSource = wl_event_loop_add_fd(
        LCompositor::eventLoop(), shared().fd[1].fd, WL_EVENT_READABLE,
        &LInputBackend::processInput, nullptr);
  ...
  static bool backendInitialize() {
    if (compositor()->graphicBackendId() != LGraphicBackendWayland) {
      LLog::error(
          "[%s] The Wayland input backend requires the Wayland graphic "
          "backend.",
          BKND_NAME);
      return false;
    }
  ...
  static void backendForceUpdate() { processInput(-1, 0, nullptr); }
  ...
  static void pointerHandleEnter(void *, wl_pointer *, UInt32 serial,
                                 wl_surface *, Float24 /*x*/, Float24 /*y*/) {
    pointerEnterSerial = serial;
    shared().cursorChangedBuffer = true;
  }

  static void pointerHandleLeave(void *, wl_pointer *, UInt32 /*serial*/,
                                 wl_surface *) {}

  static void pointerHandleMotion(void *, wl_pointer *, UInt32 time, Float24 x,
                                  Float24 y) {
    ...
    if (cursor()->output()) {
      ...
      switch (cursor()->output()->transform()) {
        ...
        case LTransform::Rotated270:
          ...
          pos.setX(tmp);
          break;
        case LTransform::Flipped: pos.setX(sizeF.w() - pos.x()); break;
        case LTransform::Flipped90:
          tmp = pos.y();
          pos.setY(pos.x());
          pos.setX(tmp);
          break;
        case LTransform::Flipped180: pos.setY(sizeF.h() - pos.y()); break;
  ...
  static void pointerHandleButton(void *, wl_pointer *, UInt32 serial,
                                  UInt32 time, UInt32 button, UInt32 state) {
  ...
  static void pointerHandleAxis(void *, wl_pointer *, UInt32 time, UInt32 axis,
                                Float24 value) {
  ...
  static void keyboardHandleEnter(void *data, wl_keyboard *keyboard,
                                  UInt32 /*serial*/, wl_surface *,
                                  wl_array *keysArr) {
    if (keysArr->size == 0) return;

    UInt32 *key{static_cast<UInt32 *>(keysArr->data)};
    std::size_t n{keysArr->size / sizeof(*key)};
    while (n > 0) {
      keyboardHandleKey(data, keyboard, LTime::nextSerial(), LTime::ms(), *key,
  ...
  static void keyboardHandleLeave(void *, wl_keyboard *, UInt32 /*serial*/,
                                  wl_surface *) {
    while (!Louvre::seat()->keyboard()->pressedKeys().empty())
      keyboardHandleKey(nullptr, nullptr, LTime::nextSerial(), LTime::ms(),
                        Louvre::seat()->keyboard()->pressedKeys().back(),
                        WL_KEYBOARD_KEY_STATE_RELEASED);
  }

  static void keyboardHandleKey(void *, wl_keyboard *, UInt32 serial,
                                UInt32 time, UInt32 key, UInt32 state) {
  ...
  static LPointF normalizedTouchPoint(Float24 x, Float24 y) {
    LPointF point((Float32)wl_fixed_to_double(x),
                  (Float32)wl_fixed_to_double(y));

    if (point.x() < 0.f) point.setX(0.f);

    if (point.y() < 0.f) point.setY(0.f);
  ...
  static void touchHandleDown(void *, wl_touch *, UInt32 serial, UInt32 time,
                              wl_surface *, Int32 id, Float24 x, Float24 y) {
  ...
  static void touchHandleUp(void *, wl_touch *, UInt32 serial, UInt32 time,
                            Int32 id) {
  ...
  static void touchHandleMotion(void *, wl_touch *, UInt32 time, Int32 id,
                                Float24 x, Float24 y) {
    const LPointF normalizedPoint{normalizedTouchPoint(x, y)};
