...
static EGLint eglConfigAttribs[]{EGL_SURFACE_TYPE,
                                 EGL_WINDOW_BIT,
                                 EGL_RED_SIZE,
                                 8,
                                 EGL_GREEN_SIZE,
...
class Louvre::LGraphicBackend {
 public:
  ...
  inline static PFNEGLSWAPBUFFERSWITHDAMAGEKHRPROC eglSwapBuffersWithDamageKHR{
      nullptr};
  ...
  static UInt32 backendGetId() { return LGraphicBackendWayland; }

  static void *backendGetContextHandle() { return display; }

  static void unlockInputThread() {
    if (shared.fd[2].fd != -1) eventfd_write(shared.fd[2].fd, 1);
  }

  static bool initCursor() {
    if (!shared.shm) return false;

    shared.cursorMapFd = Louvre::createSHM(LOUVRE_WAYLAND_BACKEND_CURSOR_SIZE *
                                           LOUVRE_WAYLAND_BACKEND_CURSORS);

    if (shared.cursorMapFd < 0) return false;

    shared.cursorMap = (UInt8 *)mmap(
        NULL,
        LOUVRE_WAYLAND_BACKEND_CURSOR_SIZE * LOUVRE_WAYLAND_BACKEND_CURSORS,
        PROT_READ | PROT_WRITE, MAP_SHARED, shared.cursorMapFd, 0);
    ...
    shared.shmPool = wl_shm_create_pool(
        shared.shm, shared.cursorMapFd,
        LOUVRE_WAYLAND_BACKEND_CURSOR_SIZE * LOUVRE_WAYLAND_BACKEND_CURSORS);
  ...
  static void unitCursor() {
    ...
    if (shared.cursorMap) {
      munmap(shared.cursorMap, LOUVRE_WAYLAND_BACKEND_CURSOR_SIZE *
                                   LOUVRE_WAYLAND_BACKEND_CURSORS);
      shared.cursorMap = nullptr;
    }
  ...
  static bool initWayland() {
    ...
    outputListener.geometry = [](auto, auto, auto, auto, auto, auto, auto, auto,
                                 auto, auto) {};
  ...
  static bool initEGL() {
    ...
    if (!eglChooseConfig(eglDisplay, eglConfigAttribs, &eglConfig, 1, &n) ||
        n != 1) {
    ...
    if (extensions &&
        srmEGLHasExtension(extensions, "EGL_KHR_swap_buffers_with_damage"))
  ...
  static bool initRenderThread() {
    LOutput::Params params{.callback =
                               [](LOutput *output) {
    ...
    while (!windowInitialized) usleep(10000);
  ...
  static void createWindow() {
    windowEGLContext =
        eglCreateContext(eglDisplay, eglConfig, eglContext, eglContextAttribs);
    surface = wl_compositor_create_surface(compositor);
    wl_surface_add_listener(surface, &surfaceListener, nullptr);

    opaqueRegion = wl_compositor_create_region(compositor);
    wl_region_add(opaqueRegion, 0, 0, std::numeric_limits<std::int32_t>::max(),
    ...
    if (xdgDecorationManager) {
      xdgDecoration = zxdg_decoration_manager_v1_get_toplevel_decoration(
          xdgDecorationManager, xdgToplevel);
      zxdg_toplevel_decoration_v1_set_mode(
          xdgDecoration, ZXDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE);
    }
    ...
    defaultMode.m_sizeB = shared.bufferSize;
    eglWindow = wl_egl_window_create(surface, shared.bufferSize.w(),
                                     shared.bufferSize.h());
  ...
  static void renderLoop() {
    ...
    while (windowInitialized == 1) {
      ...
      poll(shared.fd, 2,
           output->state() == LOutput::PendingInitialize ? 100 : -1);

      if (shared.fd[0].revents & POLLIN) eventfd_read(shared.fd[0].fd, &value);
    ...
    paintGLChangedBufferIsLocked:
      ...
      if (bufferIsLocked) repaint = false;
      ...
      if (output->state() == LOutput::Initialized && repaint) {
        ...
        if (pendingSurfaceSize != shared.surfaceSize ||
            pendingBufferScale != shared.bufferScale) {
          shared.surfaceSize = pendingSurfaceSize;
          shared.bufferScale = pendingBufferScale;
          shared.bufferSize = shared.surfaceSize * shared.bufferScale;
          defaultMode.m_sizeB = shared.bufferSize;
          output->imp()->updateRect();
          wl_egl_window_resize(eglWindow, shared.bufferSize.w(),
                               shared.bufferSize.h(), 0, 0);
        }

        output->imp()->backendPaintGL();

        if (bufferIsLocked) goto paintGLChangedBufferIsLocked;
  ...
  static bool backendInitialize() {
    ...
    if (!initWayland()) goto fail;

    if (!initEGL()) goto fail;

    if (!initRenderThread()) goto fail;
  ...
  static void backendSuspend() { /* No TTY switching so no required */ }

  static void backendResume() { /* No TTY switching so no required */ }

  static const std::vector<LOutput *> *backendGetConnectedOutputs() {
    return &dummyOutputs;
  }

  static const std::vector<LGPU *> *backendGetDevices() { return &devices; }
  ...
  static EGLDisplay backendGetAllocatorEGLDisplay() { return eglDisplay; }

  static EGLContext backendGetAllocatorEGLContext() { return eglContext; }

  static LGPU *backendGetAllocatorDevice() { return &allocator; }

  static bool textureCreateFromCPUBuffer(LTexture *texture, const LSize &size,
                                         UInt32 stride, UInt32 format,
                                         const void *pixels) {
    const SRMGLFormat *glFmt{srmFormatDRMToGL(format)};

    if (!glFmt) return false;

    UInt32 depth, bpp, pixelSize;

    if (!srmFormatGetDepthBpp(format, &depth, &bpp)) return false;

    if (bpp % 8 != 0) return false;
    ...
    if (pixels) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, glFmt->glInternalFormat, size.w(),
                   size.h(), 0, glFmt->glFormat, glFmt->glType, pixels);
      glPixelStorei(GL_UNPACK_ROW_LENGTH_EXT, 0);
    } else {
      glTexImage2D(GL_TEXTURE_2D, 0, glFmt->glInternalFormat, size.w(),
                   size.h(), 0, glFmt->glFormat, glFmt->glType, NULL);
    }
  ...
  static bool textureCreateFromWaylandDRM(LTexture *texture, void *wlBuffer) {
    ...
    if (Louvre::compositor()->imp()->eglQueryWaylandBufferWL(
            LCompositor::eglDisplay(), (wl_resource *)wlBuffer,
            EGL_TEXTURE_FORMAT, &format)) {
      Louvre::compositor()->imp()->eglQueryWaylandBufferWL(
          LCompositor::eglDisplay(), (wl_resource *)wlBuffer, EGL_WIDTH,
          &width);
      Louvre::compositor()->imp()->eglQueryWaylandBufferWL(
          LCompositor::eglDisplay(), (wl_resource *)wlBuffer, EGL_HEIGHT,
          &height);
      ...
      const static EGLAttrib attribs[3]{EGL_IMAGE_PRESERVED_KHR, EGL_TRUE,
                                        EGL_NONE};

      image = eglCreateImage(LCompositor::eglDisplay(), EGL_NO_CONTEXT,
                             EGL_WAYLAND_BUFFER_WL, wlBuffer, attribs);
  ...
  static bool textureCreateFromDMA(LTexture * /*texture*/,
                                   const LDMAPlanes * /*planes*/) {
    /* Not supported yet, wl_drm can be used instead */
    return false;
  }

  static bool textureCreateFromGL(LTexture *texture, GLuint id, GLenum target,
                                  UInt32 format, const LSize & /*size*/,
  ...
  static bool textureUpdateRect(LTexture *texture, UInt32 stride,
                                const LRect &dst, const void *pixels) {
    if (texture->sourceType() != LTexture::CPU) return false;

    CPUTexture *cpuTexture = (CPUTexture *)texture->m_graphicBackendData;

    if (!cpuTexture) return false;
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, dst.x(), dst.y(), dst.w(), dst.h(),
                    cpuTexture->glFmt->glFormat, cpuTexture->glFmt->glType,
                    pixels);
  ...
  static bool textureWriteUpdate(LTexture *texture, UInt32 stride,
                                 const LRect &dst, const void *pixels) {
    CPUTexture *cpuTexture = (CPUTexture *)texture->m_graphicBackendData;
    glBindTexture(GL_TEXTURE_2D, cpuTexture->texture.id);
    glPixelStorei(GL_UNPACK_ROW_LENGTH, stride / cpuTexture->pixelSize);
  ...
  static UInt32 textureGetID(LOutput * /*output*/, LTexture *texture) {
    ...
    if (bkndTexture) return bkndTexture->id;
  ...
  static GLenum textureGetTarget(LTexture *texture) {
    ...
    if (bkndTexture) return bkndTexture->target;
  ...
  static void textureDestroy(LTexture *texture) {
    switch (texture->sourceType()) {
      ...
      case LTexture::GL: {
        ...
        if (cpuTexture) {
          if (cpuTexture->destroy) glDeleteTextures(1, &cpuTexture->texture.id);
          delete cpuTexture;
        }
      } break;
  ...
  static const char *outputGetSerial(LOutput * /*output*/) { return nullptr; }
  ...
  static LGPU *outputGetDevice(LOutput * /*output*/) { return &allocator; }

  static UInt32 outputGetID(LOutput * /*output*/) { return 0; }

  static bool outputIsNonDesktop(LOutput * /*output*/) { return false; }

  static UInt32 outputGetFramebufferID(LOutput * /*output*/) { return 0; }

  static Int32 outputGetCurrentBufferIndex(LOutput * /*output*/) { return 0; }

  static UInt32 outputGetCurrentBufferAge(LOutput * /*output*/) {
    GLint age{0};
    if (eglQuerySurface(eglDisplay, eglSurface, EGL_BUFFER_AGE_KHR, &age) ==
        EGL_TRUE)
      return age;
    return 0;
  }
  ...
  static LTexture *outputGetBuffer(LOutput * /*output*/,
                                   UInt32 /*bufferIndex*/) {
    return nullptr;
  }
  ...
  static UInt32 outputGetGammaSize(LOutput * /*output*/) { return 0; }

  static bool outputSetGamma(LOutput * /*output*/,
                             const LGammaTable & /*table*/) {
    return false;
  }
  ...
  static bool outputIsVSyncEnabled(LOutput * /*output*/) { return vSync; }
  ...
  static void outputSetCursorTexture(LOutput * /*output*/, UChar8 *buffer) {
    ...
    if (buffer) {
      ...
      if (shared.currentCursor) {
        memcpy(shared.currentCursor->map, buffer,
               LOUVRE_WAYLAND_BACKEND_CURSOR_SIZE);
        wl_surface_attach(shared.cursorSurface, shared.currentCursor->buffer, 0,
                          0);
      }
  ...
  static void outputSetCursorPosition(LOutput * /*output*/,
                                      const LPoint & /*position*/) {
    static LPointF prevHotspotB;

    if (prevHotspotB != cursor()->hotspotB()) {
      prevHotspotB = cursor()->hotspotB();
      shared.mutex.lock();
      shared.cursorHotspot =
          ((cursor()->pos() - cursor()->rect().pos()) * shared.bufferScale) / 2;
  ...
  static const std::vector<LOutputMode *> *outputGetModes(
      LOutput * /*output*/) {
  ...
  static bool outputSetScanoutBuffer(LOutput * /*output*/,
                                     LTexture * /*texture*/) {
    return false;
  }
  ...
  static void registryHandleGlobal(void * /*data*/, wl_registry *registry,
                                   UInt32 name, const char *interface,
                                   UInt32 version) {
    if (!compositor && strcmp(interface, wl_compositor_interface.name) == 0)
      compositor = (wl_compositor *)wl_registry_bind(
          registry, name, &wl_compositor_interface, version >= 3 ? 3 : 1);

    else if (!xdgWmBase && strcmp(interface, xdg_wm_base_interface.name) == 0) {
      xdgWmBase = (xdg_wm_base *)wl_registry_bind(registry, name,
                                                  &xdg_wm_base_interface, 1);
      xdg_wm_base_add_listener(xdgWmBase, &xdgWmBaseListener, nullptr);
    }

    else if (!xdgDecorationManager &&
             strcmp(interface, zxdg_decoration_manager_v1_interface.name) ==
                 0) {
      xdgDecorationManager = (zxdg_decoration_manager_v1 *)wl_registry_bind(
      ...
      output->name = name;
      waylandOutputs.emplace_back((wl_output *)wl_registry_bind(
  ...
  static void registryHandleGlobalRemove(void * /*data*/,
    ...
    for (std::size_t i{0}; i < waylandOutputs.size(); i++) {
      waylandOutput = static_cast<WaylandOutput *>(
          (wl_output_get_user_data(waylandOutputs[i])));
  ...
  static void surfaceHandleEnter(void * /*data*/, wl_surface *surface,
                                 wl_output *output) {
    if (surface == shared.cursorSurface) return;
    LVectorPushBackIfNonexistent(surfaceOutputs, output);
    updateSurfaceScale();
  }

  static void surfaceHandleLeave(void * /*data*/, wl_surface * /*surface*/,
                                 wl_output *output) {
    if (surface == shared.cursorSurface) return;
  ...
  static void outputHandleMode(void *data, wl_output *, UInt32 /*flags*/,
  ...
  static void xdgWmBaseHandlePing(void * /*data*/, xdg_wm_base *wm_base,
                                  UInt32 serial) {
    xdg_wm_base_pong(wm_base, serial);
  }

  static void xdgSurfaceHandleConfigure(void *, xdg_surface *xdgSurface,
                                        UInt32 serial) {
  ...
  static void xdgToplevelHandleConfigure(void *, xdg_toplevel *, Int32 w,
                                         Int32 h, wl_array *) {
    if (w > 0) pendingSurfaceSize.setW(w);

    if (h > 0) pendingSurfaceSize.setH(h);

    if (pendingSurfaceSize != shared.surfaceSize) outputRepaint(nullptr);
  }
};
...
extern "C" LGraphicBackendInterface *getAPI() {
  ...
  API.backendGetScanoutDMAFormats =
      &LGraphicBackend::backendGetScanoutDMAFormats;
  API.backendGetAllocatorEGLDisplay =
      &LGraphicBackend::backendGetAllocatorEGLDisplay;
  API.backendGetAllocatorEGLContext =
      &LGraphicBackend::backendGetAllocatorEGLContext;
  ...
  API.textureCreateFromWaylandDRM =
      &LGraphicBackend::textureCreateFromWaylandDRM;
  ...
  API.outputHasBufferDamageSupport =
      &LGraphicBackend::outputHasBufferDamageSupport;
  ...
  API.outputGetCurrentBufferIndex =
      &LGraphicBackend::outputGetCurrentBufferIndex;
  ...
  /* OUTPUT V-SYNC */
  API.outputHasVSyncControlSupport =
      &LGraphicBackend::outputHasVSyncControlSupport;
  ...
  /* OUTPUT CURSOR */
  API.outputHasHardwareCursorSupport =
      &LGraphicBackend::outputHasHardwareCursorSupport;
