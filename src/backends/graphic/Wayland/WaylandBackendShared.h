...
#define LOUVRE_WAYLAND_BACKEND_CURSORS \
  4  // For compositors that don't release buffers immediately
...
struct WaylandBackendShared {
   ...
   public:
    SHMCursor(WaylandBackendShared &shared, std::size_t n) noexcept {
      buffer = wl_shm_pool_create_buffer(
          shmPool, n * LOUVRE_WAYLAND_BACKEND_CURSOR_SIZE, 64, 64, 64 * 4,
    ...
    ~SHMCursor() noexcept { wl_buffer_destroy(buffer); }
  ...
  SHMCursor *getFreeCursor() noexcept {
    for (auto &cursor : cursors)
      if (cursor.released) return &cursor;
