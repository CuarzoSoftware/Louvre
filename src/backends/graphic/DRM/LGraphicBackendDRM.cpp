...
#define SRM_VERSION_GREATER_EQUAL_THAN(major, minor, patch) \
  (major < SRM_VERSION_MAJOR ||                             \
   (major == SRM_VERSION_MAJOR &&                           \
    (minor < SRM_VERSION_MINOR ||                           \
     (minor == SRM_VERSION_MINOR && patch <= SRM_VERSION_PATCH))))
#define SRM_VERSION_LESS_THAN(major, minor, patch) \
  !SRM_VERSION_GREATER_EQUAL_THAN(major, minor, patch)
 ...
 public:
  ...
  ~DRMLease() {
    if (fd >= 0) {
      ...
      if (ret != 0)
        LLog::error(
            "[%s] drmModeRevokeLease failed (%d). Cleaning up resources once "
            "the session (DRM master) is restored.",
            BKND_NAME, ret);
    }
...
int LGraphicBackend::openRestricted(const char *path, int flags,
                                    void *userData) {
...
void LGraphicBackend::closeRestricted(int fd, void *userData) {
  ...
  if (libseatEnabled) compositor->seat()->closeDevice(dev->id());
...
static void initConnector(Backend *bknd, SRMConnector *conn) {
  if (srmConnectorGetUserData(conn)) return;
          ...
          [=](LOutput *output) {
            ...
            SRMListForeach(modeIt, srmConnectorGetModes(conn)) {
              SRMConnectorMode *mode =
                  (SRMConnectorMode *)srmListItemGetData(modeIt);
              LOutputMode *outputMode =
                  new LOutputMode(output,
                                  LSize(srmConnectorModeGetWidth(mode),
                                        srmConnectorModeGetHeight(mode)),
                                  srmConnectorModeGetRefreshRate(mode) * 1000,
                                  srmConnectorModeIsPreferred(mode), mode);
...
static void connectorPluggedEventHandler(SRMListener *listener,
                                         SRMConnector *conn) {
...
static void connectorUnpluggedEventHandler(SRMListener *listener,
                                           SRMConnector *conn) {
  Backend *bknd = (Backend *)srmListenerGetUserData(listener);
  LCompositor *compositor = (LCompositor *)srmCoreGetUserData(bknd->core);
...
UInt32 LGraphicBackend::backendGetId() { return LGraphicBackendDRM; }
...
bool LGraphicBackend::backendInitialize() {
  ...
  auto formatInVector = [](const std::vector<LDMAFormat> &vec,
                           const LDMAFormat &fmt) -> bool {
  ...
  if (version->major == 0 && version->minor < 13) {
    LLog::fatal("[%s] Using SRM v%d.%d.%d but version >= v0.13.0 is required.",
                BKND_NAME, version->major, version->minor, version->patch);
  ...
  SRMListForeach(planeIt, srmDeviceGetPlanes(allocatorDevice)) {
    ...
    if (srmPlaneGetType(plane) != SRM_PLANE_TYPE_PRIMARY) continue;

    SRMListForeach(fmtIt, srmPlaneGetFormats(plane)) {
      SRMFormat *fmt = (SRMFormat *)srmListItemGetData(fmtIt);

      if (!formatInVector(bknd->scanoutFormats, *(LDMAFormat *)fmt) &&
          formatInVector(bknd->dmaFormats, *(LDMAFormat *)fmt))
      ...
      if (alphaSubstitute != fmt->format &&
          !formatInVector(bknd->scanoutFormats,
                          {alphaSubstitute, fmt->modifier}) &&
  ...
  SRMListForeach(devIt, srmCoreGetDevices(bknd->core)) {
    ...
    SRMListForeach(connIt, srmDeviceGetConnectors(dev)) {
      ...
      if (srmConnectorIsConnected(conn)) initConnector(bknd, conn);
    }
  }

  // Listen to connector hotplug events
  srmCoreAddConnectorPluggedEventListener(bknd->core,
                                          &connectorPluggedEventHandler, bknd);
  srmCoreAddConnectorUnpluggedEventListener(
      bknd->core, &connectorUnpluggedEventHandler, bknd);

  bknd->monitor = wl_event_loop_add_fd(
      LCompositor::eventLoop(), srmCoreGetMonitorFD(bknd->core),
      WL_EVENT_READABLE, &monitorEventHandler, bknd);
...
void LGraphicBackend::backendResume() {
  ...
  SRMListForeach(devIt, srmCoreGetDevices(bknd->core)) {
    ...
    for (UInt32 i = 0; i < lessees->count; i++) {
      LLog::debug("[%s] Removing previously leaked DRM lease (%d).", BKND_NAME,
                  lessees->lessees[i]);
...
bool LGraphicBackend::textureCreateFromCPUBuffer(LTexture *texture,
                                                 const LSize &size,
                                                 UInt32 stride, UInt32 format,
                                                 const void *pixels) {
  Backend *bknd = (Backend *)compositor()->imp()->graphicBackendData;
  SRMBuffer *bkndBuffer = srmBufferCreateFromCPU(
      bknd->core, NULL, size.w(), size.h(), stride, pixels, format);
...
bool LGraphicBackend::textureCreateFromWaylandDRM(LTexture *texture,
                                                  void *wlBuffer) {
  Backend *bknd = (Backend *)compositor()->imp()->graphicBackendData;
...
bool LGraphicBackend::textureCreateFromDMA(LTexture *texture,
                                           const LDMAPlanes *planes) {
  Backend *bknd = (Backend *)compositor()->imp()->graphicBackendData;
  SRMBuffer *bkndBuffer =
      srmBufferCreateFromDMA(bknd->core, NULL, (SRMBufferDMAData *)planes);
...
bool LGraphicBackend::textureCreateFromGL(LTexture *texture, GLuint id,
                                          GLenum target, UInt32 format,
                                          const LSize &size,
                                          bool transferOwnership) {
  Backend *bknd = (Backend *)compositor()->imp()->graphicBackendData;
  SRMBuffer *bkndBuffer = srmBufferCreateGLTextureWrapper(
      srmCoreGetAllocatorDevice(bknd->core), id, target, format, size.w(),
      size.h(), transferOwnership);
...
bool LGraphicBackend::textureUpdateRect(LTexture *texture, UInt32 stride,
                                        const LRect &dst, const void *pixels) {
  SRMBuffer *bkndBuffer = (SRMBuffer *)texture->m_graphicBackendData;
  return srmBufferWrite(bkndBuffer, stride, dst.x(), dst.y(), dst.w(), dst.h(),
                        pixels);
}
...
bool LGraphicBackend::textureWriteUpdate(LTexture *texture, UInt32 stride,
                                         const LRect &dst, const void *pixels) {
  SRMBuffer *bkndBuffer = (SRMBuffer *)texture->m_graphicBackendData;
  return srmBufferWrite2Update(bkndBuffer, stride, dst.x(), dst.y(), dst.w(),
                               dst.h(), pixels);
}
...
UInt32 LGraphicBackend::textureGetID(LOutput *output, LTexture *texture) {
  ...
  if (output) {
    Output *bkndOutput = (Output *)output->imp()->graphicBackendData;
    bkndRendererDevice =
        srmDeviceGetRendererDevice(srmConnectorGetDevice(bkndOutput->conn));
  } else {
    Backend *bknd = (Backend *)compositor()->imp()->graphicBackendData;
    bkndRendererDevice = srmCoreGetAllocatorDevice(bknd->core);
  }

  return srmBufferGetTexture(bkndRendererDevice,
                             (SRMBuffer *)texture->m_graphicBackendData)
      .id;
}
...
void LGraphicBackend::textureDestroy(LTexture *texture) {
  ...
  if (buffer) srmBufferDestroy(buffer);
}
...
void LGraphicBackend::outputSetBufferDamage(LOutput *output, LRegion &region) {
  ...
  if (!srmConnectorHasBufferDamageSupport(bkndOutput->conn) ||
      srmConnectorGetState(bkndOutput->conn) != SRM_CONNECTOR_STATE_INITIALIZED)
    return;
...
LTexture *LGraphicBackend::outputGetBuffer(LOutput *output,
                                           UInt32 bufferIndex) {
  Output *bkndOutput = (Output *)output->imp()->graphicBackendData;
  ...
  if (!buffer || !buffersCount) return nullptr;
...
bool LGraphicBackend::outputSetGamma(LOutput *output,
                                     const LGammaTable &table) {
  Output *bkndOutput = (Output *)output->imp()->graphicBackendData;

  if (table.size() != srmConnectorGetGammaSize(bkndOutput->conn)) {
    LLog::error(
        "[%s] Failed to set gamma to output %s. Invalid size %d != real gamma "
        "size %d.",
        BKND_NAME, output->name(), table.size(), output->gammaSize());
    return false;
  }
...
void LGraphicBackend::outputSetCursorPosition(LOutput *output,
...
const std::vector<LOutputMode *> *LGraphicBackend::outputGetModes(
    LOutput *output) {
  Output *bkndOutput = (Output *)output->imp()->graphicBackendData;
...
bool LGraphicBackend::outputSetMode(LOutput *output, LOutputMode *mode) {
  Output *bkndOutput = (Output *)output->imp()->graphicBackendData;
  return srmConnectorSetMode(bkndOutput->conn,
                             (SRMConnectorMode *)mode->m_data);
}

LContentType LGraphicBackend::outputGetContentType(LOutput *output) {
  Output *bkndOutput = (Output *)output->imp()->graphicBackendData;
  return static_cast<LContentType>(
      srmConnectorGetContentType(bkndOutput->conn) - 1);
}

void LGraphicBackend::outputSetContentType(LOutput *output, LContentType type) {
  Output *bkndOutput = (Output *)output->imp()->graphicBackendData;
  return srmConnectorSetContentType(bkndOutput->conn,
                                    (SRM_CONNECTOR_CONTENT_TYPE)(type + 1));
}

/* DIRECT SCANOUT */

bool LGraphicBackend::outputSetScanoutBuffer(LOutput *output,
                                             LTexture *texture) {
...
int LGraphicBackend::backendCreateLease(const std::vector<LOutput *> &outputs) {
  ...
  for (LOutput *output : outputs) {
    ...
    if (!lease->gpu) {
      ...
      // When using the legacy DRM API there is no way to know which cursor and
      // primary plane will a connector use
      if (!srmDeviceGetClientCapAtomic(dev)) {
        LLog::error(
            "[%s] DRM lease is disabled for the legacy DRM API. Make sure to "
            "set SRM_FORCE_LEGACY_API=0.",
            BKND_NAME);
        return -1;
      }
    }
    ...
    if (!srmConnectorGetBestConfiguration(bkndOutput->conn, &encoder, &crtc,
                                          &primaryPlane, &cursorPlane)) {
      LLog::error(
          "[%s] DRM lease: Failed to find a free CRTC for connector %s.",
          BKND_NAME, output->name());
      return -1;
    }
  ...
  lease->fd = drmModeCreateLease(lease->gpu->fd(), resources.data(),
                                 resources.size(), O_CLOEXEC, &lease->lessee);
