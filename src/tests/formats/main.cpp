...
/*
 * This test creates 12 (or more) textures: 6 from the main thread and 6 from
 * each LOutput thread. Tested formats: DRM_FORMAT_ARGB8888 and
 * DRM_FORMAT_ABGR8888. Textures are rendered on each output, arranged as shown
 * below:
 *
 * Main Thread:   [ARGB Static] [ABGR Static] [ARGB Updated] [ABGR Updated]
 * [ARGB Copy] [ABGR Copy] Output Thread: [ARGB Static] [ABGR Static] [ARGB
 * Updated] [ABGR Updated] [ARGB Copy] [ABGR Copy]
 *
 * Textures on the left are created once. Textures in the center are updated
 * (pixel data rotates 90°) every 500 ms. Textures on the right are copies of
 * the initial ones.
 *
 * Missing textures (displayed as black spaces) could indicate unsupported
 * formats or synchronization issues.
 */
...
static UInt8 pixelsARGB[]{0,   0, 255, 255, 0,   255, 0,   255,
                          255, 0, 0,   255, 255, 255, 255, 255};

static UInt8 pixelsABGR[]{255, 0, 0,   255, 0,   255, 0,   255,
...
class Compositor final : public LCompositor {
 public:
  void initialized() override {
    mainThreadARGB.setDataFromMainMemory(LSize(2, 2), 2 * 4,
                                         DRM_FORMAT_ARGB8888, pixelsARGB);
    mainThreadABGR.setDataFromMainMemory(LSize(2, 2), 2 * 4,
                                         DRM_FORMAT_ABGR8888, pixelsABGR);

    mainThreadARGB.updateRect(LRect(0, 0, 2, 2), 2 * 4, pixelsARGB);
    mainThreadABGR.updateRect(LRect(0, 0, 2, 2), 2 * 4, pixelsABGR);

    mainThreadARGBUpdated.setDataFromMainMemory(
        LSize(2, 2), 2 * 4, DRM_FORMAT_ARGB8888, pixelsARGB);
    mainThreadABGRUpdated.setDataFromMainMemory(
        LSize(2, 2), 2 * 4, DRM_FORMAT_ABGR8888, pixelsABGR);
...
class Output final : public LOutput {
 public:
  ...
  void initializeGL() override {
    ...
    outputThreadARGB.setDataFromMainMemory(LSize(2, 2), 2 * 4,
                                           DRM_FORMAT_ARGB8888, pixelsARGB);
    outputThreadABGR.setDataFromMainMemory(LSize(2, 2), 2 * 4,
                                           DRM_FORMAT_ABGR8888, pixelsABGR);

    outputThreadARGB.updateRect(LRect(0, 0, 2, 2), 2 * 4, pixelsARGB);
    outputThreadABGR.updateRect(LRect(0, 0, 2, 2), 2 * 4, pixelsABGR);

    outputThreadARGBUpdated.setDataFromMainMemory(
        LSize(2, 2), 2 * 4, DRM_FORMAT_ARGB8888, pixelsARGB);
    outputThreadABGRUpdated.setDataFromMainMemory(
        LSize(2, 2), 2 * 4, DRM_FORMAT_ABGR8888, pixelsABGR);
  ...
  void paintGL() override {
    ...
    LTexture *textures[ROWS][COLS]{
        // [y][x]
        {&c.mainThreadARGB, &c.mainThreadABGR, &c.mainThreadARGBUpdated,
         &c.mainThreadABGRUpdated, c.mainThreadARGBCopy, c.mainThreadABGRCopy},
        {&outputThreadARGB, &outputThreadABGR, &outputThreadARGBUpdated,
         &outputThreadABGRUpdated, outputThreadARGBCopy, outputThreadABGRCopy},
    };
  ...
  void uninitializeGL() override {
    if (outputThreadARGBCopy) delete outputThreadARGBCopy;

    if (outputThreadABGRCopy) delete outputThreadABGRCopy;
  }
