 ...
 */
class CZ::LLayerRole : public LBaseSurfaceRole
{
public:
    ...
    struct State
    {
        ...
        CZEdge exclusiveEdge { CZEdgeNone };

        /// LLayerRole::layer()
        LSurfaceLayer layer;

        bool requestedConfigure { false };
    };
     ...
     */
    virtual void stateChanged(CZBitset<Changes> changes, const State &prev) noexcept;
 ...
 */
    ...
    {
        return state().exclusiveZoneSize;
    }
    ...
    {
        return state().keyboardInteractivity;
    }
     ...
     * and this property is @ref CZEdgeNone, the zone size is treated as 0.
     */
    CZEdge exclusiveEdge() const noexcept
    {
        return state().exclusiveEdge;
    }
...
private:
    ...
    enum Flags : UInt32
    {
        HasPendingInitialConf           = static_cast<UInt32>(1) << 0,
        HasConfToSend                   = static_cast<UInt32>(1) << 1,
        ClosedSent                      = static_cast<UInt32>(1) << 2,
        MappedByClient                  = static_cast<UInt32>(1) << 3,
    };

    CZEdge edgesToSingleEdge() const noexcept;
    ...
    LExclusiveZone m_exclusiveZone { CZEdgeNone, 0 };
    CZBitset<Flags> m_flags { HasPendingInitialConf };
    State m_current {};
    State m_pending {};
