 ...
 */
class CZ::LSubsurfaceRole : public LBaseSurfaceRole
{
public:
...
private:
    ...
    void handleParentMappingChange() noexcept override;

    void handleParentCommit() noexcept;
    void updateMapping() noexcept;

    mutable SkIPoint m_rolePos {};
    SkIPoint m_currentLocalPos, m_pendingLocalPos;
    bool m_isSynced { true };

    std::shared_ptr<LSurfaceLock> m_lock;

    bool m_changedOrder { false };

    // Used by the parent to skip this surface in LSurfacePrivate::notifyParentCommitToSubsurfaces
    bool m_parentAlreadyCommitted { false };
};
