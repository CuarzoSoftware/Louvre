...
CZBitset<CZEdge> LPopupRole::constrainedEdges(const SkIRect &rect) const noexcept
{
    CZBitset<CZEdge> edges;

    if (bounds().width() > 0)
    {
        if (rect.x() < bounds().x())
            edges.add(CZEdgeLeft);

        if (rect.right() > bounds().right())
            edges.add(CZEdgeRight);
    }
...
SkIRect LPopupRole::calculateUnconstrainedRect(const SkIPoint *futureParentPos) const noexcept
{
    ...
    CZBitset<CZEdge> constrained { constrainedEdges(conf.finalRect) };

    if (constrained == 0)
        goto final;

    // Handle X axis
    if (constrained.has(CZEdgeLeft | CZEdgeRight))
    {
        if (positioner().constraintAdjustments().has(LPositioner::FlipX))
        {
            ...
            if (constrained == 0)
                goto final;
            else if (!constrained.has(CZEdgeLeft | CZEdgeRight))
                goto handleYAxis;
            else
            {
                // Revert
        ...
        if (positioner().constraintAdjustments().has(LPositioner::SlideX) && !constrained.hasAll(CZEdgeLeft | CZEdgeRight))
        {
            if (constrained.has(CZEdgeLeft))
        ...
        if (positioner().constraintAdjustments().has(LPositioner::ResizeX))
        {
            ...
            else if (constrained.has(CZEdgeRight))
            {
    ...
    if (constrained.has(CZEdgeTop | CZEdgeBottom))
    {
        ...
        if (positioner().constraintAdjustments().has(LPositioner::SlideY) && !constrained.hasAll(CZEdgeTop | CZEdgeBottom))
        {
            if (constrained.has(CZEdgeTop))
        ...
        if (positioner().constraintAdjustments().has(LPositioner::ResizeY))
        {
            ...
            else if (constrained.has(CZEdgeBottom))
            {
...
LPopupRole::~LPopupRole() noexcept
{
    validateDestructor();
    notifyDestruction();
}
...
{
    return !surface()->parent() || !surface()->parent()->popup();
}
...
void LPopupRole::dismiss() noexcept
{
    for (auto it = m_childPopups.rbegin(); it != m_childPopups.rend(); it++)
        (*it)->dismiss();

    if (m_flags.has(Dismissed))
        return;

    m_flags.add(Dismissed);

    auto *res { static_cast<XdgShell::RXdgPopup*>(resource()) };
    res->popupDone();
    // TODO: surface()->imp()->setKeyboardGrabToParent();
    surface()->imp()->setMapped(false);

}

void LPopupRole::cacheCommit() noexcept
{
...
void LPopupRole::applyCommit() noexcept
{
    ...
    if (!surface()->parent())
    {
        xdgSurfaceResource()->postError(XDG_WM_BASE_ERROR_INVALID_POPUP_PARENT, "The popup has no parent");
        return;
    }
...
void LPopupRole::applyState(State *pending) noexcept
{
    if (!pending->windowGeometrySet)
        pending->windowGeometry = XdgShell::RXdgSurface::CalculateGeometryWithSubsurfaces(surface());
...
bool LPopupRole::setParent(LSurface *parent) noexcept
{
    if (parent)
    {
        assert(!surface()->parent());

        if (auto *p = parent->toplevel())
        {
            p->m_childPopups.emplace_back(this);
            m_parentLink = std::prev(p->m_childPopups.end());
        }
        else if (auto *p = parent->popup())
        {
            p->m_childPopups.emplace_back(this);
            m_parentLink = std::prev(p->m_childPopups.end());
        }
        else if (auto *p = parent->layerRole())
        {
            p->m_childPopups.emplace_back(this);
            m_parentLink = std::prev(p->m_childPopups.end());
        }
    ...
    else
    {
        ...
        if (auto *p = parent->toplevel())
        {
            p->m_childPopups.erase(m_parentLink);
        }
        else if (auto *p = parent->popup())
        {
            p->m_childPopups.erase(m_parentLink);
