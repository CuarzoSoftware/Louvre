...
public:
     ...
     */
    void configureState(CZBitset<State> flags) noexcept
    {
        updateSerial();
     ...
     */
    CZBitset<State> state() const noexcept
    {
        return atoms().state;
    }
     ...
     */
    CZBitset<State> supportedStates() const noexcept
    {
    ...
    {
        return state().has(TiledLeft | TiledTop | TiledRight | TiledBottom);
    }
    ...
    */
     ...
     */
    CZBitset<Capabilities> capabilities() const noexcept
    {
...
/// @{
     ...
     */
    virtual void atomsChanged(CZBitset<AtomChanges> changes, const Atoms &prevAtoms);
     ...
     */
    virtual void startResizeRequest(const LEvent &triggeringEvent, CZBitset<LEdge> edge);
...
private:
    ...
    CZBitset<State> m_supportedStates { Activated | Maximized | Fullscreen | Resizing };
