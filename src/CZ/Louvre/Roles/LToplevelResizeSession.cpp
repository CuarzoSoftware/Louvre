...
{
    if (m_isActive)
        CZVectorUtils::RemoveOneUnordered(seat()->imp()->resizeSessions, this);
}
...
void LToplevelResizeSession::updateDragPoint(SkIPoint point)
{
    ...
    // Top
    if (m_constraints.top != CZEdgeDisabled && m_edge.has(CZEdgeTop))
    {
        if (pos.y() - (newSize.height() - size.height()) < m_constraints.top)
            newSize.fHeight = pos.y() + size.height() - m_constraints.top;
    }
    // Bottom
    else if (m_constraints.bottom != CZEdgeDisabled && m_edge.has(CZEdgeBottom))
    {
    ...
    // Left
    if (m_constraints.left != CZEdgeDisabled && m_edge.has(CZEdgeLeft))
    {
        if (pos.x() - (newSize.width() - size.width()) < m_constraints.left)
            newSize.fWidth = pos.x() + size.width() - m_constraints.left;
    }
    // Right
    else if (m_constraints.right != CZEdgeDisabled && m_edge.has(CZEdgeRight))
    {
    ...
    toplevel()->configureState(CZWinActivated | CZWinResizing);
    m_lastSerial = m_toplevel->pendingConfiguration().serial;
}

bool LToplevelResizeSession::start(const CZEvent &triggeringEvent, CZBitset<CZEdge> edge, SkIPoint initDragPoint)
{
    if (m_isActive)
        return false;
    ...
    if (m_constraints.left != CZEdgeDisabled && m_toplevel->surface()->pos().x() < m_constraints.left)
        m_toplevel->surface()->setX(m_constraints.left);

    if (m_constraints.top != CZEdgeDisabled && m_toplevel->surface()->pos().y() < m_constraints.top)
        m_toplevel->surface()->setY(m_constraints.top);

    m_initPos = m_toplevel->surface()->pos();

    m_toplevel->configureState(m_toplevel->pendingConfiguration().windowState | CZWinActivated | CZWinResizing);
    m_lastSerial = m_toplevel->pendingConfiguration().serial;
...
const std::vector<LToplevelResizeSession*>::const_iterator LToplevelResizeSession::stop()
{
    ...
    m_isActive = false;
    toplevel()->configureState(toplevel()->pendingConfiguration().windowState & ~CZWinResizing);
