...
LGLOBAL_INTERFACE_IMP(GLayerShell, LOUVRE_LAYER_SHELL_VERSION, zwlr_layer_shell_v1_interface)

bool GLayerShell::Probe(CZWeak<LGlobal> **slot) noexcept
{
    if (compositor()->wellKnownGlobals.WlrLayerShell)
    {
        LLog(CZError, CZLN, "Failed to create {} global (already created)", Interface()->name);
        return false;
    }

    *slot = &compositor()->wellKnownGlobals.WlrLayerShell;
    return true;
}
...
{
    CZVectorUtils::RemoveOneUnordered(client()->imp()->layerShellGlobals, this);
}
