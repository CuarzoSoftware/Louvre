...
void RDataDevice::start_drag(wl_client */*client*/,
                             ...
                             UInt32 serial)
{
    const CZEvent *triggeringEvent;
    LDNDSession *session = new LDNDSession();
    auto *res { static_cast<RDataDevice*>(wl_resource_get_user_data(resource)) };
    session->origin.reset(static_cast<RWlSurface*>(wl_resource_get_user_data(origin))->surface());
    ...
    {
        LLog(CZWarning, CZLN, "Start drag request cancelled. There already is an active drag & drop session");
        goto fail;
    }
    ...
    if (!triggeringEvent)
    {
        LLog(CZWarning, CZLN, "Start drag & drop request without serial match. Ignoring it...");
        goto fail;
    }
    ...
    if (icon)
    {
        LSurface *iconSurface { static_cast<RWlSurface*>(wl_resource_get_user_data(icon))->surface() };
    ...
    seat()->dnd()->m_triggeringEvent = triggeringEvent->copy();
...
void RDataDevice::set_selection(wl_client *client, wl_resource *resource, wl_resource *source, UInt32 serial)
{
    ...
    const CZEvent *triggeringEvent { rDataDevice->client()->findEventBySerial(serial) };

    if (!triggeringEvent)
    {
        LLog(CZWarning, CZLN, "Set clipboard request without valid triggering event. Ignoring it...");
        return;
    }
    ...
    if (source)
    {
        ...
        {
            LLog(CZWarning, "Set clipboard request already made with the same data source. Ignoring it...");
            return;
        }
        ...
        {
            LLog(CZDebug, CZLN, "Set clipboard request denied by user");
            return;
        }
...
void RDataDevice::enter(UInt32 serial, RWlSurface *surface, Float24 x, Float24 y, RDataOffer *offer) noexcept
{
