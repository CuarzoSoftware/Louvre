...
LGLOBAL_INTERFACE_IMP(GSeat, LOUVRE_WL_SEAT_VERSION, wl_seat_interface)

bool GSeat::Probe(CZWeak<LGlobal> **slot) noexcept
{
    if (compositor()->wellKnownGlobals.WlSeat)
    {
        LLog(CZError, CZLN, "Failed to create {} global (already created)", Interface()->name);
        return false;
    }

    *slot = &compositor()->wellKnownGlobals.WlSeat;
    return true;
}
...
{
    CZVectorUtils::RemoveOneUnordered(client()->imp()->seatGlobals, this);
}
