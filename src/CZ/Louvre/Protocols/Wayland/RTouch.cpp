...
{
    if (seatRes())
        CZVectorUtils::RemoveOneUnordered(seatRes()->m_touchRes, this);
}
...
void RTouch::down(const CZTouchDownEvent &event, RWlSurface *surfaceRes) noexcept
{
    ...
    auto it = std::find_if(
        clientEvents.begin(),
        clientEvents.end(),
        [event](const CZTouchDownEvent &ev)
    {
...
void RTouch::up(const CZTouchUpEvent &event) noexcept
{
