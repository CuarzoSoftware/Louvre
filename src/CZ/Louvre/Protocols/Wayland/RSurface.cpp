...
void RSurface::apply_commit(LSurface *surface, LBaseSurfaceRole::CommitOrigin origin)
{
    ...
    for (LSurface *s : surface->children())
    {
        if (!s->imp()->stateFlags.has(LSurface::LSurfacePrivate::ParentCommitNotified))
        {
            ...
            if (imp.stateFlags.has(LSurface::LSurfacePrivate::ChildrenListChanged))
                goto retryParentCommitNotif;
        }
    }

    if (imp.stateFlags.has(LSurface::LSurfacePrivate::BufferAttached))
    {
    ...
    {
        if (imp.stateFlags.has(LSurface::LSurfacePrivate::InfiniteInput))
        {
     ...
     *********** INVISIBLE REGION ***********
     ****************************************/
    if (imp.stateFlags.has(LSurface::LSurfacePrivate::InfiniteInvisible))
    {
    ...
    if (imp.stateFlags.has(LSurface::LSurfacePrivate::VSync) != preferVSync)
    {
    ...
    if (changes.has(Changes::BufferSizeChanged))
        surface->bufferSizeChanged();

    if (changes.has(Changes::SizeChanged))
        surface->sizeChanged();

    if (changes.has(Changes::SourceRectChanged))
        surface->srcRectChanged();

    if (changes.has(Changes::BufferScaleChanged))
        surface->bufferScaleChanged();

    if (changes.has(Changes::BufferTransformChanged))
        surface->bufferTransformChanged();

    if (changes.has(Changes::DamageRegionChanged))
        surface->damageChanged();

    if (changes.has(Changes::InputRegionChanged))
        surface->inputRegionChanged();

    if (changes.has(Changes::PointerConstraintRegionChanged))
        surface->pointerConstraintRegionChanged();

    if (changes.has(Changes::LockedPointerPosHintChanged))
        surface->lockedPointerPosHintChanged();

    if (changes.has(Changes::OpaqueRegionChanged))
        surface->opaqueRegionChanged();

    if (changes.has(Changes::InvisibleRegionChanged))
        surface->invisibleRegionChanged();

    if (changes.has(Changes::VSyncChanged))
        surface->preferVSyncChanged();

    if (changes.has(Changes::ContentTypeChanged))
