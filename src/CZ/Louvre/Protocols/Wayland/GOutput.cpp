...
bool GOutput::Probe(CZWeak<LGlobal> **slot) noexcept
{
    CZ_UNUSED(slot)
    return true;
}
...
{
    if (output())
        CZVectorUtils::RemoveOneUnordered(client()->imp()->outputGlobals, this);
}
...
void GOutput::geometry(Int32 x, Int32 y, Int32 physicalWidth, Int32 physicalHeight,
                       RSubpixel subpixel, const char *make, const char *model, CZTransform transform) noexcept
{
