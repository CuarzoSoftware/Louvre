...
static const struct lvr_background_blur_interface imp
{
    ...
    .set_color_scheme = &RBackgroundBlur::set_color_scheme,
...
void RBackgroundBlur::set_color_scheme(wl_client */*client*/, wl_resource *resource, UInt32 colorScheme)
{
    auto &res { *static_cast<RBackgroundBlur*>(wl_resource_get_user_data(resource)) };
    ...
    if (colorScheme > LVR_BACKGROUND_BLUR_COLOR_SCHEME_LIGHT)
    {
        res.postError(LVR_BACKGROUND_BLUR_ERROR_INVALID_COLOR_SCHEME, "invalid color scheme value");
        return;
    }

    auto &blur { *res.surfaceRes()->surface()->backgroundBlur() };

    if (blur.pendingProps().colorScheme != (CZColorScheme)colorScheme)
    {
        blur.pendingProps().colorScheme = (CZColorScheme)colorScheme;
        blur.m_flags.add(LBackgroundBlur::SchemeModified);
    }
}
...
void RBackgroundBlur::clear_mask(wl_client */*client*/, wl_resource *resource)
{
    ...
    blur.pendingProps().roundRectMask = {};
    blur.pendingProps().svgPathMask = {};
...
void RBackgroundBlur::set_round_rect_mask(wl_client */*client*/, wl_resource *resource, Int32 x, Int32 y, Int32 width, Int32 height, Int32 radTL, Int32 radTR, Int32 radBR, Int32 radBL)
{
    ...
    const CZRRect newRRect { SkIRect::MakeXYWH(x, y, width, height), radTL, radTR, radBR, radBL };
...
void RBackgroundBlur::set_svg_path_mask(wl_client */*client*/, wl_resource *resource, wl_resource *svgPath)
{
    ...
    blur.m_flags.add(LBackgroundBlur::MaskModified);
    blur.pendingProps().roundRectMask = {};
    blur.pendingProps().svgPathMask = rSvgPath.path();
