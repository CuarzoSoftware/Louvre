    ...
    m_output(output)
{
    ...
    physicalSize(output->mmSize());

    RWlrOutputMode *currMode { nullptr };
    for (std::shared_ptr<LOutputMode> m : output->modes())
    {
    ...
    if (make(output->manufacturer().c_str()))
    {
        model(output->model().c_str());
        serialNumber(output->serialNumber().c_str());
        adaptiveSync(false);
    }
}
...
RWlrOutputHead::~RWlrOutputHead() noexcept
{
    if (m_wlrOutputManager)
        CZVectorUtils::RemoveOneUnordered(m_wlrOutputManager->m_heads, this);

    if (m_output)
        CZVectorUtils::RemoveOneUnordered(m_output->imp()->wlrOutputHeads, this);
}
...
RWlrOutputMode *RWlrOutputHead::mode(std::shared_ptr<LOutputMode> mode) noexcept
{
    for (auto *m : m_modes)
        if (m->mode() == mode)
            return nullptr;
