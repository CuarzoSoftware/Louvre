...
LExclusiveZone::LExclusiveZone(CZEdge edge, Int32 size, LOutput *output) noexcept
{
...
void LExclusiveZone::setEdgeAndSize(CZEdge edge, Int32 size) noexcept
{
    ...
    if (edge != m_edge)
    {
        assert((edge == 0 || edge == CZEdgeLeft || edge == CZEdgeTop || edge == CZEdgeRight || edge == CZEdgeBottom) && "Invalid edge passed to LExclusiveZone constructor.");
        m_edge = edge;
        needsUpdate = true;
    }
...
void LExclusiveZone::setEdge(CZEdge edge) noexcept
{
    if (edge == m_edge)
        return;
