...
void LNinePatchView::init(LTexture *texture, Float32 bufferScale,
  ...
  m_texture.setOnDestroyCallback(
      [this](auto) { setTexture(nullptr, 1.f, {0.f}); });
...
void LNinePatchView::updateSubViews() noexcept {
  ...
  m_subViews[BR].setPos(nativeSize().w() - m_subViews[BR].nativeSize().w(),
  ...
  m_subViews[L].setDstSize(
      ...
      nativeSize().h() - m_subViews[TL].nativeSize().h() -
          m_subViews[BL].nativeSize().h());
  ...
  m_subViews[R].setPos(nativeSize().w() - m_subViews[R].nativeSize().w(),
                       m_center.y());
  m_subViews[R].setDstSize(
      m_subViews[R].nativeSize().w(),  // Keep the same width
      nativeSize().h() - m_subViews[TR].nativeSize().h() -
          m_subViews[BR].nativeSize().h());

  m_subViews[B].setPos(m_center.x(),
                       nativeSize().h() - m_subViews[B].nativeSize().h());
  m_subViews[B].setDstSize(
      nativeSize().w() - m_subViews[TL].nativeSize().w() -
          m_subViews[TR].nativeSize().w(),
      m_subViews[B].nativeSize().h());  // Keep the same height

  m_subViews[C].setPos(m_subViews[TL].nativeSize());
  m_subViews[C].setDstSize(m_subViews[T].nativeSize().w(),
...
void LNinePatchView::setSize(Int32 width, Int32 height) noexcept {
  if (width < m_minSize.w()) width = m_minSize.w();

  if (height < m_minSize.h()) height = m_minSize.h();
...
void LNinePatchView::setTexture(LTexture *texture, Float32 bufferScale,
                                const LRectF &center) noexcept {
  ...
  if (m_nativeSize.w() < m_minSize.w()) m_nativeSize.setW(m_minSize.w());

  if (m_nativeSize.h() < m_minSize.h()) m_nativeSize.setH(m_minSize.h());
  ...
  if (x0 < 0.f)
    x0 = 0.f;
  else if (x0 > m_minSize.w())
    x0 = m_minSize.w();

  if (y0 < 0.f)
    y0 = 0.f;
  else if (y0 > m_minSize.h())
    y0 = m_minSize.h();
  ...
  m_subViews[R].setSrcRect(LRectF(m_center.x() + m_center.w(), m_center.y(),
                                  m_minSize.w() - m_center.x() - m_center.w(),
                                  m_center.h()));
  m_subViews[R].setDstSize(m_subViews[R].srcRect().size());

  m_subViews[B].setSrcRect(LRectF(m_center.x(), m_center.y() + m_center.h(),
                                  m_center.w(),
  ...
  m_subViews[TR].setSrcRect(LRectF(m_center.x() + m_center.w(), 0,
                                   m_minSize.w() - m_center.x() - m_center.w(),
                                   m_center.y()));
  m_subViews[TR].setDstSize(m_subViews[TR].srcRect().size());

  m_subViews[BR].setSrcRect(
      LRectF(m_center.x() + m_center.w(), m_center.y() + m_center.h(),
             m_minSize.w() - m_center.x() - m_center.w(),
             m_minSize.h() - m_center.y() - m_center.h()));
  m_subViews[BR].setDstSize(m_subViews[BR].srcRect().size());

  m_subViews[BL].setSrcRect(
      LRectF(0, m_center.y() + m_center.h(), m_center.x(),
             m_minSize.h() - m_center.y() - m_center.h()));
...
LTextureView *LNinePatchView::getSubView(LBitset<LEdge> edge) noexcept {
  switch (edge.get()) {
    case LEdgeLeft: return &m_subViews[L];
    case LEdgeTop: return &m_subViews[T];
    case LEdgeRight: return &m_subViews[R];
    case LEdgeBottom: return &m_subViews[B];
    case LEdgeTop | LEdgeLeft: return &m_subViews[TL];
    case LEdgeTop | LEdgeRight: return &m_subViews[TR];
    case LEdgeBottom | LEdgeRight: return &m_subViews[BR];
    case LEdgeBottom | LEdgeLeft: return &m_subViews[BL];
    default: return &m_subViews[C];
  }
}
...
const LPoint &LNinePatchView::nativePos() const noexcept { return m_nativePos; }
