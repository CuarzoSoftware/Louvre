 ...
 * By default, it inherits the damage, input, translucent, and opaque regions,
 * as well as the surface position from the LSurface.
 *
 ...
 * - enableCustomTranslucentRegion(): Enables the customization of the
 * translucent region.
 *
 * An LSurface can have multiple views, but only views with the primary()
 * property set to true will clear its damage when requestNextFrame() is called
 * and also send LSurface::sendOutputEnterEvent() and
 * LSurface::sendOutputLeaveEvent() to its client. Use the setPrimary() method
 * to set the primary property of a view.
 *
...
class Louvre::LSurfaceView : public LView {
 public:
   ...
   *
   * Only primary views will clear their damage when requestNextFrame() is
   * called. The default value is true.
   *
   * @return True if the view is a primary view, false otherwise.
   */
  bool primary() const noexcept { return m_state.check(Primary); }
   ...
   */
  void setPrimary(bool primary) noexcept { m_state.setFlag(Primary, primary); }

  /**
   * @brief Check if custom position is enabled for the view.
   *
   * If enabled, nativePos() will return the position set with setCustomPos()
   * instead of the surface position. The default value is false.
   *
   * @return True if custom position is enabled, false otherwise.
   */
  bool customPosEnabled() const noexcept { return m_state.check(CustomPos); }

  /**
   * @brief Enable or disable custom position for the view.
   *
   * If enabled, nativePos() will return the position set with setCustomPos()
   * instead of the surface position.
   *
   ...
   *
   * If enabled, inputRegion() will return the input region set with
   * setCustomInputRegion() instead of the surface input region. The default
   * value is false.
   *
   ...
   *
   * This method allows you to set a custom position for the view, overriding
  ...
  void setCustomPos(Int32 x, Int32 y) noexcept {
    if (x == m_customPos.x() && y == m_customPos.y()) return;

    m_customPos.setX(x);
    m_customPos.setY(y);

    if (!repaintCalled() && customPosEnabled() && mapped()) repaint();
  }
   ...
   */
  const LPoint &customPos() const noexcept { return m_customPos; }

  /**
   * @brief Set a custom input region for the view.
   *
   * This method allows you to set a custom input region for the view,
   * overriding the surface input region.
   *
   ...
   *
   * If enabled, the view will use the custom translucent region set with
   * setCustomTranslucentRegion() instead of the surface translucent region.
   *
   * @param enabled True to enable the custom translucent region, false to
   * disable.
   */
  void enableCustomTranslucentRegion(bool enabled) noexcept {
    if (enabled == customTranslucentRegionEnabled()) return;
   ...
   *
   * This method allows you to set a custom translucent region for the view,
   * overriding the surface translucent region.
   *
   * @param region Pointer to the custom translucent region as an LRegion
   * object.
   */
  ...
  /**
   * @brief Gets the source rect of the surface, equivalent to
   * LSurface::srcRect().
   */
   ...
   *
   * If enabled nativeMapped() will return `true` even if the surface is
   * unmapped as long as it contains a texture.
   *
   * Disabled by default.
   */
  void enableAlwaysMapped(bool enabled) noexcept {
    if (enabled == alwaysMappedEnabled()) return;
