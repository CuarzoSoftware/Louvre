 ...
 *
 * The LTextureView class enables you to use an LTexture as a view within a
 * scene.\n You can set the view's texture using setTexture(), and passing
 * `nullptr` unsets the texture, effectively unmapping the view.\n Multiple
 * views can share the same LTexture, and if the texture is destroyed, their
 * texture() property is automatically set to `nullptr`.
 *
 * Texture views can also have a custom destination size, which may differ from
 * their buffer size. In such cases, regions for damage, input, translucent, and
 * opaque must be defined based on the destination size.\n To enable a custom
 * destination size, use the enableDstSize() and setDstSize() methods.\n
 *
 * @note Using a custom destination size is recommended instead of relying on
 * the scalingVector() option, as it allows for continued damage tracking within
 * the scene.
 *
 * When destination size is disabled, the view size is by default equal to the
 * texture size divided by its buffer scale if no other transformations are
 * applied.
 *
 * As of Louvre version 1.2.0, you can define a source rect with applied
 * transformations, adhering to the behavior outlined in the
 * [Viewporter](https://wayland.app/protocols/viewporter) protocol.
 *
 * For additional methods and properties available, please refer to the
 * documentation of the `LView` class.
 */
class Louvre::LTextureView : public LView {
 public:
  /**
   * @brief Construct an LTextureView with an optional LTexture and parent
   * LView.
   *
   * @param texture The LTexture to be used as the view's texture. Default is
   * nullptr.
  ...
  LTextureView(LTexture *texture = nullptr, LView *parent = nullptr) noexcept
      : LView(LView::TextureType, true, parent) {
   ...
   *
   * Passing `nullptr` as the region means that the entire view is considered
   * translucent, which is the default value. Passing an empty LRegion (not
   * `nullptr`), on the other hand, means the entire view is opaque.
   *
   ...
   *
   * @note If the current texture is destroyed, this property is automatically
   * set to `nullptr`.
   *
   ...
   */
  LTexture *texture() const noexcept { return m_texture.get(); }
  ...
  /**
   * @brief Check if the custom destination size is enabled for the
   * LTextureView.
   *
   * @return True if custom destination size is enabled, false otherwise.
   */
  bool dstSizeEnabled() const noexcept { return m_state.check(CustomDstSize); }
  ...
  void setDstSize(Int32 w, Int32 h) noexcept {
    ...
    if (w == m_customDstSize.w() && h == m_customDstSize.h()) return;

    m_customDstSize.setW(w);
    m_customDstSize.setH(h);
    updateDimensions();

    if (!repaintCalled() && dstSizeEnabled() && mapped()) repaint();
  }

  /**
   * @brief Set the custom destination size of the LTextureView using an LSize
   * object.
   *
   ...
   *
   * This method enables or disables custom coloring for the texture rendering
   * process. When custom coloring is enabled, the texture color is replaced by
   * a custom color while preserving the texture's alpha channel.
   *
   * @param enabled A boolean value indicating whether custom coloring should be
   ...
   *
   * This method returns a boolean value indicating whether custom coloring is
   * currently enabled for the texture rendering process.
   *
   ...
   *
   * This method sets a custom color for the texture rendering process,
   * replacing the original texture color while keeping the texture's alpha
   * channel intact.
   *
  ...
  void setCustomColor(const LRGBF &color) noexcept {
    ...
    if (customColorEnabled()) damageAll();
  }

  /**
   * @brief Gets the current custom color used for texture rendering.
   *
   * This method retrieves the current custom color that is being used for the
   * texture rendering process. The custom color replaces the original texture
   * color while keeping the texture's alpha channel intact.
   *
   * @return A constant reference to the LRGBF object representing the current
   * custom color.
   */
  const LRGBF &customColor() const noexcept { return m_customColor; }
   ...
   *
   * @param enabled If `true`, the custom source rectangle is enabled; if
   * `false`, the entire texture is used as the source.
   */
  void enableSrcRect(bool enabled) noexcept {
    if (srcRectEnabled() == enabled) return;
   ...
   * @see setSrcRect()
   */
  bool srcRectEnabled() const noexcept { return m_state.check(CustomSrcRect); }

  /**
   * @brief Sets the source rect of the texture to use.
   *
   * The source rect must be specified in surface coordinates, taking into
   * account the space generated after the inverse transform() (from transform()
   * to LTransform::Normal) is applied.
   *
  ...
  void setSrcRect(const LRectF &srcRect) noexcept {
    if (m_customSrcRect == srcRect) return;

    m_customSrcRect = srcRect;
    updateDimensions();

    if (srcRectEnabled()) damageAll();
  }

  /**
   * @brief Gets the source rect set with setSrcRect().
   *
   * @note When srcRectEnabled() returns `false` the rect covers the entire
   * texture.
   *
   * @return A constant reference to the source rect specified using
   * setSrcRect().
   */
  const LRectF &srcRect() const noexcept { return m_srcRect; }

  /**
   * @brief Sets the transform of the texture.
   *
   * Use this method to tell the scene the transformation that the texture's
   * content **ALREADY HAS** and not the transformation you want to apply.
   *
   * For example, pass LTransform::Rotated90 if the texture's content is rotated
   * 90 degrees **counter-clockwise**, and the scene (if the current LOutput has
   * a normal transform) will apply a 90-degree **clockwise** rotation to
   * display it normally.
   *
   * Changing the transform affects how the source rectangle is defined and may
   * also swap the original width and height of the view when using transforms
   * with 90 or 270 degrees rotation.
   *
   ...
   */
  LTransform transform() const noexcept { return m_transform; }
