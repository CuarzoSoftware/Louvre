 ...
 *
 * The LScene class is an optional utility that significantly simplifies
rendering.\n
 * It encompasses a primary LSceneView which can host multiple children and even
nested scenes.\n
 * A single LScene can drive multiple outputs and also manage Wayland input
events, while also providing per-view input events.\n
 * You might opt to leverage LScene for rendering instead of relying solely on
the basic rendering functions from the LPainter class or your custom OpenGL
shaders.
 *
 * LScene achieves high efficiency by rendering only damaged regions and
avoiding rendering content obscured by opaque areas.
 *
 * Alternatively, you can still use your own OpenGL shaders or LPainter
functions for rendering, either in conjunction with LScene or independently.\n
 * These approaches can be applied before or after handlePaintGL() is called, or
by creating custom LViews that override the LView::paintEvent() virtual method.
 *
 * ### Rendering
 *
 * For proper rendering with LScene, you need to "plug" the following methods
into each LOutput you intend to manage:
 *
 ...
 *
 * @warning Make sure to "plug" the scene to all the output events mentioned
earlier.
 *          Failing to do so may result in scene initialization issues and
memory leaks.
 *
 ...
 * ### Input Events
 *
 * Similar to rendering, you can integrate LScene into LPointer, LKeyboard and
LTouch for managing input events.
 *
 * @see EventOptions
 */
class Louvre::LScene : public LObject {
 public:
  ...
  enum EventOptions : UInt8 {
    /// All options disabled, input events will only be dispatched to views
    /// acordingly.
    ...
    /// The scene will constraint the cursor within surfaces when allowed
    /// clients require it.
    PointerConstraints = static_cast<UInt8>(1) << 1,

    /// Auxiliary functionality of the default Louvre implementation, such as
    /// exiting the compositor when Ctrl + Shift + Esc is pressed, will be
    /// handled.
   ...
   *
   * When disabled, LOutput::repaint() requests from child views will be
   * ignored.
   *
   ...
   *
   * This vector contains views that have pointer events enabled and whose input
   * region intersects with the current pointer position.
   *
   * The vector is ordered with the topmost views first and the bottommost views
   * last.\n Views with the property LView::blockInputEnabled() disabled allow
   * views behind them to also receive pointer events, which is why multiple
   * views can be in focus at the same time.
   */
   ...
   *
   * Views with keyboard focus are simply those whose
   * LView::keyboardEventsEnabled() property is enabled.
   */
   ...
   *
   * This method should be integrated into LOutput::initializeGL() to properly
   * handle OpenGL initialization for the associated output.
   *
   ...
   *
   * This method should be integrated into LOutput::paintGL() to properly handle
   * OpenGL painting for the associated output.
   *
   ...
   *
   * This method should be integrated into LOutput::moveGL() to properly handle
   * OpenGL movement for the associated output.
   *
   ...
   *
   * This method should be integrated into LOutput::resizeGL() to properly
   * handle OpenGL resizing for the associated output.
   *
   ...
   *
   * This method should be integrated into LOutput::uninitializeGL() to properly
   * handle OpenGL uninitialization for the associated output.
   *
   ...
   *
   * This method should be integrated into LPointer::pointerMoveEvent() to
   * effectively manage pointer movement events.
   *
  ...
  void handlePointerMoveEvent(const LPointerMoveEvent &event,
                              LBitset<EventOptions> options =
                                  WaylandEvents | PointerConstraints | AuxFunc);

  /**
   * @brief Handle pointer button event.
   *
   * This method should be integrated into LPointer::pointerButtonEvent() to
   * handle pointer button events.
   *
   * @param event The pointer button event to handle.
   */
  void handlePointerButtonEvent(
      const LPointerButtonEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);

  /**
   * @brief Handle pointer scroll event.
   *
   * This method should be integrated into LPointer::pointerScrollEvent() to
   * manage pointer scroll events.
   *
   * @param event The pointer scroll event to handle.
   */
  void handlePointerScrollEvent(
      const LPointerScrollEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LPointer::pointerSwipeBeginEvent() to
   * manage pointer swipe begin events.
   *
   * @param event The pointer swipe begin event to handle.
   */
  void handlePointerSwipeBeginEvent(
      const LPointerSwipeBeginEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LPointer::pointerSwipeUpdateEvent()
   * to manage pointer swipe update events.
   *
   * @param event The pointer swipe update event to handle.
   */
  void handlePointerSwipeUpdateEvent(
      const LPointerSwipeUpdateEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LPointer::pointerSwipeEndEvent() to
   * manage pointer swipe end events.
   *
   * @param event The pointer swipe end event to handle.
   */
  void handlePointerSwipeEndEvent(
      const LPointerSwipeEndEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LPointer::pointerPinchBeginEvent() to
   * manage pointer pinch begin events.
   *
   * @param event The pointer pinch begin event to handle.
   */
  void handlePointerPinchBeginEvent(
      const LPointerPinchBeginEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LPointer::pointerPinchUpdateEvent()
   * to manage pointer pinch update events.
   *
   * @param event The pointer pinch update event to handle.
   */
  void handlePointerPinchUpdateEvent(
      const LPointerPinchUpdateEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LPointer::pointerPinchEndEvent() to
   * manage pointer pinch end events.
   *
   * @param event The pointer pinch end event to handle.
   */
  void handlePointerPinchEndEvent(
      const LPointerPinchEndEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LPointer::pointerHoldBeginEvent() to
   * manage pointer hold begin events.
   *
   * @param event The pointer hold begin event to handle.
   */
  void handlePointerHoldBeginEvent(
      const LPointerHoldBeginEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LPointer::pointerHoldEndEvent() to
   * manage pointer hold end events.
   *
   * @param event The pointer hold end event to handle.
   */
  void handlePointerHoldEndEvent(
      const LPointerHoldEndEvent &event,
      LBitset<EventOptions> options = WaylandEvents | PointerConstraints |
                                      AuxFunc);
   ...
   *
   * This method should be integrated into LKeyboard::keyEvent() to handle key
   * events.
   */
   ...
   *
   * This method should be integrated into LTouch::touchDownEvent() to manage
   * touch down events.
   *
  ...
  void handleTouchDownEvent(const LTouchDownEvent &event,
                            ...
                            LBitset<EventOptions> options = WaylandEvents |
                                                            PointerConstraints |
                                                            AuxFunc);
   ...
   *
   * This method should be integrated into LTouch::touchMoveEvent() to manage
   * touch move events.
   *
   ...
   *
   * This method should be integrated into LTouch::touchUpEvent() to manage
   * touch up events.
   *
   ...
   *
   * This method should be integrated into LTouch::touchFrameEvent() to manage
   * touch frame events.
   *
   ...
   *
   * This method should be integrated into LTouch::touchCancelEvent() to manage
   * touch cancel events.
   *
   ...
   *
   * This method returns the first LView whose input region intersects the given
   * position.
   *
   * @param pos The position to query.
   * @param type The type of view to search for. Passing LView::Type::Undefined
   * disables the type filter.
   * @param filter Additional flags for searching only views with pointer,
   * keyboard and/or touch events enabled.
   *
   * @return A pointer to the LView at the specified position, or `nullptr` if
   * no view is found.
   */
  LView *viewAt(const LPoint &pos, LView::Type type = LView::UndefinedType,
