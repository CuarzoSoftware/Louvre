 ...
 *
 * The LNinePatchView class is useful for implementing buttons, containers,
 * borders, etc. in an efficient way. It consists of 9 sub LTextureView s
 * arranged as shown below:
 *
 * <center><img height="512px"
 * src="https://lh3.googleusercontent.com/pw/AP1GczOl2R_1xInk2iUzNxSpwmVISEqAwcNP9chbXLMf4_0EwyZyzxeLSR0Blmq-_QWnnAFT8NdRkGOmLAQuZy1tK6-aIJkPPh_4SaQX63yw4-4GUgDF_HE=w2400"></center>
 *
 * The portion of the texture, position and size of each subview is
 * automatically calculated from the provided center() (E) and the dimensions
 * set with setSize().
 *
 * For more information about the nine-patch technique, check out [this
 * link](https://en.wikipedia.org/wiki/9-slice_scaling).
 *
 * Subviews can be accessed directly via subViews() or getSubView(), for
 * example, to toggle the visibility of a specific one, change its color, etc.
 *
 * @warning If used as borders (with an invisible center), remember to hide the
 * central view, otherwise, the scene will unnecessarily paint invisible pixels.
 */
class Louvre::LNinePatchView : public LView {
 public:
   ...
   * @param center The subrect of the texture corresponding to the central view
   * in surface coordinates. If extended beyond the texture bounds, it will be
   * clipped.
   * @param parent The parent view, or `nullptr` if there is no parent. Defaults
   * to `nullptr`.
   */
  LNinePatchView(LTexture *texture, Float32 bufferScale, const LRectF &center,
   ...
   *
   * The minimum size is equal to the texture's size in surface coordinates (the
   * buffer size divided by the buffer scale) minus the center rect size.
   *
   ...
   * clipped.
   */
  void setTexture(LTexture *texture, Float32 bufferScale,
                  const LRectF &center) noexcept;

  /**
   * @brief Returns the texture subrect for the central view in surface
   * coordinates.
   *
   * @see setTexture()
   *
   ...
   */
  const LRectF &center() const noexcept { return m_center; }
   ...
   *
   * @code
   * ninePatchView->getSubView(LEdgeTop | LEdgeLeft) // Returns the top-left
   * sub-view
   * @endcode
   *
   * @param edge The edge or corner to retrieve. Passing @ref LEdgeNone (or a
   * value that is not a corner or edge) returns the central view.
   ...
   *
   * If a specific view needs to be retrieved, consider using getSubView()
   * instead.
   * @return A reference to the vector of sub-views.
   */
  std::array<LTextureView, 9> &subViews() noexcept { return m_subViews; }
 ...
 protected:
  void init(LTexture *texture, Float32 bufferScale,
            const LRectF &center) noexcept;
