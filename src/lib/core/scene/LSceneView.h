 ...
 *
 * An LSceneView is a unique type of view. Instead of rendering its content
 * directly into an LOutput, it possesses its own framebuffer. This capability
 * allows for advanced blending effects, such as applying masks (check
 * LView::setBlendFunc()).
 *
 * Unlike other with other views, child views of an LSceneView are always
 * clipped to its boundaries.\n LSceneViews can function as children of other
 * views or LSceneViews (just like any other view), but they can also be used
 * independently.\n For example, you might create an LSceneView, populate it
 * with arranged views, execute the render() function, and then use the output
 * as a texture for other porpuses.
 *
 * The main view of the LScene class (LScene::mainView()) is a unique LSceneView
 * designed to render its content onto one or more LOutputs instead of using its
 * own framebuffer.\n
 *
 * @warning Use LSceneViews judiciously. When nested within another scene, they
 * are rendered twice: first, into to its framebuffer, and then into an LOutput
 * framebuffer or another LSurfaceView parent.
 */
class Louvre::LSceneView : public LView {
 public:
  /**
   * @brief Construct an LSceneView with a specified buffer size, buffer scale,
   * and optional parent.
   *
   ...
   */
  LSceneView(const LSize &sizeB, Float32 bufferScale,
             LView *parent = nullptr) noexcept
      : LView(LView::SceneType, true, parent), m_fb(new LRenderBuffer(sizeB)) {
   ...
   */
  const LRGBAF &clearColor() const noexcept { return m_clearColor; }
  ...
  void setClearColor(const LRGBAF &color) noexcept {
    if (m_clearColor.r == color.r && m_clearColor.g == color.g &&
        m_clearColor.b == color.b && m_clearColor.a == color.a)
      return;
   ...
   *
   * This method initiates rendering for the view, excluding specified regions
   * if provided.
   *
   * @note The rendered content can be accessed as a texture using the texture()
   * method.
   *
   ...
   * LSceneViews always have a single texture, with the exception of the main
   * view of an LScene. The main view may possess multiple textures, depending
   * on the current LOutput thread and hardware configuration.
   *
  ...
  void setPos(Int32 x, Int32 y) noexcept {
    if (x != m_customPos.x() || y != m_customPos.y()) {
      ...
      if (!isLScene()) static_cast<LRenderBuffer *>(m_fb)->setPos(m_customPos);
   ...
   *
   * @warning Changing the framebuffer size will destroy the current framebuffer
   * and create a new one. Therefore, this operation should not be performed
   * frequently.
   *
   * @note Calling this method on the main view of an LScene is a no-op.
   * Instead, use LOutput::setMode() to control the size of the output
   * framebuffer.
   *
  ...
  void setSizeB(const LSize &size) noexcept {
    if (!isLScene() && size != m_fb->sizeB()) {
      ...
      for (LOutput *o : compositor()->outputs()) damageAll(o);
      repaint();
    }
  }
 ...
 private:
  friend class LScene;
  friend class LView;
  LSceneView(LFramebuffer *framebuffer = nullptr,
             LView *parent = nullptr) noexcept
      : LView(LView::SceneType, true, parent), m_fb(framebuffer) {}
  ...
  void drawBackground(bool addToOpaqueSum) noexcept {
    ...
    pixman_region32_subtract(&backgroundDamage.m_region,
                             &ctd.newDamage.m_region, &ctd.opaqueSum.m_region);
    ctd.p->setColor(
        {.r = m_clearColor.r, .g = m_clearColor.g, .b = m_clearColor.b});
    ...
    if (addToOpaqueSum) ctd.opaqueSum.addRegion(backgroundDamage);
  }
  ...
  void checkRectChange(ThreadData &ctd) noexcept {
    ...
    if (ctd.o) {
      ...
      if ((ctd.o->fractionalOversamplingEnabled() != ctd.oversampling &&
           ctd.o->usingFractionalScale()) ||
          ctd.o->usingFractionalScale() != ctd.fractionalScale) {
        ctd.fractionalScale = ctd.o->usingFractionalScale();
        ctd.oversampling = ctd.o->fractionalOversamplingEnabled();
        needsDamage = true;
      }

      if (ctd.o->needsFullRepaint()) needsDamage = true;
