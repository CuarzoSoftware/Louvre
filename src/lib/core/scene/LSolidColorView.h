 ...
 *
 * This class provides functionality to display solid color rectangles within a
 * scene.
 */
class Louvre::LSolidColorView : public LView {
 public:
  ...
  LSolidColorView(LView *parent = nullptr) noexcept
      : LView(LView::SolidColorType, true, parent), m_color(0.f, 0.f, 0.f) {}

  /**
   * @brief Construct a solid color view with the specified color components and
   * alpha.
   *
   ...
   * @param parent The parent view that will contain this solid color view.
   */
  LSolidColorView(Float32 r = 0.f, Float32 g = 0.f, Float32 b = 0.f,
                  Float32 a = 1.f, LView *parent = nullptr) noexcept
      : LView(SolidColorType, true, parent), m_color(r, g, b) {
    if (a < 0.f)
      a = 0.f;
    else if (a > 1.f)
      a = 1.f;
   ...
   * @param parent The parent view that will contain this solid color view.
   */
  LSolidColorView(const LRGBF &color, Float32 a = 1.f,
                  LView *parent = nullptr) noexcept
      : LView(SolidColorType, true, parent), m_color(color) {
    if (a < 0.f)
      a = 0.f;
    else if (a > 1.f)
      a = 1.f;
   ...
   */
  const LRGBF &color() const noexcept { return m_color; }
  ...
  /**
   * @brief Set the position of the solid color view using individual X and Y
   * coordinates.
   *
