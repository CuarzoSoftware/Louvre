...
void LTextureView::setTexture(LTexture *texture) noexcept {
  if (texture == m_texture) return;

  m_texture.reset(texture);

  if (m_texture) m_textureSerial = m_texture->serial();
...
const LPoint &LTextureView::nativePos() const noexcept { return m_nativePos; }

const LSize &LTextureView::nativeSize() const noexcept {
  if (m_texture && m_texture->serial() != m_textureSerial) {
    m_textureSerial = m_texture->serial();
    updateDimensions();
  }
...
Float32 LTextureView::bufferScale() const noexcept { return m_bufferScale; }
...
const LRegion *LTextureView::opaqueRegion() const noexcept { return nullptr; }
