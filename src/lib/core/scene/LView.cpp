...
LView::LView(UInt32 type, bool renderable, LView *parent) noexcept
    : m_type(type) {
...
void LView::enableKeyboardEvents(bool enabled) noexcept {
  if (enabled == keyboardEventsEnabled()) return;
  ...
  if (scene()) {
    ...
    scene()->imp()->state.add(
        LScene::LScenePrivate::KeyboardFocusVectorChanged);
  }
}

void LView::enableTouchEvents(bool enabled) noexcept {
  if (enabled == touchEventsEnabled()) return;
...
void LView::enablePointerEvents(bool enabled) noexcept {
  if (enabled == pointerEventsEnabled()) return;
  ...
  if (!enabled) {
    if (m_state.check(PointerIsOver)) {
      if (scene()) {
        ...
        scene()->imp()->state.add(
            LScene::LScenePrivate::PointerFocusVectorChanged);
      }

      m_state.remove(PointerIsOver | PendingSwipeEnd | PendingPinchEnd |
                     PendingHoldEnd);
    }
  }
}
...
void LView::repaint() const noexcept {
  if (m_state.check(RepaintCalled) || !scene() ||
      !scene()->autoRepaintEnabled())
    return;
...
void LView::setParent(LView *view) noexcept {
  if (parent() == view || view == this) return;

  LScene *s{scene()};

  if (s) s->imp()->state.add(LScene::LScenePrivate::ChildrenListChanged);

  if (parent()) parent()->m_children.erase(m_parentLink);

  if (view) {
    view->m_children.push_back(this);
    m_parentLink = std::prev(view->m_children.end());

    if (view->scene() != s) sceneChanged(view->scene());
  } else {
    damageScene(parentSceneView(), true);

    if (s != nullptr) sceneChanged(nullptr);
  }
...
void LView::insertAfter(LView *prev) noexcept {
  ...
  if (prev) {
    ...
    if (prev == parent()->children().back()) {
      ...
      parent()->m_children.erase(m_parentLink);
      m_parentLink =
          parent()->m_children.insert(std::next(prev->m_parentLink), this);
    }
  }
  ...
  // If prev == nullptr, insert to the front of current parent children list
    ...
    // Already in front
    if (parent()->children().front() == this) return;
...
void LView::removeThread(std::thread::id thread) {
  ...
  if (it != m_threadsMap.end()) {
    if (it->second.o) leftOutput(it->second.o);
    m_threadsMap.erase(it);
  }

  if (type() != SceneType) return;
...
void LView::markAsChangedOrder(bool includeChildren) {
  for (auto &pair : m_threadsMap) pair.second.changedOrder = true;

  if (scene()) damageScene(scene()->mainView(), false);

  if (includeChildren)
    for (LView *child : children()) child->markAsChangedOrder();
}

void LView::damageScene(LSceneView *scene, bool includeChildren) {
  if (scene) {
    for (auto &pair : m_threadsMap) {
      if (!pair.second.prevMapped) continue;
...
void LView::sceneChanged(LScene *newScene) {
  if (scene()) {
    ...
    if (m_state.check(PointerIsOver)) {
      ...
      scene()->imp()->state.add(
          LScene::LScenePrivate::PointerFocusVectorChanged);
      m_state.remove(PointerIsOver | PendingHoldEnd | PendingPinchEnd |
                     PendingSwipeEnd);
    }
  ...
  for (LView *child : children()) child->sceneChanged(newScene);
}
