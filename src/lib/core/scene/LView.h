 ...
 *
 * The LView class provides a base interface for creating views that can be
 * shown in an LScene.\n This class should be used for creating custom views
 * that are different from those shipped with Louvre.\n
 *
 * Louvre provides a pre-made set of views for most common purposes:
 *
 * * LLayerView: This view is used only as a container for other views and is
 * not renderable on its own.
 ...
 * * LSceneView: This view is a scene on its own, and its content is rendered in
 * its own framebuffer.
 *
 * ## Stacking
 *
 * Each view can have a parent and multiple children. This hierarchy and the
 * order given by their children views list determine the stacking order in
 * which they are rendered.\n Children views are always stacked on top of its
 * parent, and the last view in a children list is the one stacked on top the
 * rest.
 *
 * ## Creating custom views
 *
 * To create a custom view, you must implement several virtual methods. In
 * summary, for an LScene or LSceneView to be able to render an LView, it must
 * provide the following information:
 *
 * ### Position
 *
 * The position of the view in surface coordinates must be returned by the
 * nativePos() virtual method.\n This method is used by the pos() method, which
 * returns the position equal to nativePos(), or transformed if parent offset or
 * parent scaling is enabled.
 *
 * ### Size
 *
 * The size of the view in surface coordinates should be implemented in the
 * nativeSize() virtual method. This size is then returned by the size() method
 * if no transformation is applied, or may differ if scaling or parent scaling
 * is enabled.
 *
 * ### Mapping
 *
 * To tell a scene if the view is visible, the nativeMapped() bool method must
 * be implemented. This is then returned by the mapped() method, which may
 * differ from nativeMapped() if the visible() property is different, or if the
 * view has no parent, or if its parent is mapped.
 * In summary, a view can only be mapped if nativeMapped() is `true`, the
 * visible() property is `true`, it has a parent, and the parent is also mapped.
 *
 * ### Renderable
 *
 * If the view has content that can be rendered, for example, a texture, then
 * the isRenderable() method must return `true`. On the other hand, if the view
 * is simply a container for other views and is not renderable on its own, then
 * it should return `false`.
 *
 * ### Intersected Outputs
 *
 * The outputs() virtual method must return a vector with the outputs where the
 * view is currently visible. When the view changes, repaint() is called, and
 * all those intersected outputs are scheduled for repainting. The scene informs
 * which outputs a view is currently on based on its rect through the
 * enteredOutput() and leftOutput() virtual methods. So, those methods must be
 * used to update the intersected outputs vector.
 *
 * ### Painting
 *
 * If the view is renderable, the paintEvent() method must be implemented.
 * This method is called by the scene to request the view to render itself into
 * the current framebuffer. It provides an LPainter, the region of the view to
 * be rendered in global compositor coordinates, and a boolean variable
 * indicating whether blending is enabled. The paintEvent() method is called
 * twice per frame: once for drawing only the opaque regions of the view
 * (blending disabled), and a second pass for the translucent regions (blending
 * enabled).
 *
 * @note If you're using custom tools for rendering that can't handle opaque and
 * translucent regions separately, set the view's translucent region to
 *       `nullptr` (see the "Opaque and Translucent Regions" section). This way,
 * only a single paintEvent() with blending enabled will be triggered.
 *
 * ### Damage
 *
 * To inform the scene about which rects within the view must be re-rendered
 * (due to changes), the damage() virtual method must return an LRegion
 * containing the damaged rects. If `nullptr` is returned, the entire view is
 * considered damaged.\n The scene automatically damages the entire view if its
 * position, size, or stacking order changes from one frame to another, so it's
 * not necessary to add damage in those cases. Usually, the damage should be
 * cleared once the view has been rendered. To determine if a view was rendered
 * on a specific output, the requestNextFrame() method is invoked by the scene.
 * So within that method, the damage should be cleared.
 *
 * ### Opaque and Translucent region
 *
 * To prevent the scene view from rendering views occluded by opaque regions,
 * the translucentRegion() and opaqueRegion() must be implemented. They must
 * return an LRegion with the translucent and opaque regions, respectively, in
 * surface coordinates.\n If translucentRegion() returns `nullptr`, the entire
 * view is considered translucent. On the other hand, if opaqueRegion is
 * `nullptr`, the opaque region is considered the inverse of the
 * translucentRegion(). So normally, it's only needed to specify the translucent
 * region. The opaque region must be specified if it has been calculated before
 * so that the scene can use that information and prevent re-calculating the
 * inverse translucent region.
 *
 * ### Input region
 *
 * To define which parts of the view can receive pointer or touch events, the
 * inputRegion() must be implemented.\n If `nullptr` is returned, the entire
 * view is considered capable of receiving events.
 *
 * ## Input events
 *
 * Scenes can trigger specific input events on views through the
 * pointerEnterEvent(), pointerMoveEvent(), keyEvent(), and other related
 * methods.\n Implement these virtual methods to listen and respond to those
 * events if needed.
 */
class Louvre::LView : public LObject {
 public:
  ...
  struct PaintEventParams {
    ...
    /// Indicates if the region to render is opaque (false) or translucent
    /// (true)
    bool blending;
  };
   ...
   *
   * Only views with this property enabled can appear on the
   * LScene::pointerFocus() list after an LScene::handlePointerMoveEvent().
   *
   ...
   *
   * @returns `true` if the view receives pointer and touch events, `false`
   * otherwise.
   */
   ...
   *
   * Even if the scene pointer is over the input region it may return `false` if
   * another view with block pointer enabled is in front.
   *
   * @note Requires pointerEventsEnabled() to be `true`.
   *
   * @return `true` if the pointer/cursor is inside the input region; otherwise,
   * `false`.
   */
  bool pointerIsOver() const noexcept { return m_state.check(PointerIsOver); }

  /**
   * @brief Toggles blocking pointer events to views behind the view's input
   * region.
   *
   * @note Requires pointerEventsEnabled() to be `true`.
   *
   ...
   *
   * When enabled, the view will receive keyboard events and will be added to
   * its parent LScene::keyboardFocus() vector.\n Unlike pointer or touch
   * events, keyboard events for views do not have focus semantics, this means
   * that when enabled, the view will always receive all keyboard events emitted
   * by its parent scene.\n
   *
   ...
   *
   * A view can be touched by multiple touch points simultaneously, see
   * LScene::touchPoints() and LSceneTouchPoint::views()
   *
   * @note The inputRegion() property affects which part of the view can receive
   * events.
   *
   * Disabled by default in all view types except for LSurfaceView.
   *
   * @param enabled Set to `true` to enable touch events, `false` to disable
   * them.
   */
  ...
  /**
   * @brief Toggles blocking touch events to views behind the view's input
   * region.
   *
  ...
  /**
   * @brief Checks if blocking of touch events to views behind the view's input
   * region is enabled.
   *
   * @return `true` if blocking is enabled; otherwise, `false`.
   */
  bool blockTouchEnabled() const noexcept { return m_state.check(BlockTouch); }
   ...
   *
   * @returns A pointer to the scene that contains this view, or `nullptr` if
   * the view is not part of any scene.
   */
   ...
   */
  bool isLScene() const noexcept { return m_state.check(IsScene); }

  /**
   * @brief Gets the closest parent LSceneView.
   *
   * @returns A pointer to the closest LSceneView that contains this view, or
   * `nullptr` if the view is not part of any LSceneView.
   */
  LSceneView *parentSceneView() const noexcept {
    if (parent()) {
      if (parent()->type() == SceneType) return (LSceneView *)parent();
   ...
   *
   * This property indicates whether the view is capable of rendering content
   * (check paintEvent()).\n For example, all view types included in Louvre are
   * renderable, except for LLayerView, which serves as a container for other
   * views but does not produce any output by itself.
   *
  ...
  /**
   * @brief Schedules a repaint for all outputs where this view is currently
   * visible.
   *
   * This method triggers a repaint for all outputs where this view is currently
   ...
   *
   * @returns A pointer to the parent view, or `nullptr` if the view has no
   * parent.
   */
  LView *parent() const noexcept { return m_parent; }

  /**
   * @brief Sets the new parent for the view and insert it at the end of its
   * children list.
   *
   * This method sets the new parent for the view and inserts it at the end of
   * its parent's children list. If 'view' is set to `nullptr`, the parent is
   * unset, and the view is unmapped.
   *
   ...
   *
   * @param prev The view after which this view will be inserted. If set to
   * `nullptr`, the view will be inserted at the beginning of its current
   ...
   *
   * If enabled, the position returned by pos() will include the parent's
   * position (parent()->pos()).
   *
   * Enabled by default.
   *
   * @param enabled If `true`, the parent's position will be applied to the
   * view's position.
   */
  void enableParentOffset(bool enabled) noexcept {
    if (enabled == m_state.check(ParentOffset)) return;
   ...
   *
   * @returns `true` if the parent's offset is applied to the view position,
   * `false` otherwise.
   */
   ...
   *
   * This method returns the current position of the view with any applied
   * transformations.
   *
   * @see nativePos().
   *
  ...
  const LPoint &pos() const noexcept {
    ...
    if (parent()) {
      ...
      if (parentOffsetEnabled()) m_tmpPoint += parent()->pos();
    }
   ...
   *
   * This method returns the current size of the view with any applied
   * transformations.
   *
   * @see nativeSize().
   *
  ...
  const LSize &size() const noexcept {
    ...
    if (scalingEnabled()) m_tmpSize *= scalingVector(true);
  ...
  /**
   * @brief Checks if the view is currently being clipped to the clippingRect()
   * property.
   *
   * This method returns `true` if the view is currently being clipped to the
   * clippingRect() property.
   *
   * @returns `true` if the view is being clipped to clippingRect(), `false`
   * otherwise.
   */
  bool clippingEnabled() const noexcept { return m_state.check(Clipping); }
   ...
   *
   * @note The rect is not local to the view's position, if you need to clip the
   * view without having to update the rect position each time the view moves,
   * use parent clipping instead.
   *
   ...
   */
  const LRect &clippingRect() const noexcept { return m_clippingRect; }
   ...
   * Disabled by default.
   *
   * @param enabled If `true`, the view will be clipped to the current parent
   * view rect.
   */
  void enableParentClipping(bool enabled) noexcept {
    if (enabled == m_state.check(ParentClipping)) return;
   ...
   *
   * @returns `true` if the view is clipped to the current parent view rect,
   * `false` otherwise.
   */
  ...
  void enableScaling(bool enabled) noexcept {
    if (enabled == m_state.check(Scaling)) return;
   ...
   */
  bool scalingEnabled() const noexcept { return m_state.check(Scaling); }

  /**
   * @brief Sets the scalingVector() property.
   *
   * If scalingEnabled() is `true`, the view's size will be scaled using the
   * provided scaling vector (nativeSize() * scalingVector()).
   *
   * The default value is (1.f, 1.f).
   *
   * @warning Scaling should be used with moderation, preferably during
   * animations, as damage tracking is disabled due to precision loss caused by
   * scaling. This means the entire view is repainted if changes occur, which
   * can be inefficient.
   */
  void setScalingVector(const LSizeF &scalingVector) noexcept {
    if (scalingVector == m_scalingVector) return;
   ...
   *
   * @param forceIgnoreParent If set to `false`, the vector is multiplied by the
   * parent scaling vector.
   * @returns The scaling vector for the view's size.
   */
  const LSizeF &scalingVector(bool forceIgnoreParent = false) const noexcept {
    if (forceIgnoreParent) return m_scalingVector;
  ...
  /**
   * @brief Toggles the scaling of size and position by the parent's scaling
   * vector.
   *
   * If enabled, the view's size and position will be scaled by the parent's
   * scalingVector().
   *
   * Disabled by default.
   *
   * @param enabled If `true`, the view's size and position will be scaled by
   * the parent's scaling vector.
   */
  void enableParentScaling(bool enabled) noexcept {
    if (enabled == m_state.check(ParentScaling)) return;
  ...
  /**
   * @brief Checks if the size and position are scaled by the parent's
   * scalingVector().
   *
   * Disabled by default.
   */
   ...
   *
   * @note Enabling visibility does not guarantee that the view will be
   * rendered, see mapped().
   *
   * Enabled by default.
   */
  void setVisible(bool visible) noexcept {
    if (m_state.check(Visible) == visible) return;

    const bool prev{mapped()};
    m_state.setFlag(Visible, visible);

    if (!repaintCalled() && prev != mapped()) repaint();
  }
   ...
   */
  bool visible() const noexcept { return m_state.check(Visible); }

  /**
   * @brief Checks if the view should be rendered, taking into consideration
   * several conditions.
   *
   * This method indicates whether the view should be rendered, considering the
   * nativeMapped() && visible() && parent() && parent()->mapped() condition.
   *
   * @returns `true` if the view should be rendered, `false` otherwise.
   */
  bool mapped() const noexcept {
    if (type() == SceneType && !parent()) return visible();
   ...
   *
   * @warning Opacity values != 1.f should be used with moderation, for example,
   * only during animations, as the opaqueRegion() is not ignored. This means
   * that content behind the view is always repainted when the view changes.
   *
  ...
  void enableParentOpacity(bool enabled) noexcept {
    if (m_state.check(ParentOpacity) == enabled) return;
   ...
   * of [glBlendFuncSeparate()](https://docs.gl/es2/glBlendFuncSeparate) for
   * more information.
   *
   * @note Requires autoBlendFuncEnabled() to be disabled.
   */
  void setBlendFunc(const LBlendFunc &blendFunc) noexcept {
    if (blendFunc.sRGBFactor != m_blendFunc.sRGBFactor ||
        blendFunc.dRGBFactor != m_blendFunc.dRGBFactor ||
        blendFunc.sAlphaFactor != m_blendFunc.sAlphaFactor ||
        blendFunc.dAlphaFactor != m_blendFunc.dAlphaFactor) {
   ...
   */
  const LBlendFunc &blendFunc() const noexcept { return m_blendFunc; }

  /**
   * @brief Toggles the automatic blend function adjustment.
   *
   * When enabled, the blend function dynamically adjusts based on whether the
   * rendered content is premultiplied alpha or not.
   *
  ...
  void enableAutoBlendFunc(bool enabled) noexcept {
    if (enabled == m_state.check(AutoBlendFunc)) return;
  ...
  void setColorFactor(const LRGBAF &colorFactor) noexcept {
    if (m_colorFactor != colorFactor) {
      ...
      repaint();
      m_state.setFlag(ColorFactor,
                      m_colorFactor.r != 1.f || m_colorFactor.g != 1.f ||
                          m_colorFactor.b != 1.f || m_colorFactor.a != 1.f);
    }
  }
   ...
   * See setColorFactor().
   */
  const LRGBAF &colorFactor() const noexcept { return m_colorFactor; }

  /**
   * @brief Gets the bounding box of the view and all its mapped children.
   *
   * This method returns a box containing the view and all its mapped children,
   * even if the children are outside or clipped by the view's rect.
   *
  ...
  LBox boundingBox() const noexcept {
    ...
    for (LView *child : children()) {
      ...
      if (childBox.x1 < box.x1) box.x1 = childBox.x1;

      if (childBox.y1 < box.y1) box.y1 = childBox.y1;

      if (childBox.x2 > box.x2) box.x2 = childBox.x2;

      if (childBox.y2 > box.y2) box.y2 = childBox.y2;
    }
   ...
   *
   * @return `true` if the view should be rendered without considering
   ...
   *
   * This property is primarily used by views that contain a buffer like for
   * example the LSceneView, LSurfaceView and LTextureView types.
   *
   ...
   *
   * This method is invoked by a LScene when the view's rect intersects an
   * output.
   *
   ...
   *
   * This method is invoked by a LScene when the view's rect no longer
   * intersects an output.
   *
  ...
  /**
   * @brief Gets a vector of output pointers on which the view is currently
   * visible.
   *
   * Must return a vector of output pointers where the view is currently
   * visible.\n Use the enteredOutput() and leftOutput() methods to update the
   * vector.
   *
   * @return A reference to a vector of LOutput pointers representing the
   * outputs where the view is visible.
   */
   ...
   *
   * This method is called by the closest parent scene view and should be used
   * to clear the previous view damage or update its content.\n If
   * forceRequestNextFrameEnabled() is `true`, this method is always called.
   *
   ...
   *
   * @return A pointer to the LRegion specifying the damaged area within the
   * view, or `nullptr` if the entire view rect is damaged.
   */
   ...
   *
   * The region must be specified in surface-local coordinates, without any
   * scaling, clipping, or offset transformations applied.
   *
   * If `nullptr` is returned, the entire view rect will be considered
   * translucent.
   */
   ...
   * scaling, clipping, or offset transformations applied.
   *
   * If `nullptr` is returned, the inverse of the translucent region will be
   * considered opaque.
   *
   ...
   *
   * This method is used by the closest parent LSceneView to request the view to
   * paint a specified region on the current framebuffer. Painting can be
   * performed using the provided LPainter object.
   *
   ...
   *
   * This event is only triggered if pointerEnterEvent() was called before, and
   * therefore when hasPointerFocus() returns `true`.
   */
   ...
   *
   * Keyboard events are triggered only if keyboardEventsEnabled() is set to
   * `true`.
   */
  ...
  // This is used for detecting changes on a view since the last time it was
  // drawn on a specific output
 ...
 protected:
  ...
  mutable LBitset<LViewState> m_state{Visible | ParentOffset | ParentOpacity |
                                      BlockPointer | AutoBlendFunc};
  ...
  LBlendFunc m_blendFunc{GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_SRC_ALPHA,
  ...
  bool repaintCalled() const noexcept { return m_state.check(RepaintCalled); }

  static void removeFlagWithChildren(LView *view, UInt64 flag) {
    view->m_state.remove(flag);

    for (LView *child : view->children()) removeFlagWithChildren(child, flag);
  }
