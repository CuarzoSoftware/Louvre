...
LSurfaceView::LSurfaceView(LSurface *surface, LView *parent) noexcept
    : LView(LView::SurfaceType, true, parent), m_surface(surface) {
  setPrimary(true);
...
LSurfaceView::~LSurfaceView() noexcept {
  ...
  if (surface()) LVectorRemoveOneUnordered(surface()->imp()->views, this);
}

const LRectF &LSurfaceView::srcRect() const {
  if (surface()) return surface()->srcRect();
...
bool LSurfaceView::nativeMapped() const noexcept {
  if (surface()) {
    if (alwaysMappedEnabled()) return true;
...
void LSurfaceView::requestNextFrame(LOutput *output) noexcept {
  ...
  if (forceRequestNextFrameEnabled()) {
    ...
    m_threadsMap[output->threadId()].lastRenderedDamageId =
        surface()->damageId();
    return;
  }
  ...
  for (LOutput *o : surface()->outputs()) {
    // If the view is visible on another output and has not rendered the new
    // damage prevent clearing the damage immediately
    if (o != output && (m_threadsMap[o->threadId()].lastRenderedDamageId <
                        surface()->damageId())) {
      clearDamage = false;
      o->repaint();
    }
...
const LRegion *LSurfaceView::damage() const noexcept {
  if (surface()) return &surface()->damage();
...
const LRegion *LSurfaceView::opaqueRegion() const noexcept {
  if (customTranslucentRegionEnabled() || !surface()) return nullptr;
