...
void LScene::handlePointerMoveEvent(const LPointerMoveEvent &event,
                                    LBitset<EventOptions> options) {
  // Prevent recursive calls
  if (imp()->state.check(LSS::HandlingPointerMoveEvent)) return;
  ...
  if ((options & (WaylandEvents | PointerConstraints)) ==
          (WaylandEvents | PointerConstraints) &&
      seat()->pointer()->focus() &&
      seat()->pointer()->focus()->imp()->lastPointerEventView) {
    ...
    if ((firstSurfaceView->scalingEnabled() ||
         firstSurfaceView->parentScalingEnabled()) &&
        firstSurfaceView->scalingVector().area() != 0.f)
      scalingVector = firstSurfaceView->scalingVector();

    const LPointF scaledPosDiff{(cursor()->pos() - pos) / scalingVector};

    if (surface->pointerConstraintMode() !=
        LSurface::PointerConstraintMode::Free) {
    ...
    if (surface->pointerConstraintEnabled()) {
      if (surface->pointerConstraintMode() ==
          LSurface::PointerConstraintMode::Lock) {
        if (surface->lockedPointerPosHint().x() >= 0.f)
          cursor()->setPos(pos +
                           surface->lockedPointerPosHint() * scalingVector);
        else {
          cursor()->move(-event.delta().x(), -event.delta().y());

          const LPointF closestPoint{
              surface->pointerConstraintRegion().closestPointFrom(
                  (cursor()->pos() - pos) / scalingVector, 0.5f) *
              scalingVector};
        ...
        const LPointF closestPoint{
            surface->pointerConstraintRegion().closestPointFrom(scaledPosDiff,
                                                                0.5f) *
            scalingVector};
  ...
  if (!(options & WaylandEvents)) return;

  const bool sessionLocked{sessionLockManager()->state() !=
                           LSessionLockManager::Unlocked};
  LPointF localPos;

  if (!firstSurfaceView || !surface || !surface->hasPointerFocus() ||
      !surface->pointerConstraintEnabled()) {
  ...
  if (firstSurfaceView) {
    if (!sessionLocked || (sessionLockManager()->client() ==
                           firstSurfaceView->surface()->client())) {
      localPos = imp()->viewLocalPos(firstSurfaceView, cursor()->pos());
      surface = firstSurfaceView->surface();
    }
  }

  const bool activeDND{seat()->dnd()->dragging() &&
                       seat()->dnd()->triggeringEvent().type() !=
                           LEvent::Type::Touch};
  ...
  if (seat()->pointer()->draggingSurface()) {
    if (seat()->pointer()->draggingSurface()->imp()->lastPointerEventView) {
      imp()->currentPointerMoveEvent.localPos = imp()->viewLocalPos(
          seat()->pointer()->draggingSurface()->imp()->lastPointerEventView,
          cursor()->pos());
      seat()->pointer()->sendMoveEvent(imp()->currentPointerMoveEvent);
    } else {
      imp()->currentPointerMoveEvent.localPos =
          cursor()->pos() - seat()->pointer()->draggingSurface()->rolePos();
      seat()->pointer()->sendMoveEvent(imp()->currentPointerMoveEvent);
    }
...
void LScene::handlePointerButtonEvent(const LPointerButtonEvent &event,
                                      LBitset<EventOptions> options) {
  // Prevent recursive calls
  if (imp()->state.check(LSS::HandlingPointerButtonEvent)) return;
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerButtonDone)) continue;

    view->m_state.add(LVS::PointerButtonDone);
    view->pointerButtonEvent(event);

    if (imp()->state.check(LSS::PointerFocusVectorChanged)) goto retry;
  }
  ...
  const bool sessionLocked{sessionLockManager()->state() !=
                           LSessionLockManager::Unlocked};
  const bool activeDND{dnd.dragging() &&
                       dnd.triggeringEvent().type() != LEvent::Type::Touch};
  ...
  if (!pointer.focus()) {
    ...
    LView *view{
        viewAt(cursor()->pos(), LView::UndefinedType, InputFilter::Pointer)};
  ...
  if (event.state() == LPointerButtonEvent::Pressed) {
    ...
    if (pointer.focus()->toplevel() &&
        !pointer.focus()->toplevel()->activated())
      pointer.focus()->toplevel()->configureState(
          pointer.focus()->toplevel()->pendingConfiguration().state |
  ...
  // Button released
    ...
    if (pointer.focus()->imp()->lastPointerEventView) {
      if (!imp()->pointIsOverView(pointer.focus()->imp()->lastPointerEventView,
                                  cursor()->pos(), InputFilter::Pointer))
        pointer.setFocus(nullptr);
    } else if (!pointer.focus()->inputRegion().containsPoint(
                   cursor()->pos() - pointer.focus()->pos()))
      pointer.setFocus(nullptr);
  }
}
...
void LScene::handlePointerScrollEvent(const LPointerScrollEvent &event,
  ...
  // Prevent recursive calls
  if (imp()->state.check(handlingEventFlag)) return;
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerScrollDone)) continue;
...
void LScene::handlePointerSwipeBeginEvent(const LPointerSwipeBeginEvent &event,
                                          LBitset<EventOptions> options) {
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerSwipeBeginDone)) continue;
...
void LScene::handlePointerSwipeUpdateEvent(
    const LPointerSwipeUpdateEvent &event, LBitset<EventOptions> options) {
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerSwipeUpdateDone)) continue;
...
void LScene::handlePointerSwipeEndEvent(const LPointerSwipeEndEvent &event,
                                        LBitset<EventOptions> options) {
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerSwipeEndDone)) continue;
...
void LScene::handlePointerPinchBeginEvent(const LPointerPinchBeginEvent &event,
                                          LBitset<EventOptions> options) {
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerPinchBeginDone)) continue;
...
void LScene::handlePointerPinchUpdateEvent(
    const LPointerPinchUpdateEvent &event, LBitset<EventOptions> options) {
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerPinchUpdateDone)) continue;
...
void LScene::handlePointerPinchEndEvent(const LPointerPinchEndEvent &event,
                                        LBitset<EventOptions> options) {
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerPinchEndDone)) continue;
...
void LScene::handlePointerHoldBeginEvent(const LPointerHoldBeginEvent &event,
                                         LBitset<EventOptions> options) {
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerHoldBeginDone)) continue;
...
void LScene::handlePointerHoldEndEvent(const LPointerHoldEndEvent &event,
                                       LBitset<EventOptions> options) {
...
retry:
  ...
  for (LView *view : imp()->pointerFocus) {
    if (view->m_state.check(LVS::PointerHoldEndDone)) continue;
...
void LScene::handleKeyboardKeyEvent(const LKeyboardKeyEvent &event,
                                    LBitset<EventOptions> options) {
  ...
  for (LView *view : imp()->keyboardFocus) view->m_state.remove(LVS::KeyDone);
...
retry:
  ...
  for (LView *view : imp()->keyboardFocus) {
    if (view->m_state.check(LVS::KeyDone)) continue;

    view->m_state.add(LVS::KeyDone);
    view->keyEvent(event);

    if (imp()->state.check(LSS::KeyboardFocusVectorChanged)) goto retry;
  }

  imp()->state.remove(handlingEventFlag);

  if (options & WaylandEvents) seat()->keyboard()->sendKeyEvent(event);

  if (!(options & AuxFunc)) return;
  ...
  if (event.state() == LKeyboardKeyEvent::Released) {
    ...
    if (event.keyCode() == KEY_F1 && !mods)
      ...
      if (keyboard.focus()) keyboard.focus()->client()->destroyLater();
    } else if (L_CTRL && (sym == XKB_KEY_m || sym == XKB_KEY_M)) {
      if (keyboard.focus() && keyboard.focus()->toplevel() &&
          !keyboard.focus()->toplevel()->fullscreen())
    ...
    // Screenshot
      ...
      if (cursor()->output() && cursor()->output()->bufferTexture(0)) {
        ...
        std::strftime(timeString, sizeof(timeString), "%Y-%m-%d %H:%M:%S.png",
                      std::localtime(&time));
...
void LScene::handleTouchDownEvent(const LTouchDownEvent &event,
  ...
  if (surfaceView) {
    if (sessionLocked &&
        sessionLockManager()->client() != surfaceView->surface()->client())
      goto end;

    event.localPos = globalPos - surfaceView->pos();

    if (!seat()->keyboard()->focus() ||
        !surfaceView->surface()->isSubchildOf(seat()->keyboard()->focus()))
...
void LScene::handleTouchMoveEvent(const LTouchMoveEvent &event,
                                  const LPointF &globalPos,
                                  LBitset<EventOptions> options) {
  auto &&handlingEventFlag{LSS::HandlingTouchEvent};
  ...
  if (!imp()->currentTouchPoint) goto skipViews;
...
skipViews:
  ...
  if (dnd.dragging() && dnd.triggeringEvent().type() == LEvent::Type::Touch) {
    ...
    if (touchEvent.id() == tp->id()) {
      ...
      LSurfaceView *surfaceView{static_cast<LSurfaceView *>(
          viewAt(globalPos, LView::SurfaceType,
                 InputFilter::Pointer | InputFilter::Touch))};

      if (surfaceView) {
        if (dnd.focus() == surfaceView->surface())
          dnd.sendMoveEvent(imp()->viewLocalPos(surfaceView, globalPos),
                            event.ms());
        else
          dnd.setFocus(surfaceView->surface(),
                       imp()->viewLocalPos(surfaceView, globalPos));
      } else
  ...
  if (tp->surface()) {
    if (tp->surface()->imp()->lastTouchEventView)
      event.localPos = imp()->viewLocalPos(
          tp->surface()->imp()->lastTouchEventView, globalPos);
    else
...
void LScene::handleTouchUpEvent(const LTouchUpEvent &event,
                                LBitset<EventOptions> options) {
  auto &&handlingEventFlag{LSS::HandlingTouchEvent};
...
retry:
  ...
  for (LView *view : imp()->currentTouchPoint->views()) {
    if (view->m_state.check(LVS::TouchUpDone)) continue;
...
void LScene::handleTouchFrameEvent(const LTouchFrameEvent &event,
                                   LBitset<EventOptions> options) {
  auto &&handlingEventFlag{LSS::HandlingTouchEvent};
...
retry:
  ...
  for (LView *view : imp()->currentTouchPoint->views()) {
    if (view->m_state.check(LVS::TouchFrameDone)) continue;
...
void LScene::handleTouchCancelEvent(const LTouchCancelEvent &event,
                                    LBitset<EventOptions> options) {
  auto &&handlingEventFlag{LSS::HandlingTouchEvent};
...
retry:
  ...
  for (LView *view : imp()->currentTouchPoint->views()) {
    if (view->m_state.check(LVS::TouchCancelDone)) continue;
  ...
  while (!imp()->touchPoints.empty()) imp()->touchPoints.back()->destroy();
...
LSceneView *LScene::mainView() const { return &imp()->view; }

LView *LScene::viewAt(const LPoint &pos, LView::Type type,
                      LBitset<InputFilter> filter) {
