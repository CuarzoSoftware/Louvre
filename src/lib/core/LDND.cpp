...
LDND::LDND(const void *params) noexcept : LFactoryObject(FactoryObjectType) {
  ...
  assert(*ptr == nullptr && *ptr == seat()->dnd() &&
         "Only a single LDND instance can exist.");
  *ptr = this;
}
...
void LDND::setFocus(LSurface *surface, const LPointF &localPos) noexcept {
  ...
  if (!surface) {
    ...
    // If the source is NULL, only surfaces from the src client are allowed to
    // gain focus
  ...
  if (m_session->source) {
    m_session->offer.reset(
        m_session->dstDataDevice->createOffer(RDataSource::DND));
    m_session->offer->m_dndSession = m_session;

    m_session->dstDataDevice->m_enterSerial = serial;
    m_session->dstDataDevice->enter(serial, surface->surfaceResource(), x, y,
                                    m_session->offer);

    m_session->offer->sourceActions(m_session->source->actions());
  }
  /* If source is NULL, enter, leave and motion events are sent only to the
   * client that initiated the drag and the client is expected to handle the
   * data passing internally */
  else {
    m_session->dstDataDevice->m_enterSerial = serial;
    m_session->dstDataDevice->enter(serial, focus()->surfaceResource(), x, y,
                                    nullptr);
  }
}

void LDND::sendMoveEvent(const LPointF &localPos, UInt32 ms) noexcept {
  if (!focus() || !m_session || !m_session->dstDataDevice) return;

  m_session->dstDataDevice->motion(ms, wl_fixed_from_double(localPos.x()),
...
LDNDIconRole *LDND::icon() const noexcept {
  if (m_session) return m_session->icon;
  return nullptr;
}

LSurface *LDND::origin() const noexcept {
  if (m_session) return m_session->origin;
  return nullptr;
}

LSurface *LDND::focus() const noexcept {
  if (m_session) return m_session->focus;
  return nullptr;
}

bool LDND::dragging() const noexcept { return m_session != nullptr; }
...
void LDND::drop() noexcept {
  ...
  if (icon() && icon()->surface()) icon()->surface()->imp()->setMapped(false);
  ...
  const bool cancelled{m_session->offer && m_session->offer->version() >= 3 &&
                       !m_session->offer->matchedMimeType()};

  if (m_session->dstDataDevice) {
    if (!cancelled) m_session->dstDataDevice->drop();
  ...
  if (m_session->source) {
    ...
    if (cancelled) m_session->source->cancelled();
  }
...
LDND::Action LDND::action() const noexcept {
  if (!m_session || !m_session->source) return NoAction;
...
void LDND::setPreferredAction(LDND::Action action) noexcept {
  if (m_compositorAction == action) return;
