...
LTouchPoint::LTouchPoint(const LTouchDownEvent &event) noexcept
    : m_lastDownEvent(event) {
  seat()->touch()->m_touchPoints.push_back(this);

  m_surface.setOnDestroyCallback([this](auto) { resetSerials(); });
}
...
void LTouchPoint::sendTouchFrameEvent() noexcept {
  ...
  for (GSeat *s : surface()->client()->seatGlobals())
    for (RTouch *t : s->touchRes()) t->frame();
}
...
void LTouchPoint::sendTouchCancelEvent() noexcept {
  ...
  for (GSeat *s : surface()->client()->seatGlobals())
    for (RTouch *t : s->touchRes()) t->cancel();
}
...
bool LTouchPoint::sendDownEvent(const LTouchDownEvent &event,
                                LSurface *surf) noexcept {
  ...
  if (surface() && (!surf || surface() != surf)) {
    for (GSeat *s : surface()->client()->seatGlobals()) {
      for (RTouch *t : s->touchRes()) {
        t->up(LTouchUpEvent(id(), LTime::nextSerial(), event.ms(), event.us(),
                            event.device()));
        t->frame();
      }
    }
...
bool LTouchPoint::sendMoveEvent(const LTouchMoveEvent &event) noexcept {
  ...
  for (GSeat *s : surface()->client()->seatGlobals())
    for (RTouch *t : s->touchRes()) t->motion(event.ms(), id(), x, y);
...
bool LTouchPoint::sendUpEvent(const LTouchUpEvent &event) noexcept {
  ...
  for (GSeat *s : surface()->client()->seatGlobals())
    for (RTouch *t : s->touchRes()) t->up(event);
