 ...
 * Lock](https://wayland.app/protocols/ext-session-lock-v1) protocol can request
 * the compositor to lock the user session and display arbitrary graphics (see
 * LSessionLockRole) such as an authentication form to allow the user to unlock
 * the session.\n This class allows you to accept/decline such requests (see
 * lockRequest()) and monitor changes in the session state (stateChanged()).
 */
class Louvre::LSessionLockManager : public LFactoryObject {
 public:
  static constexpr LFactoryObject::Type FactoryObjectType =
      LFactoryObject::Type::LSessionLockManager;
   ...
   *
   * There is only one instance of LSessionLockManager, which can be accessed
   * from LCompositor::sessionLockManager().
   *
   ...
   *
   * @return A pointer to the client locking the session, or `nullptr` if the
   * session is unlocked or the locking client died.
   */
  ...
  State state() const noexcept {
    if (m_state == Locked) return client() == nullptr ? DeadLocked : Locked;
    return Unlocked;
  }
   ...
   *
   * If the locking client dies without unlocking the session, the session
   * remains locked (@ref DeadLocked state). This method allows the compositor
   * to forcibly unlock the session, ideally after a fallback user
   ...
   *
   * This method determines whether to grant permission for the client to lock
   * the session. Returning `true` indicates permission, while `false` denies
   * the request.
   *
   * @warning It's recommended that the client be a trusted and well-known
   * entity.
   *
   * This method is only invoked when the session state is @ref Unlocked or @ref
   * DeadLocked. Louvre automatically ignores requests in the @ref Locked state.
   *
