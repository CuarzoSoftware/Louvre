 ...
 *
 * The LCursor class is designed to make cursor rendering easier and take
 * advantage of compositing properties of certain graphic backends to improve
 * performance.
 *
 * @see LXCursor, LClientCursor, LCursorRole.
 *
 * @note Clients can request to set the cursor through
 * LPointer::setCursorRequest(), see LClientCursor.
 *
 * @subsection hw_composition Hardware Composition
 *
 * Some graphic backends, such as DRM, allow for hardware cursor compositing,
 * which can improve performance by reducing the need to repaint an output every
 * time the cursor changes position.\n To check if an output supports hardware
 * cursor compositing, use hasHardwareSupport().\n If hardware compositing is
 * not supported for a specific output, the cursor is rendered by Louvre using
 * OpenGL after a paintGL() event (when needed). In such cases the damage region
 * provided by damage() has to be repainted.
 */
class Louvre::LCursor final : public LObject {
 public:
   ...
   *
   * @note Louvre automatically repositions the cursor if the specified position
   * is not within any output.
   */
   ...
   *
   * @note Louvre automatically repositions the cursor if the new position is
   * not within any output.
   */
   ...
   *
   * Returns the cursor rect, which is defined as LRect(pos - hotspot, size), in
   * compositor-global coordinates.
   *
   * You can use this rect and texture() to paint the cursor with LPainter if
   * needed.
   */
   ...
   *
   * Sets the cursor size in surface coordinates. The texture and hotspot are
   * automatically scaled, with the hotspot maintaining its proportion to the
   * texture buffer size.
   *
   * @note You don't need to set the cursor size every time you change its
   * texture or hotspot, Louvre automatically updates it.
   *
   ...
   *
   * While an LClientCursor is assigned, the cursor is automatically updated
   * when the client updates its LCursorRole surface texture, hotspot, and
   * visibility.
   *
   * If the LClientCursor is destroyed while set, useDefault() is called and the
   * cursor is set to visible.
   *
   ...
   *
   * Assigns the texture and hotspot of the cursor. The texture size does not
   * necessarily define the cursor size, see setSize().
   *
   ...
   *
   * The default texture initially matches defaultLouvreTexture() with a hotspot
   * at (8, 8).
   */
   ...
   *
   * This method allows you to replace the Louvre's default cursor texture and
   * hotspot, which is set when useDefault() is called.
   *
   * @param texture The new texture to use as the default cursor, or `nullptr`
   * to restore the default Louvre cursor.
   *
   * @param hotspot The hotspot position for the new cursor in buffer
   * coordinates.
   */
  void replaceDefaultB(const LTexture *texture,
                       const LPointF &hotspot) noexcept;

  /**
   * @brief Gets the default cursor texture.
   *
   * This method returns the texture that has been set using
   * replaceDefaultB().\n Initially, the default texture is the same as
   ...
   *
   * <center><IMG
   * SRC="https://lh3.googleusercontent.com/MSUUg3LSS6lYtpyLnKzbECf9eeZeFscmnLGJLRCdADwcjjcVd4xT07AMvQoHUTGptJFzY4tZrQ3IdLKyEbM_O0WyWYk8Pvc-Jf8xZHXoFUkFo2RRYTP8zN_LeOhsvIc6SlsO83TJUw=w2400"></center>
   *
   * The default Louvre's cursor has a size of 64x64 pixels and hotspot at
   * (8,8).
   */
   ...
   *
   * The cursor hotspot is defined by coordinates relative to the origin of its
   * buffer (upper left corner).
   *
   * @note The hotspot is automatically scaled proportionally to the cursor
   * size().
   *
   ...
   * @return `true` if LCursor is enabled for the given output, `false`
   * otherwise.
   */
   ...
   *
   * When disabled, Louvre will render the cursor using OpenGL after an
   * LOutput::paintGL() event.
   *
   * @see damage()
   *
   * Hardware compositing is enabled by default if the output supports it, see
   * hasHardwareSupport().
   *
   * @param output The output for which to enable or disable hardware
   * compositing.
   * @param enabled Set to `true` to enable hardware compositing, or `false` to
   * disable it.
   */
   ...
   *
   * @note This method always returns `false` if hasHardwareSupport() for the
   * given output returns `false`.
   *
   * @param output The output to check.
   * @return `true` if hardware compositing is enabled for the given output,
   * `false` otherwise.
   */
   ...
   *
   * Provides the region that needs to be repainted when hardware compositing
   * isn't supported or is disabled, allowing Louvre to properly render it after
   * an LOutput::paintGL() event.
   *
   ...
   *
   * @note This method always returns a valid output unless there are none
   * initialized.
   */
   ...
   *
   * Returns a vector of initialized outputs that intersect with the cursor's
   * rect() property.
   */
   ...
   *
   * Invokes LOutput::repaint() for each output in the vector of intersected
   * outputs.
   *
   * @param nonHardwareOnly If `true`, only repaints outputs that do not support
