...
Int8 LScreenshotRequest::copy() noexcept {
  ...
  if (wl_shm_buffer_get(resource().buffer())) {
    if (resource().screenCopyManagerRes()) {
      auto &outputDamage{
          resource().screenCopyManagerRes()->damage[resource().output()]};
      outputDamage.damage.clip(resource().rectB());

      // No damage, wait...
      if (resource().waitForDamage() && outputDamage.damage.empty()) return 0;
    ...
    glReadPixels(resource().rectB().x(),
                 ...
                 resource().rectB().w(), resource().rectB().h(), format,
    ...
    resource().flags(
        currentFramebuffer == 0 ? ZWLR_SCREENCOPY_FRAME_V1_FLAGS_Y_INVERT : 0);
  } else  // DMA buffer
  {
    LDMABuffer *dmaBuffer{static_cast<LDMABuffer *>(
        wl_resource_get_user_data(resource().buffer()))};
    UInt32 i{0};
    ...
    EGLImage image{eglCreateImage(compositor()->eglDisplay(), EGL_NO_CONTEXT,
    ...
    compositor()->imp()->glEGLImageTargetRenderbufferStorageOES(GL_RENDERBUFFER,
                                                                image);
    ...
    LFramebufferWrapper glFb(fb,
                             LSize((Int32)dmaBuffer->planes()->width,
                                   (Int32)dmaBuffer->planes()->height),
                             resource().rectB().pos());

    LTexture *outputTexture{resource().output()->bufferTexture(
        resource().output()->currentBuffer())};
