...
// The wp_fractional_v1 scale set with setScale() returned with
// fractionalScale()
...
/* Size in buffer coorinates, width and height are swapped if transform has 90°
 * rotation. It also differs from the current mode size when using a fractional
 * scale */
LSize sizeB;

// Checked witin backend...GL() methods below, trigger resizeGL() and moveGL()
// when != to rect
LPoint lastPos;
LSize lastSize;

/* Damage set with LOutput::setBufferDamage(), used in fractional scaling with
 * oversampling or when the graphic backend uses DUMB buffers or CPU copy. */
UInt64 frame{0};
LRegion damage;
...
struct ScanoutBuffer {
  wl_listener bufferDestroyListener{
      .link{nullptr, nullptr}, .notify = [](wl_listener *listener, void *) {
