...
static void clientDisconnectedEvent(wl_listener *listener, void *data) {
  ...
  LClient *disconnectedClient{
      compositor()->getClientFromNativeResource(client)};
...
bool LCompositor::LCompositorPrivate::initWayland() {
  ...
  if (!display) {
    LLog::fatal(
        "[LCompositorPrivate::initWayland] Unable to create Wayland "
        "display.\n");
    return false;
  }
  ...
  if (socket) {
    ...
    if (socketFd == -1) {
      LLog::error(
          "[LCompositorPrivate::initWayland] Failed to add custom socket %s. "
          "Trying wl_display_add_socket_auto instead.",
          socket);
      goto useAutoSocket;
    }
  ...
  useAutoSocket:
    ...
    if (!socket) {
      LLog::fatal(
          "[LCompositorPrivate::initWayland] Failed to add auto socket.");
      return false;
    }
  }
  ...
  eventFdEventSource = wl_event_loop_add_fd(
      LCompositor::eventLoop(), eventFd, WL_EVENT_READABLE,
  ...
  wl_display_set_global_filter(
      display,
      [](const wl_client *client, const wl_global *global,
         void * /*data*/) -> bool {
        ...
        // Always accept globals not created with LCompositor::createGlobal(),
        // e.g. SHM, WL_DRM, etc
        if (!lGlobal) return true;

        LClient *lClient{compositor()->getClientFromNativeResource(client)};

        if (!lClient) return true;
...
void LCompositor::LCompositorPrivate::unitCompositor() {
  ...
  while (!oneShotTimers.empty()) delete oneShotTimers.back();
...
bool LCompositor::LCompositorPrivate::initGraphicBackend() {
  ...
  if (graphicBackend) {
    if (!graphicBackend->backendInitialize()) {
      ...
      LLog::error(
          "[LCompositorPrivate::initGraphicBackend] Could not initialize "
          "pre-loaded backend.");
      goto loadEnvBackend;
    }
  } else {
  loadEnvBackend:
    ...
    if (backendsPath.empty()) backendsPath = LOUVRE_DEFAULT_BACKENDS_PATH;
    ...
    std::filesystem::path backendPathName{backendsPath / "graphic" /
                                          backendName};
    backendPathName += ".so";
  ...
  retry:
    ...
    if (loadGraphicBackend(backendPathName)) {
      if (!graphicBackend->backendInitialize()) {
        ...
        graphicBackend = nullptr;
        LLog::error(
            "[LCompositorPrivate::initGraphicBackend] Failed to initialize %s "
            "backend.",
            backendName.c_str());
        ...
        if (usingEnvs) {
          usingEnvs = false;
          backendPathName =
              std::filesystem::path(LOUVRE_DEFAULT_BACKENDS_PATH) / "graphic" /
              LOUVRE_DEFAULT_GRAPHIC_BACKEND;
          backendPathName += ".so";
          goto retry;
        }
      ...
      LLog::error(
          "[LCompositorPrivate::initGraphicBackend] Failed to load %s backend.",
          backendPathName.c_str());

      if (usingEnvs) {
        usingEnvs = false;
        backendPathName = std::filesystem::path(LOUVRE_DEFAULT_BACKENDS_PATH) /
                          "graphic" / LOUVRE_DEFAULT_GRAPHIC_BACKEND;
        backendPathName += ".so";
        goto retry;
      }
  ...
  LLog::debug(
      "[LCompositorPrivate::initGraphicBackend] Graphic backend initialized "
      "successfully.");
  ...
  WL_bind_wayland_display =
      LOpenGL::hasExtension(eglExts, "EGL_WL_bind_wayland_display");

  if (WL_bind_wayland_display) eglBindWaylandDisplayWL(eglDisplay(), display);
...
bool LCompositor::LCompositorPrivate::initInputBackend() {
  ...
  if (inputBackend) {
    if (!inputBackend->backendInitialize()) {
      ...
      LLog::error(
          "[LCompositorPrivate::initInputBackend] Could not initialize "
          "pre-loaded backend.");
      goto loadEnvBackend;
    }
  } else {
  loadEnvBackend:
    ...
    std::filesystem::path backendPathName = {backendsPath / "input" /
                                             backendName};
    backendPathName += ".so";
  ...
  retry:
    ...
    if (loadInputBackend(backendPathName)) {
      if (!inputBackend->backendInitialize()) {
        ...
        inputBackend = nullptr;
        LLog::error(
            "[LCompositorPrivate::initInputBackend] Failed to initialize %s "
            "backend.",
            backendName.c_str());
        ...
        if (usingEnvs) {
          usingEnvs = false;
          backendPathName =
              std::filesystem::path(LOUVRE_DEFAULT_BACKENDS_PATH) / "input" /
              LOUVRE_DEFAULT_INPUT_BACKEND;
          backendPathName += ".so";
          goto retry;
        }
      ...
      LLog::error(
          "[LCompositorPrivate::initInputBackend] Failed to load %s backend.",
          backendPathName.c_str());

      if (usingEnvs) {
        usingEnvs = false;
        backendPathName = std::filesystem::path(LOUVRE_DEFAULT_BACKENDS_PATH) /
                          "input" / LOUVRE_DEFAULT_INPUT_BACKEND;
        backendPathName += ".so";
        goto retry;
      }
  ...
  LLog::debug(
      "[LCompositorPrivate::initInputBackend] Input backend initialized "
      "successfully.");
...
void LCompositor::LCompositorPrivate::unitInputBackend(bool closeLib) {
  if (inputBackend && isInputBackendInitialized) {
    ...
    LLog::debug(
        "[LCompositorPrivate::unitInputBackend] Input backend uninitialized "
        "successfully.");
  }
  ...
  if (closeLib) {
    if (inputBackendHandle) dlclose(inputBackendHandle);
...
void LCompositor::LCompositorPrivate::unitGraphicBackend(bool closeLib) {
  ...
  if (isGraphicBackendInitialized && graphicBackend) {
    ...
    LLog::debug(
        "[LCompositorPrivate::unitGraphicBackend] Graphic backend "
  ...
  if (closeLib) {
    if (graphicBackendHandle) dlclose(graphicBackendHandle);
...
void LCompositor::LCompositorPrivate::unitSeat() {
  if (seat) {
    ...
    if (seat->dnd()) compositor()->onAnticipatedObjectDestruction(seat->dnd());
    ...
    if (seat->keyboard()) {
      ...
      LLog::debug(
          "[LCompositorPrivate::unitSeat] Keyboard uninitialized "
          "successfully.");
    }
    ...
    if (seat->pointer()) {
      ...
      LLog::debug(
          "[LCompositorPrivate::unitSeat] Pointer uninitialized successfully.");
    }
    ...
    if (seat->touch()) {
      ...
      LLog::debug(
          "[LCompositorPrivate::unitSeat] Touch uninitialized successfully.");
    }
    ...
    if (seat->dnd()) {
      ...
      seat->m_dnd = nullptr;
      LLog::debug(
          "[LCompositorPrivate::unitSeat] DND Manager uninitialized "
          "successfully.");
    }
    ...
    if (seat->clipboard()) {
      ...
      LLog::debug(
          "[LCompositorPrivate::unitSeat] Clipboard Manager uninitialized "
          "successfully.");
    }
    ...
    LLog::debug(
        "[LCompositorPrivate::unitSeat] Seat uninitialized successfully.");
  }
}

bool LCompositor::LCompositorPrivate::loadGraphicBackend(
  ...
  if (!graphicBackendHandle) {
    LLog::warning(
        "[LCompositorPrivate::loadGraphicBackend] No graphic backend found at "
        "(%s)",
        path.c_str());
    return false;
  }

  LGraphicBackendInterface *(*getAPI)() =
      (LGraphicBackendInterface * (*)()) dlsym(graphicBackendHandle, "getAPI");

  if (!getAPI) {
    LLog::error(
        "[LCompositorPrivate::loadGraphicBackend] Failed to load graphic "
        "backend (%s)",
        path.c_str());
    dlclose(graphicBackendHandle);
    return false;
  ...
  if (graphicBackend)
    LLog::debug(
        "[LCompositorPrivate::loadGraphicBackend] Graphic backend loaded "
        "successfully (%s).",
        path.c_str());
...
bool LCompositor::LCompositorPrivate::loadInputBackend(
    const std::filesystem::path &path) {
  if (inputBackendHandle) dlclose(inputBackendHandle);
  ...
  if (!inputBackendHandle) {
    LLog::warning(
        "[LCompositorPrivate::loadInputBackend] No input backend found at "
        "(%s).",
        path.c_str());
    return false;
  }

  LInputBackendInterface *(*getAPI)() =
      (LInputBackendInterface * (*)()) dlsym(inputBackendHandle, "getAPI");

  if (!getAPI) {
    LLog::warning(
        "[LCompositorPrivate::loadInputBackend] Failed to load input backend "
        "(%s).",
        path.c_str());
    dlclose(inputBackendHandle);
    return false;
  ...
  if (inputBackend)
    LLog::debug(
        "[LCompositorPrivate::loadInputBackend] Input backend loaded "
        "successfully (%s).",
        path.c_str());
...
void LCompositor::LCompositorPrivate::notifyOrderChangeFromSurface(
    LSurface *from) {
  ...
  if (from)
    ...
    for (LSurface *surface : surfaces) surface->orderChanged();

  compositor()->imp()->surfaceRaiseAllowedCounter--;
}

void LCompositor::LCompositorPrivate::insertSurfaceAfter(
    LSurface *prevSurface, LSurface *surfaceToInsert,
    LBitset<InsertOptions> options) {
  if (options.check(UpdateLayers)) {
    if (prevSurface) {
      assert(prevSurface->layer() == surfaceToInsert->layer() &&
             "Surfaces do not belong to the same layer.");
      ...
      if (prevSurface == layer.back()) {
        ...
        surfaceToInsert->imp()->layerLink = layer.insert(
            std::next(prevSurface->imp()->layerLink), surfaceToInsert);
    } else {
  ...
  if (options.check(UpdateSurfaces) &&
      surfaceToInsert->prevSurface() != prevSurface) {
    if (prevSurface) {
      ...
      if (prevSurface == surfaces.back()) {
        ...
        surfaceToInsert->imp()->compositorLink = surfaces.insert(
            std::next(prevSurface->imp()->compositorLink), surfaceToInsert);
    } else {
      surfaces.erase(surfaceToInsert->imp()->compositorLink);
...
void LCompositor::LCompositorPrivate::insertSurfaceBefore(
    LSurface *nextSurface, LSurface *surfaceToInsert,
    LBitset<InsertOptions> options) {
  if (options.check(UpdateLayers)) {
    assert(nextSurface->layer() == surfaceToInsert->layer() &&
           "Surfaces do not belong to the same layer.");
    auto &layer{layers[nextSurface->layer()]};
    layer.erase(surfaceToInsert->imp()->layerLink);
    surfaceToInsert->imp()->layerLink =
        layer.insert(nextSurface->imp()->layerLink, surfaceToInsert);
  }

  if (options.check(UpdateSurfaces) &&
      surfaceToInsert->nextSurface() != nextSurface) {
    surfaces.erase(surfaceToInsert->imp()->compositorLink);
    surfaceToInsert->imp()->compositorLink =
        surfaces.insert(nextSurface->imp()->compositorLink, surfaceToInsert);
    surfacesListChanged = true;
...
bool LCompositor::LCompositorPrivate::runningAnimations() {
  ...
  for (LAnimation *anim : animations) {
    ...
    if (anim->m_running || anim->m_pendingDestroy) running = true;
  }
...
retry:
  ...
  for (LAnimation *a : animations) {
    ...
    if (!a->m_running) continue;

    elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                  std::chrono::steady_clock::now() - a->m_beginTime)
                  .count();
    ...
    if (elapsed >= duration)
      ...
      a->m_value =
          static_cast<Float64>(elapsed) / static_cast<Float64>(duration);

    if (a->m_onUpdate) {
      a->m_onUpdate(a);

      if (animationsVectorChanged) goto retry;
    }
...
void LCompositor::LCompositorPrivate::destroyPendingRenderBuffers(
  ...
  if (id != nullptr) threadId = *id;

  auto it{threadsMap.find(threadId)};

  if (it == threadsMap.end()) return;

  while (!it->second.renderBuffersToDestroy.empty()) {
    glDeleteFramebuffers(
        1, &it->second.renderBuffersToDestroy.back().framebufferId);
    it->second.renderBuffersToDestroy.pop_back();
  }
}

void LCompositor::LCompositorPrivate::addRenderBufferToDestroy(
    std::thread::id thread, LRenderBuffer::ThreadData &data) {
...
void LCompositor::LCompositorPrivate::lock() { renderMutex.lock(); }

void LCompositor::LCompositorPrivate::unlock() { renderMutex.unlock(); }

void LCompositor::LCompositorPrivate::unlockPoll() {
  if (pollUnlocked) return;
...
LPainter *LCompositor::LCompositorPrivate::findPainter() {
  ...
  if (it != compositor()->imp()->threadsMap.end()) return &it->second.painter;
...
void LCompositor::LCompositorPrivate::sendPendingConfigurations() {
  ...
  for (LClient *c : clients)
    for (auto *g : c->wlrOutputManagerGlobals()) g->done();
}

void LCompositor::LCompositorPrivate::sendPresentationTime() {
  for (LOutput *o : outputs) {
    o->imp()->pageflipMutex.lock();
    if (o->imp()->stateFlags.check(
            LOutput::LOutputPrivate::HasUnhandledPresentationTime))
      for (LSurface *s : surfaces) s->imp()->sendPresentationFeedback(o);
...
void LCompositor::LCompositorPrivate::initDMAFeedback() noexcept {
  if (graphicBackend->backendGetDMAFormats()->empty()) {
    LLog::warning(
        "[LCompositorPrivate::initDMAFeedback] DMA formats not available.");
    return;
  }
...
retryName:
  i = 8;
  while (i < 15) name[i++] = 48 + ((LTime::ms() + rand()) % 10);
  name[i] = '\0';
  ...
  map = (UInt8 *)mmap(NULL, dmaFeedback.tableSize, PROT_READ | PROT_WRITE,
                      MAP_SHARED, rw, 0);
  ...
  for (const auto &format : *graphicBackend->backendGetDMAFormats()) {
    ...
    for (const auto &scanoutFmt :
         *graphicBackend->backendGetScanoutDMAFormats()) {
      if (scanoutFmt == format) {
        *(UInt16 *)wl_array_add(&dmaFeedback.scanoutIndices, sizeof(UInt16)) =
            formatIndex;
        break;
      }
    }

    // Repeating formats across different tranches of the same device is
    // permitted if they have different flags
    *(UInt16 *)wl_array_add(&dmaFeedback.formatIndices, sizeof(UInt16)) =
        formatIndex;
  ...
  LLog::debug(
      "[LCompositorPrivate::initDMAFeedback] DMA Feedback formats table "
      "created successfully.");
  return;
err:
  dmaFeedback.tableFd = -1;
  LLog::error(
      "[LCompositorPrivate::initDMAFeedback] Failed to create DMA Feedback "
      "formats table: %s.",
      name);
}
...
void LCompositor::LCompositorPrivate::handleDestroyedClients() {
  while (!destroyedClients.empty()) (*destroyedClients.begin())->destroy();
}
...
void LCompositor::LCompositorPrivate::initDRMLeaseGlobals() {
  for (LGPU *gpu : *graphicBackend->backendGetDevices())
    if (gpu->fd() != -1 && gpu->roFd() != -1)
      gpu->m_leaseGlobal.reset(
          compositor()->createGlobal<Protocols::DRMLease::GDRMLeaseDevice>(
              0, gpu));
}

void LCompositor::LCompositorPrivate::unitDRMLeaseGlobals() {
  // Not yet initialized
  if (!graphicBackend) return;

  for (LGPU *gpu : *graphicBackend->backendGetDevices())
    if (gpu->m_leaseGlobal) compositor()->removeGlobal(gpu->m_leaseGlobal);
}

LCompositor::LCompositorPrivate::ThreadData &
LCompositor::LCompositorPrivate::initThreadData(LOutput *output) noexcept {
...
LCompositor::LCompositorPrivate::ThreadData::ThreadData(
    ...
    : output(output), painter(output) {
  LLog::debug("[LCompositorPrivate::ThreadData] Data created for thread %zu.",
...
LCompositor::LCompositorPrivate::ThreadData::~ThreadData() {
  LLog::debug("[LCompositorPrivate::ThreadData] Thread %zu data destroyed.",
              std::hash<std::thread::id>{}(std::this_thread::get_id()));
}
...
void LCompositor::LCompositorPrivate::disablePendingPosixSignals() noexcept {
  ...
  while (!it->second.posixSignalsToDisable.empty()) {
    LLog::debug(
        "[LCompositorPrivate::disablePendingPosixSignals] Posix signal %d "
        "blocked in thread %zu.",
        *it->second.posixSignalsToDisable.begin(),
        std::hash<std::thread::id>{}(std::this_thread::get_id()));
    sigemptyset(&sigset);
    sigaddset(&sigset, *it->second.posixSignalsToDisable.begin());
    pthread_sigmask(SIG_BLOCK, &sigset, nullptr);
    it->second.posixSignalsToDisable.erase(
        it->second.posixSignalsToDisable.begin());
  }
}
...
void LCompositor::LCompositorPrivate::handlePosixSignalChanges() noexcept {
  ...
  for (auto sourceIt = posixSignalSources.begin();
       sourceIt != posixSignalSources.end();) {
    ...
    {
      LLog::debug(
          "[LCompositorPrivate::handlePosixSignalChanges] Event source removed "
          "for posix signal %d.",
          sourceIt->first);
  ...
  while (!signalsCopy.empty()) {
    LLog::debug(
        "[LCompositorPrivate::handlePosixSignalChanges] Event source created "
        "for posix signal %d.",
        *signalsCopy.begin());
    posixSignalSources[*signalsCopy.begin()] = wl_event_loop_add_signal(
        eventLoop, *signalsCopy.begin(), &posixSignalHandler, nullptr);
