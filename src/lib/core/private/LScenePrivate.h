...
LView *viewAt(LView *view, const LPoint &pos, LView::Type type,
              LBitset<LScene::InputFilter> flags);
LPoint viewLocalPos(LView *view, const LPoint &pos);
bool handlePointerMove(LView *view);
bool handleTouchDown(LView *view);

bool pointIsOverView(LView *view, const LPointF &pos,
                     LBitset<LScene::InputFilter> flags) {
  if (!view->mapped() ||
  ...
  if (pointClippedByParent(view, pos)) return false;

  if (pointClippedByParentScene(view, pos)) return false;

  if ((view->scalingEnabled() || view->parentScalingEnabled()) &&
      view->scalingVector() != LSizeF(1.f, 1.f)) {
    if (view->scalingVector().area() == 0.f) return false;
    ...
    if (view->inputRegion()) {
      if (view->inputRegion()->containsPoint(pos - view->pos())) return true;
    } else {
      if (LRect(view->pos(), view->size()).containsPoint(pos)) return true;
    }
  }
