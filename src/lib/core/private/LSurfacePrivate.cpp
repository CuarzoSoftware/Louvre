...
void LSurface::LSurfacePrivate::setParent(LSurface *parent) {
  if (stateFlags.check(Destroyed)) return;
  ...
  if (this->parent) this->parent->imp()->children.erase(parentLink);

  this->parent = parent;

  if (parent == nullptr) return;

  const bool isSubsurfaceOrToplevel{surface->subsurface() != nullptr ||
                                    surface->toplevel() != nullptr};

  using OP = LCompositor::LCompositorPrivate::InsertOptions;

  if (isSubsurfaceOrToplevel) {
    if (parent->layer() != surface->layer()) setLayer(parent->layer());

    if (parent->children().empty())
      compositor()->imp()->insertSurfaceAfter(
          parent, surface, OP::UpdateSurfaces | OP::UpdateLayers);
    else {
      bool inserted{false};

      for (auto child = parent->children().rbegin();
           child != parent->children().rend(); child++) {
        if ((*child)->layer() == surface->layer()) {
          inserted = true;
          compositor()->imp()->insertSurfaceAfter(
              *child, surface, OP::UpdateSurfaces | OP::UpdateLayers);
          break;
        }
      }
  ...
  if (surface->role()) surface->role()->handleParentChange();
}
...
void LSurface::LSurfacePrivate::setMapped(bool state) {
  ...
  if (stateFlags.check(Mapped) != state) {
    ...
    for (LSurface *s : children) childrenTmp.emplace_back(s);

    while (!childrenTmp.empty()) {
      if (!childrenTmp.front().get()) goto skip;
...
void LSurface::LSurfacePrivate::setRole(LBaseSurfaceRole *role) noexcept {
  ...
  assert(!stateFlags.check(UnnotifiedRoleChange) &&
         "Setting new role without notifying previous change");
...
void LSurface::LSurfacePrivate::applyPendingChildren() {
  ...
  if (pendingChildren.empty()) return;
  ...
  while (!pendingChildren.empty()) {
    ...
    if (child->imp()->pendingParent != surface) continue;

    // If the child already had a parent
    if (child->imp()->parent)
      child->imp()->parent->imp()->children.erase(child->imp()->parentLink);

    const bool isSubsurfaceOrToplevel{child->subsurface() != nullptr ||
    ...
    if (isSubsurfaceOrToplevel) {
      ...
      if (surface->children().empty())
        compositor()->imp()->insertSurfaceAfter(
            surface, child, OP::UpdateSurfaces | OP::UpdateLayers);
      else {
        bool inserted{false};

        for (auto c = surface->children().rbegin();
             c != surface->children().rend(); c++) {
          if ((*c)->layer() == child->layer()) {
            inserted = true;
            compositor()->imp()->insertSurfaceAfter(
                *c, child, OP::UpdateSurfaces | OP::UpdateLayers);
            break;
          }
        }
    ...
    if (child->role()) child->role()->handleParentChange();
  }
...
bool LSurface::LSurfacePrivate::bufferToTexture() noexcept {
  ...
  if (current.bufferRes) {
    ...
    if (wl_shm_buffer_get(current.bufferRes)) {
      ...
      UInt32 format =
          LTexture::waylandFormatToDRM(wl_shm_buffer_get_format(shm_buffer));
      ...
      if (!updateDimensions(widthB, heightB)) return false;

      if (!texture->initialized() ||
          changesToNotify.check(SizeChanged | SourceRectChanged |
                                BufferSizeChanged | BufferTransformChanged |
                                BufferScaleChanged)) {
        currentDamageB.clear();
        currentDamageB.addRect(LRect(0, sizeB));
        currentDamage.clear();
        currentDamage.addRect(LRect(0, size));
        texture->setDataFromMainMemory(LSize(widthB, heightB), stride, format,
                                       pixels);
        ...
        if (stateFlags.check(ViewportIsScaled | ViewportIsCropped)) {
          Float32 xInvScale =
              (Float32(current.bufferScale) * srcRect.w()) / Float32(size.w());
          Float32 yInvScale =
              (Float32(current.bufferScale) * srcRect.h()) / Float32(size.h());

          Int32 xOffset =
              roundf(srcRect.x() * Float32(current.bufferScale)) - 2;
          Int32 yOffset =
              roundf(srcRect.y() * Float32(current.bufferScale)) - 2;

          while (!pendingDamage.empty()) {
            LRect &r = pendingDamage.back();
            onlyPending.addRect(
                (r.x() * xInvScale + xOffset), (r.y() * yInvScale + yOffset),
                (r.w() * xInvScale + 4), (r.h() * yInvScale + 4));
            pendingDamage.pop_back();
          }

          while (!pendingDamageB.empty()) {
            LRect &r = pendingDamageB.back();
            onlyPending.addRect(r.x() - 1, r.y() - 1, r.w() + 2, r.h() + 2);
          ...
          onlyPending.transform(
              sizeB,
              Louvre::requiredTransform(current.transform, LTransform::Normal));
          ...
          while (!pendingDamageB.empty()) {
            LRect &r = pendingDamageB.back();
            onlyPending.addRect(r.x() - 2, r.y() - 2, r.w() + 4, r.h() + 4);
            pendingDamageB.pop_back();
          }
          ...
          LRegion::multiply(&currentDamage, &currentDamageB,
                            1.f / Float32(current.bufferScale));
        }
      } else {
    ...
    // WL_DRM
    else if (compositor()->imp()->WL_bind_wayland_display &&
             compositor()->imp()->eglQueryWaylandBufferWL(
                 LCompositor::eglDisplay(), current.bufferRes,
      ...
      texture = textureBackup;
      compositor()->imp()->eglQueryWaylandBufferWL(
          LCompositor::eglDisplay(), current.bufferRes, EGL_WIDTH, &widthB);
      compositor()->imp()->eglQueryWaylandBufferWL(
          LCompositor::eglDisplay(), current.bufferRes, EGL_HEIGHT, &heightB);
      if (!updateDimensions(widthB, heightB)) return false;
      updateDamage();
    ...
    // DMA-Buf
      ...
      LDMABuffer *dmaBuffer =
          (LDMABuffer *)wl_resource_get_user_data(current.bufferRes);
    ...
    // Single pixel buffer
      ...
      LSinglePixelBuffer &singlePixelBuffer{*static_cast<LSinglePixelBuffer *>(
          wl_resource_get_user_data(current.bufferRes))};

      UInt8 buffer[4]{
          static_cast<UInt8>(
              (static_cast<UInt64>(singlePixelBuffer.pixel().b) *
               static_cast<UInt64>(255)) /
              static_cast<UInt64>(std::numeric_limits<UInt32>::max())),
          static_cast<UInt8>(
              (static_cast<UInt64>(singlePixelBuffer.pixel().g) *
               static_cast<UInt64>(255)) /
              static_cast<UInt64>(std::numeric_limits<UInt32>::max())),
          static_cast<UInt8>(
              (static_cast<UInt64>(singlePixelBuffer.pixel().r) *
               static_cast<UInt64>(255)) /
              static_cast<UInt64>(std::numeric_limits<UInt32>::max())),
          static_cast<UInt8>(
              (static_cast<UInt64>(singlePixelBuffer.pixel().a) *
               static_cast<UInt64>(255)) /
              static_cast<UInt64>(std::numeric_limits<UInt32>::max())),
      };

      texture->setDataFromMainMemory(LSize(1, 1), 4, DRM_FORMAT_ARGB8888,
                                     buffer);
      updateDamage();
    } else {
      LLog::error(
          "[LSurfacePrivate::bufferToTexture] Unknown buffer type. Killing "
          "client.");
...
void LSurface::LSurfacePrivate::sendPresentationFeedback(
    LOutput *output) noexcept {
  if (presentationFeedbackResources.empty()) return;
  ...
  for (std::size_t i = 0; i < presentationFeedbackResources.size();) {
    ...
    if (!scanout && (feedback->m_commitId == -2 ||
                     (feedback->m_outputSet && !feedback->m_output))) {
      feedback->discarded();
      feedback->m_surface.reset();
      presentationFeedbackResources[i] =
          std::move(presentationFeedbackResources.back());
      presentationFeedbackResources.pop_back();
      wl_resource_destroy(feedback->resource());
      continue;
    } else if (scanout || feedback->m_output == output) {
      for (Wayland::GOutput *gOutput :
           surfaceResource->client()->outputGlobals())
        if (gOutput->output() == output) feedback->syncOutput(gOutput);

      feedback->presented(
          output->imp()->presentationTime.time.tv_sec >> 32,
...
void LSurface::LSurfacePrivate::setPendingParent(
    LSurface *pendParent) noexcept {
  if (pendingParent)
    pendingParent->imp()->pendingChildren.erase(pendingParentLink);
...
bool LSurface::LSurfacePrivate::isInChildrenOrPendingChildren(
    LSurface *child) noexcept {
  if (child == surfaceResource->surface()) return true;

  if (child->imp()->pendingParent) {
    if (isInChildrenOrPendingChildren(child->imp()->pendingParent)) return true;
  }

  for (LSurface *s : children) {
    if (s == child) return true;

    if (s->imp()->isInChildrenOrPendingChildren(child)) return true;
  }
...
void LSurface::LSurfacePrivate::updateDamage() noexcept {
  if (!texture->initialized() ||
    ...
    if (stateFlags.check(ViewportIsScaled | ViewportIsCropped)) {
      ...
      while (!pendingDamageB.empty()) {
        LRect &r = pendingDamageB.back();
        currentDamageB.addRect(r.x() - 1, r.y() - 1, r.w() + 2, r.h() + 2);
        pendingDamageB.pop_back();
      }
...
bool LSurface::LSurfacePrivate::updateDimensions(Int32 widthB,
                                                 Int32 heightB) noexcept {
  ...
  if (prevSizeB != sizeB) changesToNotify.add(BufferSizeChanged);
  ...
  if (surfaceResource->viewportRes()) {
    ...
    if (surfaceResource->viewportRes()->srcRect().x() != -1.f ||
      ...
      if (srcRect.x() < 0.f || srcRect.y() < 0.f || srcRect.w() <= 0.f ||
          srcRect.h() <= 0.f) {
        surfaceResource->viewportRes()->postError(
            WP_VIEWPORT_ERROR_BAD_VALUE,
            "Invalid source rect ({}, {}, {}, {}).", srcRect.x(), srcRect.y(),
            srcRect.w(), srcRect.h());
        return false;
      }

      if (roundf((srcRect.x() + srcRect.w()) * Float32(current.bufferScale)) >
              sizeB.w() ||
          roundf((srcRect.y() + srcRect.h()) * Float32(current.bufferScale)) >
              sizeB.h()) {
        surfaceResource->viewportRes()->postError(
    ...
    if (surfaceResource->viewportRes()->dstSize().w() != -1 ||
        surfaceResource->viewportRes()->dstSize().h() != -1) {
      size = surfaceResource->viewportRes()->dstSize();

      if (size.w() <= 0 || size.h() <= 0) {
        surfaceResource->viewportRes()->postError(
            WP_VIEWPORT_ERROR_BAD_VALUE, "Invalid destination size ({}, {}).",
            size.w(), size.h());
        return false;
      }
    ...
    // Using the viewport source rect size or normal surface size
      ...
      if (usingViewportSrc) {
        if (fmod(srcRect.w(), 1.f) != 0.f || fmod(srcRect.h(), 1.f) != 0.f) {
          surfaceResource->viewportRes()->postError(
              WP_VIEWPORT_ERROR_BAD_SIZE, "Destination size is not integer");
          return false;
        }
  ...
  if (prevSize != size) changesToNotify.add(SizeChanged);

  if (prevSrcRect != srcRect) changesToNotify.add(SourceRectChanged);

  return true;
}

void LSurface::LSurfacePrivate::simplifyDamage(
    std::vector<LRect> &vec) noexcept {
  if (vec.size() >= LOUVRE_MAX_DAMAGE_RECTS) {
    ...
    while (!vec.empty()) {
      if (vec.back().x() < extents.x1) extents.x1 = vec.back().x();

      if (vec.back().y() < extents.y1) extents.y1 = vec.back().y();
    ...
    vec.emplace_back(extents.x1, extents.y1, extents.x2 - extents.x1,
...
void LSurface::LSurfacePrivate::setLayer(LSurfaceLayer newLayer) {
  ...
  if (layerChanged) surfaceResource->surface()->layerChanged();

  compositor()->imp()->insertSurfaceAfter(
      prev, surfaceResource->surface(),
      LCompositor::LCompositorPrivate::UpdateSurfaces);

  for (LSurface *child : pendingChildren)
    if (child->subsurface() || child->toplevel()) child->imp()->setLayer(layer);
...
void LSurface::LSurfacePrivate::notifyRoleChange() {
  assert(stateFlags.check(UnnotifiedRoleChange) &&
         "There is no role change to notify");
  assert(!surfaceResource->surface()->mapped() &&
         "The surface can't be mapped while roleChanged() is called");
