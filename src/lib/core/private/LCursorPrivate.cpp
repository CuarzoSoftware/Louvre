...
void LCursor::LCursorPrivate::textureUpdate() noexcept {
  ...
  if (!textureChanged && !posChanged) return;
  ...
  for (LOutput *o : compositor()->outputs()) {
    if (isVisible && o->rect().intersects(rect)) {
      const bool found{std::find(intersectedOutputs.begin(),
                                 intersectedOutputs.end(),
                                 o) != intersectedOutputs.end()};

      if (!found) intersectedOutputs.push_back(o);
      ...
      if (cursor()->hasHardwareSupport(o) && (textureChanged || !found)) {
        if (cursor()->enabled(o) && cursor()->hwCompositingEnabled(o)) {
          ...
          compositor()->imp()->graphicBackend->outputSetCursorTexture(o,
                                                                      buffer);
        } else
          compositor()->imp()->graphicBackend->outputSetCursorTexture(o,
                                                                      nullptr);
    ...
    if (cursor()->enabled(o) && cursor()->hasHardwareSupport(o)) {
      ...
      compositor()->imp()->graphicBackend->outputSetCursorPosition(
          o, o->scale() * p / (o->scale() / o->fractionalScale()));
    }
  }
...
void texture2Buffer(LCursor *cursor, const LSizeF &size,
                    LTransform transform) noexcept {
  ...
  painter->bindTextureMode({
      ...
      .pos = LPoint(0, 0),
      .srcRect = LRect(0, 0, cursor->texture()->sizeB().w(),
                       cursor->texture()->sizeB().h()),
      .dstSize = size,
  ...
  if (painter->imp()->openGLExtensions.EXT_read_format_bgra)
    glReadPixels(0, 0, 64, 64, GL_BGRA_EXT, GL_UNSIGNED_BYTE,
                 cursor->imp()->buffer);
  else {
