...
LView *LScene::LScenePrivate::viewAt(LView *view, const LPoint &pos,
  ...
  if (!view->mapped()) return nullptr;

  if (type != LView::UndefinedType && view->type() != type) return nullptr;

  if (flags != 0 &&
      !((flags.check(InputFilter::Touch) && view->touchEventsEnabled()) ||
        (flags.check(InputFilter::Pointer) && view->pointerEventsEnabled()) ||
        (flags.check(InputFilter::Keyboard) && view->keyboardEventsEnabled())))
    return nullptr;

  if (view->clippingEnabled() && !view->clippingRect().containsPoint(pos))
    return nullptr;

  if (pointClippedByParent(view, pos)) return nullptr;

  if (pointClippedByParentScene(view, pos)) return nullptr;

  if (flags == 0) return view;

  if ((view->scalingEnabled() || view->parentScalingEnabled()) &&
      view->scalingVector() != LSizeF(1.f, 1.f)) {
    if (view->scalingVector().area() == 0.f) return nullptr;
    ...
    if (view->inputRegion()) {
      ...
      if (LRect(view->pos(), view->size())
              .containsPoint((pos - view->pos()) / view->scalingVector()))
        v = view;
    }
  } else {
    if (view->inputRegion()) {
      if (view->inputRegion()->containsPoint(pos - view->pos())) v = view;
    } else {
      if (LRect(view->pos(), view->size()).containsPoint(pos)) v = view;
    }
  }
...
bool LScene::LScenePrivate::pointClippedByParent(LView *view,
                                                 const LPoint &point) {
  if (!view->parent()) return false;

  if (view->parentClippingEnabled()) {
    if (!LRect(view->parent()->pos(), view->parent()->size())
             .containsPoint(point))
      return true;
  }
...
bool LScene::LScenePrivate::pointClippedByParentScene(LView *view,
                                                      const LPoint &point) {
  LSceneView *parentScene = view->parentSceneView();

  if (!parentScene) return false;

  if (parentScene->isLScene()) return false;

  if (!parentScene->m_fb->rect().containsPoint(point)) return true;

  return pointClippedByParentScene(parentScene, point);
}

bool LScene::LScenePrivate::handlePointerMove(LView *view) {
  if (state.check(LSS::ChildrenListChanged)) goto listChangedErr;

  for (std::list<LView *>::const_reverse_iterator it =
           view->children().crbegin();
       it != view->children().crend(); it++)
    if (!handlePointerMove(*it)) return false;

  if (!state.check(LSS::PointerIsBlocked) &&
      pointIsOverView(view, cursor()->pos(), InputFilter::Pointer)) {
    if (view->blockPointerEnabled()) state.add(LSS::PointerIsBlocked);
  ...
  // If a list was modified, start again, serials are used to prevent resend
  // events
listChangedErr:
...
bool LScene::LScenePrivate::handleTouchDown(LView *view) {
  if (state.check(ChildrenListChanged)) goto listChangedErr;

  for (std::list<LView *>::const_reverse_iterator it =
           view->children().crbegin();
       it != view->children().crend(); it++)
    if (!handleTouchDown(*it)) return false;

  if (!state.check(TouchIsBlocked) &&
      pointIsOverView(view, touchGlobalPos, InputFilter::Touch)) {
    ...
    if (view->blockTouchEnabled()) state.add(TouchIsBlocked);
  }
