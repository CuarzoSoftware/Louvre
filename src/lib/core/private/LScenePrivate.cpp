...
void LScene::LScenePrivate::calcNewDamage(LView *view, OutputData *oD)
{
    // Children first
...
void LScene::LScenePrivate::drawOpaqueDamage(LView *view, OutputData *oD)
{
    // Children first
    for (list<LView*>::const_reverse_iterator it = view->children().crbegin(); it != view->children().crend(); it++)
...
void LScene::LScenePrivate::drawTranslucentDamage(LView *view, OutputData *oD)
{
    LView::LViewPrivate::ViewCache *cache = &view->imp()->cache;
