...
void LOutput::LOutputPrivate::backendInitializeGL() {
  ...
  output->imp()->global.reset(
      compositor()->createGlobal<Protocols::Wayland::GOutput>(0, output));
...
void LOutput::LOutputPrivate::damageToBufferCoords() noexcept {
  ...
  if (screenshotCursorTimeout > 0 && cursor()->visible() &&
      cursor()->enabled(output)) {
    damage.addRect(
        LRect(prevCursorRect.pos() + output->pos(), prevCursorRect.size()));
    stateFlags.add(CursorNeedsRendering);
  }

  if (!damage.empty() &&
      (stateFlags.checkAll(UsingFractionalScale |
    ...
    while (n > 0) {
      tmp.addRect(floorf(Float32(box->x1) * fractionalScale) - 2,
                  floorf(Float32(box->y1) * fractionalScale) - 2,
                  ceilf(Float32(box->x2 - box->x1) * fractionalScale) + 4,
                  ceilf(Float32(box->y2 - box->y1) * fractionalScale) + 4);
      n--;
      box++;
    }
    ...
    if (output->hasBufferDamageSupport())
      compositor()->imp()->graphicBackend->outputSetBufferDamage(output,
                                                                 damage);
    ...
    if (compositor()->imp()->screenshotManagers > 0) {
      for (LClient *client : compositor()->clients()) {
        for (auto *screenCpyManager : client->imp()->screenCopyManagerGlobals) {
          ...
          if (!outputDamage.firstFrame) outputDamage.damage.addRegion(damage);
        }
      }
    }
  }
...
void LOutput::LOutputPrivate::blitFractionalScaleFb(bool cursorOnly) noexcept {
  ...
  painter->bindTextureMode({.texture = fractionalFb.texture(0),
                            .pos = rect.pos(),
  ...
  if (cursorOnly) {
    ...
    while (n > 0) {
      tmp.addRect(floorf(Float32(box->x1) * prevFracScale) - 2,
                  floorf(Float32(box->y1) * prevFracScale) - 2,
                  ceilf(Float32(box->x2 - box->x1) * prevFracScale) + 4,
                  ceilf(Float32(box->y2 - box->y1) * prevFracScale) + 4);
      n--;
      box++;
    }
...
void LOutput::LOutputPrivate::handleScreenshotRequests(
  ...
  for (std::size_t i = 0; i < screenshotRequests.size();) {
    if (screenshotRequests[i]->resource().compositeCursor() == withCursor) {
      ...
      if (screenshotRequests[i]->copy() == 0)
        screenshotRequests[i]->resource().m_stateFlags.remove(
            ScreenCopy::RScreenCopyFrame::Accepted);
...
void LOutput::LOutputPrivate::blitFramebuffers() noexcept {
  if (stateFlags.checkAll(UsingFractionalScale |
                          FractionalOversamplingEnabled)) {
    ...
    } else if (stateFlags.check(ScreenshotsWithCursor) &&
               !stateFlags.check(ScreenshotsWithoutCursor)) {
      drawCursor();
      blitFractionalScaleFb(false);
      handleScreenshotRequests(true);
    } else if (!stateFlags.check(ScreenshotsWithCursor) &&
...
void LOutput::LOutputPrivate::backendPaintGL() {
  ...
  if (!callLock) callLockACK.store(true);

  if (output->imp()->state != LOutput::Initialized) return;

  if (callLock) compositor()->imp()->lock();
  ...
  if (!repaintFilter()) {
    ...
    if (callLock) compositor()->imp()->unlock();
  ...
  if (scanout[0].buffer) {
    ...
    wl_resource_add_destroy_listener((wl_resource *)scanout[1].buffer,
                                     &scanout[1].bufferDestroyListener);
  ...
  if (scanout[0].buffer || scanout[1].buffer) output->repaint();

  stateFlags.setFlag(NeedsFullRepaint, needsFullRepaintPrev);

  /* This ensures that all active outputs have been repainted at least once
   * after a client requests to lock the session. Protocol quote: The locked
   * event must not be sent until a new "locked" frame has been presented on all
   * outputs and no security sensitive normal/unlocked content is possibly
   * visible. */
  ...
  /* Ensure clients receive frame callbacks and pending roles configurations on
   * time */
  compositor()->flushClients();

  /* Destroy render buffers created from this thread and marked as destroyed by
   * the user */
...
void LOutput::LOutputPrivate::backendUninitializeGL() {
  ...
  if (output->imp()->state != LOutput::PendingUninitialize) return;
...
void LOutput::LOutputPrivate::updateRect() {
  if (stateFlags.check(UsingFractionalScale)) {
    sizeB = compositor()
                ->imp()
                ->graphicBackend->outputGetCurrentMode(output)
                ->sizeB();
  ...
  if (!stateFlags.check(IsBlittingFramebuffers)) updateExclusiveZones();
}

void LOutput::LOutputPrivate::updateGlobals() {
  for (LClient *c : compositor()->clients())
    for (GOutput *global : c->outputGlobals())
      if (output == global->output()) global->sendConfiguration();
...
void LOutput::LOutputPrivate::calculateCursorDamage() noexcept {
  ...
  if (cursor()->enabled(output) && !cursor()->hwCompositingEnabled(output) &&
      cursor()->visible()) {
    stateFlags.add(CursorNeedsRendering);
    cursorDamage.addRect(cursor()->rect());
    cursorDamage.addRect(
        LRect(prevCursorRect.pos() + output->pos(), prevCursorRect.size()));
  ...
  if (stateFlags.check(CursorRenderedInPrevFrame) &&
      cursor()->hwCompositingEnabled(output)) {
    cursorDamage.addRect(
        LRect(prevCursorRect.pos() + output->pos(), prevCursorRect.size()));
    output->repaint();
  ...
  prevCursorRect =
      LRect(cursor()->rect().pos() - output->pos(), cursor()->rect().size());
}
...
void LOutput::LOutputPrivate::drawCursor() noexcept {
  ...
  if (stateFlags.check(CursorNeedsRendering)) {
    ...
    painter->setAlpha(1.f);
    painter->setColorFactor(1.f, 1.f, 1.f, 1.f);
    painter->bindTextureMode({.texture = cursor()->texture(),
                              .pos = cursor()->rect().pos(),
...
void LOutput::LOutputPrivate::validateScreenshotRequests() noexcept {
  ...
  for (std::size_t i = 0; i < screenshotRequests.size();) {
    if (stateFlags.check(HasScanoutBuffer) ||  // Deny screenshoots when there
                                               // is a custom scanout buffer
        !screenshotRequests[i]->resource().accepted() ||
        !screenshotRequests[i]->resource().m_bufferContainer.buffer ||
        screenshotRequests[i]->resource().m_initOutputModeSize !=
            output->currentMode()->sizeB() ||
        screenshotRequests[i]->resource().m_initOutputSize != output->size() ||
        screenshotRequests[i]->resource().m_initOutputTransform !=
            output->transform()) {
...
void LOutput::LOutputPrivate::removeFromSessionLockPendingRepaint() noexcept {
  if (sessionLockManager()->state() == LSessionLockManager::Locked &&
      !sessionLockManager()->m_sessionLockRes->m_lockedOnce) {
    LVectorRemoveOneUnordered(
        sessionLockManager()->m_sessionLockRes->m_pendingRepaint, output);
...
void LOutput::LOutputPrivate::updateExclusiveZones() noexcept {
  ...
  for (LExclusiveZone *zone : exclusiveZones) {
    ...
    switch (zone->edge()) {
      ...
      case LEdgeTop:
        ...
        zone->m_rect.setW(rect.w() - exclusiveEdges.left -
                          exclusiveEdges.right);
  ...
  if (availableGeometryChanged) output->availableGeometryChanged();
}
...
bool LOutput::LOutputPrivate::isBufferScannedByOtherOutputs(
    wl_buffer *buffer) const noexcept {
...
void LOutput::LOutputPrivate::releaseScanoutBuffer(UInt8 index) noexcept {
  if (scanout[index].buffer) {
    ...
    if (isBufferScannedByOtherOutputs(scanout[index].buffer)) goto skipRelease;

    /* The surface will release it */
    if (scanout[index].surface &&
        scanout[index].surface->bufferResource() == scanout[index].buffer)
      goto skipRelease;
