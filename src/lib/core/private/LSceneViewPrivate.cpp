...
void LSceneView::LSceneViewPrivate::calcNewDamage(LView *view)
{
    ...
    view->m_state.remove(LVS::RepaintCalled);
    ...
    bool colorFactorChanged = cache.voD->prevColorFactorEnabled != view->m_state.check(LVS::ColorFactor);

    if (!colorFactorChanged && view->m_state.check(LVS::ColorFactor))
    {
    ...
    if (mappingChanged || rectChanged || cache.voD->changedOrder || opacityChanged || cache.scalingEnabled || colorFactorChanged)
    {
        ...
        {
            cache.voD->prevColorFactorEnabled = view->m_state.check(LVS::ColorFactor);
...
void LSceneView::LSceneViewPrivate::drawOpaqueDamage(LView *view)
{
    ...
    if (view->m_state.check(LVS::ColorFactor))
    {
        oD.p->imp()->shaderSetColorFactor(view->m_colorFactor.r,
