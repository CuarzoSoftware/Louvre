...
enum ShaderMode : GLint { LegacyMode = 0, TextureMode = 1, ColorMode = 2 };

struct Uniforms {
  GLuint texSize, srcRect, activeTexture, mode, color, colorFactorEnabled,
      texColorEnabled, alpha, premultipliedAlpha, has90deg;
} uniforms, uniformsExternal;

Uniforms *currentUniforms;

struct UniformsScaler {
  GLuint texSize, srcRect, activeTexture, pixelSize, samplerBounds, iters;
...
struct UserState {
  ...
  LBlendFunc customBlendFunc{GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE,
...
GLuint vertexShader, fragmentShader, fragmentShaderExternal,
    fragmentShaderScaler, fragmentShaderScalerExternal;
...
// Program
GLuint programObject, programObjectExternal, programObjectScaler,
    programObjectScalerExternal, currentProgram;
...
void shaderSetSrcRect(const LRectF &rect) noexcept {
  if (currentState->srcRect != rect) {
    ...
    glUniform4f(currentUniforms->srcRect, rect.x(), rect.y(), rect.w(),
                rect.h());
  }
}
...
void setViewport(Int32 x, Int32 y, Int32 w, Int32 h) noexcept {
  ...
  if (currentState->mode == TextureMode) {
    shaderSetSrcRect(LRectF((Float32(x) - srcRect.x()) / srcRect.w(),
    ...
    /* Normal texture */
      ...
      if (userState.texture.get() && userState.texture->premultipliedAlpha()) {
        if (userState.autoBlendFunc) {
          ...
          const LRGBF colorFactor{userState.colorFactor.r,
