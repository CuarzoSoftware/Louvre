 ...
 * @return `true` if the transformation includes a 90° or 270° rotation, `false`
 * otherwise.
 */
static inline constexpr bool is90Transform(LTransform transform) noexcept {
  return static_cast<Int32>(transform) &
         static_cast<Int32>(LTransform::Rotated90);
}
 ...
 */
static inline constexpr LTransform requiredTransform(LTransform a,
                                                     LTransform b) noexcept {
  const Int32 bitmask{static_cast<Int32>(LTransform::Rotated270)};
  const Int32 flip{(static_cast<Int32>(a) & ~bitmask) ^
                   (static_cast<Int32>(b) & ~bitmask)};
  Int32 rotation;

  if (flip)
    rotation = ((static_cast<Int32>(b) & bitmask) +
                (static_cast<Int32>(a) & bitmask)) &
               bitmask;
  else {
    rotation =
        (static_cast<Int32>(b) & bitmask) - (static_cast<Int32>(a) & bitmask);
