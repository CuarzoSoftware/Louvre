 ...
 *
 * This class represents a token used in
...
class Louvre::LActivationToken final : public LObject {
 public:
   ...
   *
   * @note This may, but does not necessarily, refer to the same client that
   * requests surface activation.
   *
   * @return A valid pointer during
   * LActivationTokenManager::createTokenRequest(), but it may return `nullptr`
   *         if accessed later, for example, if the client has disconnected.
   */
  LClient *creator() const noexcept { return m_creator.get(); }
   ...
   *
   * @warning Clients are not obligated to provide this hint, so this method may
   * return `nullptr`.
   */
  LSurface *origin() const noexcept { return m_origin.get(); }

  /**
   * @brief Triggering event
   *
   * Clients typically request token creation in response to an input event. The
   * default implementation of LActivationTokenManager::createTokenRequest()
   * only accepts requests containing a valid triggeringEvent().
   *
   * @see origin()
   *
   * @warning This value is optional, if the client does not specify a
   * triggering event, this method returns `nullptr`.
   */
   ...
   *
   * Incremented by 1 before each
   * LActivationTokenManager::activateSurfaceRequest().
   */
  UInt32 timesUsed() const noexcept { return m_timesUsed; }
   ...
   */
  const std::chrono::time_point<std::chrono::steady_clock> &created()
      const noexcept {
    return m_created;
  }
   ...
   *
   * This value is optional, if the token creator does not assign an app ID,
   ...
   */
  const std::string &token() const noexcept { return *m_key; }
   ...
   *
   * @return An iterator to the next element in
   * LActivationTokenManager::tokens().
   */
  const std::unordered_map<std::string, LActivationToken *>::iterator destroy()
      const noexcept;
 ...
 private:
  ...
  LActivationToken(LClient *creator, LSurface *origin, LEvent *triggeringEvent,
