...
bool LCursor::enabled(LOutput *output) const noexcept {
  ...
  return output->imp()->stateFlags.check(
      LOutput::LOutputPrivate::CursorEnabled);
}

void LCursor::enable(LOutput *output, bool enabled) noexcept {
  if (!output || output->imp()->stateFlags.check(
                     LOutput::LOutputPrivate::CursorEnabled) == enabled)
    return;

  output->imp()->stateFlags.setFlag(LOutput::LOutputPrivate::CursorEnabled,
                                    enabled);
...
void LCursor::enableHwCompositing(LOutput *output, bool enabled) noexcept {
  if (!output || output->imp()->stateFlags.check(
                     LOutput::LOutputPrivate::HwCursorEnabled) == enabled)
    return;

  output->imp()->stateFlags.setFlag(LOutput::LOutputPrivate::HwCursorEnabled,
                                    enabled);
  imp()->textureChanged = true;
  imp()->update();
  output->repaint();
...
bool LCursor::hwCompositingEnabled(LOutput *output) const noexcept {
  ...
  return hasHardwareSupport(output) &&
         output->imp()->stateFlags.check(
             LOutput::LOutputPrivate::HwCursorEnabled);
}

const LRegion &LCursor::damage(LOutput *output) const noexcept {
  if (!output) return LRegion::EmptyRegion();

  return output->imp()->cursorDamage;
}

LCursor::LCursor() noexcept : LPRIVATE_INIT_UNIQUE(LCursor) {
  eglMakeCurrent(compositor()->eglDisplay(), EGL_NO_SURFACE, EGL_NO_SURFACE,
                 compositor()->eglContext());
  ...
  if (!imp()->louvreTexture.setDataFromMainMemory(
          ...
          LOUVRE_DEFAULT_CURSOR_STRIDE, DRM_FORMAT_ARGB8888,
          louvre_default_cursor_data()))
    LLog::warning(
        "[LCursor::LCursor] Failed to create default cursor texture.");
  ...
  glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8, LOUVRE_DEFAULT_CURSOR_WIDTH,
                        LOUVRE_DEFAULT_CURSOR_HEIGHT);
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                            GL_RENDERBUFFER, imp()->glRenderbuffer);
...
LCursor::~LCursor() {
  ...
  if (imp()->glRenderbuffer) glDeleteRenderbuffers(1, &imp()->glRenderbuffer);

  if (imp()->glFramebuffer) glDeleteFramebuffers(1, &imp()->glFramebuffer);
}

void LCursor::useDefault() noexcept {
  if (compositor()->state() == LCompositor::Uninitializing) return;

  imp()->clientCursor.reset();

  if (imp()->texture == imp()->defaultTexture &&
      imp()->hotspotB == imp()->defaultHotspotB)
    return;

  setTextureB(imp()->defaultTexture, imp()->defaultHotspotB);
}

void LCursor::replaceDefaultB(const LTexture *texture,
  ...
  if (update) useDefault();
}

void LCursor::setTextureB(const LTexture *texture,
                          const LPointF &hotspot) noexcept {
  imp()->clientCursor.reset();

  if (!texture) return;

  if (imp()->texture == texture &&
      imp()->lastTextureSerial == texture->serial() &&
      hotspot == imp()->hotspotB)
    return;
...
void LCursor::setCursor(const LClientCursor &clientCursor) noexcept {
  if (clientCursor.cursorRole()) {
    setTextureB(clientCursor.cursorRole()->surface()->texture(),
                clientCursor.cursorRole()->hotspotB());
...
void LCursor::setCursor(const LXCursor *xcursor) noexcept {
  if (xcursor) setTextureB(xcursor->texture(), xcursor->hotspotB());
}
...
void LCursor::move(const LPointF &delta) noexcept { setPos(m_pos + delta); }

void Louvre::LCursor::setPos(const LPointF &pos) noexcept {
  for (LOutput *output : compositor()->outputs())
    if (output->rect().containsPoint(pos) && output) imp()->setOutput(output);
  ...
  if (m_pos.x() > area.x() + area.w()) m_pos.setX(area.x() + area.w());
  if (m_pos.x() < area.x()) m_pos.setX(area.x());

  if (m_pos.y() > area.y() + area.h()) m_pos.setY(area.y() + area.h());
  if (m_pos.y() < area.y()) m_pos.setY(area.y());

  imp()->update();
}

void LCursor::setPos(Float32 x, Float32 y) noexcept { setPos(LPointF(x, y)); }
...
void LCursor::repaintOutputs(bool nonHardwareOnly) noexcept {
  for (LOutput *o : intersectedOutputs())
    if (!nonHardwareOnly || !hwCompositingEnabled(o)) o->repaint();

  if (clientCursor() && clientCursor()->cursorRole() &&
      clientCursor()->cursorRole()->surface()) {
    for (LOutput *output : compositor()->outputs()) {
...
bool LCursor::visible() const noexcept { return imp()->isVisible; }

bool LCursor::hasHardwareSupport(const LOutput *output) const noexcept {
  if (!imp()->hasFb) return false;

  return compositor()->imp()->graphicBackend->outputHasHardwareCursorSupport(
      (LOutput *)output);
}

const LPointF &LCursor::hotspotB() const noexcept { return imp()->hotspotB; }

LTexture *LCursor::texture() const noexcept { return imp()->texture; }
...
const LRect &LCursor::rect() const noexcept { return imp()->rect; }
