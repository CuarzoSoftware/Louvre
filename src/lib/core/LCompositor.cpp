...
LCompositor *Louvre::compositor() noexcept { return s_compositor; }

LSeat *Louvre::seat() noexcept { return s_compositor->imp()->seat; }

LCursor *Louvre::cursor() noexcept { return s_compositor->imp()->cursor; }
...
bool LCompositor::addPosixSignal(int signal) noexcept {
  if (state() == CompositorState::Uninitialized) {
    LLog::error(
        "[LCompositor::addPosixSignal] Failed to add posix signal %d. Signals "
        "must be added after the compositor is initialized.",
        signal);
    return false;
  }
  ...
  if (posixSignals().contains(signal)) {
    LLog::error(
        "[LCompositor::addPosixSignal] Failed to add posix signal %d. Already "
        "added.",
        signal);
    return false;
  }
...
bool LCompositor::removePosixSignal(int signal) noexcept {
  if (state() == CompositorState::Uninitialized ||
      !posixSignals().contains(signal))
    return false;

  if (state() == CompositorState::Uninitialized) {
    LLog::error(
        "[LCompositor::removePosixSignal] Failed to remove posix signal %d. "
        "Signals must be removed while the compositor is initialized.",
        signal);
    return false;
  }
  ...
  if (!posixSignals().contains(signal)) {
    LLog::error(
        "[LCompositor::removePosixSignal] Attempt to remove unregistered posix "
        "signal %d.",
        signal);
    return false;
  }
...
LOutput *LCompositor::mostIntersectedOutput(
    const LRect &rect, bool initializedOnly) const noexcept {
...
bool LCompositor::loadGraphicBackend(
...
bool LCompositor::start() {
  if (compositor() != this) {
    LLog::warning(
        "[LCompositor::start] Compositor already running. Two Louvre "
        "compositors can not live in the same process.");
    return false;
  }
  ...
  if (state() != CompositorState::Uninitialized) {
    LLog::warning(
        "[LCompositor::start] Attempting to start a compositor already "
        "running. Ignoring...");
    return false;
  }
...
Int32 LCompositor::processLoop(Int32 msTimeout) {
  if (state() == CompositorState::Uninitialized) return 0;

  pollfd pollfd{.fd = imp()->eventLoopFd, .events = POLLIN, .revents = 0};
  ...
  if (ret == 1) wl_event_loop_dispatch(imp()->eventLoop, 0);
  ...
  if (state() == CompositorState::Uninitializing) {
    ...
    for (LTexture *texture : imp()->textures) texture->reset();

    imp()->unitInputBackend(true);

    if (imp()->cursor) delete imp()->cursor;
  ...
  retry:
    ...
    while (!imp()->oneShotTimers.empty()) delete imp()->oneShotTimers.back();
    ...
    if (seat()->imp()->ttyNumber != -1) {
      ...
      if (libseat_switch_session(seat()->libseatHandle(),
                                 seat()->imp()->ttyNumber) == 0)
...
Int32 LCompositor::fd() const noexcept { return imp()->eventLoopFd; }

void LCompositor::finish() noexcept {
  if (state() == CompositorState::Uninitialized) return;
...
wl_event_source *LCompositor::addFdListener(int fd, void *userData,
                                            int (*callback)(int, unsigned int,
                                                            void *),
                                            UInt32 flags) {
  return wl_event_loop_add_fd(compositor()->imp()->eventLoop, fd, flags,
                              callback, userData);
}
...
LCursor *LCompositor::cursor() const noexcept { return imp()->cursor; }

LSeat *LCompositor::seat() const noexcept { return imp()->seat; }
...
void LCompositor::repaintAllOutputs() noexcept {
  for (LOutput *o : imp()->outputs) o->repaint();
}
...
bool LCompositor::addOutput(LOutput *output) {
  ...
  if (output->lease()) output->lease()->finished();

  imp()->outputs.push_back(output);

  if (imp()->outputs.size() == 1) cursor()->imp()->setOutput(output);

  output->imp()->initACK = false;

  if (!output->imp()->initialize()) {
    LLog::error("[LCompositor::addOutput] Failed to initialize output %s.",
                output->name());
    LVectorRemoveOne(imp()->outputs, output);

    if (imp()->outputs.empty()) cursor()->imp()->setOutput(nullptr);

    return false;
  }

  while (!output->imp()->initACK) usleep(10000);

  for (auto *head : output->imp()->wlrOutputHeads) head->enabled(true);

  return true;
}

void LCompositor::removeOutput(LOutput *output) {
  if (!isGraphicBackendInitialized()) return;
  ...
  for (LOutput *o : imp()->outputs) {
    ...
    if (o == output) {
      ...
      if (output->threadId() == std::this_thread::get_id()) return;
      ...
      while (output->imp()->state != LOutput::Uninitialized) usleep(1000);

      output->imp()->callLock.store(true);

      for (auto *head : output->imp()->wlrOutputHeads) head->enabled(false);

      for (LSurface *s : surfaces()) s->sendOutputLeaveEvent(output);

      for (LView *v : imp()->views) v->removeThread(o->threadId());
      ...
      for (LClient *client : clients()) {
        for (std::size_t i = 0; i < client->imp()->outputGlobals.size();) {
          ...
          if (output == global->output()) {
            ...
            client->imp()->outputGlobals[i] =
                std::move(client->imp()->outputGlobals.back());
      ...
      if (output->imp()->global) removeGlobal(output->imp()->global);

      LVectorRemoveOneUnordered(cursor()->imp()->intersectedOutputs, output);

      if (cursor()->m_output == output) cursor()->m_output = nullptr;
...
const std::list<LSurface *> &LCompositor::layer(
...
LClient *LCompositor::getClientFromNativeResource(
    const wl_client *client) noexcept {
...
LGlobal *LCompositor::globalCreate(const wl_interface *interface, Int32 version,
                                   void *data, wl_global_bind_func_t bind) {
  imp()->globals.emplace_back(new LGlobal(
      wl_global_create(display(), interface, version, nullptr, bind)));
  wl_global_set_user_data(
      const_cast<wl_global *>(imp()->globals.back()->global()),
      imp()->globals.back());
