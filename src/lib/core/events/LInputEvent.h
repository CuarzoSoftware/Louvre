...
class Louvre::LInputEvent : public LEvent {
 public:
   ...
   *
   * @param device If `nullptr` is passed, a generic fake input device is
   * assigned.
   */
   ...
   */
  LInputDevice *device() const noexcept { return m_device; }

 protected:
  LInputEvent(Type type, Subtype subtype, UInt32 serial, UInt32 ms, UInt64 us,
