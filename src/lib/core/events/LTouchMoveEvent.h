...
class Louvre::LTouchMoveEvent final : public LTouchEvent {
 public:
   ...
   */
  LTouchMoveEvent(Int32 id = 0, const LPointF &pos = LPointF(0.f, 0.f),
   ...
   */
  void setPos(const LPointF &pos) noexcept { m_pos = pos; }
   ...
   */
  void setX(Float32 x) noexcept { m_pos.setX(x); }

  /**
   * @brief Sets the y-coordinate of the touch point position.
   */
  void setY(Float32 y) noexcept { m_pos.setY(y); }

  /**
   * @brief Gets the position of the touch point.
   *
   * @note The position is typically normalized to the range [0, 1] for both
   * axes.
   */
   ...
   */
  void setId(Int32 id) noexcept { m_id = id; }
  ...
  /**
   * @brief The surface or view local position where the touch point is
   * positioned in surface coordinates.
   */
