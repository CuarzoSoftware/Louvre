...
class Louvre::LEvent {
 public:
   ...
   */
  Subtype subtype() const noexcept { return m_subtype; }

  /**
   * @brief Sets the serial of the event.
   */
  void setSerial(UInt32 serial) noexcept { m_serial = serial; }
 ...
 protected:
  LEvent(Type type, Subtype subtype, UInt32 serial, UInt32 ms,
         UInt64 us) noexcept
      : m_type(type),
        m_subtype(subtype),
        m_serial(serial),
