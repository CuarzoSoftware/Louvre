...
class Louvre::LPointerButtonEvent final : public LPointerEvent {
 public:
   ...
   *
   * You can find the complete list of pointer button codes in the
   ...
   */
  LPointerButtonEvent(Button button = Left, State state = Pressed,
   ...
   */
  void setButton(Button button) noexcept { m_button = button; }

  /**
   * @brief Gets the button code associated with the event.
   */
  Button button() const noexcept { return m_button; }
