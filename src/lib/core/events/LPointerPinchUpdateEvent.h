...
class Louvre::LPointerPinchUpdateEvent final : public LPointerEvent {
 public:
   ...
   * @param deltaUnaccelerated The unaccelerated movement delta of the pinch
   * gesture.
  ...
  LPointerPinchUpdateEvent(
      UInt32 fingers = 0, const LPointF &delta = LPointF(0.f, 0.f),
      const LPointF &deltaUnaccelerated = LPointF(0.f, 0.f),
   ...
   */
  void setDelta(const LPointF &delta) noexcept { m_delta = delta; }

  /**
   * @brief Sets the movement delta along the x-axis of the pinch gesture.
   */
  void setDx(Float32 dx) noexcept { m_delta.setX(dx); }

  /**
   * @brief Sets the movement delta along the y-axis of the pinch gesture.
   */
  void setDy(Float32 dy) noexcept { m_delta.setY(dy); }

  /**
   * @brief Gets the movement delta of the pinch gesture.
   */
  const LPointF &delta() const noexcept { return m_delta; }
  ...
  /**
   * @brief Sets the unaccelerated movement delta along the x-axis of the pinch
   * gesture.
   */
  ...
  /**
   * @brief Sets the unaccelerated movement delta along the y-axis of the pinch
   * gesture.
   */
   ...
   */
  void setScale(Float32 scale) noexcept { m_scale = scale; }

  /**
   * @brief Gets the scale factor of the pinch gesture.
   *
   * The scale begins at 1.0, and if e.g. the fingers moved together by 50% then
   * the scale will become 0.5, if they move twice as far apart as initially the
   * scale becomes 2.0, etc.
   */
  Float32 scale() const noexcept { return m_scale; }

  /**
   * @brief Sets the rotation angle of the pinch gesture.
   */
  void setRotation(Float32 rotation) noexcept { m_rotation = rotation; }

  /**
   * @brief Gets the rotation angle of the pinch gesture.
   *
   * The angle delta is defined as the change in angle of the line formed by the
   * 2 fingers of a pinch gesture. Clockwise rotation is represented by a
   ...
   */
  Float32 rotation() const noexcept { return m_rotation; }
