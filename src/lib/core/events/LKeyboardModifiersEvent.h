 ...
 *
 * Keyboard modifiers events are automatically sent to client surfaces when they
...
class Louvre::LKeyboardModifiersEvent final : public LKeyboardEvent {
 public:
  ...
  struct Modifiers {
    ...
    /// Hooked modifiers that will be disabled after a non-modifier key is
    /// pressed
    UInt32 latched = 0;
    ...
    /// Group the above states (use this value if the source of a modifier
    /// change is not of your interest)
    UInt32 group = 0;
  };
   ...
   */
  const Modifiers &modifiers() const noexcept { return m_modifiers; }
