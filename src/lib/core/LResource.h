...
#define LGLOBAL_INTERFACE                                         \
  ...
  static void bind(wl_client *client, void *data, UInt32 version, \
                   UInt32 id) noexcept;                           \
  static Int32 maxVersion() noexcept;                             \
  static const wl_interface *interface() noexcept;

#define LRES_CAST(type, res) \
  (*static_cast<type *>(wl_resource_get_user_data(res)))

/**
 * @brief Wrapper for native **wl_resource** structs
 *
 * This class serves as a wrapper for the **wl_resource** struct from the
 * original Wayland library.\n Numerous objects expose their LResource objects,
 * allowing you to utilize native Wayland methods if desired.
 *
 * To explore all the globals/resources implemented by Louvre, refer to the
 * ```Louvre/src/lib/protocols``` directory.
 *
 * @note This class is primarily intended for implementing additional protocols
 * beyond those provided by Louvre.
 */
class Louvre::LResource : public LObject {
 public:
   ...
   *
   * This method returns the original **wl_resource** pointer associated with
   * this LResource.
   *
   ...
   *
   * This method returns the version of the global interface associated with
   * this resource.
   *
   ...
   *
   * This method returns the ID of the resource, which is the same as calling
   * **wl_resource_get_id()**.
   *
   ...
   *
   * This method destroys the resource, equivalent to calling
   * **wl_resource_destroy()**.
   */
  void destroy();
  ...
  void postError(UInt32 code, std::format_string<Args...> message,
                 Args &&...args) {
 ...
 protected:
   ...
   *
   * This constructor creates an LResource and registers it with the provided
   * parameters.
   *
   ...
   * @param id ID of the resource; passing 0 automatically generates an
   * increasing ID.
   * @param implementation Pointer to the interface implementation (struct with
   * callback functions to handle client requests).
   */
  LResource(wl_client *client, const wl_interface *interface, Int32 version,
            UInt32 id, const void *implementation) noexcept;
   ...
   * callback functions to handle client requests).
   */
  LResource(LClient *client, const wl_interface *interface, Int32 version,
            UInt32 id, const void *implementation) noexcept;
