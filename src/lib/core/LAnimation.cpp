...
LAnimation::LAnimation(UInt32 durationMs, const Callback &onUpdate,
                       const Callback &onFinish) noexcept
    : m_onUpdate(onUpdate), m_duration(durationMs), m_onFinish(onFinish) {
...
LAnimation::~LAnimation() {
  ...
  auto it{std::find(compositor()->imp()->animations.begin(),
                    compositor()->imp()->animations.end(), this)};
...
void LAnimation::oneShot(UInt32 durationMs, const Callback &onUpdate,
                         const Callback &onFinish) noexcept {
...
void LAnimation::stop() {
  ...
  if (m_onFinish) m_onFinish(this);

  if (m_destroyOnFinish) m_pendingDestroy = true;
}
