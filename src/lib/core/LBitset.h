 ...
 *
 * The LBitset class template is similar to `std::bitset` in that it enables the
 * compact storage of a set of conditions or states using bits.\n Unlike
 * `std::bitset`, LBitset functions are designed to modify and retrieve bit
 * states using flags rather than indices, which are ideally defined within an
 * enum.
 *
 * It is widely used in the private API of Louvre classes to optimize memory
 * usage.
 *
 * Example usage:
 *
...
class Louvre::LBitset {
 ...
 public:
   ...
   *
   * Initializes the bitset with the specified initial flags. If no flags are
   * provided, the bitset is constructed empty without any flags.
   *
   ...
   *
   * Sets the specified flags by combining them with the existing flags using
   * the bitwise OR operator '|'.
   *
   * @param flags The flag or combination of flags to be added
   */
  constexpr void add(Flag flags) noexcept { m_flags |= flags; }

  /**
   * @brief Remove flags from the bitfield
   *
   * Clears the specified flags by performing a bitwise AND operation with the
   * complement of the provided flags.
   *
   * @param flags The flag or combination of flags to be removed
   */
  constexpr void remove(Flag flags) noexcept { m_flags &= ~flags; }
   ...
   */
  constexpr Flag get() const noexcept { return m_flags; }
   ...
   */
  constexpr void set(Flag flags) noexcept { m_flags = flags; }

  /**
   * @brief Set or unset a specific flag in the bitfield
   *
   * Modifies the specified flag (or flags) in the bitfield. If `enable` is
   * `true`, the flag is set, otherwise, it is removed.
   *
  ...
  friend constexpr T operator~(T flag) noexcept { return T(~Flag(flag)); }
  ...
  constexpr operator T() const noexcept { return T(m_flags); }
