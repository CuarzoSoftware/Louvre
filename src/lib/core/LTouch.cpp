...
LTouch::LTouch(const void *params) noexcept
    : LFactoryObject(FactoryObjectType) {
  assert(params != nullptr &&
         "Invalid parameter passed to LTouch constructor.");
  LTouch **ptr{(LTouch **)params};
  assert(*ptr == nullptr && *ptr == seat()->touch() &&
         "Only a single LTouch instance can exist.");
  *ptr = this;
}
...
LTouchPoint *LTouch::createOrGetTouchPoint(
    const LTouchDownEvent &event) noexcept {
  for (LTouchPoint *tp : touchPoints())
    if (tp->id() == event.id()) return tp;
...
LPointF LTouch::toGlobal(LOutput *output,
                         const LPointF &touchPointPos) noexcept {
  if (!output) return touchPointPos;

  LPointF point;

  switch (output->transform()) {
    case LTransform::Normal: point = output->size() * touchPointPos; break;
