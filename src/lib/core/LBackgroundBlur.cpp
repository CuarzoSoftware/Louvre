...
LBackgroundBlur::LBackgroundBlur(const void *params) noexcept
    : LFactoryObject(FactoryObjectType), m_surface(*((LSurface *)params)) {}

Protocols::BackgroundBlur::RBackgroundBlur *
LBackgroundBlur::backgroundBlurResource() const noexcept {
...
void LBackgroundBlur::sendPendingConfiguration() noexcept {
  ...
  if (m_flags.check(HasStateToSend)) res.state(m_pendingConfiguration.state);
