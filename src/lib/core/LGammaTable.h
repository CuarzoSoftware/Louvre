 ...
 *
 * The LGammaTable class allows to define the gamma correction curves for each
 * RGB component of an LOutput, applicable through the LOutput::setGamma()
 * method.
 *
 * @note Clients using the Wlr Gamma Control protocol can also request to set
 * the gamma table of an output via LOutput::setGammaRequest().
 *
 * The curves (red, green and blue) are stored in a single array of type @ref
 * UInt16. The table size (size()) indicates how many @ref UInt16 values each
 * curve contains. For example, if size() is 256, the array would have 256 * 3
 * @ref UInt16 values, where the first 256 represent the red curve, the next 256
 * represent the green curve, and the remaining values represent the blue curve.
 *
 * The table size must match the LOutput::gammaSize() property of the output to
 * which the correction is applied. If not, LOutput::setGamma() will fail. To
 * modify table values, you can use the fill() auxiliary method or directly
 * modify each curve using red(), green(), and blue(), which return the address
 * of each curve in the array.
 *
...
class Louvre::LGammaTable {
 public:
   ...
   */
  LGammaTable(UInt32 size = 0) noexcept { setSize(size); }
  ...
  LGammaTable(const LGammaTable &other) noexcept {
    setSize(other.size());
    if (size() > 0) memcpy(m_table, other.m_table, size() * 3 * sizeof(UInt16));
  }
  ...
  void setSize(UInt32 size) noexcept {
    if (size != m_size) {
      ...
      if (size > 0) m_table = new UInt16[size * 3];
    }
  }
   ...
   *
   * The size represents the number of @ref UInt16 values used to independently
   * represent each RGB curve.
   *
   ...
   *
   * This auxiliary method allows setting gamma, brightness, and contrast values
   * for the table.
   *
   ...
   *
   * @return Pointer to the red curve in the array, or `nullptr` if the table
   * size is 0.
   */
  UInt16 *red() const noexcept { return m_table; }

  /**
   * @brief Gets a pointer to the beginning of the green curve in the array.
   *
   * @return Pointer to the green curve in the array, or `nullptr` if the table
   * size is 0.
   */
   ...
   *
   * @return Pointer to the blue curve in the array, or `nullptr` if the table
   * size is 0.
   */
 ...
 private:
  ...
  // This is only set for tables created by a client and is not copied across
  // tables
