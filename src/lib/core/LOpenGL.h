...
class Louvre::LOpenGL {
 public:
   ...
   * @returns A string with the contents of the shader or `nullptr` in case of
   * error.
   */
   ...
   *
   * This function converts an OpenGL error code obtained from glGetError() into
   ...
   * @returns An integer with the maximum number of texture units supported by
   * OpenGL.
   */
   ...
   *
   * @note The image format is always converted to `DRM_FORMAT_ARGB8888` or
   * `DRM_FORMAT_ABGR8888`.
   *
   * @param file Path to the image file. Must be an image format supported by
   * [STB Image](https://github.com/nothings/stb) (JPG, PNG, TGA, BMP, PSD, GIF,
   * HDR, PIC).
   ...
   *
   * This function queries the list of supported OpenGL extensions and
   * determines whether the specified extension is among them.
   *
