 ...
 *
 * The LPopupRole class is a role for surfaces commonly used by clients to
 * display context menus and tooltips.
 *
 * <center><img
 * src="https://lh3.googleusercontent.com/6caGayutKKWqndpd6ogno2lPw8XGELxnFums4gvkWZKOJYO0762yVG3mHLrc1rw63r1eEJabEdW9F5AA2BDTFCtpB_hiPlTY4FkKlHfH1B-2MdLvXCD6RuwZOZvhOl6EhydtsOYGPw=w2400"></center>
 *
 * The popup role is part of the [XDG
 * Shell](https://wayland.app/protocols/xdg-shell#xdg_popup). The Wayland
 * protocol also has its own popup role, but it is considered obsolete and
 * therefore not included in the library.
 */
class Louvre::LPopupRole : public LBaseSurfaceRole {
 public:
  ...
  static constexpr LFactoryObject::Type FactoryObjectType =
      LFactoryObject::Type::LPopupRole;
   ...
   *
   * This struct holds the last configuration parameters assigned with
   * configureRect().
   *
   * The pending configuration parameters are sent to the client at most once
   * per Louvre main loop iteration.
   *
   * The pending serial is also updated at most once per loop iteration if a
   * configure was done.
   *
   ...
   *
   * This method instructs the client to set the size and local position of the
   * popup and stores the value in pendingConfiguration().\n The position is
   * relative to its parent's window geometry, and the size does not include the
   * popup decorations, see windowGeometry().
   *
  ...
  /**
   * @brief Calculates the size and parent-local position the popup should be
   * configured to in order to be unconstrained.
   *
   * @note Ensure that bounds() is properly set before using this method.
   *
   * @param futureParentPos The future position of the parent surface, or
   * `nullptr` to use the current position. See
   * LPositioner::hasParentConfigureSerial() and LPositioner::hasParentSize().
   *
   * @return The calculated rect representing the unconstrained size and
   * position.
   */
  LRect calculateUnconstrainedRect(
      const LPoint *futureParentPos = nullptr) const noexcept;

  /**
   * @brief Set the positioning bounds constraints for the popup.
   *
   * This method is used to define the area within which the popup must be
   * positioned.
   *
   * @param bounds The constraint rect in compositor-global coordinates.
   * Providing a rect with zero area deactivates the constraint.
   */
  void setBounds(const LRect &bounds) noexcept { m_bounds = bounds; }

  /**
   * @brief Gets the constraint bounds.
   *
   * This method returns the constraint bounds for positioning the popup which
   * can be set using setBounds().
   */
  const LRect &bounds() const noexcept { return m_bounds; }

  /**
   * @brief Checks which toplevel edges would be constrained if the popup is
   * positioned within the given rect.
   *
   ...
   *
   * The window geometry is a rect within the popups's surface that excludes its
   * decorations (typically shadows).
   *
  ...
  /**
   * @brief Retrieves the current local position relative to the parent window
   * geometry.
   *
   ...
   */
  const LPoint &localPos() const noexcept { return atoms().localPos; }

  /**
   * @brief Gets the positioning rules for the popup.
   */
  const LPositioner &positioner() const { return m_positioner; }

  /**
   * @brief Dismiss the popup.
   *
   * This method closes the popup along with all its children, starting from the
   * topmost and descending downwards.
   */
  void dismiss();
   ...
   * prevent displaying the given popup on other outputs.
   *
   * @see exclusiveOutput()
   *
  ...
  /**
   *  @brief [xdg_popup](https://wayland.app/protocols/xdg-shell#xdg_popup)
   ...
   *
   * Client request to redirect all keyboard events to the popup. See
   * LKeyboard::setGrab().\n During a keyboard grab, no other surfaces can
   * acquire keyboard focus.\n Additionally, if the surface undergoing the grab
   * is destroyed, the keyboard grab is transferred to its parent surface.
   *
