...
static void invertGravity(LPositioner::Gravity &gravity, bool x,
                          bool y) noexcept {
  if (x) {
...
static void updateFinalRectPos(Config &config) noexcept {
  config.finalRect.setPos(config.parentPos + config.anchorOrigin -
                          config.popupOrigin + config.slide +
...
LBitset<LEdge> LPopupRole::constrainedEdges(const LRect &rect) const noexcept {
  ...
  if (bounds().w() > 0) {
    if (rect.x() < bounds().x()) edges.add(LEdgeLeft);
  ...
  if (bounds().h() > 0) {
    if (rect.y() < bounds().y()) edges.add(LEdgeTop);
...
LRect LPopupRole::calculateUnconstrainedRect(
    const LPoint *futureParentPos) const noexcept {
  Config conf;
  conf.popup = this;
  ...
  if (futureParentPos)
    ...
    if (surface()->parent()->toplevel())
      conf.parentPos +=
          surface()->parent()->toplevel()->windowGeometry().topLeft();
    else if (surface()->parent()->popup())
      conf.parentPos +=
          surface()->parent()->popup()->windowGeometry().topLeft();
  }
  ...
  if (constrained.check(LEdgeLeft | LEdgeRight)) {
    ...
    if (positioner().constraintAdjustments().check(LPositioner::SlideX) &&
        !constrained.checkAll(LEdgeLeft | LEdgeRight)) {
      if (constrained.check(LEdgeLeft))
        conf.slide.setX(bounds().x() - conf.finalRect.x());
      else
        conf.slide.setX(bounds().x() + bounds().w() - conf.finalRect.x() -
                        conf.finalRect.w());
...
handleYAxis:
  ...
  if (constrained.check(LEdgeTop | LEdgeBottom)) {
    ...
    if (positioner().constraintAdjustments().check(LPositioner::SlideY) &&
        !constrained.checkAll(LEdgeTop | LEdgeBottom)) {
      if (constrained.check(LEdgeTop))
        conf.slide.setY(bounds().y() - conf.finalRect.y());
      else  // Bottom
        conf.slide.setY(bounds().y() + bounds().h() - conf.finalRect.y() -
                        conf.finalRect.h());
...
LPopupRole::LPopupRole(const void *params) noexcept
    : LBaseSurfaceRole(FactoryObjectType,
                       ...
                       LSurface::Role::Popup),
      m_positioner(
          *static_cast<const LPopupRole::Params *>(params)->positioner) {}
...
const LPopupRole::Configuration *LPopupRole::findConfiguration(
...
bool LPopupRole::isTopmostPopup() const noexcept {
  std::list<LSurface *>::const_reverse_iterator s =
...
void LPopupRole::configureRect(const LRect &rect) const noexcept {
  ...
  if (!res.xdgSurfaceRes()) return;

  if (!m_flags.check(CanBeConfigured)) return;
...
void LPopupRole::handleSurfaceCommit(CommitOrigin origin) {
  ...
  if (m_flags.check(Dismissed)) return;

  if (surface()->imp()->pendingParent) {
    if (surface()->imp()->isInChildrenOrPendingChildren(
            surface()->imp()->pendingParent)) {
  ...
  if (!surface()->parent()) {
    xdgSurfaceResource()->postError(XDG_WM_BASE_ERROR_INVALID_POPUP_PARENT,
                                    "Popup has no parent.");
    return;
  }
  ...
  // Request unmap
  if (surface()->mapped() &&
      (!surface()->hasBuffer() || !surface()->parent())) {
    dismiss();
    return;
  }
...
void LPopupRole::sendPendingConfiguration() noexcept {
  if (!m_flags.check(HasConfigurationToSend)) return;
