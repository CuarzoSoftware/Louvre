...
LSessionLockRole::LSessionLockRole(const void *params) noexcept
    : LBaseSurfaceRole(
          FactoryObjectType, static_cast<const Params *>(params)->resource,
          static_cast<const Params *>(params)->surface, LSurface::SessionLock),
...
void LSessionLockRole::handleSurfaceCommit(CommitOrigin /*origin*/) {
  ...
  switch (sessionLockRes.reply()) {
    ...
    case RSessionLock::Locked: {
      if (exclusiveOutput() &&
          exclusiveOutput()->state() != LOutput::Uninitialized) {
    ...
    case RSessionLock::Finished: break;
  };
}
...
void LSessionLockRole::configure(const LSize &size) noexcept {
  ...
  if (m_pendingSize.w() < 0) m_pendingSize.setW(0);

  if (m_pendingSize.h() < 0) m_pendingSize.setH(0);
...
void LSessionLockRole::sendPendingConfiguration() noexcept {
  if (!m_hasPendingConf) return;

  m_hasPendingConf = false;
  m_sentConfs.emplace(m_pendingSize, m_pendingSerial);
  static_cast<Protocols::SessionLock::RSessionLockSurface *>(resource())
      ->configure(m_pendingSerial, m_pendingSize.w(), m_pendingSize.h());
}
