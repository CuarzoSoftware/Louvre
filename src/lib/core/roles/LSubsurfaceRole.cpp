...
bool LSubsurfaceRole::acceptCommitRequest(
...
static void checkMapping(LSurface *surface) {
  surface->imp()->setMapped(surface->parent() && surface->parent()->mapped() &&
                            surface->hasBuffer());
}
...
void LSubsurfaceRole::handleParentCommit() {
  ...
  if (m_pendingPlaceAbove) {
    compositor()->imp()->insertSurfaceAfter(
        m_pendingPlaceAbove, surface(), OP::UpdateSurfaces | OP::UpdateLayers);

    if (m_pendingPlaceAbove == surface()->parent()) {
      if (surface()->parent()->children().front() != surface()) {
        surface()->parent()->imp()->children.erase(
            surface()->imp()->parentLink);
        surface()->parent()->imp()->children.push_front(surface());
        surface()->imp()->parentLink =
            surface()->parent()->imp()->children.begin();
        surface()->parent()->imp()->stateFlags.add(
            LSurface::LSurfacePrivate::ChildrenListChanged);
        placedAbove(m_pendingPlaceAbove);
      }
    } else if (*std::next(m_pendingPlaceAbove->imp()->parentLink) !=
               surface()) {
      surface()->parent()->imp()->children.erase(surface()->imp()->parentLink);
      surface()->imp()->parentLink =
          surface()->parent()->imp()->children.insert(
              std::next(m_pendingPlaceAbove->imp()->parentLink), surface());
      surface()->parent()->imp()->stateFlags.add(
          LSurface::LSurfacePrivate::ChildrenListChanged);
      placedAbove(m_pendingPlaceAbove);
    }
  ...
  if (m_pendingPlaceBelow) {
    if (*std::prev(m_pendingPlaceBelow->imp()->parentLink) != surface()) {
      compositor()->imp()->insertSurfaceBefore(
          m_pendingPlaceBelow, surface(),
          OP::UpdateSurfaces | OP::UpdateLayers);
      surface()->parent()->imp()->children.erase(surface()->imp()->parentLink);
      surface()->imp()->parentLink =
          surface()->parent()->imp()->children.insert(
              m_pendingPlaceBelow->imp()->parentLink, surface());
      surface()->parent()->imp()->stateFlags.add(
          LSurface::LSurfacePrivate::ChildrenListChanged);
  ...
  if (isSynced() && m_hasCache)
    Wayland::RSurface::apply_commit(surface(),
                                    LBaseSurfaceRole::CommitOrigin::Parent);
}

void LSubsurfaceRole::handleParentChange() { checkMapping(surface()); }

void LSubsurfaceRole::handleParentMappingChange() { checkMapping(surface()); }
