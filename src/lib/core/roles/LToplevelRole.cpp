...
LToplevelRole::LToplevelRole(const void *params) noexcept
  ...
  if (resource()->version() >= 6) m_supportedStates.add(Suspended);
}
...
const LToplevelRole::Configuration *LToplevelRole::findConfiguration(
    UInt32 serial) const noexcept {
  for (auto &conf : m_sentConfigurations)
    if (conf.serial == serial) return &conf;

  if (m_pendingConfiguration.serial == serial) return &m_pendingConfiguration;
  if (m_lastACKConfiguration.serial == serial) return &m_lastACKConfiguration;
...
void LToplevelRole::setExclusiveOutput(LOutput *output) noexcept {
  ...
  if (output) surface()->sendOutputEnterEvent(output);
}
...
void LToplevelRole::handleSurfaceCommit(LBaseSurfaceRole::CommitOrigin origin) {
  ...
  if (m_flags.check(HasPendingInitialConf)) {
    if (surface()->hasBuffer()) {
      resource()->postError(XDG_SURFACE_ERROR_UNCONFIGURED_BUFFER,
  ...
  if (!surface()->mapped() && surface()->hasBuffer()) {
    ...
    if (m_flags.check(HasPendingFirstMap)) {
      ...
      for (LClient *client : compositor()->clients())
        for (auto *foreignToplevelManager :
             client->foreignToplevelManagerGlobals())
...
void LToplevelRole::handleParentChange() {
  LToplevelRole *parent{(surface()->parent() && surface()->parent()->toplevel())
                            ? surface()->parent()->toplevel()
                            : nullptr};
    ...
    for (auto *controller : m_foreignControllers) {
      for (auto *parentController : parent->m_foreignControllers) {
        if (controller->resource().foreignToplevelManagerRes() &&
            controller->resource().foreignToplevelManagerRes() ==
                parentController->resource().foreignToplevelManagerRes()) {
...
void LToplevelRole::fullAtomsUpdate() {
  ...
  if (currentAtoms().serial != pendingAtoms().serial) {
    ...
    if (currentAtoms().state != pendingAtoms().state) {
      ...
      const LBitset<State> stateChanges{pendingAtoms().state ^
                                        currentAtoms().state};

      if (stateChanges.check(Activated) &&
          pendingAtoms().state.check(Activated)) {
        if (seat()->activeToplevel() && seat()->activeToplevel() != this) {
          seat()->activeToplevel()->m_flags.add(ForceRemoveActivatedFlag);
          seat()->activeToplevel()->configureState(
              seat()->activeToplevel()->pendingConfiguration().state &
              ~Activated);
        }
      ...
      if (stateChanges.check(Fullscreen)) {
        ...
        surface()->imp()->setLayer(
            pendingAtoms().state.check(Fullscreen) ? LLayerTop : defaultLayer);
      }
    }
  ...
  if (xdgSurfaceResource()->m_hasPendingWindowGeometry)
    xdgSurfaceResource()->m_currentWindowGeometry =
        xdgSurfaceResource()->m_pendingWindowGeometry;
  // If never assigned, use the surface + subsurfaces bounds
  else if (!xdgSurfaceResource()->m_windowGeometrySet)
    xdgSurfaceResource()->m_currentWindowGeometry =
        xdgSurfaceResource()->calculateGeometryWithSubsurfaces();
...
void LToplevelRole::partialAtomsUpdate() {
  ...
  const LBitset<State> stateChanges{pendingAtomsBackup.state ^
                                    currentAtoms().state};
  ...
  if (stateChanges.check(Activated)) {
    ...
    pendingAtoms().state.setFlag(Activated,
                                 pendingAtomsBackup.state.check(Activated));
  }

  if (stateChanges.check(Suspended)) {
    changesToNotify.add(StateChanged);
    pendingAtoms().state.setFlag(Suspended,
                                 pendingAtomsBackup.state.check(Suspended));
  }

  if (stateChanges.check(Resizing)) {
    changesToNotify.add(StateChanged);
    pendingAtoms().state.setFlag(Resizing,
                                 pendingAtomsBackup.state.check(Resizing));
  }
...
void LToplevelRole::sendPendingConfiguration() noexcept {
  if (!m_flags.check(HasSizeOrStateToSend | HasDecorationModeToSend |
                     HasBoundsToSend | HasCapabilitiesToSend))
    return;
  ...
  m_flags.remove(HasSizeOrStateToSend | HasDecorationModeToSend |
                 HasBoundsToSend | HasCapabilitiesToSend);
}
...
void LToplevelRole::reset() noexcept {
  ...
  if (seat()->activeToplevel() == this) seat()->imp()->activeToplevel = nullptr;
...
LMargins LToplevelRole::calculateConstraintsFromOutput(
    LOutput *output, bool includeExtraGeometry) const noexcept {
  LMargins constraints{LEdgeDisabled, LEdgeDisabled, LEdgeDisabled,
                       LEdgeDisabled};
  ...
  if (output) {
    ...
    if (includeExtraGeometry) {
      if (output->exclusiveEdges().right != 0)
        constraints.right = output->pos().x() + output->size().w() -
                            output->exclusiveEdges().right -
                            extraGeometry().right - extraGeometry().left;

      if (output->exclusiveEdges().bottom != 0)
        constraints.bottom = output->pos().y() + output->size().h() -
                             output->exclusiveEdges().bottom -
                             extraGeometry().bottom - extraGeometry().top;
    } else {
      if (output->exclusiveEdges().right != 0)
        constraints.right = output->pos().x() + output->size().w() -
                            output->exclusiveEdges().right;
