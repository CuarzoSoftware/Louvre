...
LLayerRole::LLayerRole(const void *params) noexcept
  ...
  m_exclusiveZone.setOnRectChangeCallback([this](auto) {
    if (surface()->mapped() && surface()->hasBuffer()) configureRequest();
  });
}
...
void LLayerRole::configureSize(const LSize &size) noexcept {
  if (m_flags.check(ClosedSent)) return;
  ...
  if (currentAtoms().size.w() == 0) {
    ...
    if (!m_flags.check(HasPendingSize)) pendingAtoms().size.setW(size.w());
  }

  if (currentAtoms().size.h() == 0) {
    currentAtoms().size.setH(size.h());

    if (!m_flags.check(HasPendingSize)) pendingAtoms().size.setH(size.h());
  }
}
...
LEdge LLayerRole::edgesToSingleEdge() const noexcept {
  if (anchor() == (LEdgeTop | LEdgeLeft)) {
    ...
    if (exclusiveEdge())
      ...
      return LEdgeBottom;
  } else if (anchor() == LEdgeTop ||
             anchor() == (LEdgeLeft | LEdgeTop | LEdgeRight)) {
    return LEdgeTop;
  } else if (anchor() == LEdgeBottom ||
             anchor() == (LEdgeLeft | LEdgeBottom | LEdgeRight)) {
    return LEdgeBottom;
  } else if (anchor() == LEdgeLeft ||
             anchor() == (LEdgeBottom | LEdgeLeft | LEdgeTop)) {
    return LEdgeLeft;
  } else if (anchor() == LEdgeRight ||
             anchor() == (LEdgeBottom | LEdgeRight | LEdgeTop)) {
    return LEdgeRight;
  }
  // Center
  else {
...
void LLayerRole::handleSurfaceCommit(CommitOrigin /*origin*/) noexcept {
  ...
  if (m_flags.check(Flags::HasPendingInitialConf) && surface()->hasBuffer()) {
    res.postError(ZWLR_LAYER_SHELL_V1_ERROR_ALREADY_CONSTRUCTED,
  ...
  if (m_flags.check(Flags::HasPendingSize)) {
    if (pendingAtoms().size.w() == 0) {
      if (pendingAtoms().anchor.checkAll(LEdgeLeft | LEdgeRight) == 0) {
        res.postError(
            ...
            "width is 0 but anchors do not include left and right (anchor must "
            "be set to opposite edges in the omitted dimensions)");
        return;
      }
    }
    ...
    if (pendingAtoms().size.h() == 0) {
      if (pendingAtoms().anchor.checkAll(LEdgeTop | LEdgeBottom) == 0) {
        res.postError(
            ZWLR_LAYER_SURFACE_V1_ERROR_INVALID_SIZE,
            "height is 0 but anchors do not include top and bottom (anchor "
            "must be set to opposite edges in the omitted dimensions)");
        return;
      }
    }
  ...
  LBitset<AtomChanges> changesToNotify =
      m_flags & (AnchorChanged | ExclusiveEdgeChanged |
                 ExclusiveZoneSizeChanged | KeyboardInteractivityChanged |
                 LayerChanged | MarginsChanged | SizeChanged);
  ...
  if (changesToNotify.check(AnchorChanged | ExclusiveEdgeChanged |
                            ExclusiveZoneSizeChanged | MarginsChanged)) {
    ...
    if (zoneSize > 0) {
      switch (edge) {
        case LEdgeTop: zoneSize += margins().top; break;
        case LEdgeLeft: zoneSize += margins().left; break;
        case LEdgeRight: zoneSize += margins().right; break;
        case LEdgeBottom: zoneSize += margins().bottom; break;
  ...
  if (changesToNotify != 0) atomsChanged(changesToNotify, pendingAtoms());
  ...
  if (needsConfigure) configureRequest();
}
...
void LLayerRole::updateMappingState() noexcept {
  surface()->imp()->setMapped(
      ...
      m_flags.check(MappedByClient) && surface()->hasBuffer());
}
