 ...
 *
 * The toplevel surface role represents typical desktop windows that usually
 * have a title and buttons to close, minimize, and maximize.
 *
 * <center><img height="250px"
 * src="https://lh3.googleusercontent.com/PvNaxLgkjaPryJ8W_P3bDOccQu1m-zNEcI7aH_R8WggzylV5LQZtuzLTUSImThDI8IVsAI9DERF4cwvSqPAEAyjsihHuPCQlZAbvu33iMC2iXvpTswZC3RBNDyKm1YEWDnnKeCn2Qw=w2400"></center>
 *
 * The toplevel role is part of the [XDG
 * Shell](https://wayland.app/protocols/xdg-shell#xdg_toplevel) protocol.
 *
 * @note The Wayland protocol also provides its own toplevel role but it is
 * considered obsolete and therefore not included in Louvre.
 *
 * @section toplevel-configuration Configuration
 *
 * When a client wants to map an LToplevelRole surface, it triggers a
 * configureRequest(), where the compositor is expected to define the size,
 * state, bounds, and other properties of the toplevel using different configure
 * methods, such as configureSize(), configureState(), configureBounds(), etc.
 *
 * The configuration is not applied immediately, the compositor must wait for
 * the client to ACK the changes through the atomsChanged() event which is also
 * triggered whenever one or more properties of the atoms() struct change,
 * allowing to access previous values as well.
 *
 * @note The compositor can configure any parameters at any time, but should
 * always wait for the atomsChanged() event to detect when they are actually
 * applied.
 *
 * @section toplevel-state-change-requests State Change Requests
 *
 * Depending on the flags set with configureCapabilities(), clients can request
 * to minimize or change the state() of the toplevel to @ref Maximized or @ref
 * Fullscreen through setMinimizedRequest(), set / unsetMaximizedRequest() and
 * set / unsetFullscreenRequest(). The compositor is then expected to minimize
 * or properly configure the state and size, but it's free to ignore the
 * requests if desired.
 *
 * @note Minimized is not a toplevel @ref State and can be applied immediately.
 *
 * @section foreign-controller Foreign Toplevel Controllers and XDG Activation
 * Tokens
 *
 * Requests, including those previously mentioned as well as
 * `unsetMinimizedRequest()`, `activateRequest()`, and `closeRequest()`, can be
 * triggered by external clients through the [Wlr Foreign Toplevel
 * Management](https://wayland.app/protocols/wlr-foreign-toplevel-management-unstable-v1)
 * and the [XDG Activation](https://wayland.app/protocols/xdg-activation-v1)
 * protocols. For a comprehensive understanding of this functionality, please
 * refer to the LForeignToplevelController and LActivationTokenManager classes
 * documentation.
 *
 * @section toplevel-sessions Interactive Sessions
 *
 * Clients can request to start interactive move and resize sessions through
 * startMoveRequest() and startResizeRequest(). To properly handle such
 * requests, each toplevel has an auxiliary moveSession() and resizeSession()
 * which can handle the request for you and constrain the toplevel to the
 * available geometry of outputs (LOutput::availableGeometry()) or to custom
 * constraints specified by you. These are simply auxiliary and you are free to
 * not use them if desired.
 *
 * @section toplevel-decoration Decorations
 *
 * By default, all clients draw their own decorations on the toplevel surface,
 * such as shadows, and the windowGeometry() property indicates which part of
 * the surface are decorations and which are the actual window content.
 *
 * Some clients also supportServerSideDecorations() (SSD). To enable SSD, the
 * compositor must send a configureDecorationMode() with the @ref ServerSide
 * value.\n When the decorationMode() changes to @ref ServerSide, the client
 * does not draw any decorations, and the windowGeometry() equals the entire
 * surface size.\n If the compositor draws additional elements that are meant to
 * be part of the geometry (such as a title bar), the setExtraGeometry() method
 * can be employed, which allows for proper positioning of the toplevel and
 * constraining it during interactive sessions.
 *
 * @section toplevel-position Positioning
 *
 * The toplevel position can be set with LSurface::setPos(). The default
 * implementation of rolePos() then returns the given position minus the
 * decoration part of windowGeometry() and adds the top and left margins of
 * extraGeometry().
 */
class Louvre::LToplevelRole : public LBaseSurfaceRole {
 public:
  ...
  static constexpr LFactoryObject::Type FactoryObjectType =
      LFactoryObject::Type::LToplevelRole;
  ...
  /**
   * @brief Flags indicating which atomic properties have changed during an
   * atomsChanged() event.
   *
   * @see @ref Atoms
   * @see atoms()
  ...
  enum Capabilities : UInt8 {
    ...
    /// When set, the toplevel can trigger setMaximizedRequest() and
    /// unsetMaximizedRequest().
    MaximizeCap = static_cast<UInt8>(1) << 1,

    /// When set, the toplevel can trigger setFullscreenRequest() and
  ...
  struct Atoms {
    ...
    LBitset<Capabilities> capabilities{WindowMenuCap | MaximizeCap |
                                       FullscreenCap | MinimizeCap};
   ...
   *
   * @warning The `surface()` handle always remains valid during the destructor
   * call. However, `LSurface::role()` returns `nullptr` because
   * `LSurface::roleChanged()` is notified beforehand and requires the role to
   * be valid.
   */
  ~LToplevelRole();
   ...
   *
   * The pending serial is updated at most once per loop iteration if a
   * configure was done.
   *
   ...
   *
   * Asks the client to configure the toplevel with the given states and stores
   * the flags in pendingConfiguration().\n The state is not applied
   * immediately, see atomsChanged() and state().
   *
   * @note A client may not support all states, in such cases unsupported states
   * are filtered out. See supportedStates().
   *
   ...
   *
   * Asks the client to configure the toplevel with the given size and stores it
   * in pendingConfiguration().\n The size is not applied immediately, see
   * atomsChanged() and windowGeometry().
   *
   * @param size The size is in surface coordinates and does not include
   * decorations.\n If either width or height is 0, the client is free to pick
   * its own size.
   */
   ...
   *
   * Asks the client to configure the toplevel with the given decoration mode
   * and stores it in pendingConfiguration().\n The decoration mode is not
   * applied immediately, see atomsChanged() and decorationMode().
   *
   * @note The decoration mode is @ref ClientSide by default. If a client
   * doesn't support @ref ServerSide decorations, the configuration mode will
   * fallback to @ref ClientSide. See supportServerSideDecorations().
   *
   ...
   *
   * Informs the toplevel which @ref Capabilities are supported by the
   * compositor and stores them in pendingConfiguration().\n For example, if the
   * @ref MaximizeCap is not available, clients should hide the maximize button
   * in their decorations. Additionally, requests such as setMaximizedRequest()
   * and unsetMaximizedRequest() will not be triggered.
   *
   * The capabilities are not applied immediately, see atomsChanged() and
   * capabilities().
   *
   * All capabilities are enabled by default.
   *
   * @note Some poorly behaved clients, such as Firefox, always expect the
   * compositor to obey their requests and freeze when it does not. In such
   * cases, it is better to leave @ref MaximizeCap and @ref FullscreenCap
   * enabled.
   *
  ...
  void configureCapabilities(LBitset<Capabilities> caps) noexcept {
    ...
    m_pendingConfiguration.capabilities =
        caps & (WindowMenuCap | MaximizeCap | FullscreenCap | MinimizeCap);
  }
   ...
   * larger than the given bounds, for example, to prevent exceeding the
   * available geometry of an output.
   *
   * The bounds are stored in pendingConfiguration() and not applied
   * immediately, see atomsChanged() and bounds().
   *
   * @note This is merely a suggestion, clients are free to ignore it.
   *
   * @param bounds The suggested maximum size. Setting a component to 0 disables
   * the constraint for that component.
   */
   ...
   *
   * This struct contains all the current toplevel atomic properties, which are
   * updated each time atomsChanged() is triggered.
   *
   * The current properties can also be accessed via aliases such as
   * windowGeometry(), state(), bounds(), etc.
   */
   ...
   *
   * The state flags can also be checked via aliases such as activated(),
   ...
   */
  LBitset<State> state() const noexcept { return atoms().state; }

  /**
   * @brief Returns the states supported by the toplevel.
   *
   * - @ref Activated, @ref Maximized , @ref Fullscreen , and @ref Resizing are
   * always supported.
   ...
   */
  LBitset<State> supportedStates() const noexcept { return m_supportedStates; }
   ...
   */
  bool activated() const noexcept { return state().check(Activated); }
   ...
   * @return `true` if the flag is present, `false` otherwise.
   */
  bool maximized() const noexcept { return state().check(Maximized); }
   ...
   * @return `true` if the flag is present, `false` otherwise.
   */
  bool fullscreen() const noexcept { return state().check(Fullscreen); }
   ...
   * @return `true` if the flag is present, `false` otherwise.
   */
  bool resizing() const noexcept { return state().check(Resizing); }
   ...
   * @return `true` if the flag is present, `false` otherwise.
   */
  bool suspended() const noexcept { return state().check(Suspended); }

  /**
   * @brief Window geometry in surface coordinates
   *
   * <center><img height="300px"
   * src="https://lh3.googleusercontent.com/pw/AIL4fc_le5DeTa6b-yBnChX6YPbkr12gAp38ghVyvsv4SjHCd2L4fTL8agYls0AcGlBeplJyc0FNQCIeb6sR4WbSUyAHM4_LrKLNjhZ0SniRdaSUsjS9IGQ=w2400"></center>
   *
   * The window geometry is a rect within the toplevel's surface that excludes
   * its decorations (typically shadows).
   *
   ...
   *
   * This optional method allows you to modify the `extraGeometry()` property,
   * providing additional margins for each toplevel edge to be considered part
   * of the geometry. For example, when using server-side decorations, the
   * compositor could draw a custom title bar or borders that are not part of
   * the client surfaces. This method allows you to define those sizes and
   * avoids the need to override `rolePos()`, the move and resizing constraints,
   * and other default implementations such as LOutput::geometryChanged().
   *
   * @note This does not affect the `windowGeometry()` property and the
   * functioning of `configureSize()`.
   *
   * @see extraGeometry()
   *
   ...
   */
  const LMargins &extraGeometry() const noexcept { return m_extraGeometry; }
   ...
   */
  const LSize &minSize() const noexcept { return atoms().minSize; }
   ...
   * Components with a value of 0 indicate the limit is disabled.
   */
  const LSize &maxSize() const noexcept { return atoms().maxSize; }

  /**
   * @brief Check if the provided size falls within the range defined by
   * minSize() and maxSize().
   *
   * @param size The size to be checked.
   * @return `true` if the size is within the specified range, `false`
   * otherwise.
   */
   ...
   *
   * Returns the left, top, right, and bottom constraints in global-compositor
   * coordinates during a moveSession() and resizeSession() so that the toplevel
   * stays within the given LOutput::availableGeometry(). See
   * LToplevelMoveSession::setConstraints() and
   * LToplevelResizeSession::setConstraints()
   *
   * @param includeExtraGeometry If `true`, extraGeometry() is considered.
   */
  LMargins calculateConstraintsFromOutput(
      LOutput *output, bool includeExtraGeometry = true) const noexcept;
   ...
   */
  const LSize &bounds() const noexcept { return atoms().bounds; }

  /**
   * @brief Current capabilities.
   *
   * Flags representing the @ref Capabilities supported by the compositor, as
   * notified to the client via configureCapabilities().
   *
   ...
   * in which `LOutput` a toplevel is positioned while maximized or in
   * fullscreen mode.
   *
   * The default implementation of `LOutput::paintGL()` uses this information to
   * prevent displaying the given toplevel on other outputs.
   *
   * @see exclusiveOutput()
   *
   ...
   *
   * Returns the exclusive output hint set with setExclusiveOutput(), or
   * `nullptr` if unset.
   */
  ...
  /**
   *  @brief
   * [xdg_toplevel](https://wayland.app/protocols/xdg-shell#xdg_toplevel)
   * resource from the [XDG Shell](https://wayland.app/protocols/xdg-shell)
   * protocol.
   */
  Protocols::XdgShell::RXdgToplevel *xdgToplevelResource() const;

  /**
   *  @brief [xdg_surface](https://wayland.app/protocols/xdg-shell#xdg_surface)
   * resource from the [XDG Shell](https://wayland.app/protocols/xdg-shell)
   * protocol.
   */
   ...
   * @see appIdChanged()
   *
   * @return A string containing the application ID (e.g.,
   * "org.cuarzosoftware.Desk").
   */
   ...
   *
   * Requests the client to close the toplevel (equivalent to pressing the close
   * button on the window).
   *
   * @note The client may choose to ignore this request, or show a dialog to ask
   * the user to save their data, etc.
   */
  void close() noexcept;
   ...
   */
  const std::vector<LForeignToplevelController *> &foreignControllers()
      const noexcept {
   ...
   *
   * If this method does not return `nullptr` during a state-changing request,
   * it means it is being triggered by the specified
   * LForeignToplevelController::client().
   *
   ...
   *
   * A vector containing all the handles used by foreign clients to identify
   * this toplevel.
   *
   * @see foreignHandleFilter() class for more details.
   */
  const std::vector<Protocols::ForeignToplevelList::RForeignToplevelHandle *>
  foreignHandles() const noexcept {
   ...
   *
   * This ID is used by foreign clients using the [Foreign Toplevel
   * List](https://wayland.app/protocols/ext-foreign-toplevel-list-v1#ext_foreign_toplevel_handle_v1:event:identifier)
   * protocol (see `foreignHandleFilter()` for details).
   *
   * The string is initially empty and is updated after the toplevel is mapped
   * or re-mapped.
   */
   ...
   *
   * Override this virtual method if you need to define your own logic for
   * positioning the toplevel.
   *
   * The default implementation returns the position set with LSurface::setPos()
   * minus the decoration part of windowGeometry() and adds the top and left
   * margins of extraGeometry().
   *
   ...
   *
   * This request is triggered each time the client intends to map the toplevel
   * surface.
   *
   * #### Default Implementation
   ...
   *
   * Triggered by a an LActivationTokenManager::token() or requesterController()
   * expecting the compositor to configure the toplevel with the @ref Activated
   * state.
   *
   * @see configureState(), requesterController() and
   ...
   *
   * Triggered by the client or a requesterController() expecting the compositor
   * to configure the toplevel with the @ref Maximized state. See
   * configureState().
   *
   * @note This event is only triggered if the @ref MaximizeCap is set. See
   ...
   *
   * Triggered by the client or a requesterController() expecting the compositor
   * to configure the toplevel without the @ref Maximized state. See
   * configureState().
   *
   * @note This event is only triggered if the @ref MaximizeCap is set. See
   * configureCapabilities().
   ...
   *
   * Triggered by the client or a requesterController() expecting the compositor
   * to configure the toplevel with the @ref Fullscreen state. See
   * configureState().
   *
   * @note This event is only triggered if the @ref FullscreenCap is set. See
   * configureCapabilities().
   * @note The compositor is free to ignore this request.
   *
   * @param destOutput Output on which the client wishes to display the
   * toplevel. If `nullptr` the compositor must choose the output.
   *
   * #### Default implementation
   ...
   *
   * Triggered by the client or a requesterController() expecting the compositor
   * to configure the toplevel without the @ref Fullscreen state. See
   * configureState().
   *
   * @note This event is only triggered if the @ref FullscreenCap is set. See
   * configureCapabilities().
   ...
   *
   * Triggered by the client or a requesterController() expecting the compositor
   * to minimize the toplevel window.
   *
   * @see LSurface::setMinimized(), requesterController() and
   * LForeignToplevelController.
   *
   * @note This event is only triggered if the @ref MinimizeCap is set. See
   * configureCapabilities().
   * @note The compositor is free to ignore this request.
   *
   * #### Default implementation
   ...
   *
   * Triggered by a requesterController() expecting the compositor to unminimize
   * the toplevel window.
   *
   * @see LSurface::setMinimized(), requesterController() and
   * LForeignToplevelController.
   ...
   * @brief Close request.
   *
   * Triggered by a requesterController() expecting the compositor to close()
   * the toplevel window.
   *
   ...
   *
   * This method allows you to filter which foreign clients can control this
   * toplevel. See LForeignToplevelController for more details.
   *
   * If accepted, a new LForeignToplevelController object will be added to the
   * foreignControllers() vector.
   *
   * @param manager The `GForeignToplevelManager` resource requesting to control
   * the toplevel.
   *
   * @return `true` to allow the foreign client to control the toplevel, `false`
   * to deny it.
   *
   * #### Default implementation
   * @snippet LToplevelRoleDefault.cpp foreignControllerFilter
   */
  virtual bool foreignControllerFilter(
      Protocols::ForeignToplevelManagement::GForeignToplevelManager *manager);

  /**
   * @brief Filter for foreign toplevel handle requests
   *
   * This method allows you to control which foreign clients can obtain a handle
   * for this toplevel.
   *
   * The handle enables clients to identify other clients' toplevels and, in
   * conjunction with other protocols, perform operations such as selecting
   * which toplevel to capture using protocols like Image Copy Capture.
   *
   * If accepted, a new `ForeignToplevelList::RForeignToplevelHandle` object
   * will be added to the foreignHandles() vector.\n Alternatively, you can
   * disable the protocol for specific clients using
   * LCompositor::globalsFilter().
   *
   * @note This interface is related to the [Foreign Toplevel
   * List](https://wayland.app/protocols/ext-foreign-toplevel-list-v1) protocol.
   * Do not confuse it with the [Wlr Foreign Toplevel
   * Management](https://wayland.app/protocols/wlr-foreign-toplevel-management-unstable-v1)
   * protocol (see foreignControllerFilter()).
   *
   * This protocol can be tested with clients like
   * [lswt](https://git.sr.ht/~leon_plickat/lswt).
   *
   * @param foreignList The `GForeignToplevelList` resource requesting to get a
   * handle.
   *
   * @return `true` to allow the foreign client to obtain a handle for this
   * toplevel, `false` to deny it.
   *
   * #### Default implementation
   * @snippet LToplevelRoleDefault.cpp foreignHandleFilter
   */
  virtual bool foreignHandleFilter(
      Protocols::ForeignToplevelList::GForeignToplevelList *foreignList);

  /**
   * @brief Show window menu request.
   *
   * Triggered by the client expecting the compositor to display a popup menu
   * with options for minimizing, maximizing and/or turning the toplevel into
   * fullscreen mode.
   *
   * @note This event is only triggered if the @ref WindowMenuCap is set. See
   * configureCapabilities().
   * @note The compositor is free to ignore this request.
   *
   * #### Default Implementation
   ...
   */
  virtual void showWindowMenuRequest(const LEvent &triggeringEvent, Int32 x,
                                     Int32 y);
 ...
 private:
  ...
  Atoms &currentAtoms() noexcept { return m_atoms[m_currentAtomsIndex]; }

  Atoms &pendingAtoms() noexcept { return m_atoms[1 - m_currentAtomsIndex]; }
  ...
  LBitset<State> m_supportedStates{Activated | Maximized | Fullscreen |
                                   Resizing};
  ...
  // This is for clients that do maximize or fullscreen requests before the
  // first conf
  ...
  LForeignToplevelController *m_requesterController{
      nullptr};  // Only set during requests

  /* Foreign toplevel list */
  std::string m_identifier;
  std::vector<Protocols::ForeignToplevelList::RForeignToplevelHandle *>
      m_foreignToplevelHandles;
};
