...
LToplevelResizeSession::LToplevelResizeSession(LToplevelRole *toplevel) noexcept
  ...
  m_ackTimer.setCallback([this](auto) { m_lastSerialHandled = true; });
}
...
void LToplevelResizeSession::handleGeometryChange() {
  if (!m_lastSerialHandled) {
    if (m_edge.check(LEdgeTop))
      m_toplevel->surface()->setY(
          m_initPos.y() + (m_initSize.h() - m_toplevel->windowGeometry().h()));

    if (m_edge.check(LEdgeLeft))
      m_toplevel->surface()->setX(
          m_initPos.x() + (m_initSize.w() - m_toplevel->windowGeometry().w()));

    if (!m_isActive &&
        (!m_toplevel->resizing() || m_lastSerial < m_toplevel->serial())) {
      m_ackTimer.cancel();
...
void LToplevelResizeSession::updateDragPoint(const LPoint &point) {
  ...
  LSize newSize = {
      calculateResizeSize(m_initDragPoint - point, m_initSize, m_edge)};
  ...
  if (newSize.w() < m_minSize.w()) newSize.setW(m_minSize.w());

  if (newSize.h() < m_minSize.h()) newSize.setH(m_minSize.h());

  toplevel()->configureSize(newSize);
  toplevel()->configureState(LToplevelRole::Activated |
                             LToplevelRole::Resizing);
  m_lastSerial = m_toplevel->pendingConfiguration().serial;
}

bool LToplevelResizeSession::start(const LEvent &triggeringEvent,
                                   LBitset<LEdge> edge,
  ...
  if (m_constraints.left != LEdgeDisabled &&
      m_toplevel->surface()->pos().x() < m_constraints.left)
    m_toplevel->surface()->setX(m_constraints.left);

  if (m_constraints.top != LEdgeDisabled &&
      m_toplevel->surface()->pos().y() < m_constraints.top)
    m_toplevel->surface()->setY(m_constraints.top);

  m_initPos = m_toplevel->surface()->pos();

  m_toplevel->configureState(m_toplevel->pendingConfiguration().state |
...
const std::vector<LToplevelResizeSession *>::const_iterator
LToplevelResizeSession::stop() {
  if (!m_isActive) return seat()->imp()->resizeSessions.begin();

  m_isActive = false;
  toplevel()->configureState(toplevel()->pendingConfiguration().state &
                             ~LToplevelRole::Resizing);
  m_lastSerialHandled = toplevel()->pendingConfiguration().size ==
                        toplevel()->m_lastACKConfiguration.size;

  if (!m_lastSerialHandled) m_ackTimer.start(600);

  auto it = std::find(seat()->imp()->resizeSessions.begin(),
                      seat()->imp()->resizeSessions.end(), this);
