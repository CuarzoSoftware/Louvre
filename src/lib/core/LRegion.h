 ...
 *
 * The LRegion class provides an efficient mechanism for creating sets of
 * rectangles that do not overlap in their geometries. It offers methods for
 * performing operations such as additions, subtractions, intersections, and
 * more on rectangles. This class is extensively used by the library for tasks
 * like calculating surface damage, defining opaque, translucent, and input
 * regions, among others. Internally, LRegion employs the algorithm and
 * functions from the [Pixman](http://www.pixman.org/) library.
 */
class Louvre::LRegion {
 public:
   ...
   */
  LRegion() noexcept { pixman_region32_init(&m_region); }

  /**
   * @brief Initialize the region with a rect.
   */
  LRegion(const LRect &rect) noexcept {
    pixman_region32_init_rect(&m_region, rect.x(), rect.y(), rect.w(),
                              rect.h());
  }
   ...
   */
  ~LRegion() noexcept { pixman_region32_fini(&m_region); }
   ...
   *
   * Constructs an LRegion by moving the contents from another LRegion, leaving
   * the source LRegion empty.\n The source LRegion remains valid and can be
   * used afterwards.
   *
  ...
  LRegion &operator=(const LRegion &other) noexcept {
    if (&other != this) pixman_region32_copy(&m_region, &other.m_region);
    return *this;
  }
   ...
   *
   * Assigns the contents of another LRegion to this LRegion by moving, leaving
   * the source LRegion empty.\n The source LRegion remains valid and can be
   * used afterwards.
   *
   * @param other The LRegion to move from.
   ...
   */
  void clear() noexcept { pixman_region32_clear(&m_region); }
  ...
  void addRect(const LRect &rect) noexcept {
    pixman_region32_union_rect(&m_region, &m_region, rect.x(), rect.y(),
                               rect.w(), rect.h());
  }
  ...
  void addRect(const LPoint &pos, const LSize &size) noexcept {
    pixman_region32_union_rect(&m_region, &m_region, pos.x(), pos.y(), size.w(),
                               size.h());
  }
  ...
  /**
   * @brief Adds the content of another LRegion to this LRegion (union
   * operation).
   *
  ...
  /**
   * @brief Multiplies the components of each rectangle in the LRegion by the
   * given factor.
   *
  ...
  bool containsPoint(const LPoint &point) const noexcept {
    return pixman_region32_contains_point(&m_region, point.x(), point.y(),
                                          NULL);
  }
  ...
  void inverse(const LRect &rect) noexcept {
    pixman_box32_t r{rect.x(), rect.y(), rect.x() + rect.w(),
                     rect.y() + rect.h()};
   ...
   */
  bool empty() const noexcept { return !pixman_region32_not_empty(&m_region); }
  ...
  void clip(const LRect &rect) noexcept {
    pixman_region32_intersect_rect(&m_region, &m_region, rect.x(), rect.y(),
                                   rect.w(), rect.h());
  }
  ...
  void clip(const LPoint &pos, const LSize &size) noexcept {
    pixman_region32_intersect_rect(&m_region, &m_region, pos.x(), pos.y(),
                                   size.w(), size.h());
  }
   ...
   *
   * This method returns a pointer to an LBox that represents the bounding box
   * of the LRegion. The LBox contains the minimum and maximum coordinates that
   * enclose the LRegion.
   *
   ...
   *
   * @param n A pointer to an integer that will be set to the number of
   * rectangles.
   * @return A pointer to an array of LBox objects representing the rectangles.
   *
   * @note This variant allows you to access the rectangles using the native
   * Pixman method, which is considerably faster.
   */
  ...
  /**
   * @brief Applies the specified transform to all rectangles within the given
   * size.
   *
   * This function clips the rectangles within the specified size and applies
   ...
   *
   * If the point is inside the region or the region is empty, the same point is
   * returned.
   *
  ...
  LPointF closestPointFrom(const LPointF &point,
                           Float32 padding = 0.f) const noexcept;

  /**
   * @brief Replace the rectangles with a single enclosing one if the number is
   * >= threshold.
   *
   * @param threshold The threshold value. If the number of rectangles is
   * greater than or equal to this value, the rectangles will be simplified to a
  ...
  void simplifyToExtents(Int32 threshold) noexcept {
    ...
    if (n >= threshold) {
      LBox ext{extents()};
      clear();
      addRect(ext.x1, ext.y1, ext.x2 - ext.x1, ext.y2 - ext.y1);
    }
  }
