...
LOutput::LOutput(const void *params) noexcept
    : LFactoryObject(FactoryObjectType),
      m_imp(std::make_unique<LOutputPrivate>(this)) {
  imp()->output = this;
  imp()->rect.setX(0);
  imp()->rect.setY(0);
  ...
  if (p->callback) p->callback(this);
}

LOutput::~LOutput() { notifyDestruction(); }
...
void LOutput::enableFractionalOversampling(bool enabled) noexcept {
  if (imp()->stateFlags.check(LOutputPrivate::FractionalOversamplingEnabled) !=
      enabled) {
    imp()->stateFlags.setFlag(LOutputPrivate::FractionalOversamplingEnabled,
                              enabled);

    if (usingFractionalScale()) repaint();
  }
}
...
LOutput::SubPixel LOutput::subPixel() const noexcept {
  return (LOutput::SubPixel)compositor()
      ->imp()
      ->graphicBackend->outputGetSubPixel((LOutput *)this);
}

bool LOutput::hasVSyncControlSupport() const noexcept {
  return compositor()->imp()->graphicBackend->outputHasVSyncControlSupport(
      (LOutput *)this);
}

bool LOutput::vSyncEnabled() const noexcept {
  return compositor()->imp()->graphicBackend->outputIsVSyncEnabled(
      (LOutput *)this);
}

bool LOutput::enableVSync(bool enabled) noexcept {
  return compositor()->imp()->graphicBackend->outputEnableVSync((LOutput *)this,
                                                                enabled);
}

Int32 LOutput::refreshRateLimit() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetRefreshRateLimit(
      (LOutput *)this);
}

void LOutput::setRefreshRateLimit(Int32 hz) noexcept {
  return compositor()->imp()->graphicBackend->outputSetRefreshRateLimit(
      (LOutput *)this, hz);
}

UInt32 LOutput::gammaSize() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetGammaSize(
      (LOutput *)this);
}

bool LOutput::setGamma(const LGammaTable *gamma) noexcept {
  if (gamma) {
    if (gamma->size() != gammaSize()) return false;

    if (imp()->gammaTable.m_gammaControlResource &&
        imp()->gammaTable.m_gammaControlResource !=
            gamma->m_gammaControlResource)
  ...
  return compositor()->imp()->graphicBackend->outputSetGamma((LOutput *)this,
                                                             imp()->gammaTable);
}

LFramebuffer *LOutput::framebuffer() const noexcept { return &imp()->fb; }
...
LTransform LOutput::transform() const noexcept { return imp()->transform; }

void LOutput::setTransform(LTransform transform) noexcept {
  if (transform == imp()->transform) return;
  ...
  for (auto *head : imp()->wlrOutputHeads) head->transform(transform);
}

UInt32 LOutput::currentBufferAge() const noexcept {
  if (needsFullRepaint()) return 0;

  return compositor()->imp()->graphicBackend->outputGetCurrentBufferAge(
      (LOutput *)this);
}
...
const LOutputMode *LOutput::preferredMode() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetPreferredMode(
      (LOutput *)this);
}

const LOutputMode *LOutput::currentMode() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetCurrentMode(
      (LOutput *)this);
}
...
void LOutput::setMode(const LOutputMode *mode) noexcept {
  ...
  for (LOutput *o : compositor()->outputs())
    if (o->threadId() == std::this_thread::get_id()) return;
...
Int32 LOutput::currentBuffer() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetCurrentBufferIndex(
      (LOutput *)this);
}

UInt32 LOutput::buffersCount() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetBuffersCount(
      (LOutput *)this);
}

LTexture *LOutput::bufferTexture(UInt32 bufferIndex) noexcept {
  return compositor()->imp()->graphicBackend->outputGetBuffer((LOutput *)this,
                                                              bufferIndex);
}

bool LOutput::hasBufferDamageSupport() const noexcept {
  return compositor()->imp()->graphicBackend->outputHasBufferDamageSupport(
      (LOutput *)this);
}
...
const LRegion &LOutput::bufferDamage() const noexcept { return imp()->damage; }

void LOutput::setScale(Float32 scale) noexcept {
  if (scale < 0.25f) scale = 0.25f;

  if (imp()->fractionalScale == scale) return;
  ...
  if (fmod(imp()->fractionalScale, 1.f) != 0.f) {
    ...
    fbSize = LSize(
        roundf(Float32(fbSize.w()) * imp()->scale / imp()->fractionalScale),
  ...
  if (!imp()->stateFlags.check(LOutputPrivate::IsBlittingFramebuffers)) {
    ...
    cursor()->imp()->textureChanged = true;
    repaint();
    for (LSurface *s : compositor()->surfaces()) s->imp()->sendPreferredScale();
  }

  for (auto *head : imp()->wlrOutputHeads) head->scale(imp()->fractionalScale);
}

Float32 LOutput::scale() const noexcept { return imp()->scale; }

void LOutput::repaint() noexcept {
  if (imp()->stateFlags.check(LOutputPrivate::IsInRepaintFilter)) return;

  compositor()->imp()->graphicBackend->outputLockCurrentBuffer(
      this, !imp()->repaintFilter());
...
Int32 LOutput::dpi() noexcept {
  if (physicalSize().area() == 0) return 0;
...
const LSize &LOutput::physicalSize() const noexcept {
  return *compositor()->imp()->graphicBackend->outputGetPhysicalSize(
      (LOutput *)this);
}

const LSize &LOutput::sizeB() const noexcept { return imp()->sizeB; }

const LSize &LOutput::realBufferSize() const noexcept {
  return usingFractionalScale() ? imp()->fractionalFb.sizeB()
                                : currentMode()->sizeB();
}

const std::vector<LScreenshotRequest *> &LOutput::screenshotRequests()
    const noexcept {
  return imp()->screenshotRequests;
}

LContentType LOutput::contentType() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetContentType(
      (LOutput *)this);
}
...
bool LOutput::isNonDesktop() const noexcept {
  return compositor()->imp()->graphicBackend->outputIsNonDesktop(
      (LOutput *)this);
}

void LOutput::setLeasable(bool leasable) noexcept {
  if (leasable == imp()->leasable) return;
  ...
  if (leasable) {
    for (LClient *client : compositor()->clients()) {
      for (auto *global : client->drmLeaseDeviceGlobals()) {
        if (global->gpu() == gpu()) {
          if (global->connector(this)) global->done();
          break;
        }
      }
    }
...
bool LOutput::leasable() noexcept { return imp()->leasable; }

DRMLease::RDRMLease *LOutput::lease() const noexcept { return imp()->lease; }

const LRect &LOutput::rect() const noexcept { return imp()->rect; }

const LPoint &LOutput::pos() const noexcept { return imp()->rect.pos(); }

const LSize &LOutput::size() const noexcept { return imp()->rect.size(); }

LOutput::State LOutput::state() const noexcept { return imp()->state; }
...
const char *LOutput::model() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetModelName(
      (LOutput *)this);
}

const char *LOutput::manufacturer() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetManufacturerName(
      (LOutput *)this);
}

const char *LOutput::description() const noexcept {
  return compositor()->imp()->graphicBackend->outputGetDescription(
      (LOutput *)this);
}
...
void LOutput::setPos(const LPoint &pos) noexcept {
  ...
  for (auto *head : imp()->wlrOutputHeads) head->position(pos);
}

LPainter *LOutput::painter() const noexcept { return imp()->painter; }
...
bool LOutput::setCustomScanoutBuffer(LTexture *texture) noexcept {
  if (!imp()->stateFlags.check(LOutputPrivate::IsInPaintGL)) {
    LLog::error(
        "Calling LOutput::setCustomScanoutBuffer() outside LOutput::paintGL() "
        "is not allowed.");
    return false;
  }
   ...
   * Scanning out surface textures from SHM buffers would require additional
   * buffering (extra CPU copies), which would actually be slower than simply
   * rendering them. User SHM buffers are allowed, though the user must take
   * care of not updating the pixels data while being scanned. */
  if (texture && texture->m_surface && texture->sourceType() != LTexture::DMA &&
      texture->sourceType() != LTexture::WL_DRM) {
    imp()->stateFlags.remove(LOutputPrivate::HasScanoutBuffer);
    return false;
  }

  const bool ret{compositor()->imp()->graphicBackend->outputSetScanoutBuffer(
      this, texture)};
