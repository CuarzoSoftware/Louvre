...
LSurface::LSurface(const void *params) noexcept
    : LFactoryObject(FactoryObjectType), LPRIVATE_INIT_UNIQUE(LSurface) {
  ...
  imp()->current.onBufferDestroyListener.notify = [](wl_listener *listener,
                                                     void *) {
    LSurfacePrivate::State *state{(LSurfacePrivate::State *)listener};
    state->bufferRes = nullptr;
  };
  imp()->pending.onBufferDestroyListener.notify =
      imp()->current.onBufferDestroyListener.notify;
...
LSurface::~LSurface() {
  ...
  if (imp()->texture && imp()->texture != imp()->textureBackup &&
      imp()->texture->m_pendingDelete)
    delete imp()->texture;
...
LBaseSurfaceRole *LSurface::role() const noexcept { return imp()->role; }

void LSurface::setPos(const LPoint &newPos) noexcept { imp()->pos = newPos; }
...
void LSurface::setX(Int32 x) noexcept { imp()->pos.setX(x); }

void LSurface::setY(Int32 y) noexcept { imp()->pos.setY(y); }

const LSize &LSurface::sizeB() const noexcept { return imp()->sizeB; }

const LSize &LSurface::size() const noexcept { return imp()->size; }
...
void LSurface::setMinimized(bool state) {
  if (state != minimized()) {
    ...
    for (LSurface *child : children()) child->setMinimized(state);
  }
}

void LSurface::repaintOutputs() noexcept {
  for (LOutput *o : outputs()) o->repaint();
}
...
void LSurface::enablePointerConstraint(bool enabled) {
  if (enabled && !hasPointerFocus()) return;
...
LSurfaceLayer LSurface::layer() const noexcept { return imp()->layer; }
...
const LPoint &LSurface::pos() const noexcept { return imp()->pos; }

const LPoint &LSurface::rolePos() const {
  if (role()) return role()->rolePos();

  return imp()->pos;
}

void LSurface::sendOutputEnterEvent(LOutput *output) noexcept {
  if (imp()->stateFlags.check(LSurfacePrivate::Destroyed)) return;
  ...
  for (GOutput *global : client()->outputGlobals())
    if (global->output() == output) surfaceResource()->enter(global);
...
void LSurface::sendOutputLeaveEvent(LOutput *output) noexcept {
  ...
  if (!output || (role() && role()->exclusiveOutput() == output &&
                  output->state() == LOutput::Initialized))
    return;
  ...
  for (std::size_t i = 0; i < imp()->outputs.size(); i++) {
    if (imp()->outputs[i] == output) {
      ...
      for (GOutput *global : client()->outputGlobals())
        if (global->output() == output) surfaceResource()->leave(global);
...
void LSurface::requestNextFrame(bool clearDamage) noexcept {
  ...
  for (auto *presentation : imp()->presentationFeedbackResources) {
    if (presentation->m_commitId >= 0 && !presentation->m_output) {
      if (presentation->m_commitId == imp()->commitId &&
          compositor()->imp()->currentOutput) {
  ...
  while (!imp()->frameCallbacks.empty()) {
    if (!imp()->frameCallbacks.front()->m_commited) break;
...
std::vector<IdleInhibit::RIdleInhibitor *> LSurface::idleInhibitorResources()
    const noexcept {
...
bool LSurface::hasBuffer() const noexcept { return imp()->current.hasBuffer; }

LClient *LSurface::client() const noexcept {
  return surfaceResource()->client();
}

LSurface *LSurface::parent() const noexcept { return imp()->parent; }

static LSurface *findTopmostParent(LSurface *surface) noexcept {
  if (surface->parent() == nullptr) return surface;
...
static bool isChildOfPopup(const LSurface *surface) noexcept {
  if (surface->parent()) {
    if (surface->parent()->popup()) return true;
...
bool LSurface::isPopupSubchild() const noexcept { return isChildOfPopup(this); }

static bool hasPopupChildren(const LSurface *surface) noexcept {
  for (LSurface *c : surface->children()) {
    if (c->popup()) return true;

    if (hasPopupChildren(c)) return true;
  }
...
void LSurface::raise() {
  if (compositor()->imp()->surfaceRaiseAllowedCounter > 0 ||
      imp()->stateFlags.check(LSurfacePrivate::Destroyed))
    return;
...
LSurface *LSurface::prevSurface() const noexcept {
  if (imp()->stateFlags.check(LSurfacePrivate::Destroyed) ||
      this == compositor()->surfaces().front())
    return nullptr;
  else
