 ...
 *
 * The LCompositor class manages the main event loop, backend systems, and
 * provides virtual constructors and destructors for Louvre objects with
...
class Louvre::LCompositor {
 public:
   ...
   *        Setting it to 0 disables the timeout, and setting it to -1 makes it
   ...
   * @param callback The callback function to handle events on the file
   * descriptor.
   * @param flags Flags to specify the type of event to listen for (e.g.,
   * WL_EVENT_READABLE).
   *
   * @return The wl_event_source associated with the added file descriptor.
   */
  [[deprecated(
      "This function will be removed in Louvre v3.0. Use "
      "LCompositor::eventLoop() and wl_event_loop_add_fd instead.")]]
  static wl_event_source *addFdListener(int fd, void *userData,
                                        int (*callback)(int, unsigned int,
                                                        void *),
                                        UInt32 flags = WL_EVENT_READABLE);

  /**
   * @brief Removes a previously added file descriptor from the compositor's
   * event loop.
   *
  ...
  [[deprecated(
      "This function will be removed in Louvre v3.0. Use "
      "wl_event_source_remove instead.")]]
  static void removeFdListener(wl_event_source *source);

  /**
   * @brief Gets the compositor cursor.
   *
   * This method must be accessed within or after the initialized() or
   * cursorInitialized() events. If the cursor has not yet been initialized,
   * this method returns `nullptr`.
   *
   * @return A pointer to the LCursor instance or `nullptr` if not yet
   * initialized.
   */
   ...
   *
   * The seat provides access to the LPointer, LKeyboard, LTouch, and LOutput
   * instances.
   *
   ...
   *
   * The session lock manager allows you to handle client requests to lock the
   * user session and display arbitrary content.
   */
   ...
   *
   * The activation token manager allows you to handle client requests to
   * activate surfaces of other clients.
   */
   ...
   *
   * This method returns a list of all surfaces created by clients, respecting
   * the stacking order of their roles protocols.\n To access surfaces from a
   * specific layer, used layer() instead.
   *
   ...
   * @return A reference to a list of pointers to LSurface objects within the
   * specified layer.
   */
   ...
   *
   * This method returns a list of clients that are currently connected to the
   * compositor.
   *
   ...
   *
   * @returns The LClient instance for a `wl_client` resource or `nullptr` if
   * not found.
   */
   ...
   *
   * This method returns a vector of all outputs that have been initialized
   * using the addOutput() method.
   *
   * @note This vector only contains initialized outputs. To get all available
   * outputs, use LSeat::outputs() instead.
   */
   ...
   *
   * This method creates a new rendering thread and initializes the specified
   * output. Once initialized, the LOutput::initializeGL() event is triggered,
   * and LOutput::repaint() can be used to unlock the rendering thread loop,
   * which triggers LOutput::paintGL() events.
   *
   ...
   *
   * This method uninitializes and removes the specified output from the
   * compositor, stopping its thread, rendering loop and triggering the
   * LOutput::uninitializeGL() event.
   *
   * @note Removing an output that has not been added to the compositor is a
   * no-op.
   *
   * @warning Calling this method from the same rendering thread of the output
   * (whithin any of its `...GL()` events) has no effect. Doing so would lead to
   * a deadlock, so Louvre simply ignores the request.
   *
   * @param output The output to remove, previously added to the compositor. See
   * outputs().
   */
   ...
   *
   * This method schedules a new rendering frame by calling the
   * LOutput::repaint() method to all initialized outputs.
   */
   ...
   *
   * Checks which output within the global compositor space intersects the given
   * rectangle the most.
   *
   * @param rect The rectangle to check in compositor-global coordinates.
   * @param initializedOnly If `true`, only considers initialized outputs.
   * Default is `true`.
   * @return A pointer to the most intersected LOutput, or `nullptr` if no
   * output is found.
   */
  LOutput *mostIntersectedOutput(const LRect &rect,
                                 bool initializedOnly = true) const noexcept;

  /**
   * @brief Identifier of the main thread.
   *
   * This ID corresponds to the primary thread responsible for managing the
   * Wayland and input backend event loops, while individual output operations
   * are performed on separate threads.
   *
   ...
   *
   * This path is automatically generated by Meson and should point to
   * `PREFIX/DATADIR/Louvre/assets`.
   *
   ...
   *
   * This path is automatically generated by Meson and should point to
   * `PREFIX/LIBDIR/Louvre/backends`.\n Used when the **LOUVRE_BACKENDS_PATH**
   ...
   * By default, Louvre tries to load the Wayland backend if **WAYLAND_DISPLAY**
   * is set, and then the DRM backend if fails.
   *
   * @note Instead of using this method, a more preferable approach is to let
   * the user choose the graphical backend by configuring the
   * **LOUVRE_BACKENDS_PATH** and **LOUVRE_GRAPHIC_BACKEND** environment
   * variables.
   *
   ...
   *
   * Use this method to determine whether the graphic backend has been
   * initialized after calling start().
   *
   ...
   *
   * This name is used when the **LOUVRE_GRAPHIC_BACKEND** environment variable
   ...
   * - In the case of the DRM backend, it returns a pointer to a
   * [SRMCore](https://cuarzosoftware.github.io/SRM/group___s_r_m_core.html)
   * struct.
   * - In the case of the Wayland backend, it returns a pointer to a
   ...
   *
   * Each graphic backend is assigned a unique @ref UInt32 ID. You can use this
   * method to retrieve the ID of the current graphic backend in use.
   *
   * The IDs of graphic backends shipped with Louvre are listed in the @ref
   * LGraphicBackendID enum.
   *
   ...
   * Use this method to load a custom graphic backend before calling start().\n
   * By default, Louvre tries to load the Wayland backend if **WAYLAND_DISPLAY**
   * is set, and then the Libinput backend if fails.
   *
   * @note Instead of using this method, a more preferable approach is to let
   * the user choose the graphical backend by configuring the
   * **LOUVRE_BACKENDS_PATH** and **LOUVRE_INPUT_BACKEND** environment
   * variables.
   *
   ...
   *
   * Use this method to determine whether the input backend has been initialized
   * after calling start().
   *
   ...
   *
   * This name is used when the **LOUVRE_INPUT_BACKEND** environment variable is
   ...
   * - In the case of the Libinput backend, it returns a pointer to a
   * [libinput](https://wayland.freedesktop.org/libinput/doc/latest/api/structlibinput.html)
   * struct.
   * - In the case of the Wayland backend, it returns a pointer to a
   * [wl_display](https://wayland.freedesktop.org/docs/html/apb.html#Client-classwl__display)
   * struct.
   ...
   *
   * Each input backend is assigned a unique Louvre::UInt32 ID. You can use this
   * method to retrieve the ID of the current input backend in use.
   *
   * The IDs of input backends shipped with Louvre are listed in the
   * Louvre::LInputBackendID enum.
   *
   ...
   *
   * This method creates and adds a global to the compositor. Globals
   * implemented by Louvre can be found in `<protocols/{protocol}/G{global}.h>`.
   *
   * Louvre automatically creates all the supported globals during compositor
   * initialization. See LCompositor::createGlobalsRequest().
   *
   * @tparam Global The type of global to create. It should be a subclass of
   * Louvre::LResource.
   * @param version The maximum supported interface version, constrained by
   * Louvre’s implemented version. If <= 0, Louvre’s highest supported version
   * is used instead.
   * @param data User data to assign to the `wl_resource`.
   * @return Pointer to the created LGlobal instance, which can be later removed
   * with removeGlobal().
   */
  ...
  LGlobal *createGlobal(Int32 version = 0, void *data = nullptr) noexcept {
    ...
    return globalCreate(Global::interface(),
                        version <= 0 ? Global::maxVersion()
                                     : std::min(version, Global::maxVersion()),
                        data, &Global::bind);
  }
   ...
   *
   * This method performs a lazy removal of an LGlobal created with
   * createGlobal(), ensuring clients are notified before it is destroyed.
   *
   * @note Calling LCompositor::removeGlobal() isn't mandatory as globals are
   * automatically destroyed during the compositor's uninitialization.
   *
   * @warning After calling this method, the LGlobal instance must no longer be
   * used as it is going to be destroyed.
   *
   ...
   *
   * This method registers POSIX signals for synchronous dispatching by the main
   * event loop, as explained in
   * [wl_event_loop_add_signal](https://wayland.freedesktop.org/docs/html/apc.html#Server-structwl__event__source_1a1706e2490502a95f24ccb59cbae3e2f8).
   *
   * All registered signals can be handled in onPosixSignal().
   *
   * Louvre automatically disables the signal in all rendering threads, but this
   * does not apply to user-created threads.
   *
   * @note If called from a rendering thread, the signal addition will be
   * scheduled for the next main loop iteration.
   *
   * @see removePosixSignal()
   * @see posixSignals()
   *
   * @return `true` if successfully added, `false` if the signal is already
   * registered or if attempted while the compositor is uninitialized.
   */
   ...
   *
   * Unregisters and unblocks a previously added POSIX signal, preventing it
   * from being handled by the main event loop.
   *
   * @note If called from a rendering thread, the signal removal will be
   * scheduled for the next main loop iteration.
   *
   * @note Signals are automatically removed after the compositor is
   * uninitialized.
   *
   * @return `true` if the signal was successfully removed, `false` if it was
   * not registered.
   */
   ...
   *
   * Use this event to handle the successful initialization of the compositor
   * after calling start().\n Here you should perform initial configuration
   * tasks, such as setting up outputs, as demonstrated in the default
   * implementation.
   *
   * @warning Avoid interacting with any LFactoryObjects (e.g., LPointer,
   * LKeyboard, LTouch, etc.) or creating LTextures before this event. These
   * objects or the input/graphic backends might be partially initialized, and
   * interacting with them could cause a segmentation fault.
   *
   ...
   *
   * This event is triggered right before the compositor is uninitialized when
   * finish() is called. At this point, both the input and graphic backends,
   * along with other resources such as connected clients and initialized
   * outputs, are still operational.\n Use this opportunity to release any
   * compositor-specific resources that you may have created.
   *
   * @par Default Implementation
   ...
   * Instances of these classes can only be created from this virtual
   * constructor when requested by the compositor.\n Refer to @ref
   * LFactoryObject::Type to see all the classes that can be overridden.
   *
   * @param objectType Indicates the type of the class instance to return.
   * @param params An opaque data type that should be passed to the object's
   * constructor.
   *
   * @return If `nullptr` is returned, Louvre will create an instance of the
   * object using the base class.
   *
   * @par Default Implementation
   ...
   *
   * When an LFactoryObject destructor is invoked, it is likely that associated
   * resources are no longer available.\n Using this event, however, provides an
   ...
   * @see removeGlobal()
   *
   * @return `true` on success, `false` on failure (prevents the compositor from
   * starting).
   *
   * @par Default Implementation
   ...
   * It is recommended to use this method to allow only well-known clients to
   * bind to protocols such as Session Lock, Wlr Screen Copy, and others.
   *
   ...
   *
   * Centralized function for managing all POSIX signals registered via
   * addPosixSignal().
   *
   * @par Default Implementation
  ...
  LGlobal *globalCreate(const wl_interface *interface, Int32 version,
