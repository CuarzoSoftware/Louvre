...
LActivationTokenManager::LActivationTokenManager(const void *params) noexcept
    : LFactoryObject(FactoryObjectType) {
  assert(params != nullptr &&
         "Invalid parameter passed to LActivationTokenManager constructor.");
  LActivationTokenManager **ptr{(LActivationTokenManager **)params};
  assert(*ptr == nullptr && *ptr == Louvre::activationTokenManager() &&
         "Only a single LActivationTokenManager instance can exist.");
  *ptr = this;
}
