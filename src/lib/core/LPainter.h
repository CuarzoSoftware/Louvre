 ...
 *
 * The LPainter class offers essential methods for 2D rendering without the need
 * to use OpenGL functions directly.\n It can draw solid colors and textures
 * with custom clipping, scaling, blending effects, and transforms.
 *
 * Each LOutput has its own LPainter, accessible via LOutput::painter().
 *
 * @note While LPainter provides convenience, it's not mandatory. You have the
 * flexibility to use OpenGL functions and your custom shaders if preferred, or
 * leverage the efficiency of rendering provided by the LScene and LView
 * classes.
 *
 * ## Coordinate System
 *
 * When specifying the destination rectangle for your painting operations, use
 * compositor-global coordinates, which operate with surface units.\n LPainter
 * automatically scales and transforms the content for you, considering the
 * scale factor/transform of the texture you are drawing and the scale
 * factor/transform of the destination framebuffer.
 *
 * The coordinate space in which the content is rendered aligns with that used
 * to arrange outputs.\n For example, to paint something in the upper-left
 * corner of an LOutput, consider the LOutput::pos().
 *
 * @note When rendering into an LRenderBuffer, also consider its position,
 * similar to how you handle outputs.
 */
class Louvre::LPainter final : LObject {
 public:
   ...
   *
   * This struct provides all the necessary parameters to map a texture into the
  ...
  struct TextureParams {
    ...
    /**
     * @brief Position of the texture (destination rect) in compositor-global
     * coordinates.
     */
    LPoint pos;
     ...
     *
     * Coordinates should be specified in the space generated after applying the
     * scale factor and transformation to the texture buffer.
     */
    LRectF srcRect;
     ...
     *
     * For example, if the texture is rotated 90 degrees counterclockwise and
     * you want to display it in a normal orientation, use LTransform::Rotated90
     * and LPainter will apply the inverse transform (LTransform::Rotated270).\n
   ...
   *
   * This method maps a texture to the compositor-global space, enabling
   * subsequent drawing of specific sub-rectangles using drawBox(), drawRect()
   * or drawRegion().
   *
   ...
   *
   * In color mode, drawBox(), drawRect() and drawRegion() can be used to draw
   * rectagles of the specified color.
   *
  ...
  /**
   * @brief Draws a texture or color box on the screen based on the current
   * rendering mode.
   *
  ...
  /**
   * @brief Draws a texture or color rect on the screen based on the current
   * rendering mode.
   *
  ...
  /**
   * @brief Draws a texture or color region on the screen based on the current
   * rendering mode.
   *
   ...
   *
   * When enabled, the bound texture RGB values are replaced by the color set
   * with setColor().
   */
   ...
   *
   * When enabled, LPainter will automatically select the appropriate OpenGL
   * blend function mode based on whether the bound texture uses premultiplied
   * alpha or not (see LTexture::premultipliedAlpha()).
   *
   * When disabled, LPainter will use the blend function set with
   * setBlendFunc().
   *
   * Enabled by default.
   */
   ...
   *
   * Setting a custom blend function can be useful for masking or other
   * sophisticated blending effects.
   *
   * To make LPainter use this blend function, the auto blend function must be
   * disabled, see enableAutoBlendFunc().
   */
   ...
   *
   * - In texture mode and if customTextureColorEnabled() is enabled, this value
   * replaces the texture RGB values while keeping the alpha intact.
   ...
   *
   * This method multiplies each component of the source color by the specified
   * factor.
   *
   ...
   *
   * This method clears the bound framebuffer using the color set with
   * setClearColor().
   */
   ...
   *
   * @note Do not use this method unless you are working with your own
   * shaders/programs.
   */
   ...
   *
   * This method binds the provided framebuffer for rendering, allowing
   * subsequent rendering operations to be performed on it.
   *
   * @note Output framebuffers are automatically bound prior a
   ...
   *
   * This method returns a pointer to the currently bound framebuffer for
   * rendering.
   *
   ...
   *
   * @note This method should be used if you are working with your own OpenGL
   * programs and want to use the LPainter methods again.
   */
