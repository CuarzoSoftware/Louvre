...
LClient::LClient(const void *params) noexcept
    : LFactoryObject(FactoryObjectType),
      m_imp{
          std::make_unique<LClientPrivate>(this, ((Params *)params)->client)} {
...
const LEvent *LClient::findEventBySerial(UInt32 serial) const noexcept {
  ...
  for (auto it = imp()->eventHistory.touch.down.begin();
       it != imp()->eventHistory.touch.down.end(); it++)
    if (it->serial() == serial) return &(*it);

  for (auto it = imp()->eventHistory.touch.up.begin();
       it != imp()->eventHistory.touch.up.end(); it++)
    if (it->serial() == serial) return &(*it);
...
void LClient::postErrorPrivate(wl_resource *resource, UInt32 code,
                               const std::string &message) {
  if (imp()->destroyed) return;
...
bool LClient::ping(UInt32 serial) const noexcept {
  if (imp()->xdgWmBaseGlobals.empty()) return false;

  imp()->xdgWmBaseGlobals.front()->ping(serial);
  return true;
}

wl_client *LClient::client() const noexcept { return imp()->client; }

void LClient::flush() noexcept { wl_client_flush(client()); }

void LClient::destroyLater() noexcept {
  if (imp()->pendingDestroyLater) return;
...
const std::vector<Wayland::GCompositor *> &LClient::compositorGlobals()
    const noexcept {
  return imp()->compositorGlobals;
}

const std::vector<Wayland::GSubcompositor *> &LClient::subcompositorGlobals()
    const noexcept {
...
const std::vector<Wayland::GDataDeviceManager *> &
LClient::dataDeviceManagerGlobals() const noexcept {
  return imp()->dataDeviceManagerGlobals;
}

const std::vector<XdgShell::GXdgWmBase *> &LClient::xdgWmBaseGlobals()
    const noexcept {
  return imp()->xdgWmBaseGlobals;
}

const std::vector<PresentationTime::GPresentation *> &
LClient::presentationTimeGlobals() const noexcept {
  return imp()->presentationTimeGlobals;
}

const std::vector<LinuxDMABuf::GLinuxDMABuf *> &LClient::linuxDMABufGlobals()
    const noexcept {
  return imp()->linuxDMABufGlobals;
}

const std::vector<FractionalScale::GFractionalScaleManager *> &
LClient::fractionalScaleManagerGlobals() const noexcept {
  return imp()->fractionalScaleManagerGlobals;
}

const std::vector<GammaControl::GGammaControlManager *> &
LClient::gammaControlManagerGlobals() const noexcept {
  return imp()->gammaControlManagerGlobals;
}

const std::vector<TearingControl::GTearingControlManager *> &
LClient::tearingControlManagerGlobals() const noexcept {
  return imp()->tearingControlManagerGlobals;
}

const std::vector<Viewporter::GViewporter *> &LClient::viewporterGlobals()
    const noexcept {
  return imp()->viewporterGlobals;
}

const std::vector<XdgDecoration::GXdgDecorationManager *> &
LClient::xdgDecorationManagerGlobals() const noexcept {
  return imp()->xdgDecorationManagerGlobals;
}

const std::vector<RelativePointer::GRelativePointerManager *> &
LClient::relativePointerManagerGlobals() const noexcept {
  return imp()->relativePointerManagerGlobals;
}

const std::vector<PointerGestures::GPointerGestures *> &
LClient::pointerGesturesGlobals() const noexcept {
  return imp()->pointerGesturesGlobals;
}

const std::vector<SessionLock::GSessionLockManager *> &
LClient::sessionLockManagerGlobals() const noexcept {
  return imp()->sessionLockManagerGlobals;
}

const std::vector<PointerConstraints::GPointerConstraints *> &
LClient::pointerConstraintsGlobals() const noexcept {
  return imp()->pointerConstraintsGlobals;
}

const std::vector<XdgOutput::GXdgOutputManager *> &
LClient::xdgOutputManagerGlobals() const noexcept {
  return imp()->xdgOutputManagerGlobals;
}

const std::vector<ScreenCopy::GScreenCopyManager *> &
LClient::screenCopyManagerGlobals() const noexcept {
  return imp()->screenCopyManagerGlobals;
}

const std::vector<ImageCaptureSource::GOutputImageCaptureSourceManager *> &
LClient::outputImageCaptureSourceManagerGlobals() const noexcept {
  return imp()->outputImageCaptureSourceManagerGlobals;
}

const std::vector<
    ImageCaptureSource::GForeignToplevelImageCaptureSourceManager *> &
LClient::foreignToplevelImageCaptureSourceManagerGlobals() const noexcept {
  return imp()->foreignToplevelImageCaptureSourceManagerGlobals;
}

const std::vector<LayerShell::GLayerShell *> &LClient::layerShellGlobals()
    const noexcept {
  return imp()->layerShellGlobals;
}

const std::vector<ForeignToplevelManagement::GForeignToplevelManager *> &
LClient::foreignToplevelManagerGlobals() const noexcept {
  return imp()->foreignToplevelManagerGlobals;
}

const std::vector<ForeignToplevelList::GForeignToplevelList *> &
LClient::foreignToplevelListGlobals() const noexcept {
  return imp()->foreignToplevelListGlobals;
}

const std::vector<SinglePixelBuffer::GSinglePixelBufferManager *> &
LClient::singlePixelBufferManagerGlobals() const noexcept {
  return imp()->singlePixelBufferManagerGlobals;
}

const std::vector<ContentType::GContentTypeManager *> &
LClient::contentTypeManagerGlobals() const noexcept {
  return imp()->contentTypeManagerGlobals;
}

const std::vector<IdleNotify::GIdleNotifier *> &LClient::idleNotifierGlobals()
    const noexcept {
  return imp()->idleNotifierGlobals;
}

const std::vector<IdleInhibit::GIdleInhibitManager *> &
LClient::idleInhibitManagerGlobals() const noexcept {
  return imp()->idleInhibitManagerGlobals;
}

const std::vector<XdgActivation::GXdgActivation *> &
LClient::xdgActivationGlobals() const noexcept {
  return imp()->xdgActivationGlobals;
}

const std::vector<DRMLease::GDRMLeaseDevice *> &LClient::drmLeaseDeviceGlobals()
    const noexcept {
  return imp()->drmLeaseDeviceGlobals;
}

const std::vector<WlrOutputManagement::GWlrOutputManager *> &
LClient::wlrOutputManagerGlobals() const noexcept {
  return imp()->wlrOutputManagerGlobals;
}

const std::vector<BackgroundBlur::GBackgroundBlurManager *> &
LClient::backgroundBlurManagerGlobals() const noexcept {
  return imp()->backgroundBlurManagerGlobals;
}

const std::vector<SvgPath::GSvgPathManager *> &LClient::svgPathManagerGlobals()
    const noexcept {
  return imp()->svgPathManagerGlobals;
}

const std::vector<InvisibleRegion::GInvisibleRegionManager *> &
LClient::invisibleRegionManagerGlobals() const noexcept {
