...
const LPoint &LLayerRole::rolePos() const {
  ...
  if (!exclusiveOutput()) return surface()->pos();
  ...
  if (anchor() == (LEdgeTop | LEdgeLeft)) {
    ...
    m_rolePos = exclusiveOutput()->pos() + exclusiveZone().rect().pos() +
                (exclusiveZone().rect().size() - surface()->size()) / 2;

  // Add extra margins.
  if (anchor().check(LEdgeTop)) m_rolePos.setY(m_rolePos.y() + margins().top);
  if (anchor().check(LEdgeBottom))
    m_rolePos.setY(m_rolePos.y() - margins().bottom);
  if (anchor().check(LEdgeLeft)) m_rolePos.setX(m_rolePos.x() + margins().left);
...
//! [atomsChanged]
void LLayerRole::atomsChanged(LBitset<AtomChanges> changes,
  ...
  if (changes.check(KeyboardInteractivityChanged)) {
    switch (keyboardInteractivity()) {
      case KeyboardInteractivity::NoInteractivity:
        ...
        if (surface()->hasKeyboardGrab()) seat()->keyboard()->setGrab(nullptr);
        break;
  ...
  if (changes.check(MarginsChanged)) surface()->requestNextFrame(false);

  if (exclusiveOutput() &&
      changes.check(ExclusiveEdgeChanged | ExclusiveZoneSizeChanged |
                    SizeChanged | AnchorChanged | LayerChanged |
                    MarginsChanged))
...
void LLayerRole::configureRequest() {
  ...
  if (!exclusiveOutput()) setExclusiveOutput(cursor()->output());
   ...
   * It can also return 0 if the anchor() flags do not contain any edges or if
   * they contain all edges.
   */
  const LBitset<LEdge> edge{exclusiveZone().edge()};

  /*
   * If either the width or height of size() is 0, it indicates that the client
   * expects the compositor to assign it. In such cases, or if the surface is
   * anchored to both opposite edges, all edges or no edge, we assign it the
   * entire available space of its exclusive zone. The client can choose a
   * different size if desired. For more details see the LExclusiveZone doc.
   */

  if (newSize.w() == 0 || edge == 0 ||
      anchor().checkAll(LEdgeLeft | LEdgeRight))
    newSize.setW(exclusiveZone().rect().w());

  if (newSize.h() == 0 || edge == 0 ||
      anchor().checkAll(LEdgeTop | LEdgeBottom))
