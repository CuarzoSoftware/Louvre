...
void LPointer::pointerMoveEvent(const LPointerMoveEvent &event) {
  ...
  if (focus()) {
    ...
    // Attempt to enable the pointer constraint mode if the cursor is within the
    // constrained region.
    if (focus()->pointerConstraintMode() !=
        LSurface::PointerConstraintMode::Free) {
      if (focus()->pointerConstraintRegion().containsPoint(cursor()->pos() -
                                                           fpos))
      ...
      if (focus()->pointerConstraintMode() ==
        ...
        if (focus()->lockedPointerPosHint().x() >= 0.f)
          ...
          const LPointF closestPoint{
              focus()->pointerConstraintRegion().closestPointFrom(
                  cursor()->pos() - fpos)};
  ...
  // Schedule repaint on outputs that intersect with the cursor where hardware
  // composition is not supported.
  cursor()->repaintOutputs(true);

  const bool sessionLocked{compositor()->sessionLockManager()->state() !=
  ...
  if (activeDND) {
    if (seat()->dnd()->icon()) {
      ...
      cursor()->setCursor(
          seat()->dnd()->icon()->surface()->client()->lastCursorRequest());
    }
...
void LPointer::pointerButtonEvent(const LPointerButtonEvent &event) {
  ...
  if (event.state() == LPointerButtonEvent::Pressed) {
    ...
    if (focus()->toplevel() && !focus()->toplevel()->activated())
      focus()->toplevel()->configureState(
          focus()->toplevel()->pendingConfiguration().state |
          LToplevelRole::Activated);

    if (!focus()->popup() && !focus()->isPopupSubchild())
      seat()->dismissPopups();

    if (!focus() || focus() == compositor()->surfaces().back()) return;
  ...
  // Left button released
    ...
    // We stop sending events to the surface on which the left button was being
    // held down
    setDraggingSurface(nullptr);

    if (!focus()->pointerConstraintEnabled() &&
        !focus()->inputRegion().containsPoint(cursor()->pos() -
                                              focus()->rolePos())) {
      setFocus(nullptr);
      cursor()->useDefault();
      cursor()->setVisible(true);
...
void LPointer::setCursorRequest(const LClientCursor &clientCursor) {
  /* During a non-touch drag & drop session, the source client typically updates
   * the cursor to reflect the DND action (e.g., copy, move, not supported,
   * etc.)
   */
  ...
  /* Allow the client to set the cursor only if one of its surfaces has pointer
   * focus */
...
//! [focusChanged]
void LPointer::focusChanged() { /* No default implementation. */ }
//! [focusChanged]
