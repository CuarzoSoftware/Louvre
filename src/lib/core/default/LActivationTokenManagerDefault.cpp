  ...
  if (token()->triggeringEvent()) {
    switch (token()->triggeringEvent()->type()) {
      case LEvent::Type::Pointer:
        allowed = seat()->pointer()->focus() &&
                  seat()->pointer()->focus()->client() == token()->creator();
        break;
      case LEvent::Type::Keyboard:
        allowed = seat()->keyboard()->focus() &&
                  seat()->keyboard()->focus()->client() == token()->creator();
        break;
      case LEvent::Type::Touch:
        const auto &touchPoints{seat()->touch()->touchPoints()};
        allowed = std::any_of(
            touchPoints.begin(), touchPoints.end(), [this](LTouchPoint *tp) {
              return tp->surface() &&
                     tp->surface()->client() == token()->creator();
            });
        break;
    }
  }

  if (!allowed) token()->destroy();
...
void LActivationTokenManager::activateSurfaceRequest(LSurface *surface) {
  ...
  if (token()->creator() && token()->triggeringEvent() &&
