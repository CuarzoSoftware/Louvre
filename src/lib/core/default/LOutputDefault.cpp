...
//! [initializeGL]
void LOutput::initializeGL() { painter()->setClearColor(1.f, 1.f, 1.f, 1.f); }
//! [initializeGL]
...
void LOutput::paintGL() {
  ...
  const LRect availGeo{pos() + availableGeometry().pos(),
  ...
  static const auto surfaceShouldBeSkipped = [](LSurface *s, LOutput *output,
                                                bool sessionLocked) -> bool {
    // An unmapped surface usually indicates it doesn't have a buffer
    if (!s->mapped()) return true;
    ...
    // Layer or screen lock surfaces should be displayed only on their exclusive
    // output or not at all
    if ((s->layerRole() && s->layerRole()->exclusiveOutput() != output) ||
        (s->sessionLock() && s->sessionLock()->exclusiveOutput() != output))
      return true;

    // Other roles with a different exclusive output, e.g. maximized or
    // fullscreen toplevels
    if (s->role() && s->role()->exclusiveOutput() &&
        s->role()->exclusiveOutput() != output)
      return true;

    // If the session is locked only surfaces from the locking client should be
    // displayed
  ...
  static const auto surfaceShouldBeClippedToAvailableGeometry =
      [](LSurface *s) -> bool {
    ...
    if (s->toplevel() && !s->toplevel()->fullscreen()) return true;
    ...
    if (s->subsurface() || s->toplevel()) {
      ...
      if (topmostParent && topmostParent->toplevel() &&
          !topmostParent->toplevel()->fullscreen())
        return true;
    }
  ...
  for (LSurface *s : compositor()->surfaces()) {
    if (surfaceShouldBeSkipped(s, this, sessionLocked)) continue;
    ...
    if (surfaceShouldBeClippedToAvailableGeometry(s))
      ...
      if (currentRect.clip(availGeo)) continue;
...
//! [resizeGL]
void LOutput::resizeGL() { repaint(); }
//! [resizeGL]

//! [moveGL]
void LOutput::moveGL() { repaint(); }
//! [moveGL]

//! [uninitializeGL]
void LOutput::uninitializeGL() { /* No default implementation */ }
//! [uninitializeGL]
...
void LOutput::availableGeometryChanged() {
  ...
  for (LSurface *surface : compositor()->surfaces()) {
    ...
    if (tl && !tl->fullscreen()) {
      const LSize toplevelExtraSize{
          tl->extraGeometry().left + tl->extraGeometry().right,
          tl->extraGeometry().top + tl->extraGeometry().bottom};
      LRect toplevelRect{surface->pos(),
                         tl->windowGeometry().size() + toplevelExtraSize};

      if (compositor()->mostIntersectedOutput(toplevelRect) != this) continue;

      if (tl->maximized()) {
        surface->setPos(availGeo.pos());
        tl->configureSize(availGeo.size() - toplevelExtraSize);
      } else {
        if (exclusiveEdges().right != 0 &&
            toplevelRect.x() + toplevelRect.w() > availGeo.x() + availGeo.w())
          toplevelRect.setX(availGeo.x() + availGeo.w() - toplevelRect.w());

        if (exclusiveEdges().bottom != 0 &&
            toplevelRect.y() + toplevelRect.h() > availGeo.y() + availGeo.h())
...
//! [repaintFilter]
bool LOutput::repaintFilter() { return true; }
//! [repaintFilter]
