...
const LPoint &LPopupRole::rolePos() const {
  ...
  if (!surface()->parent()) return m_rolePos;

  m_rolePos =
      surface()->parent()->rolePos() + localPos() - windowGeometry().topLeft();
...
void LPopupRole::grabKeyboardRequest(const LEvent &triggeringEvent) {
  ...
  if (triggeringEvent.type() == LEvent::Type::Pointer) {
    ...
    if (!touchPoint->surface()) return;

    if (touchPoint->surface()->client() != surface()->client()) return;
  }
...
//! [atomsChanged]
void LPopupRole::atomsChanged(LBitset<AtomChanges> changes,
...
void LPopupRole::configureRequest() {
  // Ensure the Popup stays within the boundaries of the current output where
