...
void LToplevelRole::startMoveRequest(const LEvent &triggeringEvent) {
  ...
  if (triggeringEvent.type() == LEvent::Type::Touch) {
    if (triggeringEvent.subtype() != LEvent::Subtype::Down) return;

    if (!activeOutput) return;

    const LTouchDownEvent &touchDownEvent{
        static_cast<const LTouchDownEvent &>(triggeringEvent)};
    LTouchPoint *touchPoint{
        seat()->touch()->findTouchPoint(touchDownEvent.id())};

    if (!touchPoint) return;

    if (touchPoint->surface() != surface()) return;

    const LPoint initDragPoint{
        LTouch::toGlobal(activeOutput, touchPoint->pos())};

    moveSession().start(triggeringEvent, initDragPoint);
  } else if (seat()->pointer()->focus() &&
             (surface()->hasPointerFocus() ||
              (seat()->pointer()->focus()->subsurface() &&
               seat()->pointer()->focus()->isSubchildOf(surface())))) {
    if (triggeringEvent.type() == LEvent::Type::Pointer) {
      if (triggeringEvent.subtype() != LEvent::Subtype::Button) return;

      const LPointerButtonEvent &pointerButtonEvent{
          static_cast<const LPointerButtonEvent &>(triggeringEvent)};
...
//! [startResizeRequest]
void LToplevelRole::startResizeRequest(const LEvent &triggeringEvent,
                                       LBitset<LEdge> edge) {
  if (fullscreen()) return;
...
//! [atomsChanged]
void LToplevelRole::atomsChanged(LBitset<AtomChanges> changes,
                                 const Atoms &prevAtoms) {
  surface()->repaintOutputs();

  if (!changes.check(StateChanged)) return;
...
//! [titleChanged]
void LToplevelRole::titleChanged() { /* No default implementation */ }
//! [titleChanged]

//! [appIdChanged]
void LToplevelRole::appIdChanged() { /* No default implementation */ }
//! [appIdChanged]
...
void LToplevelRole::setMaximizedRequest() {
  ...
  if (!output || maximized()) return;

  if (!fullscreen())
    prevRect = LRect(surface()->pos(), windowGeometry().size());

  const LSize extraGeoSize{extraGeometry().left + extraGeometry().right,
                           extraGeometry().top + extraGeometry().bottom};
...
void LToplevelRole::setFullscreenRequest(LOutput *preferredOutput) {
  LOutput *output{preferredOutput != nullptr ? preferredOutput
                                             : cursor()->output()};

  if (!output || fullscreen()) return;

  if (!maximized()) prevRect = LRect(surface()->pos(), windowGeometry().size());
...
void LToplevelRole::setMinimizedRequest() {
  ...
  if (surface()->hasPointerFocus()) seat()->pointer()->setFocus(nullptr);

  if (surface()->hasKeyboardFocus()) seat()->keyboard()->setFocus(nullptr);
...
void LToplevelRole::activateRequest() {
   ...
   * LActivationTokenManager::activateSurfaceRequest() */
...
bool LToplevelRole::foreignControllerFilter(
...
//! [foreignHandleFilter]
bool LToplevelRole::foreignHandleFilter(
...
void LToplevelRole::showWindowMenuRequest(const LEvent &triggeringEvent,
                                          Int32 x, Int32 y) {
