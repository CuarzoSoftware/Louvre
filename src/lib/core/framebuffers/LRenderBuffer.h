 ...
 *
 * The LRenderBuffer provides a means to render to a framebuffer instead of an
 * LOutput, allowing the resulting content to be used as a texture.\n It can be
 * employed with an LPainter via LPainter::bindFramebuffer().
 *
 * @note The framebuffer possesses properties such as position, size, and scale
 * factor. These properties are utilized by LPainter to appropriately position
 * and scale the rendered content.
 */
class Louvre::LRenderBuffer final : public LFramebuffer {
 public:
   ...
   *
   * This constructor creates an LRenderBuffer with the specified size in buffer
   * coordinates.
   *
   ...
   *
   * This method returns the OpenGL ID of the framebuffer associated with the
   * LRenderBuffer.
   *
   ...
   *
   * LRenderBuffers always have a single framebuffer, so 0 must always be passed
   * as an argument. The index is part of the LFramebuffer parent class, which
   * may be used by special framebuffers like those of LOutput.
   *
   ...
   *
   * This method should be called after rendering is complete on the
   * renderbuffer.
   *
   * It ensures that all rendering commands are finished before the texture is
   * used elsewhere, preventing potential data inconsistencies or rendering
   * artifacts.
   */
   ...
   *
   * @warning The existing framebuffer is destroyed and replaced by a new one
   * each time this method is called, so it should be used with moderation.
   *
   ...
   *
   * This method returns the size of the framebuffer in surface coordinates.
   * This is sizeB() / scale().
   *
   * @return The size of the framebuffer.
   */
  const LSize &size() const noexcept { return m_rect.size(); }
   ...
   */
  void setPos(const LPoint &pos) noexcept { m_rect.setPos(pos); }
   ...
   */
  const LPoint &pos() const noexcept { return m_rect.pos(); }

  /**
   * @brief Retrieve the position and size of the framebuffer in surface
   * coordinates.
   *
   ...
   *
   * For example, framebuffers used in HiDPI displays should have a scale of 2
   * or greater.
   *
   * @param scale The buffer scale factor.
   */
  void setScale(Float32 scale) noexcept {
    if (scale < 0.25f) scale = 0.25;
