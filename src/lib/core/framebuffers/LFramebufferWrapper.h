 ...
 *
 * This class facilitates the usage of an existing native OpenGL framebuffer
 * with LPainter.\n It requires providing the framebuffer ID, its buffer size,
 * scaling factor and position.
 *
 * @note Native OpenGL framebuffers are not shared across threads, and thus
 * cannot be utilized from all LOutputs.\n If a framebuffer is needed for use
 * from any thread, consider using LRenderBuffer instead.
 */
class Louvre::LFramebufferWrapper final : public LFramebuffer {
 public:
   ...
   * @param pos The position of the framebuffer in the global
   * compositor-coordinates space.
   * @param scale The scaling factor of the framebuffer (default is 1.0).
   */
  LFramebufferWrapper(GLuint id, const LSize &sizeB,
                      const LPoint &pos = LPoint(0, 0),
                      Float32 scale = 1.f) noexcept
      : LFramebuffer(Wrapper), m_sizeB(sizeB), m_fbId(id), m_scale(scale) {
    m_rect.setPos(pos);
    updateDimensions();
  }
   ...
   */
  void setId(GLuint id) noexcept { m_fbId = id; }
  ...
  /**
   * @brief Sets the position of the framebuffer in the global
   * compositor-coordinates space.
   *
 ...
 private:
  void updateDimensions() noexcept {
    if (m_sizeB.w() < 0) m_sizeB.setW(0);

    if (m_sizeB.h() < 0) m_sizeB.setH(0);

    if (m_scale < 0.25f) m_scale = 0.25f;
