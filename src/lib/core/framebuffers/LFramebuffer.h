 ...
 *
 * The LFramebuffer abstract class defines an interface for creating
 * framebuffers or groups of framebuffers compatible with LPainter.
 *
...
class Louvre::LFramebuffer : public LObject {
 public:
  ...
  /**
   * @brief Gets the scale by which the framebuffer dimensions must be
   * interpreted.
   *
   * This method must return the scale factor used to interpret the dimensions
   * of the framebuffer.\n For example, HiDPI pixmaps/displays may have a scale
   * of 2, whereas low DPI pixmaps/displays may have a scale of 1.
   *
  ...
  /**
   * @brief Gets the position and size of the framebuffer in surface
   * coordinates.
   *
   * This method must return the position and size of the framebuffer in surface
   * coordinates.
   *
   * @returns The rect representing the position and size of the framebuffer in
   * surface coordinates.
   */
   ...
   *
   * This method must return the current OpenGL framebuffer ID associated with
   * the instance.
   *
   ...
   *
   * This method is used in implementations like LOutputFramebuffer, which may
   * contain more than one internal framebuffer associated with an LFramebuffer,
   * for example, when double or triple buffering is used.
   *
   ...
   *
   * This method must return the index of the current internal framebuffer
   * (where the rendered content will be stored).
   *
   ...
   *
   * This method must return the OpenGL texture ID associated with the specified
   * framebuffer index or 0 if not available.
   *
   * @param index The index of the framebuffer.
   * @returns The OpenGL texture ID of the specified framebuffer index or 0 if
   * not available.
   */
   ...
   *
   * This method sets the region in surface coordinates that indicates the parts
   * of the framebuffer that have changed since the last painting was performed.
   *
   * @param damage A pointer to the LRegion object representing the changed
   * region.
   */
