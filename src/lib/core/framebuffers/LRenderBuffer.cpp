...
LRenderBuffer::LRenderBuffer(const LSize &sizeB) noexcept
    : LFramebuffer(RenderBuffer) {
...
void LRenderBuffer::setSizeB(const LSize &sizeB) noexcept {
  const LSize newSize{sizeB.w() <= 0 ? 1 : sizeB.w(),
                      sizeB.h() <= 0 ? 1 : sizeB.h()};
...
void LRenderBuffer::setFence() noexcept { m_texture.setFence(); }
...
Float32 LRenderBuffer::scale() const noexcept { return m_scale; }

const LSize &LRenderBuffer::sizeB() const noexcept { return m_texture.sizeB(); }

const LRect &LRenderBuffer::rect() const noexcept { return m_rect; }
  ...
  if (!data.framebufferId) {
    ...
    if (!m_texture.initialized()) {
      ...
      LTexture::LTexturePrivate::setTextureParams(
          tex, GL_TEXTURE_2D, GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_texture.sizeB().w(),
                   m_texture.sizeB().h(), 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
      m_texture.m_sourceType = LTexture::GL;
      m_texture.setDataFromGL(tex, GL_TEXTURE_2D, DRM_FORMAT_ABGR8888,
                              m_texture.sizeB(), true);
      m_texture.m_sourceType = LTexture::Framebuffer;
    }

    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                           m_texture.id(nullptr), 0);
  }

  return data.framebufferId;
}

Int32 LRenderBuffer::buffersCount() const noexcept { return 1; }

Int32 LRenderBuffer::currentBufferIndex() const noexcept { return 0; }
