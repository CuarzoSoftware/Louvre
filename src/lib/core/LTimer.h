 ...
 *
 * This class provides the capability to create and manage timers, allowing you
 * to schedule time intervals in milliseconds and execute a specified callback
 * function when the timer expires. It supports both re-usable timers and
 * one-shot timers that can be automatically destroyed after timeout or
 * cancellation.
 */
class Louvre::LTimer : public LObject {
 public:
  ...
  /**
   * @brief Construct a re-usable LTimer instance with the provided timeout
   * callback.
   * @param onTimeout The callback function to be executed when the timer
   * expires.
   */
   ...
   *
   * If the timer is destroyed while it is running, the associated callback
   * function is not invoked.
   */
  ~LTimer();

  /**
   * @brief Create a one-shot timer that is automatically started and destroyed
   ...
   */
  inline UInt32 interval() const noexcept { return m_interval; }
   ...
   *
   * If the timer was created with oneShot() it is destroyed and the onTimeout
   ...
   *
   * If the timer was created with oneShot() it is destroyed after the callback
   * is triggered.
   */
  void stop() noexcept;
   ...
   *
   * @note Calling start() on an already started timer will restart the timer
   * without calling the callback. If this method is called within the timeout
   * callback the timer is restarted even if it was created using oneShot().
   *
   * @param intervalMs The interval for the timer in milliseconds.
   *
