...
void LRegion::multiply(Float32 factor) noexcept {
  ...
  if (factor == 0.5f) {
    for (int i = 0; i < n; i++) {
      pixman_region32_union_rect(&tmp, &tmp, rects->x1 >> 1, rects->y1 >> 1,
                                 (rects->x2 - rects->x1) >> 1,
                                 (rects->y2 - rects->y1) >> 1);
      rects++;
    }
    ...
    for (int i = 0; i < n; i++) {
      pixman_region32_union_rect(&tmp, &tmp, floorf(float(rects->x1) * factor),
...
void LRegion::multiply(Float32 xFactor, Float32 yFactor) noexcept {
  if (xFactor == 1.f && yFactor == 1.f) return;
  ...
  for (int i = 0; i < n; i++) {
    pixman_region32_union_rect(&tmp, &tmp, floor(float(rects->x1) * xFactor),
...
void LRegion::transform(const LSize &size, LTransform transform) noexcept {
  pixman_region32_intersect_rect(&m_region, &m_region, 0, 0, size.w(),
                                 size.h());
  ...
  switch (transform) {
    case LTransform::Normal: return;
    case LTransform::Flipped270:
      pixman_region32_init(&tmp);
      boxes = (LBox *)pixman_region32_rectangles(&m_region, &n);
      for (i = 0; i < n; i++) {
        pixman_region32_union_rect(&tmp, &tmp, size.h() - boxes->y2,
                                   size.w() - boxes->x2, boxes->y2 - boxes->y1,
                                   boxes->x2 - boxes->x1);
        boxes++;
      }
      break;
    case LTransform::Flipped90:
      ...
      boxes = (LBox *)pixman_region32_rectangles(&m_region, &n);
      for (i = 0; i < n; i++) {
        pixman_region32_union_rect(&tmp, &tmp, boxes->y1, boxes->x1,
                                   boxes->y2 - boxes->y1,
                                   boxes->x2 - boxes->x1);
        boxes++;
      }
    ...
    case LTransform::Flipped180:
      ...
      boxes = (LBox *)pixman_region32_rectangles(&m_region, &n);
      for (i = 0; i < n; i++) {
        pixman_region32_union_rect(&tmp, &tmp, boxes->x1, size.h() - boxes->y2,
                                   boxes->x2 - boxes->x1,
                                   boxes->y2 - boxes->y1);
        boxes++;
      }
    ...
    case LTransform::Rotated180:
      ...
      boxes = (LBox *)pixman_region32_rectangles(&m_region, &n);
      for (i = 0; i < n; i++) {
        pixman_region32_union_rect(&tmp, &tmp, size.w() - boxes->x2,
                                   size.h() - boxes->y2, boxes->x2 - boxes->x1,
                                   boxes->y2 - boxes->y1);
        boxes++;
      }
      break;
    case LTransform::Flipped:
      ...
      boxes = (LBox *)pixman_region32_rectangles(&m_region, &n);
      for (i = 0; i < n; i++) {
        pixman_region32_union_rect(&tmp, &tmp, size.w() - boxes->x2, boxes->y1,
                                   boxes->x2 - boxes->x1,
                                   boxes->y2 - boxes->y1);
        boxes++;
      }
    ...
    case LTransform::Rotated90:
      ...
      boxes = (LBox *)pixman_region32_rectangles(&m_region, &n);
      for (i = 0; i < n; i++) {
        pixman_region32_union_rect(&tmp, &tmp, boxes->y1, size.w() - boxes->x2,
                                   boxes->y2 - boxes->y1,
                                   boxes->x2 - boxes->x1);
        boxes++;
      }
    ...
    case LTransform::Rotated270:
      ...
      boxes = (LBox *)pixman_region32_rectangles(&m_region, &n);
      for (i = 0; i < n; i++) {
        pixman_region32_union_rect(&tmp, &tmp, size.h() - boxes->y2, boxes->x1,
                                   boxes->y2 - boxes->y1,
                                   boxes->x2 - boxes->x1);
        boxes++;
      }
...
LPointF LRegion::closestPointFrom(const LPointF &point,
                                  Float32 margin) const noexcept {
...
void LRegion::multiply(LRegion *dst, LRegion *src, Float32 factor) noexcept {
  ...
  if (factor == 0.5f) {
    for (int i = 0; i < n; i++) {
      pixman_region32_union_rect(&dst->m_region, &dst->m_region, rects->x1 >> 1,
                                 rects->y1 >> 1, (rects->x2 - rects->x1) >> 1,
                                 (rects->y2 - rects->y1) >> 1);
      rects++;
    }
  } else if (factor == 2.f) {
    ...
    for (int i = 0; i < n; i++) {
      pixman_region32_union_rect(&dst->m_region, &dst->m_region,
