...
static Int32 daemonLoop() {
  ...
  if (setpgid(0, 0) == 0) daemonGID = getpgrp();
...
pid_t LLauncher::startDaemon(const std::string &name) {
  ...
  if (daemonPID != -1) {
    LLog::error(
        "[LLauncher::startDaemon] Failed to start daemon, already running.");
    goto error;
  }

  if (compositor()) {
    LLog::error(
        "[LLauncher::startDaemon] Failed to start daemon. Must be launched "
        "before an LCompositor instance is created.");
    goto error;
  }

  if (pipe(pipeA) != 0) {
    LLog::error(
        "[LLauncher::startDaemon] Failed to start daemon. Failed to create "
        "pipe: %s.",
        strerror(errno));
    goto error;
  }
  ...
  // Child
    ...
    for (Int32 x = sysconf(_SC_OPEN_MAX); x >= 0; x--)
      if (x != pipeA[0] && x != pipeB[1] && x != STDOUT_FILENO &&
          x != STDERR_FILENO)
        close(x);

    Int32 nullFD = open("/dev/null", O_RDONLY);

    if (nullFD != -1) dup2(nullFD, STDIN_FILENO);
    ...
    if (daemonGID != -1) kill(-daemonGID, SIGTERM);
    ...
    close(pipeB[1]);
    LLog::debug(
        "[LLauncher::startDaemon] LLauncher daemon started successfully with "
        "PID: %d.",
        daemonPID);
    return daemonPID;
  }
...
pid_t LLauncher::pid() { return daemonPID; }

pid_t LLauncher::launch(const std::string &command) {
  if (daemonPID < 0) {
    LLog::error("[LLauncher::launch] Can not launch %s. Daemon is not running.",
                command.c_str());
    return -1;
  }

  if (command.empty()) {
    LLog::error("[LLauncher::launch] Can not launch %s. Invalid command.",
                command.c_str());
    return -1;
  }
  ...
  while (true) {
    ...
    if (c == '\0') {
      ...
      if (pid > 0)
        LLog::debug(
            "[LLauncher::launch] Command %s executed successfuly. PID: %d.",
            command.c_str(), pid);
      else
        LLog::error("[LLauncher::launch] Command %s failed. PID: %d.",
                    command.c_str(), pid);
...
stop:
  LLog::error("[LLauncher::launch] Command %s failed. Daemon died.",
              command.c_str());
  stopDaemon();
  return -1;
}
