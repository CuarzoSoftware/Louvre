...
LTexture::LTexture(bool premultipliedAlpha) noexcept
...
UInt32 LTexture::formatBytesPerPixel(UInt32 format) noexcept {
  switch (format) {
    ...
    case DRM_FORMAT_BGR233: return 1; break;
    ...
    case DRM_FORMAT_BGR565: return 2; break;
    case DRM_FORMAT_RGB888:
    case DRM_FORMAT_BGR888: return 3; break;
    ...
    case DRM_FORMAT_BGRA8888: return 4; break;
    default: return 0;
  }
}
...
UInt32 LTexture::formatPlanes(UInt32 format) noexcept {
  switch (format) {
    ...
    case DRM_FORMAT_YVU444: return 3; break;
    ...
    case DRM_FORMAT_P016: return 2; break;
    default: return 1;
  }
}
...
bool LTexture::setDataFromMainMemory(const LSize &size, UInt32 stride,
                                     UInt32 format,
                                     const void *buffer) noexcept {
  if (m_sourceType == Framebuffer) return false;

  reset();

  if (compositor()->imp()->graphicBackend->textureCreateFromCPUBuffer(
          this, size, stride, format, buffer)) {
    m_format = format;
    m_sizeB = size;
    m_sourceType = CPU;
    return true;
...
bool LTexture::setDataFromWaylandDRM(wl_resource *buffer) noexcept {
  ...
  if (compositor()->imp()->graphicBackend->textureCreateFromWaylandDRM(
          this, buffer)) {
    m_sourceType = WL_DRM;
    return true;
  }
...
bool LTexture::setDataFromDMA(const LDMAPlanes &planes) noexcept {
  ...
  if (compositor()->imp()->graphicBackend->textureCreateFromDMA(this,
                                                                &planes)) {
    m_sourceType = DMA;
    return true;
  }
...
bool LTexture::setDataFromGL(GLuint id, GLenum target, UInt32 format,
                             const LSize &size,
  ...
  if (compositor()->imp()->graphicBackend->textureCreateFromGL(
          this, id, target, format, size, transferOwnership)) {
    m_sourceType = GL;
    m_format = format;
    m_sizeB = size;
    return true;
...
bool LTexture::updateRect(const LRect &rect, UInt32 stride,
                          const void *buffer) noexcept {
  if (initialized() && m_sourceType != Framebuffer) {
    m_serial++;
    return compositor()->imp()->graphicBackend->textureUpdateRect(this, stride,
                                                                  rect, buffer);
  }
...
bool LTexture::writeUpdate(const LRect &rect, UInt32 stride,
                           const void *buffer) noexcept {
  return compositor()->imp()->graphicBackend->textureWriteUpdate(this, stride,
                                                                 rect, buffer);
}
...
LTexture *LTexture::copy(const LSize &dst, const LRect &src,
                         bool highQualityScaling) const noexcept {
  if (!initialized()) return nullptr;

  if (dst.w() < 0 || dst.h() < 0) {
    LLog::error(
        "[LTexture::copyB] Failed to copy texture. Invalid destination size.");
    return nullptr;
  }
  ...
  if (srcRect.w() == 0 || srcRect.h() == 0 || dstSize.w() == 0 ||
      dstSize.h() == 0) {
  ...
  if (highQualityScaling) {
    ...
    if (wScale > limit) wScale = limit;

    if (hScale > limit) hScale = limit;
    ...
    LTexture::LTexturePrivate::setTextureParams(
        texCopy, GL_TEXTURE_2D, GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, dstSize.w(), dstSize.h(), 0,
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
      ...
      LLog::error(
          "[LTexture::copyB] glCheckFramebufferStatus failed. Skipping "
          "highQualityScaling.");
      goto skipHQ;
    }
    ...
    glUniform2f(painter->imp()->currentUniformsScaler->texSize, sizeB().w(),
                sizeB().h());
    glUniform4f(painter->imp()->currentUniformsScaler->srcRect, srcRect.x(),
                srcRect.y() + srcRect.h(), srcRect.w(), -srcRect.h());

    Float32 tmp;
    Float32 x1 = (Float32(srcRect.x()) + 0.f * Float32(srcRect.w())) /
                 Float32(sizeB().w());
    Float32 y1 = (Float32(srcRect.y() + srcRect.h()) + Float32(-srcRect.h()) -
                  0.f * Float32(-srcRect.h())) /
                 Float32(sizeB().h());
    Float32 x2 = (Float32(srcRect.x()) + 1.f * Float32(srcRect.w())) /
                 Float32(sizeB().w());
    Float32 y2 = (Float32(srcRect.y() + srcRect.h()) + Float32(-srcRect.h()) -
                  1.f * Float32(-srcRect.h())) /
                 Float32(sizeB().h());
    ...
    glUniform4f(painter->imp()->currentUniformsScaler->samplerBounds, x1, y1,
                x2, y2);
    LTexture::LTexturePrivate::setTextureParams(
        textureId, textureTarget, GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR);
    glUniform2f(painter->imp()->currentUniformsScaler->pixelSize, pixSizeW,
                pixSizeH);
    ...
    ret = textureCopy->setDataFromGL(texCopy, GL_TEXTURE_2D,
                                     DRM_FORMAT_ABGR8888, dstSize, true);
    glDeleteFramebuffers(1, &framebuffer);
    glUseProgram(prevProgram);
    ...
    LLog::error(
        "[LTexture::copyB] Failed to create texture. Graphic backend error.");
    delete textureCopy;
    return nullptr;
  }
...

{
  ...
  if (textureTarget == GL_TEXTURE_2D && dstSize.w() == srcRect.w() &&
      dstSize.h() == srcRect.h() && srcRect.x() >= 0 &&
      srcRect.x() + srcRect.w() <= sizeB().w() && srcRect.y() >= 0 &&
    ...
    LTexture::LTexturePrivate::setTextureParams(
        texCopy, GL_TEXTURE_2D, GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR);
    glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, srcRect.x(), srcRect.y(),
  ...
  // Scaled draw to new texture fb
    ...
    painter->bindTextureMode({.texture = (LTexture *)this,
                              .pos = LPoint(0, 0),
                              .srcRect = srcRect,
                              .dstSize = dstSize,
...
skipAll:
  ...
  LLog::error(
      "[LTexture::copyB] Failed to create texture. Graphica backend error.");
  if (textureCopy) delete textureCopy;
  return nullptr;
}
...
bool LTexture::save(const std::filesystem::path &name) const noexcept {
  ...
  /* First attempt to read directly from the texture using a framebuffer. */
  {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, textureTarget,
                           textureId, 0);

    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
      LLog::warning(
          "[LTexture::save] Failed to read texture directly using a "
          "framebuffer. Trying drawing the texture instead.");
      goto draw;
    }

    buffer = (UChar8 *)malloc(sizeB().w() * sizeB().h() * 4);
    LTexture::LTexturePrivate::readPixels(LRect(0, sizeB()), 0, sizeB().w(),
...
draw:
  ...
  /* If first attempt fails, then render texture into a render buffer and then
...
{
  const Int32 ret{stbi_write_png(name.c_str(), sizeB().w(), sizeB().h(), 4,
                                 buffer, sizeB().w() * 4)};
  free(buffer);
  glBindFramebuffer(GL_FRAMEBUFFER, 0);

  if (ret) {
    LLog::debug("[LTexture::save] Texture saved successfully: %s.",
                name.c_str());
    return true;
  }
...
printError:
  LLog::error("[LTexture::save] Failed to save texture: %s. %s.", name.c_str(),
              error);
  return false;
}
...
GLuint LTexture::id(LOutput *output) const noexcept {
  if (initialized())
    return compositor()->imp()->graphicBackend->textureGetID(output,
                                                             (LTexture *)this);
...
GLenum LTexture::backendTarget() const noexcept {
  return compositor()->imp()->graphicBackend->textureGetTarget(
      (LTexture *)this);
}
