...
LPainter::LPainter(LOutput *output) noexcept : LPRIVATE_INIT_UNIQUE(LPainter) {
  ...
  imp()->fragmentShader =
      LOpenGL::compileShader(GL_FRAGMENT_SHADER, fShaderStr);
  imp()->fragmentShaderExternal =
      LOpenGL::compileShader(GL_FRAGMENT_SHADER, fShaderStrExternal.c_str());
  imp()->fragmentShaderScaler =
      LOpenGL::compileShader(GL_FRAGMENT_SHADER, fShaderStrScaler);
  imp()->fragmentShaderScalerExternal = LOpenGL::compileShader(
      GL_FRAGMENT_SHADER, fShaderStrScalerExternal.c_str());
  ...
  glAttachShader(imp()->programObjectScalerExternal,
                 imp()->fragmentShaderScalerExternal);
  ...
  if (!linked) {
    GLint infoLen = 0;
    glGetProgramiv(imp()->programObjectScalerExternal, GL_INFO_LOG_LENGTH,
                   &infoLen);
    glDeleteProgram(imp()->programObjectScalerExternal);
    imp()->programObjectScalerExternal = 0;
    LLog::error(
        "[LPainter::LPainter] Failed to compile scaler shader external.");
  } else {
...
void LPainter::LPainterPrivate::setupProgram() noexcept {
  ...
  currentUniforms->texColorEnabled =
      glGetUniformLocation(currentProgram, "texColorEnabled");
  currentUniforms->colorFactorEnabled =
      glGetUniformLocation(currentProgram, "colorFactorEnabled");
  currentUniforms->alpha = glGetUniformLocation(currentProgram, "alpha");
  currentUniforms->premultipliedAlpha =
      glGetUniformLocation(currentProgram, "premultipliedAlpha");
...
void LPainter::LPainterPrivate::setupProgramScaler() noexcept {
  ...
  // Get Uniform Variables
  currentUniformsScaler->texSize =
      glGetUniformLocation(currentProgram, "texSize");
  currentUniformsScaler->srcRect =
      glGetUniformLocation(currentProgram, "srcRect");
  currentUniformsScaler->activeTexture =
      glGetUniformLocation(currentProgram, "tex");
  currentUniformsScaler->pixelSize =
      glGetUniformLocation(currentProgram, "pixelSize");
  currentUniformsScaler->samplerBounds =
      glGetUniformLocation(currentProgram, "samplerBounds");
...
void LPainter::LPainterPrivate::updateExtensions() noexcept {
  ...
  openGLExtensions.EXT_read_format_bgra =
      LOpenGL::hasExtension(exts, "GL_EXT_read_format_bgra");
  openGLExtensions.OES_EGL_image =
      LOpenGL::hasExtension(exts, "GL_OES_EGL_image");
}

void LPainter::LPainterPrivate::updateCPUFormats() noexcept {
  for (const LDMAFormat &fmt :
       *compositor()->imp()->graphicBackend->backendGetDMAFormats()) {
    if (fmt.modifier != DRM_FORMAT_MOD_LINEAR) continue;

    switch (fmt.format) {
      case DRM_FORMAT_ARGB8888: cpuFormats.ARGB8888 = true; break;
      case DRM_FORMAT_XRGB8888: cpuFormats.XRGB8888 = true; break;
      case DRM_FORMAT_ABGR8888: cpuFormats.ABGR8888 = true; break;
      case DRM_FORMAT_XBGR8888: cpuFormats.XBGR8888 = true; break;
      default: break;
    }
  }
}
...
void LPainter::bindTextureMode(const TextureParams &p) noexcept {
  ...
  if (!imp()->userState.texture.get() ||
      imp()->userState.texture.get()->premultipliedAlpha() !=
          p.texture->premultipliedAlpha())
    imp()->needsBlendFuncUpdate = true;
  ...
  LTransform invTrans =
      Louvre::requiredTransform(p.srcTransform, imp()->fb->transform());
  ...
  switch (invTrans) {
    case LTransform::Normal: break;
    case LTransform::Rotated90: xFlip = !xFlip; break;
    case LTransform::Rotated180:
      xFlip = !xFlip;
      yFlip = !yFlip;
      break;
    case LTransform::Rotated270: yFlip = !yFlip; break;
    case LTransform::Flipped: xFlip = !xFlip; break;
    case LTransform::Flipped90:
      xFlip = !xFlip;
      yFlip = !yFlip;
      break;
    case LTransform::Flipped180: yFlip = !yFlip; break;
    case LTransform::Flipped270: break;
    default: return;
  }
...
void LPainter::bindColorMode() noexcept {
  if (imp()->userState.mode == LPainterPrivate::ColorMode) return;
...
void LPainter::drawBox(const LBox &box) noexcept {
  if (imp()->needsBlendFuncUpdate) imp()->updateBlendingParams();
...
void LPainter::enableCustomTextureColor(bool enabled) noexcept {
  if (imp()->userState.customTextureColor == enabled) return;
...
void LPainter::enableAutoBlendFunc(bool enabled) const noexcept {
  if (imp()->userState.autoBlendFunc == enabled) return;
...
void LPainter::setAlpha(Float32 alpha) noexcept {
  if (imp()->userState.alpha == alpha) return;
...
void LPainter::setColor(const LRGBF &color) noexcept {
  if (imp()->userState.color == color) return;
...
LFramebuffer *LPainter::boundFramebuffer() const noexcept { return imp()->fb; }
...
void LPainter::setClearColor(Float32 r, Float32 g, Float32 b,
                             Float32 a) noexcept {
  glClearColor(r, g, b, a);
}

void LPainter::setColorFactor(Float32 r, Float32 g, Float32 b,
                              Float32 a) noexcept {
  ...
  imp()->shaderSetColorFactorEnabled(r != 1.f || g != 1.f || b != 1.f ||
                                     a != 1.f);
  imp()->needsBlendFuncUpdate = true;
}

void LPainter::setColorFactor(const LRGBAF &factor) noexcept {
  if (imp()->userState.colorFactor == factor) return;

  imp()->userState.colorFactor = factor;
  imp()->shaderSetColorFactorEnabled(factor.r != 1.f || factor.g != 1.f ||
                                     factor.b != 1.f || factor.a != 1.f);
  imp()->needsBlendFuncUpdate = true;
}
...
void LPainter::clearScreen() noexcept {
  ...
  glDisable(GL_BLEND);
  imp()->setViewport(imp()->fb->rect().x(), imp()->fb->rect().y(),
                     imp()->fb->rect().w(), imp()->fb->rect().h());
...
void LPainter::bindProgram() noexcept {
  ...
  glUniform2f(imp()->currentUniforms->texSize, imp()->currentState->texSize.w(),
              imp()->currentState->texSize.h());

  glUniform4f(imp()->currentUniforms->srcRect, imp()->currentState->srcRect.x(),
  ...
  glUniform1f(imp()->currentUniforms->alpha, imp()->currentState->alpha);

  glUniform1i(imp()->currentUniforms->mode, imp()->currentState->mode);

  glUniform3f(imp()->currentUniforms->color, imp()->currentState->color.r,
              imp()->currentState->color.g, imp()->currentState->color.b);
  ...
  glUniform1i(imp()->currentUniforms->has90deg, imp()->currentState->has90deg);
...
void LPainter::setBlendFunc(const LBlendFunc &blendFunc) const noexcept {
  ...
  if (!imp()->userState.autoBlendFunc)
    glBlendFuncSeparate(blendFunc.sRGBFactor, blendFunc.dRGBFactor,
                        blendFunc.sAlphaFactor, blendFunc.dAlphaFactor);
}
