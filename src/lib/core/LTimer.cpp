...
LTimer::LTimer(const Callback &onTimeout) noexcept
    : m_onTimeoutCallback(onTimeout) {
  if (compositor() && compositor()->display())
    m_waylandEventSource = wl_event_loop_add_timer(
        compositor()->imp()->eventLoop, &LTimer::waylandTimeoutCallback, this);
}
...
bool LTimer::oneShot(UInt32 intervalMs, const Callback &onTimeout) noexcept {
  if (!onTimeout) {
    LLog::error(
        "[LTimer::oneShot] Cannot create one shot LTimer without onTimeout "
        "callback.");
    return false;
  }
  ...
  if (!compositor() || !compositor()->display()) {
    LLog::error(
        "[LTimer::oneShot] Failed to create one shot LTimer, no active "
        "LCompositor instance.");
    return false;
  }
...
void LTimer::stop() noexcept {
  if (running()) {
    ...
    if (m_onTimeoutCallback) m_onTimeoutCallback(this);
...
bool LTimer::start(UInt32 intervalMs) noexcept {
  if (!compositor() || !compositor()->display()) {
    LLog::error(
        "[LTimer::oneShot] Failed to start LTimer, no active LCompositor "
        "instance.");
    return false;
  }

  if (!m_onTimeoutCallback) return false;
