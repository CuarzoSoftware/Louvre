 ...
 *
 * This constant indicates that an edge is disabled, for example, in
 * LToplevelRole constraints.
 */
 ...
 *
 * This function verifies if the provided bitset of edges corresponds to one of
 * the four possible corner configurations: (Top-Left, Top-Right, Bottom-Left,
 * Bottom-Right).
 *
...
inline constexpr bool edgeIsCorner(LBitset<LEdge> edges) noexcept {
  return edges == (LEdgeTop | LEdgeLeft) || edges == (LEdgeTop | LEdgeRight) ||
