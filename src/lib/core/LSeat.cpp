...
LSeat::LSeat(const void *params)
    : LFactoryObject(FactoryObjectType), LPRIVATE_INIT_UNIQUE(LSeat) {
  assert(params != nullptr && "Invalid parameter passed to LSeat constructor.");
  LSeat **ptr{(LSeat **)params};
  assert(*ptr == nullptr && *ptr == seat() &&
         "Only a single LSeat instance can exist.");
  *ptr = this;
...
const std::vector<const LIdleListener *> &LSeat::idleListeners()
    const noexcept {
...
bool LSeat::isUserIdleHint() const noexcept { return imp()->isUserIdleHint; }

const char *LSeat::name() const noexcept {
  if (imp()->libseatHandle) return libseat_seat_name(imp()->libseatHandle);
...
const std::vector<LToplevelResizeSession *> &LSeat::toplevelResizeSessions()
    const noexcept {
  return imp()->resizeSessions;
}

const std::vector<LToplevelMoveSession *> &LSeat::toplevelMoveSessions()
    const noexcept {
...
void LSeat::dismissPopups() noexcept {
  ...
  for (; s != compositor()->surfaces().rend(); s++) {
    if ((*s)->popup()) (*s)->popup()->dismiss();
  }
}

LPopupRole *LSeat::topmostPopup() const noexcept {
  for (std::list<LSurface *>::const_reverse_iterator it =
           compositor()->surfaces().crbegin();
       it != compositor()->surfaces().crend(); it++) {
    if ((*it)->mapped() && (*it)->popup()) return (*it)->popup();
  }
...
void LSeat::setTTY(UInt32 tty) noexcept {
  if (imp()->libseatHandle) imp()->ttyNumber = tty;
}

Int32 LSeat::openDevice(const char *path, Int32 *fd) noexcept {
  if (!imp()->libseatHandle) return -1;

  Int32 id = libseat_open_device(libseatHandle(), path, fd);

  if (id == -1)
    LLog::error("[LSeat::openDevice] Failed to open device %s, id %d, %d.",
                path, id, *fd);
...
libseat *LSeat::libseatHandle() const noexcept { return imp()->libseatHandle; }

bool LSeat::enabled() const noexcept { return imp()->enabled; }
