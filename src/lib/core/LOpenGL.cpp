...
LTexture *LOpenGL::loadTexture(const std::filesystem::path &file) {
  ...
  if (!image) {
    LLog::error("[LOpenGL::loadTexture] Failed to load image %s: %s.",
  ...
  if (!texture->setDataFromMainMemory(LSize(width, height), width * 4,
                                      DRM_FORMAT_ABGR8888, image)) {
    UInt8 pix{0};
    ...
    texture->setDataFromMainMemory(LSize(width, height), width * 4,
                                   DRM_FORMAT_ARGB8888, image);
  }
...
bool LOpenGL::hasExtension(const char *extensions, const char *extension) {
  ...
  while (extensions < end) {
    ...
    if (n == extlen && strncmp(extension, extensions, n) == 0) return true;
