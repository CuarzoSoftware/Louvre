...
class Louvre::LXCursor {
 public:
   ...
   * @param theme Name of the cursor theme. Pass `NULL` if you don't want to
   * specify a theme.
   * @param suggestedSize Suggested buffer size (width or height) of the pixmap.
   *        Returns the variant of the pixmap with closest dimensions to the
   * specified one.
   *
   * @returns If an XCursor matching the parameters is found, returns an
   * instance of the LXCursor class, which stores the cursor's hotspot, and
   * texture. Otherwise, it returns `nullptr`.
   */
  static LXCursor *load(const char *cursor, const char *theme = NULL,
                        Int32 suggestedSize = 64) noexcept;
   ...
   */
  const LTexture *texture() const noexcept { return &m_texture; }

  /**
   * @brief Gets the cursor's hotspot in buffer coordinates.
   */
  const LPoint &hotspotB() const noexcept { return m_hotspotB; }
