...
LExclusiveZone::LExclusiveZone(LEdge edge, Int32 size,
                               LOutput *output) noexcept {
  m_output.setOnDestroyCallback([this](auto) { setOutput(nullptr); });
...
void LExclusiveZone::setEdgeAndSize(LEdge edge, Int32 size) noexcept {
  ...
  if (edge != m_edge) {
    assert((edge == 0 || edge == LEdgeLeft || edge == LEdgeTop ||
            edge == LEdgeRight || edge == LEdgeBottom) &&
           "Invalid edge passed to LExclusiveZone constructor.");
    m_edge = edge;
    needsUpdate = true;
  }
...
bool LExclusiveZone::insertAfter(LExclusiveZone *zone) const {
  ...
  if (zone) {
    if (zone == this || zone->output() != output()) return false;

    if (zone == prevZone()) return true;

    output()->imp()->exclusiveZones.erase(m_outputLink);

    if (zone == output()->imp()->exclusiveZones.back()) {
      output()->imp()->exclusiveZones.push_back(
          const_cast<LExclusiveZone *>(this));
      m_outputLink = std::prev(output()->imp()->exclusiveZones.end());
    } else
      m_outputLink = output()->imp()->exclusiveZones.insert(
          std::next(zone->m_outputLink), const_cast<LExclusiveZone *>(this));

    output()->imp()->updateExclusiveZones();
    output()->repaint();
    return true;
  } else {
    if (output()->imp()->exclusiveZones.front() == this) return true;

    output()->imp()->exclusiveZones.erase(m_outputLink);
    output()->imp()->exclusiveZones.push_front(
        const_cast<LExclusiveZone *>(this));
