 ...
 * It is used to describe the format and memory layout of DMA planes used for
 * texture generation.\n
 */
struct LDMAFormat {
 ...
 * It provides a unified interface for generating and updating textures from
 * buffers in main memory, `wl_drm` buffers, and DMA buffers.
 *
 * @warning The texture internal buffer storage is destroyed and replaced every
 * time any of the setData() variants is used.
 *
...
class LTexture final : public LObject {
 public:
  /**
   * @brief Texture source enumeration indicating the origin of the texture
   * data.
   */
  ...
  /**
   * @brief Gets the equivalent DRM buffer format from a Wayland SHM buffer
   * format.
   *
   * DRM formats are listed in
   ...
   * @param format The DRM format of the buffer (defined in
   * [`drm_fourcc.h`](https://github.com/torvalds/linux/blob/master/include/uapi/drm/drm_fourcc.h)).
   * @param buffer The pointer to the source main memory buffer.
   * @return `true` if the data was successfully set, `false` otherwise.
   */
  bool setDataFromMainMemory(const LSize &size, UInt32 stride, UInt32 format,
   ...
   * @param transferOwnership If set to `true`, the texture will be
   * automatically destroyed when the LTexture is destroyed or replaced.
   *
   * @return `true` if the texture data was successfully set, `false` otherwise.
   */
  bool setDataFromGL(GLuint id, GLenum target, UInt32 format, const LSize &size,
                     bool transferOwnership) noexcept;
   ...
   * If invalid parameters are passed or if the texture cannot be modified, this
   * method returns `false`.
   *
   * @param rect The rect within the texture to update, specified in buffer
   * coordinates with the top-left corner as the origin.
   ...
   */
  bool updateRect(const LRect &rect, UInt32 stride,
                  const void *buffer) noexcept;

  /**
   * @brief Prepares the texture for writing.
   *
   * The writeBegin(), writeUpdate() and writeEnd() methods provide the same
   * functionality as updateRect(), but they allow for multiple writes without
   * requiring immediate internal syncing, yielding better performance.
   *
   ...
   *
   * This method can only be called if writeBegin() was previously called and
   * returned `true`. The parameters are the same as those defined in
   * updateRect().
   *
   * @return `true` on success, or `false` on failure.
   */
  bool writeUpdate(const LRect &rect, UInt32 stride,
                   const void *buffer) noexcept;

  /**
   * @brief Finalizes the write operation to the texture.
   *
   * This method should be called after completing all write operations
   * initiated by writeBegin(). It ensures that any pending writes are completed
   * and the texture is in a consistent state before using it.
   *
   * @return `true` on success, or `false` if writeBegin() wasn't called or
   * failed.
   */
   ...
   *
   * @note The resulting texture is independent of the original and must be
   * freed manually when no longer used.
   *
   * @param dst The destination size of the copied texture. Pass (0,0) to use
   * the same size as the original texture.
   * @param src The rectangular area within the source texture to be copied.
   * Pass (0,0,0,0) to copy the entire texture.
   * @param highQualityScaling Set this parameter to `true` to enable
   * high-quality scaling, which produces better results when resizing to a
   * significantly different size from the original.
   *
   * @return A pointer to the copied LTexture object or `nullptr` on failure.
   */
  LTexture *copy(const LSize &dst = LSize(), const LRect &src = LRect(),
                 bool highQualityScaling = true) const noexcept;

  /**
   * @brief Save the texture as a PNG file.
   *
   * This method allows you to save the texture as a PNG image file at the
   * specified @p name.
   *
   ...
   */
  const LSize &sizeB() const noexcept { return m_sizeB; }

  /**
   * @brief Check if the texture has been initialized.
   *
   * A texture is considered initialized when content has been assigned to it
   * using any of the **setDataXXX()** variants.
   *
   * @return `true` if the texture has been initialized, `false` otherwise.
   */
  bool initialized() const noexcept { return m_graphicBackendData != nullptr; }

  /**
   * @brief Gets the OpenGL texture ID for a specific output.
   *
   * If `nullptr` is passed as output, a texture for the main thread is
   * returned.
   *
  ...
  GLenum target() const noexcept {
    if (initialized()) {
      if (sourceType() == Framebuffer) return GL_TEXTURE_2D;
   ...
   *
   * @return The texture source type as an LTexture::BufferSourceType enum
   * value.
   */
  BufferSourceType sourceType() const noexcept { return m_sourceType; }
   ...
   */
  UInt32 format() const noexcept { return m_format; }

  /**
   * @brief Gets the serial number of the texture.
   *
   * The serial number is incremented each time the texture's backing storage,
   * or its pixel data changes.
   *
   * @return The serial number of the texture.
   */
  UInt32 serial() const noexcept { return m_serial; }

  /**
   * @brief Indicates whether the RGB components are pre-multiplied by the alpha
   * component.
   *
   * This flag is crucial for proper blending of textures within LPainter and
   * LScene.\n By default, Wayland clients typically use pre-multiplied alpha,
   * but user-defined textures may not adhere to this convention.
   *
   * To modify this value, use setPremultipliedAlpha().
   *
   * @return `true` if the RGB components are pre-multiplied by the alpha
   * component, `false` otherwise.
   */
  bool premultipliedAlpha() const noexcept { return m_premultipliedAlpha; }

  /**
   * @brief Sets a hint indicating whether the RGB components are pre-multiplied
   * by the alpha component.
   *
   * This function allows you to modify the pre-multiplied alpha flag.
   *
   * @param premultipledAlpha `true` to indicate pre-multiplied alpha, `false`
   * otherwise.
   */
