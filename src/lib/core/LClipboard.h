 ...
 *
 * LClipboard lets you manage which clients can set the clipboard and also
 * specify which MIME types to make persistent.
 *
 * Clients can access the clipboard when one of their surfaces acquires keyboard
 * focus.
 */
class Louvre::LClipboard : public LFactoryObject {
 public:
  static constexpr LFactoryObject::Type FactoryObjectType =
      LFactoryObject::Type::LClipboard;
   ...
   *
   * Returning `true` grants the client permission to set the clipboard and
   * `false` prohibits it.\n Only a single client can set the clipboard at a
   * time.
   *
