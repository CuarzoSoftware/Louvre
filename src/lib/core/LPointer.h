 ...
 *
 * The LPointer class allows you to listen to input events generated by devices
 * such as a mouse or touchpad and redirect them to client surfaces. There is a
 * single instance of LPointer, which can be accessed through LSeat::pointer().
 *
 * ### Wayland Events
 *
 * To send pointer events to clients, you must first assign focus to a surface
 * using one of the setFocus() variants.\n Subsequently, all pointer events sent
 * with any of the **sendXXXEvent()** methods are directed to the currently
 * focused surface.
 */
class Louvre::LPointer : public LFactoryObject {
 public:
  static constexpr LFactoryObject::Type FactoryObjectType =
      LFactoryObject::Type::LPointer;

  /**
   * @brief LPointer class constructor.
   *
   * There is a single instance of LPointer, which can be accessed from
   * LSeat::pointer().
   *
   * @param params Internal library parameters provided in the
   * LCompositor::createObjectRequest() virtual constructor.
   */
   ...
   *
   * This method returns the surface that has been assigned pointer focus using
   * setFocus().
   *
   ...
   *
   * This method assigns the pointer focus to the specified surface at the given
   * local surface position within the surface.
   *
   * If another surface already has pointer focus, it will lose it.
   *
   * @param surface Surface to which the pointer focus will be assigned, or
   * `nullptr` to remove focus from all surfaces.
   ...
   * @brief Sets the pointer focus.
   *
   * Sets the pointer focus to the provided surface based on the current LCursor
   * position.
   *
   * @note This method internally transforms the LCursor position to the local
   * coordinates of the focused surface, taking into account the surface's role
   * position.
   *
   * @param surface The surface to which you want to assign the pointer focus or
   * `nullptr` to remove focus from all surfaces.
   */
   ...
   *
   * This is just a utility used by the default LPointer implementation to
   * ensure that pointer focus remains on a surface while it's being actively
   * pressed by the left pointer button, for example, during text selection,
   * even if the pointer moves outside the surface boundaries.\n
   *
   ...
   *
   * This method returns the surface that is currently being actively pressed by
   * the main pointer button.
   *
   ...
   * given point.\n It takes into account the surfaces rolePos(), size(),
   * inputRegion() and the reverse order given by the LCompositor::surfaces()
   * list.
   *
   * @note Some surface roles do not have an input region such as LCursorRole or
   * LDNDIconRole so these surfaces are always ignored.
   *
   ...
   * @see isButtonPressed()
   */
  const std::vector<LPointerButtonEvent::Button> &pressedButtons()
      const noexcept;
   ...
   *
   * @note Pointer events are sent to the currently focused surface set with
   * setFocus(). If no surface has focus, calling these methods has no effect.
   */
   ...
   *
   * @note To specify the position within the surface modify the mutable @ref
   * LPointerMoveEvent::localPos property.
   */
   ...
   *
   * @note A sendSwipeBeginEvent() should have been sent before, otherwise this
   * is a no-op.
   */
   ...
   *
   * @note A sendSwipeBeginEvent() should have been sent before, otherwise this
   * is a no-op.\n This event is automatically sent if a sendSwipeBeginEvent()
   * was sent and the surface lost focus.
   */
   ...
   *
   * @note A sendPinchBeginEvent() should have been sent before, otherwise this
   * is a no-op.
   */
   ...
   *
   * @note A sendPinchBeginEvent() should have been sent before, otherwise this
   * is a no-op.\n This event is automatically sent if a sendPinchBeginEvent()
   * was sent and the surface lost focus.
   */
   ...
   *
   * @note A sendHoldBeginEvent() should have been sent before, otherwise this
   * is a no-op.\n This event is automatically sent if a sendHoldBeginEvent()
   * was sent and the surface lost focus.
   */
   ...
   *
   * Triggered when a client requests to set the cursor texture, hotspot or hide
   * it.
   *
