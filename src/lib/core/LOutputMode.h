 ...
 *
 * The LOutputMode class represents a possible configuration in which an LOutput
 * can operate, specifically the refresh rate and resolution.\n Each LOutput has
 * one or more modes which can be accessed from LOutput::modes() and assigned
 * with LOutput::setMode().
 */
class Louvre::LOutputMode final : LObject {
 public:
  LOutputMode(LOutput *output, const LSize &size, UInt32 refreshRate,
   ...
   *
   * This method returns the dimensions of the output when using this mode,
   * represented in buffer coordinates.
   */
   ...
   *
   * This method returns the refresh rate of the mode in Hertz (Hz) multiplied
   * by 1000.
   *
   * @return The refresh rate of the mode, expressed in Hz * 1000.
   */
  UInt32 refreshRate() const noexcept { return m_refreshRate; }

  /**
   * @brief Check if this mode is the preferred mode for the output.
   *
   * This method indicates whether the mode is the preferred choice for the
   * output.
   *
   * @return `true` if this mode is preferred and `false` otherwise.
   */
  bool isPreferred() const noexcept { return m_isPreferred; }
