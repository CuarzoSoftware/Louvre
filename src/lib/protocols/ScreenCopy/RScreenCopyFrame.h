...
class Louvre::Protocols::ScreenCopy::RScreenCopyFrame final : public LResource {
 public:
  ...
  GScreenCopyManager *screenCopyManagerRes() const noexcept {
    return m_screenCopyManagerRes;
  }
  ...
  bool compositeCursor() const noexcept {
    return m_stateFlags.check(CompositeCursor);
  };
  bool alreadyUsed() const noexcept { return m_stateFlags.check(AlreadyUsed); };
  bool waitForDamage() const noexcept {
    return m_stateFlags.check(WaitForDamage);
  };
  ...
  static void copy_with_damage(wl_client *client, wl_resource *resource,
                               wl_resource *buffer) noexcept;
#endif
  /******************** EVENTS ********************/
 ...
 private:
  ...
  RScreenCopyFrame(GScreenCopyManager *screenCopyManagerRes, LOutput *output,
                   bool overlayCursor, const LRect &region, UInt32 id,
                   Int32 version) noexcept;
  ~RScreenCopyFrame() noexcept;
  static void copyCommon(wl_resource *resource, wl_resource *buffer,
