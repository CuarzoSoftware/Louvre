...
void GScreenCopyManager::bind(wl_client *client, void * /*data*/,
...
GScreenCopyManager::GScreenCopyManager(wl_client *client, Int32 version,
                                       UInt32 id) noexcept
...
void GScreenCopyManager::destroy(wl_client * /*client*/,
                                 wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void GScreenCopyManager::capture_output(wl_client * /*client*/,
                                        wl_resource *resource, UInt32 id,
                                        Int32 overlayCursor,
                                        wl_resource *output) noexcept {
  auto &res{
      *static_cast<GScreenCopyManager *>(wl_resource_get_user_data(resource))};
  auto *outputRes{
      static_cast<Wayland::GOutput *>(wl_resource_get_user_data(output))};

  new RScreenCopyFrame(
      &res, outputRes->output(), overlayCursor == 1,
      LRect(LPoint(),
            outputRes->output() ? outputRes->output()->size() : LSize(1)),
      id, res.version());
}

void GScreenCopyManager::capture_output_region(wl_client * /*client*/,
                                               wl_resource *resource, UInt32 id,
                                               Int32 overlayCursor,
                                               wl_resource *output, Int32 x,
                                               Int32 y, Int32 width,
  ...
  new RScreenCopyFrame(&res, outputRes->output(), overlayCursor == 1,
                       LRect(x, y, width, height), id, res.version());
}
