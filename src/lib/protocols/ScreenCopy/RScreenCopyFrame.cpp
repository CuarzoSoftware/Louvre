...
RScreenCopyFrame::RScreenCopyFrame(GScreenCopyManager *screenCopyManagerRes,
                                   LOutput *output, bool overlayCursor,
                                   const LRect &region, UInt32 id,
                                   Int32 version) noexcept
    : LResource(screenCopyManagerRes->client(),
                &zwlr_screencopy_frame_v1_interface, version, id, &imp),
      m_output(output),
  ...
  if (Louvre::is90Transform(output->transform())) {
    scale.setW(Float32(m_initOutputModeSize.w()) /
               Float32(m_initOutputSize.h()));
    scale.setH(Float32(m_initOutputModeSize.h()) /
               Float32(m_initOutputSize.w()));
  } else {
    scale.setW(Float32(m_initOutputModeSize.w()) /
               Float32(m_initOutputSize.w()));
    scale.setH(Float32(m_initOutputModeSize.h()) /
  ...
  if (output->painter()->imp()->openGLExtensions.EXT_read_format_bgra) {
    buffer(WL_SHM_FORMAT_XRGB8888, m_rectB.size(), m_stride);
  } else {
    buffer(WL_SHM_FORMAT_XBGR8888, m_rectB.size(), m_stride);
  }

  LTexture *outputTexture;

  if ((outputTexture = output->bufferTexture(0)) &&
      output->painter()->imp()->openGLExtensions.OES_EGL_image)
...
void RScreenCopyFrame::copyCommon(wl_resource *resource, wl_resource *buffer,
                                  bool waitForDamage) noexcept {
  auto &res{
      *static_cast<RScreenCopyFrame *>(wl_resource_get_user_data(resource))};
  ...
  if (res.alreadyUsed()) {
    res.postError(ZWLR_SCREENCOPY_FRAME_V1_ERROR_ALREADY_USED,
  ...
  if (wl_shm_buffer_get(buffer)) {
    ...
    if ((res.output()
         ...
         wl_shm_buffer_get_format(shmBuffer) != WL_SHM_FORMAT_XRGB8888) ||
        (!res.output()
              ->painter()
              ->imp()
              ->openGLExtensions.EXT_read_format_bgra &&
         wl_shm_buffer_get_format(shmBuffer) != WL_SHM_FORMAT_XBGR8888)) {
      res.postError(ZWLR_SCREENCOPY_FRAME_V1_ERROR_INVALID_BUFFER,
    ...
    if (res.rectB().w() != wl_shm_buffer_get_width(shmBuffer) ||
        res.rectB().h() != wl_shm_buffer_get_height(shmBuffer)) {
      res.postError(ZWLR_SCREENCOPY_FRAME_V1_ERROR_INVALID_BUFFER,
                    "Invalid buffer size.");
      return;
    }
    ...
    LDMABuffer *dmaBuffer{
        static_cast<LDMABuffer *>(wl_resource_get_user_data(buffer))};

    if (!res.output()->bufferTexture(0)) {
      res.postError(ZWLR_SCREENCOPY_FRAME_V1_ERROR_INVALID_BUFFER,
                    "Linux DMA buffers are not supported.");
      return;
    }

    if (dmaBuffer->texture()) {
      res.postError(ZWLR_SCREENCOPY_FRAME_V1_ERROR_INVALID_BUFFER,
                    "Buffer already used by surfaces.");
      return;
    }

    if (res.output()->bufferTexture(0)->format() !=
        dmaBuffer->planes()->format) {
      res.postError(ZWLR_SCREENCOPY_FRAME_V1_ERROR_INVALID_BUFFER,
                    "Invalid buffer format.");
      return;
    }

    if (static_cast<Int32>(dmaBuffer->planes()->width) != res.rectB().w() ||
        static_cast<Int32>(dmaBuffer->planes()->height) != res.rectB().h()) {
      res.postError(ZWLR_SCREENCOPY_FRAME_V1_ERROR_INVALID_BUFFER,
                    "Invalid buffer size.");
      return;
    }
...
void RScreenCopyFrame::copy(wl_client * /*client*/, wl_resource *resource,
                            wl_resource *buffer) noexcept {
  RScreenCopyFrame::copyCommon(resource, buffer, false);
}

void RScreenCopyFrame::destroy(wl_client * /*client*/,
                               wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

#if LOUVRE_SCREEN_COPY_MANAGER_VERSION >= 2
void RScreenCopyFrame::copy_with_damage(wl_client * /*client*/,
...
void RScreenCopyFrame::buffer(UInt32 shmFormat, const LSize &size,
                              UInt32 stride) noexcept {
  zwlr_screencopy_frame_v1_send_buffer(resource(), shmFormat, size.w(),
                                       size.h(), stride);
}
...
void RScreenCopyFrame::ready(const timespec &time) noexcept {
  zwlr_screencopy_frame_v1_send_ready(resource(), time.tv_sec >> 32,
                                      time.tv_sec & 0xffffffff, time.tv_nsec);
}
...
#if LOUVRE_SCREEN_COPY_MANAGER_VERSION >= 2
  if (version() >= 2) {
    zwlr_screencopy_frame_v1_send_damage(resource(), rect.x(), rect.y(),
                                         rect.w(), rect.h());
    return true;
  }
#else
  ...
  if (version() >= 2) {
    ...
    while (n > 0) {
      zwlr_screencopy_frame_v1_send_damage(
          resource(), box->x1, box->y1, box->x2 - box->x1, box->y2 - box->y1);
      box++;
      n--;
    }
    return true;
...
bool RScreenCopyFrame::linuxDMABuf(UInt32 format, const LSize &size) noexcept {
  ...
  if (version() >= 3) {
    zwlr_screencopy_frame_v1_send_linux_dmabuf(resource(), format, size.w(),
                                               size.h());
    return true;
  }
#else
