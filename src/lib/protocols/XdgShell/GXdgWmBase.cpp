...
void GXdgWmBase::bind(wl_client *client, void * /*data*/, UInt32 version,
                      UInt32 id) noexcept {
  new GXdgWmBase(client, version, id);
}

Int32 GXdgWmBase::maxVersion() noexcept { return LOUVRE_XDG_WM_BASE_VERSION; }

const wl_interface *GXdgWmBase::interface() noexcept {
  return &xdg_wm_base_interface;
}

XdgShell::GXdgWmBase::GXdgWmBase(wl_client *client, Int32 version,
                                 UInt32 id) noexcept
...
void GXdgWmBase::destroy(wl_client * /*client*/,
...
void GXdgWmBase::create_positioner(wl_client * /*client*/,
                                   wl_resource *resource, UInt32 id) noexcept {
  new RXdgPositioner(
      static_cast<GXdgWmBase *>(wl_resource_get_user_data(resource)), id);
}

void GXdgWmBase::get_xdg_surface(wl_client * /*client*/, wl_resource *resource,
  ...
  if (!surfaceRes.surface()->imp()->canHostRole()) {
    res.postError(XDG_WM_BASE_ERROR_ROLE,
                  "Given wl_surface has another role or xdg_surface assigned.");
    return;
  }

  new RXdgSurface(
      static_cast<GXdgWmBase *>(wl_resource_get_user_data(resource)),
      surfaceRes.surface(), id);
}

void GXdgWmBase::pong(wl_client * /*client*/, wl_resource *resource,
                      UInt32 serial) {
  static_cast<GXdgWmBase *>(wl_resource_get_user_data(resource))
      ->client()
      ->pong(serial);
