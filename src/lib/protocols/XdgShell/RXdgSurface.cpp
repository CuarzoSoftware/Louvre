...
RXdgSurface::RXdgSurface(GXdgWmBase *xdgWmBaseRes, LSurface *surface,
                         UInt32 id) noexcept
    : LResource(xdgWmBaseRes->client(), &xdg_surface_interface,
                xdgWmBaseRes->version(), id, &imp),
...
RXdgSurface::~RXdgSurface() noexcept {
  if (xdgWmBaseRes()) xdgWmBaseRes()->m_xdgSurfacesCount--;
}

/******************** REQUESTS ********************/

LRect RXdgSurface::calculateGeometryWithSubsurfaces() noexcept {
  if (!surface()) return LRect(0, 0, 1, 1);
  ...
  return LRect(region.extents().x1, region.extents().y1,
...
void RXdgSurface::destroy(wl_client * /*client*/, wl_resource *resource) {
  ...
  if (res.surface()->role()) {
    res.postError(XDG_WM_BASE_ERROR_DEFUNCT_SURFACES,
                  "xdg_surface must be destroyed after its specific role");
    return;
  }
...
void RXdgSurface::get_toplevel(wl_client * /*client*/, wl_resource *resource,
                               UInt32 id) {
  auto *res{static_cast<RXdgSurface *>(wl_resource_get_user_data(resource))};

  if (res->surface()->imp()->hasBufferOrPendingBuffer()) {
    res->postError(XDG_SURFACE_ERROR_ALREADY_CONSTRUCTED,
                   "Given wl_surface already has a buffer attached.");
    return;
  }

  if (res->surface()->role()) {
    res->postError(XDG_WM_BASE_ERROR_ROLE,
...
void RXdgSurface::get_popup(wl_client * /*client*/, wl_resource *resource,
                            UInt32 id, wl_resource *parent,
                            wl_resource *positioner) {
  auto *xdgPositionerRes{
  ...
  auto *xdgParentSurfaceRes{
      parent == nullptr
          ? nullptr
          : static_cast<RXdgSurface *>(wl_resource_get_user_data(parent))};

  if (xdgParentSurfaceRes &&
      res->surface()->imp()->isInChildrenOrPendingChildren(
          xdgParentSurfaceRes->surface())) {
    xdgPositionerRes->postError(XDG_WM_BASE_ERROR_INVALID_POPUP_PARENT,
...
void RXdgSurface::set_window_geometry(wl_client * /*client*/,
                                      wl_resource *resource, Int32 x, Int32 y,
                                      Int32 width, Int32 height) {
  auto &res{*static_cast<RXdgSurface *>(wl_resource_get_user_data(resource))};

  if (!res.xdgPopupRes() && !res.xdgToplevelRes()) {
    res.postError(XDG_SURFACE_ERROR_NOT_CONSTRUCTED,
                  "Can not set window geometry with no role.");
    return;
  }

  if (width <= 0 || height <= 0) {
    res.postError(XDG_SURFACE_ERROR_INVALID_SIZE,
...
void RXdgSurface::ack_configure(wl_client * /*client*/, wl_resource *resource,
                                UInt32 serial) {
  auto &res{*static_cast<RXdgSurface *>(wl_resource_get_user_data(resource))};

  if (!res.xdgPopupRes() && !res.xdgToplevelRes()) {
    res.postError(XDG_SURFACE_ERROR_NOT_CONSTRUCTED,
                  "Can not ack xdg_surface with no role.");
    return;
  }
  ...
  if (res.surface()->roleId() == LSurface::Role::Toplevel) {
    ...
    while (!toplevel.m_sentConfigurations.empty()) {
      if (toplevel.m_sentConfigurations.front().serial == serial) {
        const bool sizeUnchanged{toplevel.m_lastACKConfiguration.size ==
                                 toplevel.m_sentConfigurations.front().size};
        ...
        toplevel.pendingAtoms().capabilities =
            toplevel.m_lastACKConfiguration.capabilities;
        toplevel.pendingAtoms().decorationMode =
            toplevel.m_lastACKConfiguration.decorationMode;
    ...
    res.postError(XDG_SURFACE_ERROR_INVALID_SERIAL,
