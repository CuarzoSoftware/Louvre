...
RXdgPositioner::RXdgPositioner(GXdgWmBase *xdgWmBaseRes, UInt32 id) noexcept
    : LResource(xdgWmBaseRes->client(), &xdg_positioner_interface,
                xdgWmBaseRes->version(), id, &imp) {}

bool RXdgPositioner::validate() {
  if (positioner().size().w() <= 0 || positioner().size().h() <= 0) {
    postError(XDG_POSITIONER_ERROR_INVALID_INPUT,
  ...
  if (positioner().anchorRect().w() <= 0 ||
      positioner().anchorRect().h() <= 0) {
    postError(XDG_POSITIONER_ERROR_INVALID_INPUT,
              "xdg_positioner.set_anchor_rect requested with non-positive "
              "dimensions ({}, {})",
  ...
  if (positioner().gravity() > 8) {
    postError(XDG_POSITIONER_ERROR_INVALID_INPUT, "Invalid gravity.");
    return false;
  }
...
void RXdgPositioner::destroy(wl_client * /*client*/,
                             wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void RXdgPositioner::set_size(wl_client * /*client*/, wl_resource *resource,
                              Int32 width, Int32 height) noexcept {
  auto &positioner{
      static_cast<RXdgPositioner *>(wl_resource_get_user_data(resource))
          ->m_positioner};
  positioner.m_size.setW(width);
  positioner.m_size.setH(height);
}

void RXdgPositioner::set_anchor_rect(wl_client * /*client*/,
...
void RXdgPositioner::set_anchor(wl_client * /*client*/, wl_resource *resource,
                                UInt32 anchor) noexcept {
  auto &positioner{
      static_cast<RXdgPositioner *>(wl_resource_get_user_data(resource))
          ->m_positioner};
...
void RXdgPositioner::set_gravity(wl_client * /*client*/, wl_resource *resource,
                                 UInt32 gravity) noexcept {
  auto &positioner{
      static_cast<RXdgPositioner *>(wl_resource_get_user_data(resource))
          ->m_positioner};
...
void RXdgPositioner::set_constraint_adjustment(
    wl_client * /*client*/, wl_resource *resource,
    UInt32 constraintAdjustment) noexcept {
  auto &positioner{
      static_cast<RXdgPositioner *>(wl_resource_get_user_data(resource))
          ->m_positioner};
  positioner.m_constraintAdjustments =
      static_cast<LPositioner::ConstraintAdjustments>(constraintAdjustment);
}

void RXdgPositioner::set_offset(wl_client * /*client*/, wl_resource *resource,
                                Int32 x, Int32 y) noexcept {
  auto &positioner{
      static_cast<RXdgPositioner *>(wl_resource_get_user_data(resource))
          ->m_positioner};
...
void RXdgPositioner::set_reactive(wl_client * /*client*/,
...
void RXdgPositioner::set_parent_size(wl_client * /*client*/,
                                     wl_resource *resource, Int32 parent_width,
                                     Int32 parent_height) {
  auto &positioner{
      static_cast<RXdgPositioner *>(wl_resource_get_user_data(resource))
          ->m_positioner};
...
void RXdgPositioner::set_parent_configure(wl_client * /*client*/,
                                          wl_resource *resource,
                                          UInt32 serial) {
  auto &positioner{
      static_cast<RXdgPositioner *>(wl_resource_get_user_data(resource))
