...
RXdgToplevel::RXdgToplevel(RXdgSurface *xdgSurfaceRes, UInt32 id)
    : LResource(xdgSurfaceRes->client(), &xdg_toplevel_interface,
                xdgSurfaceRes->version(), id, &imp),
      m_xdgSurfaceRes(xdgSurfaceRes) {
  xdgSurfaceRes->m_xdgToplevelRes.reset(this);

  LToplevelRole::Params toplevelRoleParams{this, xdgSurfaceRes->surface()};

  m_toplevelRole.reset(
      LFactory::createObject<LToplevelRole>(&toplevelRoleParams));
...
void RXdgToplevel::set_parent(wl_client * /*client*/, wl_resource *resource,
                              wl_resource *parent) {
  ...
  if (parent == NULL) {
    ...
    auto *parentRes{
        static_cast<RXdgToplevel *>(wl_resource_get_user_data(parent))};
    ...
    if (res.toplevelRole()->surface()->imp()->isInChildrenOrPendingChildren(
            parentRes->toplevelRole()->surface())) {
      res.postError(XDG_TOPLEVEL_ERROR_INVALID_PARENT,
                    "Invalid xdg_toplevel parent.");
      return;
    }

    res.toplevelRole()->surface()->imp()->setParent(
        parentRes->toplevelRole()->surface());
  }
}

void RXdgToplevel::set_title(wl_client * /*client*/, wl_resource *resource,
                             const char *title) {
  static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))
      ->toplevelRole()
      ->setTitle(title);
}

void RXdgToplevel::set_app_id(wl_client * /*client*/, wl_resource *resource,
                              const char *app_id) {
  static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))
      ->toplevelRole()
      ->setAppId(app_id);
...
void RXdgToplevel::show_window_menu(wl_client * /*client*/,
  ...
  if (triggeringEvent &&
      toplevel.capabilities().check(LToplevelRole::WindowMenuCap))
    toplevel.showWindowMenuRequest(*triggeringEvent, x, y);
}

void RXdgToplevel::move(wl_client * /*client*/, wl_resource *resource,
                        wl_resource * /*seat*/, UInt32 serial) {
  auto &res{*static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))};

  if (!res.toplevelRole()->surface()->mapped()) return;

  const LEvent *triggererEvent{res.client()->findEventBySerial(serial)};

  if (triggererEvent) res.toplevelRole()->startMoveRequest(*triggererEvent);
}

void RXdgToplevel::resize(wl_client * /*client*/, wl_resource *resource,
                          wl_resource * /*seat*/, UInt32 serial, UInt32 edges) {
  auto &res{*static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))};
...
void RXdgToplevel::set_max_size(wl_client * /*client*/, wl_resource *resource,
                                Int32 width, Int32 height) {
  auto &res{*static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))};

  if (width < 0 || height < 0) {
    res.postError(XDG_TOPLEVEL_ERROR_INVALID_SIZE,
                  "Invalid xdg_toplevel max size ({}, {}).", width, height);
    return;
  }
...
void RXdgToplevel::set_min_size(wl_client * /*client*/, wl_resource *resource,
                                Int32 width, Int32 height) {
  auto &res{*static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))};

  if (width < 0 || height < 0) {
    res.postError(XDG_TOPLEVEL_ERROR_INVALID_SIZE,
                  "Invalid xdg_toplevel min size ({}, {}).", width, height);
    return;
  }
...
void RXdgToplevel::set_maximized(wl_client * /*client*/,
                                 wl_resource *resource) {
  auto &toplevel{
      *static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))
           ->toplevelRole()};
  ...
  if (!toplevel.m_flags.check(LToplevelRole::HasSizeOrStateToSend) &&
      (toplevel.capabilities().check(LToplevelRole::MaximizeCap) ||
       toplevel.resource()->version() < 5))
    toplevel.configureState(toplevel.pendingConfiguration().state);
}

void RXdgToplevel::unset_maximized(wl_client * /*client*/,
                                   wl_resource *resource) {
  auto &toplevel{
      *static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))
           ->toplevelRole()};
...
void RXdgToplevel::set_fullscreen(wl_client * /*client*/, wl_resource *resource,
                                  wl_resource *wlOutput) {
  LOutput *output{nullptr};

  if (wlOutput)
    output =
        static_cast<Wayland::GOutput *>(wl_resource_get_user_data(wlOutput))
            ->output();
  ...
  if (!toplevel.m_flags.check(LToplevelRole::HasSizeOrStateToSend) &&
      (toplevel.capabilities().check(LToplevelRole::FullscreenCap) ||
       toplevel.resource()->version() < 5))
    toplevel.configureState(toplevel.pendingConfiguration().state);
}

void RXdgToplevel::unset_fullscreen(wl_client * /*client*/,
                                    wl_resource *resource) {
  auto &toplevel{
      *static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))
           ->toplevelRole()};
...
void RXdgToplevel::set_minimized(wl_client * /*client*/,
                                 wl_resource *resource) {
  auto &toplevel{
      *static_cast<RXdgToplevel *>(wl_resource_get_user_data(resource))
           ->toplevelRole()};

  if (toplevel.m_flags.check(LToplevelRole::HasPendingInitialConf)) return;
...
void RXdgToplevel::close() noexcept { xdg_toplevel_send_close(resource()); }
