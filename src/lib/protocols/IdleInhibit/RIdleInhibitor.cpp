...
RIdleInhibitor::RIdleInhibitor(LSurface *surface, Int32 version,
                               UInt32 id) noexcept
    : LResource(surface->client(), &zwp_idle_inhibitor_v1_interface, version,
                id, &imp),
      m_surface(surface) {
  m_surface.setOnDestroyCallback(
      [this](LSurface *surface) { handleRemoval(this, surface); });
...
RIdleInhibitor::~RIdleInhibitor() {
  if (m_surface) handleRemoval(this, m_surface.get());
}

void RIdleInhibitor::handleRemoval(RIdleInhibitor *idleInhibitorRes,
                                   LSurface *surface) noexcept {
  LVectorRemoveOneUnordered(surface->imp()->idleInhibitorResources,
                            idleInhibitorRes);
...
void RIdleInhibitor::destroy(wl_client * /*client*/,
