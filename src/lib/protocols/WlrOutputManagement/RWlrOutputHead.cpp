...
RWlrOutputHead::RWlrOutputHead(GWlrOutputManager *wlrOutputManager,
                               LOutput *output) noexcept
    : LResource(wlrOutputManager->client(), &zwlr_output_head_v1_interface,
                wlrOutputManager->version(), 0, &imp),
  ...
  if (currMode) currentMode(currMode);
  ...
  if (make(output->manufacturer())) {
    ...
    if (output->serialNumber()) serialNumber(output->serialNumber());
...
void RWlrOutputHead::markAsPendingDone() noexcept {
  if (m_wlrOutputManager) m_wlrOutputManager->m_pendingDone = true;
}
