...
void GWlrOutputManager::bind(wl_client *client, void * /*data*/, UInt32 version,
                             UInt32 id) noexcept {
...
GWlrOutputManager::GWlrOutputManager(wl_client *client, Int32 version,
                                     UInt32 id) noexcept
    : LResource(client, interface(), version, id, &imp) {
  this->client()->imp()->wlrOutputManagerGlobals.emplace_back(this);

  for (LOutput *output : seat()->outputs()) head(output);
...
void GWlrOutputManager::create_configuration(wl_client * /*client*/,
