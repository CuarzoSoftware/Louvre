...
RWlrOutputConfiguration::RWlrOutputConfiguration(
    GWlrOutputManager *wlrOutputManager, UInt32 id, UInt32 serial) noexcept
    : LResource(wlrOutputManager->client(),
                &zwlr_output_configuration_v1_interface,
                wlrOutputManager->version(), id, &imp),
      m_wlrOutputManager(wlrOutputManager),
      m_serial(serial) {}

RWlrOutputConfiguration::~RWlrOutputConfiguration() noexcept {
  while (!m_enabled.empty()) m_enabled.front()->destroy();
}
...
RWlrOutputConfiguration::Reply RWlrOutputConfiguration::validate() noexcept {
  ...
  for (auto &wo : m_disabled)
    if (!wo.get()) return Cancelled;

  for (auto *ch : m_enabled)
    if (!ch->output()) return Cancelled;
...
void RWlrOutputConfiguration::enable_head(wl_client * /*client*/,
                                          wl_resource *resource, UInt32 id,
                                          wl_resource *head) {
  auto &res{LRES_CAST(RWlrOutputConfiguration, resource)};

  if (res.m_replied) {
    res.postError(ZWLR_OUTPUT_CONFIGURATION_V1_ERROR_ALREADY_USED,
  ...
  if (headRes.output() && res.checkAlreadyConfigured(headRes.output())) return;
...
void RWlrOutputConfiguration::disable_head(wl_client * /*client*/,
  ...
  if (headRes.output()) {
    if (res.checkAlreadyConfigured(headRes.output())) return;
...
void RWlrOutputConfiguration::apply(wl_client * /*client*/,
                                    wl_resource *resource) {
  auto &res{LRES_CAST(RWlrOutputConfiguration, resource)};
  ...
  if (reply == Succeeded) {
    ...
    for (LOutput *o : seat()->outputs())
      fallback.emplace_back(*o, o->state() != LOutput::Uninitialized,
                            *o->currentMode(), o->pos(), o->transform(),
                            o->fractionalScale());

    // Fill new configuration
    for (auto &wo : res.m_disabled)
      configurations.emplace_back(*wo.get(), false, *wo->currentMode(),
                                  wo->pos(), wo->transform(), wo->scale());

    for (auto *eh : res.m_enabled)
      configurations.emplace_back(
          *eh->output(), true,
          eh->m_mode.get() != nullptr ? *eh->m_mode.get()
                                      : *eh->output()->currentMode(),
          eh->m_setProps.check(RWlrOutputConfigurationHead::Position)
              ? eh->m_pos
              : eh->output()->pos(),
          eh->m_setProps.check(RWlrOutputConfigurationHead::Transform)
              ? eh->m_transform
              : eh->output()->transform(),
          eh->m_setProps.check(RWlrOutputConfigurationHead::Scale)
              ? eh->m_scale
              : eh->output()->fractionalScale());

    reply = seat()->configureOutputsRequest(res.client(), configurations)
                ? Succeeded
                : Failed;
...
void RWlrOutputConfiguration::test(wl_client * /*client*/,
                                   wl_resource *resource) {
  auto &res{LRES_CAST(RWlrOutputConfiguration, resource)};
...
void RWlrOutputConfiguration::destroy(wl_client * /*client*/,
                                      wl_resource *resource) {
...
void RWlrOutputConfiguration::reply(Reply reply) noexcept {
  switch (reply) {
    case Failed: zwlr_output_configuration_v1_send_failed(resource()); break;
    case Succeeded:
