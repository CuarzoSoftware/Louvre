...
class Louvre::Protocols::WlrOutputManagement::RWlrOutputConfiguration final
    : public LResource {
 public:
  enum Reply { Failed, Succeeded, Cancelled };
  ...
  static void enable_head(wl_client *client, wl_resource *resource, UInt32 id,
                          wl_resource *head);
  static void disable_head(wl_client *client, wl_resource *resource,
                           wl_resource *head);
 ...
 private:
  ...
  RWlrOutputConfiguration(GWlrOutputManager *wlrOutputManager, UInt32 id,
                          UInt32 serial) noexcept;
