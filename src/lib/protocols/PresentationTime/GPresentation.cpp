...
static const struct wp_presentation_interface imp{
    .destroy = &GPresentation::destroy, .feedback = &GPresentation::feedback};

void GPresentation::bind(wl_client *client, void * /*data*/, UInt32 version,
                         UInt32 id) noexcept {
...
GPresentation::GPresentation(wl_client *client, Int32 version,
                             UInt32 id) noexcept
  ...
  if (seat()->outputs().empty())
    ...
    clockId(compositor()->imp()->graphicBackend->outputGetClock(
        seat()->outputs().front()));
}
...
void GPresentation::destroy(wl_client * /*client*/,
                            wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void GPresentation::feedback(wl_client * /*client*/, wl_resource *resource,
                             wl_resource *surface, UInt32 id) noexcept {
  new RPresentationFeedback(
      static_cast<GPresentation *>(wl_resource_get_user_data(resource)),
