...
RXdgActivationToken::RXdgActivationToken(GXdgActivation *xdgActivationRes,
                                         UInt32 id) noexcept
    : LResource(xdgActivationRes->client(), &xdg_activation_token_v1_interface,
                xdgActivationRes->version(), id, &imp) {}

/******************** REQUESTS ********************/

void RXdgActivationToken::set_serial(wl_client * /*client*/,
                                     wl_resource *resource, UInt32 serial,
                                     wl_resource * /*seat*/) {
  auto &res{
      *static_cast<RXdgActivationToken *>(wl_resource_get_user_data(resource))};

  if (res.m_commited) {
    res.postError(XDG_ACTIVATION_TOKEN_V1_ERROR_ALREADY_USED,
                  "Duplicated commit.");
    return;
  }
...
void RXdgActivationToken::set_app_id(wl_client * /*client*/,
...
void RXdgActivationToken::set_surface(wl_client * /*client*/,
                                      wl_resource *resource,
                                      wl_resource *surface) {
  auto &res{
      *static_cast<RXdgActivationToken *>(wl_resource_get_user_data(resource))};
  ...
  auto &surfaceRes{*static_cast<Protocols::Wayland::RSurface *>(
...
static std::string randomToken() noexcept {
  ...
  static std::string dic{
      "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"};
  static char random[17];

retry:

  for (int i = 0; i < 16; i++) random[i] = dic[rand() % dic.length()];

  random[16] = '\0';

  std::string token = std::to_string(n) + "_" + random;

  if (activationTokenManager()->tokens().contains(token)) goto retry;

  return token;
}

void RXdgActivationToken::commit(wl_client * /*client*/,
                                 wl_resource *resource) {
  auto &res{
      *static_cast<RXdgActivationToken *>(wl_resource_get_user_data(resource))};
  ...
  auto pair = activationTokenManager()
                  ->m_tokens
                  .emplace(randomToken(),
                           new LActivationToken(res.client(), res.m_surface,
                                                event ? event->copy() : nullptr,
                                                std::move(res.m_appId)))
                  .first;
...
void RXdgActivationToken::destroy(wl_client * /*client*/,
                                  wl_resource *resource) {
