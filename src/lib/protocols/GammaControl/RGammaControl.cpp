...
RGammaControl::RGammaControl(Wayland::GOutput *outputRes, Int32 version,
                             UInt32 id) noexcept
    : LResource(outputRes->client(), &zwlr_gamma_control_v1_interface, version,
                id, &imp),
...
void RGammaControl::set_gamma(wl_client * /*client*/, wl_resource *resource,
                              Int32 fd) {
  auto *res{static_cast<RGammaControl *>(wl_resource_get_user_data(resource))};

  if (!res->outputRes() || !res->outputRes()->output() ||
      res->outputRes()->output()->gammaSize() == 0) {
    res->failed();
    close(fd);
    return;
  }
  ...
  if (n < 0) {
    ...
    res->postError(ZWLR_GAMMA_CONTROL_V1_ERROR_INVALID_GAMMA,
                   "Invalid gamma size");
    return;
  }
