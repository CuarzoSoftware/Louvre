...
RBackgroundBlur::RBackgroundBlur(
    ...
    Wayland::RSurface *surfaceRes, UInt32 id, Int32 version) noexcept
    : LResource(surfaceRes->client(), &lvr_background_blur_interface, version,
                id, &imp),
...
void RBackgroundBlur::destroy(wl_client * /*client*/, wl_resource *resource) {
  auto &res{
      *static_cast<RBackgroundBlur *>(wl_resource_get_user_data(resource))};

  if (!res.surfaceRes()) {
    res.postError(LVR_BACKGROUND_BLUR_ERROR_DESTROYED_SURFACE,
...
void RBackgroundBlur::ack_configure(wl_client * /*client*/,
  ...
  while (!blur.m_sentConfigurations.empty()) {
    if (blur.m_sentConfigurations.front().serial == serial) {
      ...
      blur.pendingProps().colorHint =
          blur.m_sentConfigurations.front().colorHint;
  ...
  res.postError(LVR_BACKGROUND_BLUR_ERROR_INVALID_SERIAL,
                "invalid configure serial");
}

void RBackgroundBlur::set_region(wl_client * /*client*/, wl_resource *resource,
                                 wl_resource *region) {
  auto &res{
      *static_cast<RBackgroundBlur *>(wl_resource_get_user_data(resource))};
  ...
  if (region) {
    auto &rRegion{*static_cast<Protocols::Wayland::RRegion *>(
...
void RBackgroundBlur::clear_mask(wl_client * /*client*/,
  ...
  // Noop
  if (res.m_maskingCapabilities.get() == 0) return;

  auto &blur{*res.surfaceRes()->surface()->backgroundBlur()};

  if (blur.pendingProps().maskType == LBackgroundBlur::NoMask) return;
...
void RBackgroundBlur::set_round_rect_mask(wl_client * /*client*/,
                                          wl_resource *resource, Int32 x,
                                          Int32 y, Int32 width, Int32 height,
                                          Int32 radTL, Int32 radTR, Int32 radBR,
                                          Int32 radBL) {
  auto &res{
      *static_cast<RBackgroundBlur *>(wl_resource_get_user_data(resource))};
  ...
  if (!res.m_maskingCapabilities.check(LBackgroundBlur::RoundRectMaskCap)) {
    res.postError(LVR_BACKGROUND_BLUR_ERROR_UNSUPPORTED_MASK,
  ...
  if (!newRRect.isValid()) {
    res.postError(LVR_BACKGROUND_BLUR_ERROR_INVALID_ROUND_RECT,
                  "invalid round rect size or radii");
    return;
  }

  if (blur.pendingProps().maskType == LBackgroundBlur::RoundRect &&
...
void RBackgroundBlur::set_svg_path_mask(wl_client * /*client*/,
                                        wl_resource *resource,
                                        wl_resource *svgPath) {
  auto &res{
      *static_cast<RBackgroundBlur *>(wl_resource_get_user_data(resource))};
  ...
  auto &blur{*res.surfaceRes()->surface()->backgroundBlur()};
  auto &rSvgPath{*static_cast<Protocols::SvgPath::RSvgPath *>(
      wl_resource_get_user_data(svgPath))};

  if (!rSvgPath.isComplete()) {
    res.postError(LVR_BACKGROUND_BLUR_ERROR_INVALID_SVG_PATH,
                  "incomplete svg path");
    return;
  }
