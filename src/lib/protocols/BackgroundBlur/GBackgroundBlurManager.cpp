...
void GBackgroundBlurManager::bind(wl_client *client, void * /*data*/,
...
GBackgroundBlurManager::GBackgroundBlurManager(wl_client *client, Int32 version,
                                               UInt32 id) noexcept
  ...
  lvr_background_blur_manager_send_masking_capabilities(
      resource(), LBackgroundBlur::maskingCapabilities.get());
}

GBackgroundBlurManager::~GBackgroundBlurManager() noexcept {
  LVectorRemoveOneUnordered(client()->imp()->backgroundBlurManagerGlobals,
                            this);
}

/******************** REQUESTS ********************/

void GBackgroundBlurManager::destroy(wl_client * /*client*/,
                                     wl_resource *resource) {
  wl_resource_destroy(resource);
}

void GBackgroundBlurManager::get_background_blur(wl_client * /*client*/,
                                                 wl_resource *resource,
                                                 UInt32 id,
                                                 wl_resource *surface) {
  auto *res{static_cast<GBackgroundBlurManager *>(
  ...
  new RBackgroundBlur(res->m_maskingCapabilities, surfaceRes, id,
