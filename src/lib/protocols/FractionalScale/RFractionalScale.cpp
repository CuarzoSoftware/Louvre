...
RFractionalScale::RFractionalScale(Wayland::RSurface *surfaceRes, UInt32 id,
                                   Int32 version) noexcept
    : LResource(surfaceRes->client(), &wp_fractional_scale_v1_interface,
                version, id, &imp),
...
void RFractionalScale::destroy(wl_client * /*client*/,
...
void RFractionalScale::preferredScale(Float32 scale) noexcept {
  ...
  m_lastScale = scale;
  wp_fractional_scale_v1_send_preferred_scale(resource(),
                                              (UInt32)(roundf(scale * 120.f)));
}
