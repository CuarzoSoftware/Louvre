...
RTearingControl::RTearingControl(Wayland::RSurface *surfaceRes, Int32 version,
                                 UInt32 id) noexcept
    : LResource(surfaceRes->client(), &wp_tearing_control_v1_interface, version,
                id, &imp),
...
void RTearingControl::destroy(wl_client * /*client*/,
                              wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void RTearingControl::set_presentation_hint(wl_client * /*client*/,
                                            wl_resource *resource,
                                            UInt32 hint) noexcept {
  auto &res{
      *static_cast<RTearingControl *>(wl_resource_get_user_data(resource))};
