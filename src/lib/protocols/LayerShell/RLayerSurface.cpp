...
RLayerSurface::RLayerSurface(UInt32 id, GLayerShell *layerShellRes,
                             LSurface *surface, LOutput *output,
                             LSurfaceLayer layer, const char *scope) noexcept
    : LResource(layerShellRes->client(), &zwlr_layer_surface_v1_interface,
                layerShellRes->version(), id, &imp) {
...
void RLayerSurface::set_size(wl_client * /*client*/, wl_resource *resource,
...
void RLayerSurface::set_anchor(wl_client * /*client*/, wl_resource *resource,
                               UInt32 anchor) {
...
void RLayerSurface::set_exclusive_zone(wl_client * /*client*/,
                                       wl_resource *resource, Int32 zone) {
  auto &res{*static_cast<RLayerSurface *>(wl_resource_get_user_data(resource))};
  res.layerRole()->pendingAtoms().exclusiveZoneSize = zone;
  res.layerRole()->m_flags.add(LLayerRole::HasPendingExclusiveZone);
}

void RLayerSurface::set_margin(wl_client * /*client*/, wl_resource *resource,
...
void RLayerSurface::set_keyboard_interactivity(wl_client * /*client*/,
                                               wl_resource *resource,
                                               UInt32 keyboard_interactivity) {
  auto &res{*static_cast<RLayerSurface *>(wl_resource_get_user_data(resource))};

  if (keyboard_interactivity > 2) {
    res.postError(ZWLR_LAYER_SURFACE_V1_ERROR_INVALID_KEYBOARD_INTERACTIVITY,
                  "Invalid keyboard interactivity value.");
    return;
  }

  res.layerRole()->pendingAtoms().keyboardInteractivity =
      (LLayerRole::KeyboardInteractivity)keyboard_interactivity;
  res.layerRole()->m_flags.add(LLayerRole::HasPendingKeyboardInteractivity);
}

void RLayerSurface::get_popup(wl_client * /*client*/, wl_resource *resource,
                              wl_resource *popup) {
  auto &popupSurface{
      *static_cast<XdgShell::RXdgPopup *>(wl_resource_get_user_data(popup))
           ->xdgSurfaceRes()
           ->surface()};
  auto &res{*static_cast<RLayerSurface *>(wl_resource_get_user_data(resource))};

  if (popupSurface.imp()->parent || popupSurface.imp()->pendingParent) {
    res.postError(XDG_WM_BASE_ERROR_INVALID_POPUP_PARENT,
                  "Popup already has a parent.");
    return;
  }

  if (popupSurface.imp()->isInChildrenOrPendingChildren(
          res.layerRole()->surface())) {
    res.postError(XDG_WM_BASE_ERROR_INVALID_POPUP_PARENT,
                  "Popup can not have a child surface as parent.");
    return;
  }
...
void RLayerSurface::ack_configure(wl_client * /*client*/,
                                  wl_resource * /*resource*/,
                                  UInt32 /*serial*/) {}
...
#if LOUVRE_LAYER_SHELL_VERSION >= 2
void RLayerSurface::set_layer(wl_client * /*client*/, wl_resource *resource,
                              UInt32 layer) {
  auto &res{*static_cast<RLayerSurface *>(wl_resource_get_user_data(resource))};

  if (layer > 3) {
    res.postError(ZWLR_LAYER_SHELL_V1_ERROR_INVALID_LAYER,
                  "Invalid layer value.");
    return;
  }

  res.layerRole()->pendingAtoms().layer =
      static_cast<LSurfaceLayer>(layer < 2 ? layer : layer + 1);
  res.layerRole()->m_flags.add(LLayerRole::HasPendingLayer);
}
#endif

#if LOUVRE_LAYER_SHELL_VERSION >= 5
void RLayerSurface::set_exclusive_edge(wl_client * /*client*/,
                                       wl_resource *resource, UInt32 edge) {
  auto &res{*static_cast<RLayerSurface *>(wl_resource_get_user_data(resource))};

  if (!(edge == 0 || edge == 1 || edge == 2 || edge == 4 || edge == 8)) {
    res.postError(ZWLR_LAYER_SURFACE_V1_ERROR_INVALID_EXCLUSIVE_EDGE,
