...
void GLayerShell::bind(wl_client *client, void * /*data*/, UInt32 version,
                       UInt32 id) noexcept {
...
GLayerShell::GLayerShell(wl_client *client, Int32 version, UInt32 id) noexcept
...
void GLayerShell::get_layer_surface(wl_client * /*client*/,
  ...
  if (layer > 3) {
    surfaceRes.postError(ZWLR_LAYER_SHELL_V1_ERROR_INVALID_LAYER,
                         "Invalid layer value.");
    return;
  }

  if (!surfaceRes.surface()->imp()->canHostRole()) {
    surfaceRes.postError(ZWLR_LAYER_SHELL_V1_ERROR_ROLE,
                         "Given wl_surface has another role.");
    return;
  }

  if (surfaceRes.surface()->imp()->hasBufferOrPendingBuffer()) {
    surfaceRes.postError(ZWLR_LAYER_SHELL_V1_ERROR_ALREADY_CONSTRUCTED,
                         "wl_surface has a buffer attached or committed.");
    return;
  }

  const LSurfaceLayer surfaceLayer{
      static_cast<LSurfaceLayer>(layer < 2 ? layer : layer + 1)};

  new RLayerSurface(
      id, static_cast<GLayerShell *>(wl_resource_get_user_data(resource)),
      surfaceRes.surface(),
      output == NULL
          ? nullptr
          : static_cast<Wayland::GOutput *>(wl_resource_get_user_data(output))
                ->output(),
      surfaceLayer, name_space);
}

#if LOUVRE_LAYER_SHELL_VERSION >= 3
void GLayerShell::destroy(wl_client * /*client*/,
