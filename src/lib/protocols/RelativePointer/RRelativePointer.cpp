...
RRelativePointer::RRelativePointer(Wayland::RPointer *pointerRes, Int32 id,
                                   UInt32 version) noexcept
    : LResource(pointerRes->client(), &zwp_relative_pointer_v1_interface,
                version, id, &imp),
...
void RRelativePointer::destroy(wl_client * /*client*/,
                               wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void RRelativePointer::relativeMotion(
    const Louvre::LPointerMoveEvent &event) noexcept {
  zwp_relative_pointer_v1_send_relative_motion(
      resource(), event.us() >> 32, event.us() & 0xffffffff,
