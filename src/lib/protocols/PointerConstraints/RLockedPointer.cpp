...
RLockedPointer::RLockedPointer(GPointerConstraints *pointerConstraintsRes,
                               LSurface *surface, Wayland::RPointer *pointerRes,
                               Wayland::RRegion *regionRes, UInt32 lifetime,
                               UInt32 id) noexcept
    : LResource(pointerConstraintsRes->client(),
                &zwp_locked_pointer_v1_interface,
                pointerConstraintsRes->version(), id, &imp),
  ...
  if (regionRes) {
    m_surface->imp()->pendingPointerConstraintRegion =
        std::make_unique<LRegion>(regionRes->region());
    ...
    m_surface->imp()->pointerConstraintRegion =
        m_surface->imp()->currentInputRegion;
  }
...
RLockedPointer::~RLockedPointer() {
  if (surface()) {
    ...
    surface()->imp()->changesToNotify.remove(
...
void RLockedPointer::set_cursor_position_hint(wl_client * /*client*/,
                                              wl_resource *resource, Float24 x,
                                              Float24 y) noexcept {
  auto &res{
      *static_cast<RLockedPointer *>(wl_resource_get_user_data(resource))};

  if (res.surface()) {
    res.surface()->imp()->pending.lockedPointerPosHint.setX(
        wl_fixed_to_double(x));
    res.surface()->imp()->pending.lockedPointerPosHint.setY(
        wl_fixed_to_double(y));
    res.surface()->imp()->changesToNotify.add(
        LSurface::LSurfacePrivate::LockedPointerPosHintChanged);
  }
}

void RLockedPointer::set_region(wl_client * /*client*/, wl_resource *resource,
  ...
  if (res.surface()) {
    if (region) {
      auto *regionRes{
          static_cast<Wayland::RRegion *>(wl_resource_get_user_data(region))};
      res.surface()->imp()->pendingPointerConstraintRegion =
          std::make_unique<LRegion>(regionRes->region());
    } else
