...
RContentType::RContentType(Wayland::RSurface *surfaceRes, UInt32 id,
                           Int32 version) noexcept
    : LResource(surfaceRes->client(), &wp_content_type_v1_interface, version,
                id, &imp),
...
void RContentType::set_content_type(wl_client * /*client*/,
                                    wl_resource *resource, UInt32 type) {
  auto &res{*static_cast<RContentType *>(wl_resource_get_user_data(resource))};

  if (res.surfaceRes())
    res.surfaceRes()->surface()->imp()->pending.contentType =
        static_cast<LContentType>(type);
}
