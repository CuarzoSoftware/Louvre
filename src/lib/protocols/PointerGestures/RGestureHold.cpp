...
RGestureHold::RGestureHold(Wayland::RPointer *pointerRes, Int32 id,
                           UInt32 version) noexcept
    : LResource(pointerRes->client(), &zwp_pointer_gesture_hold_v1_interface,
                version, id, &imp),
...
void RGestureHold::destroy(wl_client * /*client*/,
...
void RGestureHold::begin(const LPointerHoldBeginEvent &event,
  ...
  zwp_pointer_gesture_hold_v1_send_begin(resource(), event.serial(), event.ms(),
                                         surfaceRes->resource(),
                                         event.fingers());
}
...
void RGestureHold::end(const LPointerHoldEndEvent &event) noexcept {
  ...
  if (event.fingers() == 0)
    clientPointerEvents.holdEnd.setFingers(
        clientPointerEvents.holdBegin.fingers());

  if (event.device() == &compositor()->imp()->fakeDevice)
    clientPointerEvents.holdEnd.setDevice(
        clientPointerEvents.holdBegin.device());

  zwp_pointer_gesture_hold_v1_send_end(resource(), event.serial(), event.ms(),
                                       event.cancelled());
}
