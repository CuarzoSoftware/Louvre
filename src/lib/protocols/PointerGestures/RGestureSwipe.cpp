...
RGestureSwipe::RGestureSwipe(Wayland::RPointer *pointerRes, Int32 id,
                             UInt32 version) noexcept
    : LResource(pointerRes->client(), &zwp_pointer_gesture_swipe_v1_interface,
                version, id, &imp),
...
void RGestureSwipe::destroy(wl_client * /*client*/,
                            wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void RGestureSwipe::begin(const LPointerSwipeBeginEvent &event,
  ...
  zwp_pointer_gesture_swipe_v1_send_begin(resource(), event.serial(),
...
void RGestureSwipe::end(const LPointerSwipeEndEvent &event) noexcept {
  ...
  if (event.fingers() == 0)
    clientPointerEvents.swipeEnd.setFingers(
        clientPointerEvents.swipeBegin.fingers());

  if (event.device() == &compositor()->imp()->fakeDevice)
    clientPointerEvents.swipeEnd.setDevice(
        clientPointerEvents.swipeBegin.device());

  zwp_pointer_gesture_swipe_v1_send_end(resource(), event.serial(), event.ms(),
                                        event.cancelled());
}
