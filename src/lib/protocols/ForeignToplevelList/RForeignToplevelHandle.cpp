...
RForeignToplevelHandle::RForeignToplevelHandle(
    GForeignToplevelList &foreignToplevelListRes, LToplevelRole &toplevelRole)
    : LResource(foreignToplevelListRes.client(),
                &ext_foreign_toplevel_handle_v1_interface,
                foreignToplevelListRes.version(), 0, &imp),
  ...
  ext_foreign_toplevel_list_v1_send_toplevel(foreignToplevelListRes.resource(),
                                             resource());
...
RForeignToplevelHandle::~RForeignToplevelHandle() {
  ...
  if (m_toplevelRole)
    LVectorRemoveOneUnordered(m_toplevelRole->m_foreignToplevelHandles, this);
}

bool RForeignToplevelHandle::canSendParams() const noexcept {
  return !m_closed && m_toplevelRole && m_foreignToplevelListRes &&
         !m_foreignToplevelListRes->finishedSent();
}
...
void RForeignToplevelHandle::done() noexcept {
  if (canSendParams()) ext_foreign_toplevel_handle_v1_send_done(resource());
}
...
void RForeignToplevelHandle::identifier(
    const std::string &identifier) noexcept {
  if (canSendParams())
    ext_foreign_toplevel_handle_v1_send_identifier(resource(),
                                                   identifier.c_str());
}
