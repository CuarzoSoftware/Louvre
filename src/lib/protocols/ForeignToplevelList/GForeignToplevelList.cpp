...
#include <protocols/ForeignToplevelList/ext-foreign-toplevel-list-v1.h>
...
void GForeignToplevelList::bind(wl_client *client, void * /*data*/,
...
GForeignToplevelList::GForeignToplevelList(wl_client *client, Int32 version,
                                           UInt32 id) noexcept
    : LResource(client, interface(), version, id, &imp) {
  this->client()->imp()->foreignToplevelListGlobals.emplace_back(this);

  for (LSurface *s : compositor()->surfaces())
    if (s->toplevel() && s->mapped()) toplevel(*s->toplevel());
}

GForeignToplevelList::~GForeignToplevelList() noexcept {
  if (!m_finished)
    LVectorRemoveOneUnordered(client()->imp()->foreignToplevelListGlobals,
                              this);
}
...
const std::vector<GForeignToplevelList *>::iterator
GForeignToplevelList::finished() {
  if (m_finished) return client()->imp()->foreignToplevelListGlobals.end();

  m_finished = true;
  ext_foreign_toplevel_list_v1_send_finished(resource());

  auto it{client()->imp()->foreignToplevelListGlobals.erase(
      std::find(client()->imp()->foreignToplevelListGlobals.begin(),
                client()->imp()->foreignToplevelListGlobals.end(), this))};
...
void GForeignToplevelList::stop(wl_client * /*client*/,
                                wl_resource *resource) noexcept {
  auto &res{*static_cast<GForeignToplevelList *>(
      wl_resource_get_user_data(resource))};
  res.m_stopped = true;
}

void GForeignToplevelList::destroy(wl_client * /*client*/,
                                   wl_resource *resource) noexcept {
