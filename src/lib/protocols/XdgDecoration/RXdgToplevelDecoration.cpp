...
RXdgToplevelDecoration::RXdgToplevelDecoration(
    GXdgDecorationManager *xdgDecorationManagerRes, LToplevelRole *toplevelRole,
    UInt32 id) noexcept
    : LResource(xdgDecorationManagerRes->client(),
                &zxdg_toplevel_decoration_v1_interface,
                xdgDecorationManagerRes->version(), id, &imp),
...
RXdgToplevelDecoration::~RXdgToplevelDecoration() {
  if (toplevelRole() &&
      toplevelRole()->decorationMode() == LToplevelRole::ServerSide) {
...
void RXdgToplevelDecoration::destroy(wl_client * /*client*/,
                                     wl_resource *resource) {
  auto &xdgToplevelDecorationRes{*static_cast<RXdgToplevelDecoration *>(
      wl_resource_get_user_data(resource))};

  if (!xdgToplevelDecorationRes.toplevelRole()) {
    xdgToplevelDecorationRes.postError(
        ZXDG_TOPLEVEL_DECORATION_V1_ERROR_ORPHANED,
        "Toplevel destroyed before decoration.");
    return;
  }
...
void RXdgToplevelDecoration::set_mode(wl_client * /*client*/,
                                      wl_resource *resource, UInt32 mode) {
  auto &xdgToplevelDecorationRes{*static_cast<RXdgToplevelDecoration *>(
      wl_resource_get_user_data(resource))};
  ...
  if (xdgToplevelDecorationRes.toplevelRole()->preferredDecorationMode() !=
      mode) {
    xdgToplevelDecorationRes.toplevelRole()->m_preferredDecorationMode =
        (LToplevelRole::DecorationMode)mode;
    xdgToplevelDecorationRes.toplevelRole()->preferredDecorationModeChanged();
  }

  if (!xdgToplevelDecorationRes.toplevelRole()->m_flags.check(
          LToplevelRole::HasDecorationModeToSend))
    xdgToplevelDecorationRes.toplevelRole()->configureDecorationMode(
        xdgToplevelDecorationRes.toplevelRole()
            ->pendingConfiguration()
            .decorationMode);
}

void RXdgToplevelDecoration::unset_mode(wl_client * /*client*/,
                                        wl_resource *resource) {
  auto &xdgToplevelDecorationRes{*static_cast<RXdgToplevelDecoration *>(
      wl_resource_get_user_data(resource))};
