...
RSessionLock::RSessionLock(GSessionLockManager *sessionLockManagerRes,
                           UInt32 id) noexcept
    : LResource(sessionLockManagerRes->client(), &ext_session_lock_v1_interface,
                sessionLockManagerRes->version(), id, &imp) {
  /* Wait for the client to create surfaces for each output, then ask the user
   */

  if (compositor()->sessionLockManager()->state() ==
      LSessionLockManager::Locked)
    finished();
  else {
    m_timer.setCallback([this](auto) { makeLockRequest(); });
...
void RSessionLock::destroy(wl_client * /*client*/, wl_resource *resource) {
  ...
  if (res.reply() == RSessionLock::Locked) {
    res.postError(EXT_SESSION_LOCK_V1_ERROR_INVALID_DESTROY,
...
void RSessionLock::get_lock_surface(wl_client * /*client*/,
  ...
  auto &outputRes{
      *static_cast<Wayland::GOutput *>(wl_resource_get_user_data(output))};

  if (!surfaceRes.surface()->imp()->canHostRole()) {
    res.postError(EXT_SESSION_LOCK_V1_ERROR_ROLE,
                  "Given wl_surface already has a role.");
    return;
  }

  if (surfaceRes.surface()->imp()->hasBufferOrPendingBuffer()) {
    res.postError(EXT_SESSION_LOCK_V1_ERROR_ALREADY_CONSTRUCTED,
                  "Given wl_surface has a buffer attached or committed.");
    return;
  }

  for (auto *role : res.m_roles) {
    if (role->exclusiveOutput() && outputRes.output() &&
        role->exclusiveOutput() == outputRes.output()) {
      res.postError(EXT_SESSION_LOCK_V1_ERROR_DUPLICATE_OUTPUT,
...
void RSessionLock::unlock_and_destroy(wl_client * /*client*/,
                                      wl_resource *resource) {
  auto &res{*static_cast<RSessionLock *>(wl_resource_get_user_data(resource))};

  if (!res.m_lockedOnce) {
    res.postError(EXT_SESSION_LOCK_V1_ERROR_INVALID_UNLOCK,
