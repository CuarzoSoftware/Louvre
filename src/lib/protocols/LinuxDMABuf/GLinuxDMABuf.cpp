...
void GLinuxDMABuf::bind(wl_client *client, void * /*data*/, UInt32 version,
                        UInt32 id) noexcept {
...
GLinuxDMABuf::GLinuxDMABuf(wl_client *client, Int32 version, UInt32 id) noexcept
  ...
  if (version < 3) {
    ...
    for (const LDMAFormat &dmaFormat :
    ...
    for (const LDMAFormat &dmaFormat :
         *compositor()->imp()->graphicBackend->backendGetDMAFormats())
      modifier(dmaFormat.format, dmaFormat.modifier >> 32,
...
void GLinuxDMABuf::destroy(wl_client * /*client*/,
                           wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void GLinuxDMABuf::create_params(wl_client * /*client*/, wl_resource *resource,
                                 UInt32 id) noexcept {
  new RLinuxBufferParams(
      static_cast<GLinuxDMABuf *>(wl_resource_get_user_data(resource)), id);
}

#if LOUVRE_LINUX_DMA_BUF_VERSION >= 4
void GLinuxDMABuf::get_default_feedback(wl_client * /*client*/,
                                        wl_resource *resource, UInt32 id) {
  new RLinuxDMABufFeedback(
      static_cast<GLinuxDMABuf *>(wl_resource_get_user_data(resource)), id);
}
void GLinuxDMABuf::get_surface_feedback(wl_client * /*client*/,
                                        wl_resource *resource, UInt32 id,
                                        wl_resource * /*surface*/) {
  new RLinuxDMABufFeedback(
      static_cast<GLinuxDMABuf *>(wl_resource_get_user_data(resource)), id);
...
bool GLinuxDMABuf::modifier(UInt32 format, UInt32 mod_hi,
                            UInt32 mod_lo) noexcept {
#if LOUVRE_LINUX_DMA_BUF_VERSION >= 3
  if (version() >= 3) {
    zwp_linux_dmabuf_v1_send_modifier(resource(), format, mod_hi, mod_lo);
    return true;
  }
#endif
