...
RLinuxBufferParams::RLinuxBufferParams(GLinuxDMABuf *linuxDMABufRes,
                                       UInt32 id) noexcept
    : LResource(linuxDMABufRes->client(), &zwp_linux_buffer_params_v1_interface,
                linuxDMABufRes->version(), id, &imp),
      m_dmaPlanes(std::make_unique<LDMAPlanes>()) {}

bool RLinuxBufferParams::createCommon(Int32 width, Int32 height, UInt32 format,
...
void RLinuxBufferParams::destroy(wl_client * /*client*/,
                                 wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void RLinuxBufferParams::add(wl_client * /*client*/, wl_resource *resource,
                             Int32 fd, UInt32 plane_idx, UInt32 offset,
                             UInt32 stride, UInt32 modifier_hi,
                             UInt32 modifier_lo) noexcept {
  auto *res{
      static_cast<RLinuxBufferParams *>(wl_resource_get_user_data(resource))};
  ...
  res->m_dmaPlanes->modifiers[plane_idx] =
      (static_cast<UInt64>(modifier_hi) << 32) |
      static_cast<UInt64>(modifier_lo);
}

void RLinuxBufferParams::create(wl_client * /*client*/, wl_resource *resource,
                                Int32 width, Int32 height, UInt32 format,
                                UInt32 flags) noexcept {
  auto *res{
      static_cast<RLinuxBufferParams *>(wl_resource_get_user_data(resource))};
  ...
  // Ignore the leak warning, the buffer is destroyed when the client releases
  // it or is disconnected
}

#if LOUVRE_LINUX_DMA_BUF_VERSION >= 2
void RLinuxBufferParams::create_immed(wl_client * /*client*/,
                                      wl_resource *resource, UInt32 buffer_id,
                                      Int32 width, Int32 height, UInt32 format,
                                      UInt32 flags) noexcept {
  auto *res{
      static_cast<RLinuxBufferParams *>(wl_resource_get_user_data(resource))};
