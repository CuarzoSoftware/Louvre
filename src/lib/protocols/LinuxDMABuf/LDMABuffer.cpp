...
static const struct wl_buffer_interface imp{.destroy = &LDMABuffer::destroy};

LDMABuffer::LDMABuffer(RLinuxBufferParams *bufferParamsRes, UInt32 id) noexcept
    : LResource(bufferParamsRes->client(), &wl_buffer_interface, 1, id, &imp),
...
LDMABuffer::~LDMABuffer() noexcept {
  if (texture()) {
    ...
    for (UInt32 i = 0; i < planes()->num_fds; i++) close(planes()->fds[i]);
  }
}
...
void LDMABuffer::destroy(wl_client * /*client*/,
