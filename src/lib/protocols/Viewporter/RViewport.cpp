...
RViewport::RViewport(Wayland::RSurface *surfaceRes, Int32 version,
                     UInt32 id) noexcept
    : LResource(surfaceRes->client(), &wp_viewport_interface, version, id,
                &imp),
...
void RViewport::destroy(wl_client * /*client*/,
                        wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void RViewport::set_source(wl_client * /*client*/, wl_resource *resource,
                           Float24 x, Float24 y, Float24 width,
                           Float24 height) noexcept {
  auto &res{*static_cast<RViewport *>(wl_resource_get_user_data(resource))};

  if (!res.surfaceRes()) {
    res.postError(WP_VIEWPORT_ERROR_NO_SURFACE,
...
void RViewport::set_destination(wl_client * /*client*/, wl_resource *resource,
