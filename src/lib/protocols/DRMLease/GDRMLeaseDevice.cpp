...
void GDRMLeaseDevice::bind(wl_client *client, void *data, UInt32 version,
                           UInt32 id) noexcept {
...
GDRMLeaseDevice::GDRMLeaseDevice(LGPU *gpu, wl_client *client, Int32 version,
                                 UInt32 id) noexcept
    : LResource(client, interface(), version, id, &imp), m_gpu(gpu) {
  this->client()->imp()->drmLeaseDeviceGlobals.emplace_back(this);
  drmFd();
  for (LOutput *output : seat()->outputs()) connector(output);
  done();
}
...
void GDRMLeaseDevice::drmFd() noexcept {
  if (m_roFDSent || !m_gpu || m_gpu->roFd() < 0) return;
...
bool GDRMLeaseDevice::connector(LOutput *output) noexcept {
  if (!output->leasable()) return false;

  for (RDRMLeaseConnector *conn : m_connectors)
    if (conn->m_output == output) return false;
...
void GDRMLeaseDevice::create_lease_request(wl_client * /*client*/,
