...
static const struct wp_drm_lease_v1_interface imp{.destroy =
                                                      &RDRMLease::destroy};

RDRMLease::RDRMLease(RDRMLeaseRequest *leaseRequestRes, UInt32 id)
    : LResource(leaseRequestRes->client(), &wp_drm_lease_v1_interface,
                leaseRequestRes->version(), id, &imp) {
  for (auto &output : leaseRequestRes->requestedConnectors()) {
    if (!output.get() || !output->leasable()) continue;

    if (output->leaseRequest(client())) m_leasedConnectors.emplace_back(output);
  }
  ...
  const int fd{compositor()->imp()->graphicBackend->backendCreateLease(
      m_leasedConnectors)};
  ...
  for (LOutput *output : m_leasedConnectors) {
    ...
    output->imp()->lease.setOnDestroyCallback(
        [output](auto) { output->leaseChanged(); });

    output->leaseChanged();

    if (!ref) break;  // The user called finished()
  }
}
