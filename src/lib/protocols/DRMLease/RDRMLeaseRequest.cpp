...
RDRMLeaseRequest::RDRMLeaseRequest(GDRMLeaseDevice *drmLeaseDeviceRes,
                                   UInt32 id)
    : LResource(drmLeaseDeviceRes->client(), &wp_drm_lease_request_v1_interface,
                drmLeaseDeviceRes->version(), id, &imp),
      m_gpu(drmLeaseDeviceRes->gpu()) {}

/******************** REQUESTS ********************/

void RDRMLeaseRequest::request_connector(wl_client * /*client*/,
  ...
  if (output->gpu() != res.gpu()) {
    res.postError(WP_DRM_LEASE_REQUEST_V1_ERROR_WRONG_DEVICE,
  ...
  for (auto &conn : res.requestedConnectors()) {
    if (conn.get() == output) {
      res.postError(WP_DRM_LEASE_REQUEST_V1_ERROR_DUPLICATE_CONNECTOR,
...
void RDRMLeaseRequest::submit(wl_client * /*client*/, wl_resource *resource,
                              UInt32 id) {
  auto &res{LRES_CAST(RDRMLeaseRequest, resource)};

  if (!res.m_addedConnector) {
    res.postError(WP_DRM_LEASE_REQUEST_V1_ERROR_EMPTY_LEASE,
