...
RDRMLeaseConnector::RDRMLeaseConnector(GDRMLeaseDevice *drmLeaseDeviceRes,
                                       LOutput *output)
    : LResource(drmLeaseDeviceRes->client(),
                &wp_drm_lease_connector_v1_interface,
                drmLeaseDeviceRes->version(), 0, &imp),
      m_drmLeaseDeviceRes(drmLeaseDeviceRes),
      m_output(output) {
  wp_drm_lease_device_v1_send_connector(m_drmLeaseDeviceRes->resource(),
                                        resource());
...
void RDRMLeaseConnector::destroy(wl_client * /*client*/,
...
void RDRMLeaseConnector::description() noexcept {
  assert(m_output != nullptr);
  wp_drm_lease_connector_v1_send_description(resource(),
