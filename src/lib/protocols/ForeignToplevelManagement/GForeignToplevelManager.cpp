...
void GForeignToplevelManager::bind(wl_client *client, void * /*data*/,
...
GForeignToplevelManager::GForeignToplevelManager(wl_client *client,
                                                 Int32 version,
                                                 UInt32 id) noexcept
  ...
  for (LSurface *s : compositor()->surfaces())
    if (s->toplevel() &&
        !s->toplevel()->m_flags.check(LToplevelRole::HasPendingFirstMap))
      toplevel(*s->toplevel());
}

GForeignToplevelManager::~GForeignToplevelManager() noexcept {
  if (!m_finished)
    LVectorRemoveOneUnordered(client()->imp()->foreignToplevelManagerGlobals,
                              this);
}
...
void GForeignToplevelManager::toplevel(LToplevelRole &toplevelRole) {
  if (m_stopped || !toplevelRole.foreignControllerFilter(this)) return;

  new RForeignToplevelHandle(*this, toplevelRole);
}

const std::vector<GForeignToplevelManager *>::iterator
GForeignToplevelManager::finished() {
  if (m_finished) return client()->imp()->foreignToplevelManagerGlobals.end();

  m_finished = true;
  zwlr_foreign_toplevel_manager_v1_send_finished(resource());

  auto it{client()->imp()->foreignToplevelManagerGlobals.erase(
      std::find(client()->imp()->foreignToplevelManagerGlobals.begin(),
                client()->imp()->foreignToplevelManagerGlobals.end(), this))};
...
void GForeignToplevelManager::stop(wl_client * /*client*/,
                                   wl_resource *resource) noexcept {
  auto &res{*static_cast<GForeignToplevelManager *>(
