...
RForeignToplevelHandle::RForeignToplevelHandle(
    ...
    : LResource(foreignToplevelManagerRes.client(),
                &zwlr_foreign_toplevel_handle_v1_interface,
                foreignToplevelManagerRes.version(), 0, &imp),
  ...
  zwlr_foreign_toplevel_manager_v1_send_toplevel(
      foreignToplevelManagerRes.resource(), resource());

  m_toplevelRole.setOnDestroyCallback([this](auto) { closed(); });

  title(toplevelRole.title());
  appId(toplevelRole.appId());

  for (LOutput *o : toplevelRole.surface()->outputs()) outputEnter(o);

  updateState();

  if (toplevelRole.surface()->parent() &&
      toplevelRole.surface()->parent()->toplevel()) {
    for (auto *controller :
         toplevelRole.surface()->parent()->toplevel()->m_foreignControllers) {
...
void RForeignToplevelHandle::set_maximized(wl_client * /*client*/,
                                           wl_resource *resource) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};

  if (!res.m_foreignToplevelManagerRes ||
      res.m_foreignToplevelManagerRes->stopped() || !res.m_toplevelRole ||
      !res.m_toplevelRole->surface()->mapped() ||
      !res.m_toplevelRole->capabilities().check(LToplevelRole::MaximizeCap))
    return;
...
void RForeignToplevelHandle::unset_maximized(wl_client * /*client*/,
                                             wl_resource *resource) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};
...
void RForeignToplevelHandle::set_minimized(wl_client * /*client*/,
                                           wl_resource *resource) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};
  ...
  if (!res.m_foreignToplevelManagerRes ||
      res.m_foreignToplevelManagerRes->stopped() || !res.m_toplevelRole ||
      !res.m_toplevelRole->surface()->mapped() ||
      !res.m_toplevelRole->capabilities().check(LToplevelRole::MinimizeCap))
    return;
...
void RForeignToplevelHandle::unset_minimized(wl_client * /*client*/,
                                             wl_resource *resource) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};
...
void RForeignToplevelHandle::activate(wl_client * /*client*/,
                                      wl_resource *resource,
                                      wl_resource * /*seat*/) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};
...
void RForeignToplevelHandle::close(wl_client * /*client*/,
                                   wl_resource *resource) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};
...
void RForeignToplevelHandle::set_rectangle(wl_client * /*client*/,
                                           wl_resource *resource,
                                           wl_resource *surface, Int32 x,
                                           Int32 y, Int32 width, Int32 height) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};

  if (!res.m_foreignToplevelManagerRes || !res.m_toplevelRole ||
      res.m_foreignToplevelManagerRes->stopped())
    return;

  if (width < 0 || height < 0) {
    res.postError(ZWLR_FOREIGN_TOPLEVEL_HANDLE_V1_ERROR_INVALID_RECTANGLE,
  ...
  LSurface *lsurface{static_cast<Protocols::Wayland::RSurface *>(
                         wl_resource_get_user_data(surface))
                         ->surface()};
  const LRect newRect{x, y, width, height};

  if (res.controller()->m_taskbarIconRect != newRect ||
      lsurface != res.controller()->taskbar()) {
...
void RForeignToplevelHandle::destroy(wl_client * /*client*/,
                                     wl_resource *resource) {
  wl_resource_destroy(resource);
}

#if LOUVRE_FOREIGN_TOPLEVEL_MANAGER_VERSION >= 2
void RForeignToplevelHandle::set_fullscreen(wl_client * /*client*/,
                                            wl_resource *resource,
                                            wl_resource *output) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};
  ...
  if (!res.m_foreignToplevelManagerRes ||
      res.m_foreignToplevelManagerRes->stopped() || !res.m_toplevelRole ||
      !res.m_toplevelRole->surface()->mapped() ||
      !res.m_toplevelRole->capabilities().check(LToplevelRole::FullscreenCap))
    return;

  LOutput *dstOutput{nullptr};

  if (output)
    dstOutput = static_cast<Protocols::Wayland::GOutput *>(
...
void RForeignToplevelHandle::unset_fullscreen(wl_client * /*client*/,
                                              wl_resource *resource) {
  auto &res{*static_cast<RForeignToplevelHandle *>(
      wl_resource_get_user_data(resource))};
...
bool RForeignToplevelHandle::parent(RForeignToplevelHandle *parent) noexcept {
  ...
  if (version() >= 3) {
    zwlr_foreign_toplevel_handle_v1_send_parent(
        resource(), parent == nullptr ? nullptr : parent->resource());
    return true;
  }
#endif
  L_UNUSED(parent)
