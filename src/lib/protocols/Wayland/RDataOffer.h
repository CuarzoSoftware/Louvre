...
class Louvre::Protocols::Wayland::RDataOffer final : public LResource {
 public:
  ...
  RDataSource::Usage usage() const noexcept { return m_usage; }

  /******************** DND ONLY ********************/

  UInt32 actions() const noexcept { return m_actions; }

  UInt32 preferredAction() const noexcept { return m_preferredAction; }

  bool matchedMimeType() const noexcept { return m_matchedMimeType; }

  /******************** REQUESTS ********************/

  static void destroy(wl_client *client, wl_resource *resource) noexcept;
  static void accept(wl_client *client, wl_resource *resource, UInt32 serial,
                     const char *mime_type) noexcept;
  static void receive(wl_client *client, wl_resource *resource,
                      const char *mime_type, Int32 fd) noexcept;

#if LOUVRE_WL_DATA_DEVICE_MANAGER_VERSION >= 3
  static void finish(wl_client *client, wl_resource *resource) noexcept;
  static void set_actions(wl_client *client, wl_resource *resource,
                          UInt32 dnd_actions, UInt32 preferred_action) noexcept;
#endif
 ...
 private:
  ...
  RDataOffer(RDataDevice *dataDeviceRes, UInt32 id,
