...
RDataDevice::RDataDevice(GDataDeviceManager *dataDeviceManagerRes,
                         GSeat *seatRes, Int32 id) noexcept
    : LResource(seatRes->client(), &wl_data_device_interface,
                dataDeviceManagerRes->version(), id, &imp),
      m_seatRes(seatRes) {
...
#if LOUVRE_WL_DATA_DEVICE_MANAGER_VERSION >= 2
void RDataDevice::release(wl_client * /*client*/,
                          wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}
#endif

void RDataDevice::start_drag(wl_client * /*client*/, wl_resource *resource,
  ...
  session->origin.reset(
      static_cast<RSurface *>(wl_resource_get_user_data(origin))->surface());
  session->srcDataDevice.reset(res);

  if (source) {
    session->source.reset(
        static_cast<RDataSource *>(wl_resource_get_user_data(source)));
  ...
  if (seat()->dnd()->dragging()) {
    LLog::warning(
        "[RDataDevicePrivate::start_drag] Start drag request cancelled. There "
        "already is an active drag & drop session.");
    goto fail;
  }
  ...
  if (!triggeringEvent) {
    LLog::warning(
        "[RDataDevicePrivate::start_drag] Start drag & drop request without "
        "serial match. Ignoring it.");
    goto fail;
  }

  // Check if there is an icon
  if (icon) {
    LSurface *iconSurface{
        static_cast<RSurface *>(wl_resource_get_user_data(icon))->surface()};

    if (!iconSurface->dndIcon()) {
      if (!iconSurface->imp()->canHostRole()) {
        res->postError(WL_DATA_DEVICE_ERROR_ROLE,
  ...
  if (session->source && session->source->version() >= 3 &&
      session->source->actions() == 0)
    goto fail;

  seat()->dnd()->m_triggeringEvent.reset(triggeringEvent->copy());
  seat()->dnd()->m_session.reset(session);

  if (session->source) session->source->m_dndSession = seat()->dnd()->m_session;

  seat()->dnd()->startDragRequest();
  return;

fail:
  if (session->source) session->source->cancelled();

  if (session->icon) session->icon->surface()->imp()->setMapped(false);

  delete session;
}

void RDataDevice::set_selection(wl_client *client, wl_resource *resource,
                                wl_resource *source, UInt32 serial) {
  L_UNUSED(client);

  RDataDevice *rDataDevice{
      static_cast<RDataDevice *>(wl_resource_get_user_data(resource))};

  const LEvent *triggeringEvent{
      rDataDevice->client()->findEventBySerial(serial)};

  if (!triggeringEvent) {
    LLog::warning(
        "[RDataDevicePrivate::set_selection] Set clipboard request without "
        "valid triggering event. Ignoring it.");
    return;
  }
  ...
  if (source) {
    ...
    if (seat()->clipboard()->m_dataSource == rDataSource) {
      LLog::warning(
          "[RDataDevicePrivate::set_selection] Set clipboard request already "
          "made with the same data source. Ignoring it.");
      return;
    }

    if (rDataSource->usage() != RDataSource::Undefined) {
      rDataSource->postError(WL_DATA_DEVICE_ERROR_USED_SOURCE,
                             "Source already used.");
      return;
    }

    // Ask the user if the client should set the clipboard
    if (!seat()->clipboard()->setClipboardRequest(rDataDevice->client(),
                                                  *triggeringEvent)) {
      LLog::debug(
          "[RDataDevicePrivate::set_selection] Set clipboard request denied by "
          "user.");
      return;
    }
    ...
    /* A NULL source should unset the clipboard, but we keep a copy of the
     * contents then we don't let clients unset the clipboard. */
  }
}
...
void RDataDevice::enter(UInt32 serial, RSurface *surface, Float24 x, Float24 y,
                        RDataOffer *offer) noexcept {
  wl_data_device_send_enter(resource(), serial, surface->resource(), x, y,
                            offer == nullptr ? NULL : offer->resource());
}

void RDataDevice::leave() noexcept { wl_data_device_send_leave(resource()); }

void RDataDevice::motion(UInt32 time, Float24 x, Float24 y) noexcept {
  wl_data_device_send_motion(resource(), time, x, y);
}

void RDataDevice::drop() noexcept { wl_data_device_send_drop(resource()); }
