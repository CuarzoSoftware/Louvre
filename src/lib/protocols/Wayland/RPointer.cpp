...
RPointer::RPointer(GSeat *seatRes, Int32 id) noexcept
    : LResource(seatRes->client(), &wl_pointer_interface, seatRes->version(),
                id, &imp),
      m_seatRes(seatRes) {
...
RPointer::~RPointer() noexcept {
  if (seatRes()) LVectorRemoveOneUnordered(seatRes()->m_pointerRes, this);
}

/******************** REQUESTS ********************/

void RPointer::set_cursor(wl_client * /*client*/, wl_resource *resource,
                          UInt32 serial, wl_resource *wlSurface,
                          Int32 hotspot_x, Int32 hotspot_y) {
  RPointer &pointerRes{
      *static_cast<RPointer *>(wl_resource_get_user_data(resource))};
  const LClient &client{*pointerRes.client()};

  if (client.eventHistory().pointer.enter.serial() != serial) {
    LLog::warning(
        "[RPointer::RPointerPrivate::set_cursor] Set cursor request without "
        "valid pointer enter event serial. Ignoring it.");
    return;
  }

  if (wlSurface) {
    const RSurface *surfaceRes{
        static_cast<RSurface *>(wl_resource_get_user_data(wlSurface))};
    ...
    if (!cursorRole) {
      if (!surface.imp()->canHostRole()) {
        pointerRes.postError(WL_POINTER_ERROR_ROLE,
    ...
    cursorRole->m_currentHotspotB =
        cursorRole->m_currentHotspot * surface.bufferScale();
    ...
    client.imp()->lastCursorRequest.m_triggeringEvent =
        client.eventHistory().pointer.enter;
...
void RPointer::enter(const LPointerEnterEvent &event,
                     RSurface *surfaceRes) noexcept {
  auto &clientEvent{client()->imp()->eventHistory.pointer.enter};

  if (clientEvent.serial() != event.serial()) clientEvent = event;

  wl_pointer_send_enter(resource(), event.serial(), surfaceRes->resource(),
                        wl_fixed_from_double(event.localPos.x()),
                        wl_fixed_from_double(event.localPos.y()));
}

void RPointer::leave(const LPointerLeaveEvent &event,
                     RSurface *surfaceRes) noexcept {
...
void RPointer::motion(const LPointerMoveEvent &event) noexcept {
  wl_pointer_send_motion(resource(), event.ms(),
                         wl_fixed_from_double(event.localPos.x()),
                         wl_fixed_from_double(event.localPos.y()));
}
...
void RPointer::button(const LPointerButtonEvent &event) noexcept {
  ...
  if (clientEvents.button[clientEvents.buttonIndex].serial() !=
      event.serial()) {
  ...
  wl_pointer_send_button(resource(), event.serial(), event.ms(), event.button(),
                         event.state());
}
