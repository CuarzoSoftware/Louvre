#include <protocols/Wayland/private/KeyboardResourcePrivate.h>
#include <protocols/Wayland/private/PointerResourcePrivate.h>
#include <protocols/Wayland/private/DataDeviceResourcePrivate.h>
#include <protocols/Wayland/private/SeatGlobalPrivate.h>
#include <private/LClientPrivate.h>
#include <LCompositor.h>

using namespace Louvre::Globals;


SeatGlobal::SeatGlobal(LCompositor *compositor,
                             wl_client *client,
                             const wl_interface *interface,
                             Int32 version,
                             UInt32 id,
                             const void *implementation,
                             wl_resource_destroy_func_t destroy) :
    LResource(compositor,
              client,
              interface,
              version,
              id,
              implementation,
              destroy)
{
    m_imp = new SeatGlobalPrivate();
    this->client()->imp()->seatGlobals.push_back(this);
    imp()->clientLink = std::prev(this->client()->imp()->seatGlobals.end());
    sendCapabilities(this->compositor()->seat()->capabilities());
    sendName("seat0");
}

SeatGlobal::~SeatGlobal()
{
    client()->imp()->seatGlobals.erase(imp()->clientLink);

    if(keyboardResource())
        keyboardResource()->imp()->seatGlobal = nullptr;

    if(pointerResource())
        pointerResource()->imp()->seatGlobal = nullptr;

    if(dataDeviceResource())
        dataDeviceResource()->imp()->seatGlobal = nullptr;

    delete m_imp;
}

void SeatGlobal::sendCapabilities(UInt32 capabilities)
{
    wl_seat_send_capabilities(resource(), capabilities);
}

void SeatGlobal::sendName(const char *name)
{
#if LOUVRE_SEAT_VERSION >= WL_SEAT_NAME_SINCE_VERSION
    if(version() >= WL_SEAT_NAME_SINCE_VERSION)
        wl_seat_send_name(resource(), name);
#endif
}

KeyboardResource *SeatGlobal::keyboardResource() const
{
    return imp()->keyboardResource;
}

PointerResource *SeatGlobal::pointerResource() const
{
    return imp()->pointerResource;
}

DataDeviceResource *SeatGlobal::dataDeviceResource() const
{
    return imp()->dataDeviceResource;
}

SeatGlobal::SeatGlobalPrivate *SeatGlobal::imp() const
{
    return m_imp;
}

