...
RSurface::RSurface(GCompositor *compositorRes, UInt32 id)
    : LResource(compositorRes->client(), &wl_surface_interface,
  ...
  surface()->imp()->layerLink =
      std::prev(compositor()->imp()->layers[LLayerMiddle].end());

  LSurface *prev{surface()->imp()->prevSurfaceInLayers()};

  if (prev) {
    if (prev->nextSurface())
      surface()->imp()->compositorLink = compositor()->imp()->surfaces.insert(
          prev->nextSurface()->imp()->compositorLink, surface());
    else {
      compositor()->imp()->surfaces.emplace_back(surface());
      surface()->imp()->compositorLink =
          std::prev(compositor()->imp()->surfaces.end());
    }
  } else {
...
RSurface::~RSurface() {
  ...
  compositor()->onAnticipatedObjectDestruction(
      lSurface->imp()->backgroundBlur.get());
  ...
  compositor()->imp()->layers[lSurface->imp()->layer].erase(
      lSurface->imp()->layerLink);
...
void RSurface::handleOffset(LSurface *surface, Int32 x, Int32 y) {
  if (surface->role()) surface->role()->handleSurfaceOffset(x, y);
}

void RSurface::attach(wl_client * /*client*/, wl_resource *resource,
                      wl_resource *buffer, Int32 x, Int32 y) {
  auto &surfaceRes{
      *static_cast<RSurface *>(wl_resource_get_user_data(resource))};

  surfaceRes.surface()->imp()->stateFlags.add(
      LSurface::LSurfacePrivate::BufferAttached);
  ...
  if (surfaceRes.surface()->imp()->pending.bufferRes)
    wl_list_remove(
        &surfaceRes.surface()->imp()->pending.onBufferDestroyListener.link);

  surfaceRes.surface()->imp()->pending.bufferRes = buffer;

  if (buffer) {
    wl_resource_add_destroy_listener(
        buffer, &surfaceRes.surface()->imp()->pending.onBufferDestroyListener);
  ...
  if (surfaceRes.version() < 5)
    ...
    surfaceRes.postError(
        ...
        "Buffer offset is invalid. Check wl_surface::offset (v5).");
#else
  handleOffset(surfaceRes.surface(), x, y);
#endif
}

void RSurface::frame(wl_client *client, wl_resource *resource,
                     UInt32 callback) {
  auto &imp{*static_cast<const RSurface *>(wl_resource_get_user_data(resource))
                 ->surface()
                 ->imp()};
...
void RSurface::destroy(wl_client * /*client*/, wl_resource *resource) {
  ...
  if (!surfaceRes.surface()->imp()->canHostRole()) {
    surfaceRes.postError(WL_SURFACE_ERROR_DEFUNCT_ROLE_OBJECT,
...
void RSurface::commit(wl_client * /*client*/, wl_resource *resource) {
  apply_commit(
      static_cast<const RSurface *>(wl_resource_get_user_data(resource))
          ->surface());
}
...
static bool bufferIsBeingScannedByOutputs(wl_buffer *buffer) noexcept {
  ...
  for (LOutput *o : compositor()->outputs())
    if (o->imp()->scanout[0].buffer == buffer ||
        o->imp()->scanout[1].buffer == buffer)
      return true;

  return false;
}

// The origin params indicates who requested the commit for this surface (itself
// or its parent surface)
void RSurface::apply_commit(LSurface *surface,
                            LBaseSurfaceRole::CommitOrigin origin) {
  // Check if the surface role wants to apply the commit
  if (surface->role() && !surface->role()->acceptCommitRequest(origin)) return;
  ...
  for (auto *presentation : imp.presentationFeedbackResources)
    if (presentation->m_commitId == -1) presentation->m_commitId = imp.commitId;
...
retryParentCommitNotif:
  ...
  if (imp.stateFlags.check(LSurface::LSurfacePrivate::BufferAttached)) {
    if (imp.current.bufferRes) {
      ...
      /* Release WL_DRM and DMA buffers only if a seccond buffer has been
       * attached. Also, if being scanned out, let outputs take care of
       * releasing them. SHM and Single Pixel buffers are released in
       * LSurface::LSurfacePrivate::bufferToTexture() */
    ...
    if (imp.current.bufferRes) {
      wl_resource_add_destroy_listener(imp.current.bufferRes,
                                       &imp.current.onBufferDestroyListener);
  ...
  if (!imp.frameCallbacks.empty()) {
    for (RCallback *callback : imp.frameCallbacks) callback->m_commited = true;
  ...
  if (imp.current.hasBuffer) {
    ...
    if (!imp.bufferToTexture()) {
      LLog::error(
          "[RSurface::apply_commit] Failed to convert buffer to OpenGL "
          "texture.");
      return;
    }
  }
  ...
  if (surface->receiveInput()) {
    ...
    } else if (changes.check(Changes::SizeChanged |
                             Changes::InputRegionChanged)) {
      pixman_region32_intersect_rect(&imp.currentInputRegion.m_region,
                                     &imp.pendingInputRegion.m_region, 0, 0,
  ...
  if (imp.stateFlags.check(LSurface::LSurfacePrivate::InfiniteInvisible)) {
    if (changes.check(Changes::SizeChanged | Changes::InvisibleRegionChanged)) {
      ...
      changes.add(Changes::InvisibleRegionChanged);
                           ...
                           Changes::InvisibleRegionChanged)) {
    pixman_region32_intersect_rect(&imp.currentInvisibleRegion.m_region,
                                   &imp.pendingInvisibleRegion.m_region, 0, 0,
                                   surface->size().w(), surface->size().h());
    changes.add(Changes::InvisibleRegionChanged);
  }
   ...
   ********** OPAQUE REGION ***********
   ************************************/
  if (changes.check(Changes::BufferSizeChanged | Changes::SizeChanged |
                    Changes::OpaqueRegionChanged)) {
    /*
    ...
    pixman_region32_intersect_rect(&imp.currentOpaqueRegion.m_region,
                                   &imp.pendingOpaqueRegion.m_region, 0, 0,
                                   surface->size().w(), surface->size().h());
    ...
    pixman_region32_inverse(&imp.currentTranslucentRegion.m_region,
                            &imp.currentOpaqueRegion.m_region, &box);
  }

  /*******************************************
   ***************** VSYNC *******************
   *******************************************/
  const bool preferVSync{
      surface->surfaceResource()->tearingControlRes() == nullptr ||
      surface->surfaceResource()->tearingControlRes()->preferVSync()};
   ...
   *******************************************/
  if (surface->role()) surface->role()->handleSurfaceCommit(origin);

  if (!ref) return;

  if (changes.check(Changes::BufferSizeChanged)) surface->bufferSizeChanged();

  if (changes.check(Changes::SizeChanged)) surface->sizeChanged();

  if (changes.check(Changes::SourceRectChanged)) surface->srcRectChanged();

  if (changes.check(Changes::BufferScaleChanged)) surface->bufferScaleChanged();

  if (changes.check(Changes::BufferTransformChanged))
    surface->bufferTransformChanged();

  if (changes.check(Changes::DamageRegionChanged)) surface->damageChanged();

  if (changes.check(Changes::InputRegionChanged)) surface->inputRegionChanged();
  ...
  if (changes.check(Changes::VSyncChanged)) surface->preferVSyncChanged();

  if (changes.check(Changes::ContentTypeChanged)) surface->contentTypeChanged();
...
void RSurface::damage(wl_client * /*client*/, wl_resource *resource, Int32 x,
  ...
  if (width > LOUVRE_MAX_SURFACE_SIZE) width = LOUVRE_MAX_SURFACE_SIZE;
  if (width <= 0) return;

  if (height > LOUVRE_MAX_SURFACE_SIZE) height = LOUVRE_MAX_SURFACE_SIZE;
  if (height <= 0) return;
...
void RSurface::set_opaque_region(wl_client * /*client*/, wl_resource *resource,
                                 wl_resource *region) {
  auto &imp{*static_cast<const RSurface *>(wl_resource_get_user_data(resource))
                 ->surface()
                 ->imp()};
  ...
  if (region) {
    imp.pendingOpaqueRegion =
        static_cast<const RRegion *>(wl_resource_get_user_data(region))
            ->region();
...
void RSurface::set_input_region(wl_client * /*client*/, wl_resource *resource,
                                wl_resource *region) {
  auto &imp{*static_cast<const RSurface *>(wl_resource_get_user_data(resource))
                 ->surface()
                 ->imp()};
...
#if LOUVRE_WL_COMPOSITOR_VERSION >= 2
void RSurface::set_buffer_transform(wl_client * /*client*/,
                                    wl_resource *resource, Int32 transform) {
  auto &res{*static_cast<RSurface *>(wl_resource_get_user_data(resource))};
  if (transform < 0 || transform > 7) {
    res.postError(WL_SURFACE_ERROR_INVALID_TRANSFORM,
                  "Invalid framebuffer transform %d.", transform);
    return;
  }
...
#if LOUVRE_WL_COMPOSITOR_VERSION >= 3
void RSurface::set_buffer_scale(wl_client * /*client*/, wl_resource *resource,
                                Int32 scale) {
  auto &res{*static_cast<RSurface *>(wl_resource_get_user_data(resource))};
...
#if LOUVRE_WL_COMPOSITOR_VERSION >= 4
void RSurface::damage_buffer(wl_client * /*client*/, wl_resource *resource,
...
#if LOUVRE_WL_COMPOSITOR_VERSION >= 5
void RSurface::offset(wl_client * /*client*/, wl_resource *resource, Int32 x,
                      Int32 y) {
  handleOffset(
      static_cast<const RSurface *>(wl_resource_get_user_data(resource))
          ->surface(),
...
#if LOUVRE_WL_COMPOSITOR_VERSION >= 6
  if (version() >= 6) {
    wl_surface_send_preferred_buffer_transform(resource(),
