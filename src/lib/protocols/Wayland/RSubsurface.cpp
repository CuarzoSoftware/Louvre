...
RSubsurface::RSubsurface(GSubcompositor *subcompositorRes, LSurface *surface,
                         LSurface *parent, UInt32 id)
    : LResource(surface->client(), &wl_subsurface_interface,
                subcompositorRes->version(), id, &imp) {
  LSubsurfaceRole::Params subsurfaceRoleParams{this, surface};
  m_subsurfaceRole.reset(
      LFactory::createObject<LSubsurfaceRole>(&subsurfaceRoleParams));

  // According to the wl_subsurface spec, the parent should be applied when
  // parent commits
...
void RSubsurface::set_position(wl_client * /*client*/, wl_resource *resource,
                               Int32 x, Int32 y) {
  auto &subsurface{
      *static_cast<RSubsurface *>(wl_resource_get_user_data(resource))
           ->subsurfaceRole()};
...
void RSubsurface::place_above(wl_client * /*client*/, wl_resource *resource,
                              wl_resource *sibling) {
  auto &res{*static_cast<RSubsurface *>(wl_resource_get_user_data(resource))};
  LSubsurfaceRole *subsurfaceRole{
      static_cast<RSubsurface *>(wl_resource_get_user_data(resource))
          ->subsurfaceRole()};
  LSurface *siblingSurface{
      static_cast<RSurface *>(wl_resource_get_user_data(sibling))->surface()};

  const bool siblingIsParent{subsurfaceRole->surface()->parent() &&
                             subsurfaceRole->surface()->parent() ==
                                 siblingSurface};

  if (siblingIsParent ||
      (siblingSurface->parent() == subsurfaceRole->surface()->parent() &&
       siblingSurface != subsurfaceRole->surface())) {
    subsurfaceRole->m_pendingPlaceAbove.reset(siblingSurface);
    return;
  }

  res.postError(WL_SUBSURFACE_ERROR_BAD_SURFACE,
                "Subsurface is not sibling or parent.");
}

void RSubsurface::place_below(wl_client * /*client*/, wl_resource *resource,
                              wl_resource *sibling) {
  auto &res{*static_cast<RSubsurface *>(wl_resource_get_user_data(resource))};
  LSubsurfaceRole *subsurfaceRole{
      static_cast<RSubsurface *>(wl_resource_get_user_data(resource))
  ...
  if (siblingSurface->parent() == subsurfaceRole->surface()->parent() &&
      siblingSurface != subsurfaceRole->surface()) {
