...
class Louvre::Protocols::Wayland::GOutput final : public LResource {
 public:
  ...
  const std::vector<XdgOutput::RXdgOutput *> &xdgOutputRes() const noexcept {
    return m_xdgOutputRes;
  }
  ...
  // Since 1
  void geometry(Int32 x, Int32 y, Int32 physicalWidth, Int32 physicalHeight,
                Int32 subpixel, const char *make, const char *model,
 ...
 private:
  ...
  LGLOBAL_INTERFACE
  GOutput(LOutput *output, wl_client *client, Int32 version,
          UInt32 id) noexcept;
  ~GOutput() noexcept;
