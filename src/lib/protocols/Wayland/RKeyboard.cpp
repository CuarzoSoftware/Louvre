...
RKeyboard::RKeyboard(GSeat *seatRes, Int32 id) noexcept
    : LResource(seatRes->client(), &wl_keyboard_interface, seatRes->version(),
                id, &imp),
      m_seatRes(seatRes) {
  repeatInfo(seat()->keyboard()->repeatRate(),
             seat()->keyboard()->repeatDelay());
  keymap(seat()->keyboard()->keymapFormat(), seat()->keyboard()->keymapFd(),
         seat()->keyboard()->keymapSize());
  seatRes->m_keyboardRes.emplace_back(this);
}

RKeyboard::~RKeyboard() noexcept {
  if (seatRes()) LVectorRemoveOneUnordered(seatRes()->m_keyboardRes, this);
}

/******************** REQUESTS********************/

#if LOUVRE_WL_SEAT_VERSION >= 3
void RKeyboard::release(wl_client * /*client*/,
...
void RKeyboard::enter(const LKeyboardEnterEvent &event, RSurface *surfaceRes,
  ...
  wl_keyboard_send_enter(resource(), event.serial(), surfaceRes->resource(),
                         keys);
}

void RKeyboard::leave(const LKeyboardLeaveEvent &event,
...
void RKeyboard::key(const LKeyboardKeyEvent &event) noexcept {
  ...
  wl_keyboard_send_key(resource(), event.serial(), event.ms(), event.keyCode(),
                       event.state());
}
...
void RKeyboard::modifiers(const LKeyboardModifiersEvent &event) noexcept {
  ...
  wl_keyboard_send_modifiers(resource(), event.serial(),
                             event.modifiers().depressed,
                             event.modifiers().latched,
                             event.modifiers().locked, event.modifiers().group);
}
