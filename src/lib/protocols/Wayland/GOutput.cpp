...
void GOutput::bind(wl_client *client, void *output, UInt32 version,
                   UInt32 id) noexcept {
  LGlobal *outputGlobal{static_cast<LGlobal *>(output)};
  new GOutput((LOutput *)(outputGlobal->userData()), client, version, id);
}

Int32 GOutput::maxVersion() noexcept { return LOUVRE_WL_OUTPUT_VERSION; }

const wl_interface *GOutput::interface() noexcept {
  return &wl_output_interface;
}

GOutput::GOutput(LOutput *output, wl_client *client, Int32 version,
                 UInt32 id) noexcept
    : LResource(client, interface(), version, id, &imp), m_output(output) {
  this->client()->imp()->outputGlobals.emplace_back(this);
  sendConfiguration();
}

GOutput::~GOutput() noexcept {
  if (output()) LVectorRemoveOneUnordered(client()->imp()->outputGlobals, this);
}
...
void GOutput::sendConfiguration() noexcept {
  ...
  geometry(output()->pos().x(), output()->pos().y(),
           output()->physicalSize().w(), output()->physicalSize().h(),
           output()->subPixel(), output()->manufacturer(), output()->model(),
           output()->transform());

  mode(WL_OUTPUT_MODE_CURRENT, output()->currentMode()->sizeB().w(),
  ...
  if (scale(output()->imp()->scale)) {
    if (name(output()->name())) description(output()->description());

    for (auto *xdgOutput : xdgOutputRes())
      if (xdgOutput->version() <= 2) xdgOutput->done();

    done();
  } else
    for (auto *xdgOutput : xdgOutputRes()) xdgOutput->done();
...
void GOutput::geometry(Int32 x, Int32 y, Int32 physicalWidth,
                       Int32 physicalHeight, Int32 subpixel, const char *make,
                       const char *model, LTransform transform) noexcept {
  wl_output_send_geometry(resource(), x, y, physicalWidth, physicalHeight,
                          subpixel, make, model, static_cast<Int32>(transform));
}

void GOutput::mode(UInt32 flags, Int32 width, Int32 height,
                   Int32 refresh) noexcept {
