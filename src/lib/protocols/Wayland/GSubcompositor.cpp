...
void GSubcompositor::bind(wl_client *client, void * /*data*/, UInt32 version,
                          UInt32 id) noexcept {
...
GSubcompositor::GSubcompositor(wl_client *client, Int32 version,
                               UInt32 id) noexcept
    : LResource(client, &wl_subcompositor_interface, version, id, &imp) {
...
void GSubcompositor::destroy(wl_client * /*client*/,
                             wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void GSubcompositor::get_subsurface(wl_client * /*client*/,
                                    wl_resource *resource, UInt32 id,
                                    wl_resource *surface,
                                    wl_resource *parent) noexcept {
  auto &surfaceRes{
      *static_cast<RSurface *>(wl_resource_get_user_data(surface))};
  auto &parentRes{*static_cast<RSurface *>(wl_resource_get_user_data(parent))};

  if (surface == parent) {
    parentRes.postError(WL_SUBCOMPOSITOR_ERROR_BAD_PARENT,
                        "Invalid wl_subsurface parent.");
    return;
  }

  if (!surfaceRes.surface()->imp()->canHostRole()) {
    parentRes.postError(WL_SUBCOMPOSITOR_ERROR_BAD_SURFACE,
                        "Given wl_surface already has another role.");
    return;
  }

  if (surfaceRes.surface()->imp()->isInChildrenOrPendingChildren(
          parentRes.surface())) {
    parentRes.postError(WL_SUBCOMPOSITOR_ERROR_BAD_PARENT,
                        "Parent can not be child of surface.");
    return;
  }

  new RSubsurface(
      static_cast<GSubcompositor *>(wl_resource_get_user_data(resource)),
      surfaceRes.surface(), parentRes.surface(), id);
}
