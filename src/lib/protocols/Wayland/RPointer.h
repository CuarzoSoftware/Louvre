...
class Louvre::Protocols::Wayland::RPointer final : public LResource {
 public:
  ...
  const std::vector<Protocols::RelativePointer::RRelativePointer *> &
  relativePointerRes() const noexcept {
    return m_relativePointerRes;
  }

  const std::vector<Protocols::PointerGestures::RGestureSwipe *> &
  gestureSwipeRes() const noexcept {
    return m_gestureSwipeRes;
  }

  const std::vector<Protocols::PointerGestures::RGesturePinch *> &
  gesturePinchRes() const noexcept {
    return m_gesturePinchRes;
  }

  const std::vector<Protocols::PointerGestures::RGestureHold *> &
  gestureHoldRes() const noexcept {
    return m_gestureHoldRes;
  }

  /******************** REQUESTS ********************/

  static void set_cursor(wl_client *client, wl_resource *resource,
                         UInt32 serial, wl_resource *surface, Int32 hotspot_x,
                         Int32 hotspot_y);
 ...
 private:
  ...
  std::vector<Protocols::RelativePointer::RRelativePointer *>
      m_relativePointerRes;
