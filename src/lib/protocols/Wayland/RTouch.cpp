...
RTouch::RTouch(GSeat *seatRes, Int32 id) noexcept
    : LResource(seatRes->client(), &wl_touch_interface, seatRes->version(), id,
                &imp),
      m_seatRes(seatRes) {
...
RTouch::~RTouch() noexcept {
  if (seatRes()) LVectorRemoveOneUnordered(seatRes()->m_touchRes, this);
}
...
void RTouch::down(const LTouchDownEvent &event, RSurface *surfaceRes) noexcept {
  ...
  auto it = std::find_if(
      clientEvents.begin(), clientEvents.end(),
      [event](const LTouchDownEvent &ev) { return ev.id() == event.id(); });
  ...
  wl_touch_send_down(resource(), event.serial(), event.ms(),
                     surfaceRes->resource(), event.id(),
...
void RTouch::up(const LTouchUpEvent &event) noexcept {
  ...
  auto it = std::find_if(
      clientEvents.begin(), clientEvents.end(),
      [event](const LTouchUpEvent &ev) { return ev.id() == event.id(); });
...
void RTouch::frame() noexcept { wl_touch_send_frame(resource()); }

void RTouch::cancel() noexcept { wl_touch_send_cancel(resource()); }
