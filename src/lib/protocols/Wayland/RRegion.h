...
class Louvre::Protocols::Wayland::RRegion final : public LResource {
 public:
  const LRegion &region() const noexcept {
    if (!m_subtract.empty()) {
      pixman_region32_subtract(&m_region.m_region, &m_region.m_region,
                               &m_subtract.m_region);
      m_subtract.clear();
    }
  ...
  static void add(wl_client *client, wl_resource *resource, Int32 x, Int32 y,
                  Int32 width, Int32 height) noexcept;
  static void subtract(wl_client *client, wl_resource *resource, Int32 x,
