...
static const struct
    ext_foreign_toplevel_image_capture_source_manager_v1_interface imp{
        .create_source =
            &GForeignToplevelImageCaptureSourceManager::create_source,
        .destroy = &GForeignToplevelImageCaptureSourceManager::destroy};

void GForeignToplevelImageCaptureSourceManager::bind(wl_client *client,
                                                     void * /*data*/,
                                                     UInt32 version,
                                                     UInt32 id) noexcept {
...
const wl_interface *
GForeignToplevelImageCaptureSourceManager::interface() noexcept {
...
GForeignToplevelImageCaptureSourceManager::
  ...
  this->client()
      ->imp()
      ->foreignToplevelImageCaptureSourceManagerGlobals.emplace_back(this);
}

GForeignToplevelImageCaptureSourceManager::
    ~GForeignToplevelImageCaptureSourceManager() noexcept {
  LVectorRemoveOneUnordered(
      client()->imp()->foreignToplevelImageCaptureSourceManagerGlobals, this);
}

/******************** REQUESTS ********************/

void GForeignToplevelImageCaptureSourceManager::destroy(
    wl_client * /*client*/, wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void GForeignToplevelImageCaptureSourceManager::create_source(
    wl_client * /*client*/, wl_resource *resource, UInt32 id,
    wl_resource *toplevel_handle) noexcept {
  new RImageCaptureSource(
      static_cast<GForeignToplevelImageCaptureSourceManager *>(
