...
void GOutputImageCaptureSourceManager::bind(wl_client *client, void * /*data*/,
                                            UInt32 version,
                                            UInt32 id) noexcept {
...
GOutputImageCaptureSourceManager::GOutputImageCaptureSourceManager(
    wl_client *client, Int32 version, UInt32 id) noexcept
    : LResource(client, interface(), version, id, &imp) {
  this->client()->imp()->outputImageCaptureSourceManagerGlobals.emplace_back(
      this);
}

GOutputImageCaptureSourceManager::~GOutputImageCaptureSourceManager() noexcept {
  LVectorRemoveOneUnordered(
      client()->imp()->outputImageCaptureSourceManagerGlobals, this);
}

/******************** REQUESTS ********************/

void GOutputImageCaptureSourceManager::destroy(wl_client * /*client*/,
                                               wl_resource *resource) noexcept {
  wl_resource_destroy(resource);
}

void GOutputImageCaptureSourceManager::create_source(
    wl_client * /*client*/, wl_resource *resource, UInt32 id,
    wl_resource *output) noexcept {
  new RImageCaptureSource(
      static_cast<GOutputImageCaptureSourceManager *>(
          wl_resource_get_user_data(resource))
          ->client(),
      wl_resource_get_version(resource), id, LImageCaptureSourceType::Output,
