...
RInvisibleRegion::RInvisibleRegion(Wayland::RSurface *surface, UInt32 id,
                                   Int32 version) noexcept
    : LResource(surface->client(), &lvr_invisible_region_interface, version, id,
                &imp),
      m_surfaceRes(surface) {}

RInvisibleRegion::~RInvisibleRegion() noexcept {
  if (!surfaceRes()) {
    postError(LVR_INVISIBLE_REGION_ERROR_DESTROYED_SURFACE,
              "surface destroyed before its lvr_invisible_region object");
    return;
  }

  surfaceRes()->surface()->imp()->pendingInvisibleRegion.clear();
  surfaceRes()->surface()->imp()->stateFlags.remove(
      LSurface::LSurfacePrivate::InfiniteInvisible);
  surfaceRes()->surface()->imp()->changesToNotify.add(
      LSurface::LSurfacePrivate::InvisibleRegionChanged);
}
...
void RInvisibleRegion::set_region(wl_client * /*client*/, wl_resource *resource,
                                  wl_resource *region) {
  auto &res{
      *static_cast<RInvisibleRegion *>(wl_resource_get_user_data(resource))};

  if (!res.surfaceRes()) {
    res.postError(LVR_INVISIBLE_REGION_ERROR_DESTROYED_SURFACE,
                  "surface destroyed before its lvr_invisible_region object");
    return;
  }
