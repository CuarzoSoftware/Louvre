...
RIdleNotification::RIdleNotification(LClient *client, Int32 version, UInt32 id,
                                     UInt32 timeout) noexcept
    : LResource(client, &ext_idle_notification_v1_interface, version, id, &imp),
      m_timeout(timeout == 0 ? 1 : timeout),
      m_timer([this](LTimer *timer) {
        seat()->onIdleListenerTimeout(listener());

        if (!timer->running()) idled();
      }) {
  m_listener.resetTimer();
}

RIdleNotification::~RIdleNotification() noexcept { m_timer.cancel(); }

/******************** REQUESTS ********************/

void RIdleNotification::destroy(wl_client * /*client*/,
