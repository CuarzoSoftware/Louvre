...
#include <globals/LinuxDMA-BUF/wl_buffer_dmabuf.h>
...
bool LSurface::LSurfacePrivate::bufferToTexture()
{
    ...
    else if(wl_buffer_is_dmabuf(current.buffer))
    {
        ...
        width = wl_buffer_dmabuf_get_width(current.buffer);
        height = wl_buffer_dmabuf_get_height(current.buffer);


        LSize newSize = LSize(width, height);

        if(texture->size() != newSize || bufferScaleChanged)
        {
            bufferSizeChanged = true;
            currentBufferDamages.clear();
            currentBufferDamages.addRect(LRect(0,newSize));
            currentDamages.copy(currentBufferDamages);
            currentDamages.multiply(1.0/current.bufferScale);
        }
        else if(!pending.damages.empty() || !pending.bufferDamages.empty())
        {
            LRegion p;
            p.copy(pending.damages);
            p.multiply(current.bufferScale);
            currentBufferDamages.addRegion(p);
            currentBufferDamages.addRegion(pending.bufferDamages);
            currentBufferDamages.clip(LRect(0,newSize));
            currentDamages.copy(currentBufferDamages);
            currentDamages.multiply(1.0/current.bufferScale);
        }

        struct bufstate *s = (struct bufstate*)wl_buffer_dmabuf_get_subsystem_object(current.buffer, SUBSYSTEM_DRM);

        printf("DMA BUF\n");

        texture->setData(width, height, &s->eglImage, GL_BGRA, GL_UNSIGNED_BYTE, LTexture::BufferSourceType::EGL);
        //eglDestroyImage(LWayland::eglDisplay(), image);
    }
    #endif
    else
