...
{            
    surfaces.splice( surfaces.end(), surfaces, surface->imp()->link );
...
{
    surfaces.splice(std::next(prevSurface->imp()->link), surfaces, surfaceToInsert->imp()->link);
}

void LCompositor::LCompositorPrivate::insertSurfaceBefore(LSurface *nextSurface, LSurface *surfaceToInsert)
{
    surfaces.splice(nextSurface->imp()->link, surfaces, surfaceToInsert->imp()->link);
}
