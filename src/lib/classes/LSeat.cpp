...
void LSeat::setCapabilities(UInt32 capabilitiesFlags)
{
    ...
    {
        if(c->seatResource())
            wl_seat_send_capabilities(c->seatResource(), imp()->capabilities);
    }
}

LToplevelRole *LSeat::activeToplevel() const
{
    return m_imp->activeToplevel;
}
